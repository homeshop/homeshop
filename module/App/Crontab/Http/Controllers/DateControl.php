...
class  DateControl extends BaseCronControl {
    ...
    private function _order_auto_complete() {
        ...
        $condition['delay_time'] = ['lt', TIMESTAMP - ORDER_AUTO_RECEIVE_DAY * 86400];
        //分批，每批处理100个订单，最多处理5W个订单
        for($i = 0; $i < 500; $i++){
            if($_break){
                break;
    ...
    private function _add_points() {
        ...
        for($i = 0; $i < 50000; $i = $i + 100){
            ...
            if(!empty($member_list)){
                foreach($member_list as $member_info){
                    if(C('points_isuse')){
                        $model_points->savePointsLog('login', [
                            'pl_memberid' => $member_info['member_id'],
                            'pl_membername' => $member_info['member_name']
                        ], true);
                    }
                    $model_exppoints->saveExppointsLog('login', [
                        'exp_memberid' => $member_info['member_id'],
                        'exp_membername' => $member_info['member_name']
                    ], true);
                }
            } else {
                break;
        ...
        if(C('points_isuse')){
            ...
            for($i = 0; $i < 50000; $i = $i + 100){
                ...
                if(!empty($member_list)){
                    foreach($member_list as $member_info){
                        $model_points->savePointsLog('regist', [
                            'pl_memberid' => $member_info['member_id'],
                            'pl_membername' => $member_info['member_name']
                        ], true);
                    }
        ...
        for($i = 0; $i < 50000; $i = $i + 100){
            ...
            if(!empty($order_list)){
                foreach($order_list as $order_info){
                    if(C('points_isuse')){
                        $model_points->savePointsLog('order', [
                    ...
                    $model_exppoints->saveExppointsLog('order', [
    ...
    private function _arrival_notice() {
        ...
        for($i = 0; $i <= $times; $i++){
            ...
            $goods_list = Model('goods')->getGoodsOnlineList([
        ...
        for($stime = $start_time; $stime < time(); $stime = $stime + 86400){
            ...
            for($i = 0; $i < $countnum; $i += 100){//每次查询100条
                ...
                if($orderlog_list){
                    ...
                    $order_list_tmp = $model->table('orders')->field($field)->where([
                        'order_id' => [
                            'in',
                            $orderid_arr
                        ]
                    ...
                    $order_common_list_tmp = $model->table('order_common')->field($field)->where([
                        'order_id' => [
                            'in',
                            $orderid_arr
                        ]
                    ...
                    $store_list_tmp = $model->table('store')->field($field)->where([
                        'store_id' => [
                            'in',
                            $storeid_arr
                        ]
                    ...
                    $ordergoods_list = $model->table('order_goods')->field($field)->where([
                        'order_id' => [
                            'in',
                            $orderid_arr
                        ]
                    ...
                    $goods_list_tmp = $model->table('goods')->field($field)->where([
                        'goods_id' => [
                            'in',
                            $goodsid_arr
                        ]
                    ...
                    $goods_common_list_tmp = $model->table('goods_common')->field($field)->where([
                        'goods_commonid' => [
                            'in',
                            $goods_commonid_arr
                        ]
    ...
    private function _member_stat() {
        ...
        for($stime = $start_time; $stime < time(); $stime = $stime + 86400){
            ...
            $where['payment_code'] = [
                'exp',
                "(orders.payment_code='offline' and order_log.log_orderstate = '" . ORDER_STATE_SUCCESS . "') or (orders.payment_code<>'offline' and order_log.log_orderstate = '" . ORDER_STATE_PAY . "' )"
            ];
            $orderlist_tmp = [];
            ...
            foreach((array)$update_arr as $k => $v){
                foreach($v as $m_k => $m_v){
                    ...
                    $where['payment_code'] = [
                        'exp',
                        "(payment_code='offline' and order_state = '" . ORDER_STATE_SUCCESS . "') or (payment_code<>'offline')"
                    ];
                    ...
                    $where['orders.order_state'] = [
                        ['neq', ORDER_STATE_NEW],
                        ['neq', ORDER_STATE_CANCEL],
                        'and'
                    ];//去除未支付和已取消订单
                    $where['orders.add_time'] = ['between', [$up_stime, $up_etime]];
                    //货到付款当交易成功进入统计，非货到付款当付款后进入统计
                    $where['orders.payment_code'] = [
                        'exp',
                        "(orders.payment_code='offline' and orders.order_state = '" . ORDER_STATE_SUCCESS . "') or (orders.payment_code<>'offline')"
                    ];
    ...
    private function _create_bill_cycle_by_month($start_unixtime, $store_info) {
        ...
        while(($time = strtotime('-' . $i . ' month', $current_time)) >= $start_unixtime) {
            if(date('Ym', $start_unixtime) == date('Ym', $time)){
                //如果两个月份相等检查库是里否存在
                $order_statis = Model()->cls()->table('bill_create')->where([
                    'os_month' => date('Ym', $start_unixtime),
    ...
    private function _create_real_order_bill($data) {
        ...
        if(!$this->_model_bill->getOrderBillInfo([
            'ob_store_id' => $data['ob_store_id'],
            'ob_start_date' => $data['ob_start_date']
        ])
        ){
    ...
    private function _calc_real_order_bill($data_bill) {
        ...
        $refund_condition['admin_time'] = [
            ['egt', $data_bill['ob_start_date']],
            ['elt', $data_bill['ob_end_date']],
            'and'
        ];
        ...
        //本期应结
        $update['ob_result_totals'] = $update['ob_order_totals'] + $update['ob_rpt_amount'] + $update['ob_order_book_totals'] - $update['ob_order_return_totals'] - $update['ob_commis_totals'] + $update['ob_commis_return_totals'] - $update['ob_rf_rpt_amount'] - $update['ob_store_cost_totals'];
    ...
    private function _create_vr_order_bill($data) {
        ...
        $data_bill['ob_state'] = 0;
        $data_bill['ob_store_id'] = $data['ob_store_id'];
        if(!$this->_model_vr_bill->getOrderBillInfo([
            'ob_store_id' => $data['ob_store_id'],
            'ob_start_date' => $data['ob_start_date']
        ])
        ){
