...
class  GoodsControl extends BaseChainCenterControl {
    ...
    public function indexOp() {
        ...
        if(!empty($goods_list)){
            ...
            $goodsid_array = $model_goods->getGoodsOnlineList([
                'goods_commonid' => [
                    'in',
                    $commonid_array
                ]
            ], 'min(goods_id) goods_id,goods_commonid', 0, 'goods_id desc', 0, 'goods_commonid');
            $goodsid_array = array_under_reset($goodsid_array, 'goods_commonid');
            Tpl::output('goodsid_array', $goodsid_array);
            $stock_array = Model('chain_stock')->getChainStockList([
                'chain_id' => $_SESSION['chain_id'],
                'goods_commonid' => ['in', $commonid_array]
            ]);
    ...
    public function set_stockOp() {
        ...
        $goods_info = $model_goods->getGeneralGoodsOnlineList(['goods_commonid' => $common_id], 'goods_id,goods_spec,goods_serial,goods_price');
        $stock_info = $model_chain_stock->getChainStockList([
            'chain_id' => $_SESSION['chain_id'],
    ...
    private function profile_menu($menu_type, $menu_key) {
        ...
        switch($menu_type) {
            case 'goods_list':
                $menu_array = [
                    ['menu_key' => 'goods_list', 'menu_name' => '商品列表', 'menu_url' => urlChain('goods', 'index')]
                ];
                break;
        }
