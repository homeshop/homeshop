...
class  ApiControl extends BaseCircleControl {
    ...
    public function get_theme_listOp() {
        ...
        $model = Model();
        $theme_list = $model->table('circle_theme')->field('circle_theme.*, is_recommend*rand()*10000 + has_affix*rand() as rand')->where([
            'circle_status' => 1,
            'is_closed' => 0
        ])->where(['has_affix' => 1])->order('rand desc')->limit($data_count)->select();
        if(!empty($theme_list)){
    ...
    public function get_reply_themelistOp() {
        ...
        $model = Model();
        $reply_themelist = $model->table('circle_theme')->where(['is_closed' => 0])->order('theme_commentcount desc')->limit($data_count)->select();
        if($this->data_type === 'json'){
