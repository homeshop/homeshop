...
class  IndexControl extends BaseCircleControl {
    ...
    public function indexOp() {
        ...
        $circle_list = $model->table('circle')->field('circle.*, is_hot*rand() as rand')->where([
            'circle_status' => 1,
            'is_hot' => 1
        ])->order('rand desc')->limit(3)->select();
        if(!empty($circle_list)){
            ...
            foreach($circle_list as $key => $val){
                // 最新的两条数据
                $theme_list = $model->table('circle_theme')->where([
                    'circle_id' => $val['circle_id'],
                    'is_closed' => 0
                ])->order('theme_id desc')->limit(2)->select();
                $circle_list[ $key ]['theme_list'] = $theme_list;
            }
            Tpl::output('circle_list', $circle_list);
            $now = strtotime(date('Y-m-d', time()));
            // 今天发表的主题
            $nowthemecount_array = $model->table('circle_theme')->field('count(circle_id) as count,circle_id')->group('circle_id')->where([
                'theme_addtime' => [
                    'gt',
                    $now
                ],
                'circle_id' => ['in', $circleid_array],
                'is_closed' => 0
            ])->select();
            ...
            // 今天新加入的成员
            $nowjoincount_array = $model->table('circle_member')->field('count(circle_id) as count,circle_id')->group('circle_id')->where([
                'cm_jointime' => [
                    'gt',
                    $now
                ],
        ...
        // 圈子分类
        $class_list = $model->table('circle_class')->where([
            'class_status' => 1,
            'is_recommend' => 1
        ])->order('class_sort asc')->select();
        Tpl::output('class_list', $class_list);
        // 推荐圈子
        $rcircle_list = $model->table('circle')->field('circle.*, is_recommend*rand() as rand')->where([
            'circle_status' => 1,
            'is_recommend' => 1
        ])->order('rand desc')->limit('20')->select();
        Tpl::output('rcircle_list', $rcircle_list);
        // 推荐话题
        $theme_list = $model->table('circle_theme')->field('circle_theme.*, is_recommend*rand() as rand')->where([
            'has_affix' => 1,
            'is_closed' => 0,
            'is_recommend' => 1
        ])->order('rand desc')->limit(8)->select();
        if(!empty($theme_list)){
            $theme_list = array_under_reset($theme_list, 'theme_id');
            $themeid_array = array_keys($theme_list);
            // 附件
            $affix_list = $model->table('circle_affix')->field('theme_id, min(affix_filethumb) affix_filethumb')->where([
                'theme_id' => [
                    'in',
                    $themeid_array
                ],
        ...
        // 商品话题
        $gtheme_list = $model->table('circle_theme')->where([
            'has_goods' => 1,
            'is_closed' => 0
        ])->order('theme_id desc')->limit(6)->select();
        if(!empty($gtheme_list)){
            $gtheme_list = array_under_reset($gtheme_list, 'theme_id');
            $themeid_array = array_keys($gtheme_list);
            // 圈子商品
            $thg_list = $model->table('circle_thg')->where([
        ...
        // 优秀成员
        $member_list = $model->table('circle_member')->field('circle_member.*, is_recommend*rand() as rand')->where(['is_recommend' => 1])->order('rand desc')->limit(5)->select();
    ...
    public function myjoinedcircleOp() {
        ...
        $cm_list = $model->getCircleMemberList([
            'member_id' => $_SESSION['member_id'],
            'circle_id' => ['neq', 0]
        ], 'circle_id,circle_name,is_identity', 0, 'is_identity asc');
        if(empty($cm_list)){
            echo false;
            die;
        }
    ...
    public function check_circle_nameOp() {
        ...
        $rs = Model()->table('circle')->where(['circle_name' => $name])->find();
        if(!empty($rs)){
            echo 'false';
        } else {
            echo 'true';
