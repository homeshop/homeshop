...
class  MemberControl extends SystemControl {
    ...
    public function member_listOp() {
        ...
        $where = [];
        if($_GET['searchname'] != ''){
            $where['member_name'] = ['like', '%' . $_GET['searchname'] . '%'];
        }
        if($_GET['circlename'] != ''){
            $where['circle_name'] = ['like', '%' . $_GET['circlename'] . '%'];
        }
        if($_GET['searchrecommend'] != '' && in_array(intval($_GET['searchrecommend']), [0, 1])){
            $where['is_recommend'] = intval($_GET['searchrecommend']);
        }
        if($_GET['searchidentity'] != '' && in_array(intval($_GET['searchidentity']), [1, 2, 3])){
