 ...
 * Circle Member Level
 */
class  MemberlevelControl extends SystemControl {
    ...
    public function indexOp() {
        ...
        if(chksubmit()){
            ...
            $rs = $model->table('circle_mldefault')->insertAll($insert_all, [], true);
            if($rs){
    ...
    public function refOp() {
        ...
        if(chksubmit()){
            ...
            $rs = $model->table('circle_mlref')->where(['mlref_id' => ['in', $mlrefid_array]])->delete();
            if($rs){
                showMessage(L('nc_common_op_succ'));
            } else {
                showMessage(L('nc_common_op_fail'));
    ...
    public function ref_editOp() {
        ...
        if(chksubmit()){
            ...
            $rs = $model->table('circle_mlref')->where(['mlref_id' => intval($_POST['mlref_id'])])->update($update_array);
            if($rs){
        ...
        $mlref_info = $model->table('circle_mlref')->where(['mlref_id' => intval($_GET['mlref_id'])])->find();
        if(empty($mlref_info)){
            showMessage(L('param_error'));
        }
    ...
    public function ref_delOp() {
        ...
        $rs = Model()->table('circle_mlref')->where(['mlref_id' => $mlref_id])->delete();
        if($rs){
            showMessage(L('nc_common_op_succ'));
        } else {
            showMessage(L('nc_common_op_fail'));
    ...
    public function ajaxOp() {
        switch($_GET['branch']) {
            case 'status':
                ...
                Model()->table('circle_mlref')->where(['mlref_id' => intval($_GET['id'])])->update($update_array);
                echo 'true';
                exit;
                break;
        }
