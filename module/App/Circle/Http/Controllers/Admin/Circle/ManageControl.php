...
class  ManageControl extends SystemControl {
    ...
    public function circle_addOp() {
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                ["input" => $_POST["c_name"], "require" => "true", "message" => L('circle_name_not_null')],
            ];
            ...
            if($error != ''){
                ...
                if($circleid){
                    ...
                    $model->table('circle')->where(['circle_id' => $circleid])->update($update);
        ...
        // 圈子分类
        $class_list = $model->table('circle_class')->where(['class_status' => 1])->order('class_sort asc')->select();
    ...
    public function circle_editOp() {
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                $rs = $model->table('circle')->where(['circle_id' => intval($_POST['c_id'])])->update($update);
                if($rs){
                    // 更新圈子会员表 圈主信息。
                    $cminfo = $model->table('circle_member')->where([
                        'member_id' => intval($_POST['masterid']),
                        'circle_id' => intval($_POST['c_id'])
                    ])->find();
                    if(empty($cminfo)){
                        // 取消员圈主圈主身份
                        ...
                        $model->table('circle_theme')->where([
                            'circle_id' => intval($_POST['c_id']),
                            'is_identity' => 1
                        ])->update(['is_identity' => 3]);
                        // 把圈主信息加入圈子会员表
                    ...
                    // 更新圈子成员信息
                    $count = $model->table('circle_member')->where(['circle_id' => intval($_POST['c_id'])])->count();
                    $model->table('circle')->where(['circle_id' => intval($_POST['c_id'])])->update(['circle_mcount' => $count]);
                    // 更新主题圈子名称字段
                    $model->table('circle_theme')->where(['circle_id' => intval($_POST['c_id'])])->update(['circle_name' => $_POST['c_name']]);
                    $model->table('circle_member')->where(['circle_id' => intval($_POST['c_id'])])->update(['circle_name' => $_POST['c_name']]);
        ...
        // 圈子详细
        $circle_info = $model->table('circle')->where(['circle_id' => $id])->find();
    ...
    public function choose_masterOp() {
        Tpl::output('search_url', (intval($_GET['c_id']) > 0) ? urlAdminCircle('circle_manage', 'search_member', ['c_id' => intval($_GET['c_id'])]) : urlAdminCircle('circle_manage', 'search_member'));
        Tpl::setDirquna('circle');
    ...
    public function search_memberOp() {
        ...
        $where = [];
        if($_GET['name'] != ''){
            $where['member_name'] = ['like', '%' . trim($_GET['name']) . '%'];
        }
        ...
        if(intval($_GET['c_id']) > 0){
            $where = ['circle_id' => ['neq', intval($_GET['c_id'])]];
        }
        ...
        if(!empty($count_array)){
            foreach($count_array as $val){
                if(intval($val['count']) >= C('circle_createsum')){
                    unset($member_list[ $val['circle_masterid'] ]);
                }
            }
        }
        // 允许加入圈子验证
        ...
        if(!empty($count_array)){
            foreach($count_array as $val){
                if(intval($val['count']) >= C('circle_joinsum')){
                    unset($member_list[ $val['member_id'] ]);
                }
            }
        }
    ...
    public function circle_delOp() {
        ...
        // 删除附件
        $affix_list = $model->table('circle_affix')->where(['circle_id' => $id])->select();
        if(!empty($affix_list)){
            foreach($affix_list as $val){
                @unlink(themeImagePath($val['affix_filename']));
                @unlink(themeImagePath($val['affix_filethumb']));
            }
            $model->table('circle_affix')->where(['circle_id' => $id])->delete();
        }
        // 删除商品
        $model->table('circle_thg')->where(['circle_id' => $id])->delete();
        // 删除赞表相关
        $model->table('circle_like')->where(['circle_id' => $id])->delete();
        // 删除回复
        $model->table('circle_threply')->where(['circle_id' => $id])->delete();
        // 删除话题
        $model->table('circle_theme')->where(['circle_id' => $id])->delete();
        // 删除成员
        $model->table('circle_member')->where(['circle_id' => $id])->delete();
        // 删除圈子
        $model->table('circle')->where(['circle_id' => $id])->delete();
        $this->log(L('nc_del_circle') . '[' . $id . ']');
        exit(json_encode(['state' => true, 'msg' => L('nc_common_op_succ')]));
    }
    ...
    public function check_memberOp() {
        $model = Model();
        $member_info = Model('member')->table('member')->where([
    ...
    public function ajaxOp() {
        switch($_GET['branch']) {
            ...
            case 'recommend':
                ...
                Model()->table('circle')->where(['circle_id' => intval($_GET['id'])])->update($update);
                echo 'true';
                break;
            case 'name':
                $where = [
    ...
    public function check_circle_nameOp() {
        ...
        if(intval($_GET['id']) > 0){
            $where['circle_id'] = ['neq', intval($_GET['id'])];
        }
