 ...
 * 圈子话题管理
 */
class  ThemeControl extends SystemControl {
    ...
    public function get_xmlOp() {
        ...
        // 附件
        $affix_list = $model->table('circle_affix')->field('theme_id,max(affix_filethumb) as affix_filethumb')->where([
            'theme_id' => [
                'in',
                $themeid_array
            ],
            'affix_type' => 1
        ])->group('theme_id')->select();
    ...
    public function theme_infoOp() {
        ...
        $theme_info = $model->table('circle_theme')->where(['theme_id' => $t_id])->find();
    ...
    public function theme_delOp() {
        ...
        $theme_info = $model->table('circle_theme')->where(['theme_id' => $t_id, 'circle_id' => $c_id])->find();
        if(empty($theme_info)){
        ...
        // 更新圈子主题数量
        $model->table('circle')->where(['circle_id' => $c_id])->update([
            'circle_thcount' => [
                'exp',
                'circle_thcount-1'
            ]
    ...
    public function theme_replyOp() {
        ...
        if(chksubmit()){
            ...
            if(!empty($_POST['check_reply_id']) && is_array($_POST['check_reply_id'])){
                foreach($_POST['check_reply_id'] as $r_id){
                    ...
                    // 删除回复
                    $model->table('circle_threply')->where(['theme_id' => $t_id, 'reply_id' => $r_id])->delete();
                    // 更新话题回复数
        ...
        $t_id = intval($_GET['t_id']);
        $reply_list = $model->table('circle_threply')->where(['theme_id' => $t_id])->page(10)->select();
    ...
    public function get_reply_xmlOp() {
        ...
        $order = '';
        $param = ['reply_content', 'member_name', 'reply_addtime'];
        if(in_array($_POST['sortname'], $param) && in_array($_POST['sortorder'], ['asc', 'desc'])){
            $order = $_POST['sortname'] . ' ' . $_POST['sortorder'];
        }
        $page = $_POST['rp'];
    ...
    public function theme_replydelOp() {
        ...
        $model = Model();
        // 验证回复
        $reply_info = $model->table('circle_threply')->where(['theme_id' => $t_id, 'reply_id' => $r_id])->find();
        if(empty($reply_info)){
    ...
    public function theme_recommendOp() {
        $update = ['is_recommend' => ($_GET['value'] == '1' ? 1 : 0)];
        Model()->table('circle_theme')->where(['theme_id' => intval($_GET['id'])])->update($update);
        exit(json_encode(['state' => true, 'msg' => '操作成功']));
    }
}
