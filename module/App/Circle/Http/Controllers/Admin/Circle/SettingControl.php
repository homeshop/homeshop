...
class  SettingControl extends SystemControl {
    ...
    public function super_listOp() {
        ...
        if(chksubmit()){
            ...
            $result = $model_circlemember->delSuper(['member_id' => ['in', $id_array]]);
            if($result){
        ...
        $cm_list = $model_circlemember->getSuperList([]);
    ...
    public function del_superOp() {
        ...
        $result = Model('circle_member')->delSuper(['member_id' => $member_id]);
        if($result){
            showMessage(L('nc_common_del_succ'));
        } else {
            showMessage(L('nc_common_del_fail'));
    ...
    public function member_searchOp() {
        $cm_list = Model('circle_member')->getSuperList([], 'member_id');
        $where = [];
        if(!empty($_GET['name'])){
        ...
        if(!empty($cm_list)){
            ...
            $where['member_id'] = ['not in', $memberid_array];
        }
