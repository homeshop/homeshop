 ...
 * 圈子等级
 */
class  LevelControl extends BaseCircleManageControl {
    ...
    public function levelOp() {
        ...
        if(chksubmit()){
            ...
            if($_POST['levelset'] == 'custom'){
                ...
                if($levelset){
                    $mlref_info = Model()->table('circle_mlref')->where(['mlref_id' => $levelset])->find();
                }
                if(!empty($mlref_info)){
        ...
        if(!empty($mldefault_list)){
            ...
            $mlref_list = $model->table('circle_mlref')->where(['mlref_status' => 1])->select();
            // tidy
            if(!empty($mlref_list)){
                $mlr_array = [];
