 ...
 * 圈子首页
 */
class  InformControl extends BaseCircleManageControl {
    ...
    public function informOp() {
        ...
        if(chksubmit()){
            ...
            foreach($_POST['i_id'] as $val){
                ...
                $rs = $model->table('circle_inform')->where(['inform_id' => $val])->update($update);
                // Experience increase or decrease
                if($rs && $i_rewards != 0){
                    $inform_info = $model->table('circle_inform')->field('circle_id,member_id,member_name')->where(['inform_id' => $val])->find();
            ...
            $count = $model->table('circle_inform')->where(['circle_id' => $this->c_id, 'inform_state' => 0])->count();
            $model->table('circle')->where(['circle_id' => $this->c_id])->update(['new_informcount' => $count]);
