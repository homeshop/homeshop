...
class  ThemeControl extends BaseCircleThemeControl {
    ...
    public function ajax_themeinfoOp() {
        ...
        $model = Model();
        // 话题商品
        $goods_list = $model->table('circle_thg')->where(['theme_id' => $this->t_id, 'reply_id' => 0])->select();
        $goods_list = tidyThemeGoods($goods_list, 'themegoods_id');
        $data['goods_list'] = $goods_list;
        // 附件
        $affix_list = $model->table('circle_affix')->where(['affix_type' => 1, 'theme_id' => $this->t_id])->select();
        if(!empty($affix_list)){
        ...
        // 访问数增加
        $model->table('circle_theme')->where(['theme_id' => $this->t_id])->update([
            'theme_browsecount' => [
                'exp',
                'theme_browsecount+1'
            ]
        ...
        if(isset($_SESSION['member_id'])){
            // 是否赞过话题
            $like_info = $model->table('circle_like')->where([
                'theme_id' => $this->t_id,
    ...
    public function ajax_quickreplyOp() {
        ...
        // 回复
        $reply_list = Model()->table('circle_threply')->where([
            'theme_id' => $this->t_id,
            'circle_id' => $this->c_id
        ])->order('reply_id desc')->limit(5)->select();
        if(!empty($reply_list)){
    ...
    public function save_themeOp() {
        if(chksubmit()){
            ...
            $this->circleInfo();
            // 不是圈子成员不能发帖
            if(!in_array($this->identity, [1, 2, 3])){
            ...
            $thclass_name = '';
            if($thclass_id > 0){
                $thclass_info = $model->table('circle_thclass')->where(['thclass_id' => $thclass_id])->find();
            ...
            $validate_arr[] = [
                "input" => $_POST["name"],
                "require" => "true",
                "message" => Language::get('nc_name_not_null')
            ];
            $validate_arr[] = [
                "input" => $_POST["name"],
                "validator" => 'Length',
                "min" => 4,
                "max" => 30,
                "message" => Language::get('nc_name_min_max_length')
            ];
            $validate_arr[] = [
                "input" => $_POST["themecontent"],
                "require" => "true",
                "message" => Language::get('nc_content_not_null')
            ];
            if(intval(C('circle_contentleast')) > 0){
                $validate_arr[] = [
                    "input" => $_POST["themecontent"],
                    "validator" => 'Length',
                    "min" => intval(C('circle_contentleast')),
                    "message" => Language::get('circle_contentleast')
                ];
            }
            ...
            if($themeid){
                ...
                // 更新话题附件
                $model->table('circle_affix')->where([
                    'affix_type' => 1,
                    'member_id' => $_SESSION['member_id'],
                    'theme_id' => 0
                ])->update(['theme_id' => $themeid, 'circle_id' => $this->c_id]);
                // 更新话题信息
                $affixe_count = $model->table('circle_affix')->where([
                    'affix_type' => 1,
                    'member_id' => $_SESSION['member_id'],
                    'theme_id' => $themeid
                ])->count();
                ...
                if($has_goods || $has_affix){
                    ...
                    $model->table('circle_theme')->where(['theme_id' => $themeid])->update($update);
                }
                // 更新圈子表话题数
                $update = [
                    'circle_thcount' => ['exp', 'circle_thcount+1']
                ];
                $model->table('circle')->where(['circle_id' => $this->c_id])->update($update);
                // 更新用户相关信息
                $update = [
                    'cm_thcount' => ['exp', 'cm_thcount+1'],
                    'cm_lastspeaktime' => time()
                ];
                $model->table('circle_member')->where([
                    'member_id' => $_SESSION['member_id'],
                    'circle_id' => $this->c_id
                ])->update($update);
                // Special theme
    ...
    public function save_votepollOp() {
        if(chksubmit()){
            ...
            $poll_info = $model->table('circle_thpoll')->where(['theme_id' => $this->t_id])->find();
            if(empty($poll_info)){
            ...
            $voter_info = $model->table('circle_thpollvoter')->where([
                'theme_id' => $this->t_id,
                'member_id' => $_SESSION['member_id']
            ])->find();
            if(!empty($voter_info)){
            ...
            $option_list = $model->table('circle_thpolloption')->where([
                'pollop_id' => ['in', $pollopid],
                'theme_id' => $this->t_id
            ])->select();
            ...
            foreach($option_list as $val){
                ...
                $update = [];
                $update['pollop_votes'] = ['exp', 'pollop_votes+1'];
                $update['pollop_votername'] = [
                    'exp',
                    '\'' . $_SESSION['member_name'] . ' ' . $val['pollop_votername'] . '\''
                ];
                $model->table('circle_thpolloption')->where(['pollop_id' => $val['pollop_id']])->update($update);        //
            }
            ...
            $update = [];
            $update['poll_voters'] = ['exp', 'poll_voters+' . count($option_list)];
            $model->table('circle_thpoll')->where(['theme_id' => $this->t_id])->update($update);
    ...
    public function choose_goodsOp() {
        $model = Model();
        // 三个月内 已购买的商品
        $order_goods = $model->table('order_goods,orders')->field('order_goods.goods_id,order_goods.goods_name,order_goods.goods_image,order_goods.goods_price,orders.store_id as store_id')->join('inner join')->on('order_goods.order_id=orders.order_id')->where([
            'orders.buyer_id' => $_SESSION['member_id'],
            'orders.order_state' => 40,
            'orders.finnshed_time' => ['gt', time() - 60 * 60 * 24 * 30 * 3]
        ])->distinct(true)->select();
        // 收藏的商品
        $favorites_goods = $model->table('goods,favorites')->field('goods.goods_id,goods.goods_name,goods.goods_image,goods.goods_price as goods_price,goods.store_id')->join('inner join')->on('goods.goods_id=favorites.fav_id')->where([
            'favorites.fav_type' => 'goods',
            'favorites.member_id' => $_SESSION['member_id']
        ])->distinct(true)->select();
    ...
    public function choose_imageOp() {
        ...
        $class_list = $model->table('sns_albumclass')->where(['member_id' => $_SESSION['member_id']])->select();
    ...
    public function delimgOp() {
        $id = intval($_GET['id']);
        if($id > 0){
            $affix_info = Model()->table('circle_affix')->where([
                'member_id' => $_SESSION['member_id'],
                'affix_id' => $id
            ])->find();
            if(!empty($affix_info)){
                ...
                Model()->table('circle_affix')->where(['affix_id' => $id])->delete();
                echo true;
            }
        }
    }
    ...
    public function unused_imgOp() {
        $affix_list = Model()->table('circle_affix')->field('affix_id,affix_filename,affix_filethumb')->where([
            'member_id' => $_SESSION['member_id'],
            'affix_type' => 1,
            'theme_id' => 0
        ])->select();
    ...
    public function theme_detailOp() {
        ...
        // member
        $member_list = $model->table('circle_member')->field('member_id,circle_id,cm_level,cm_levelname,is_identity')->where([
            'circle_id' => $this->c_id,
        ...
        if($this->theme_info['theme_special'] == 1){
            ...
            $option_list = $model->table('circle_thpolloption')->where(['theme_id' => $this->t_id])->order('pollop_sort asc')->select();
    ...
    public function edit_themeOp() {
        ...
        if(chksubmit()){
            ...
            $rs = $model->table('circle_theme')->where(['theme_id' => $this->t_id])->update($update);
            if($rs){
                $has_goods = 0; // 存在商品标记
                $has_affix = 0;// 存在附件标记
                // 删除原有商品
                $goods_list = Model()->table('circle_thg')->where([
                    'theme_id' => $this->t_id,
                    'reply_id' => 0
                ])->delete();
                // 插入话题商品
                ...
                $model->table('circle_affix')->where([
                    ...
                    'member_id' => $_SESSION['member_id'],
                    'theme_id' => 0
                ])->update(['theme_id' => $this->t_id, 'circle_id' => $this->c_id]);
                // 更新话题信息
                $affixe_count = $model->table('circle_affix')->where([
                    'affix_type' => 1,
                    'member_id' => $_SESSION['member_id'],
                ...
                if($has_goods || $has_affix){
                    ...
                    $update['has_goods'] = $has_goods;
                    $update['has_affix'] = $has_affix;
                    $model->table('circle_theme')->where(['theme_id' => $this->t_id])->update($update);
                }
                // Special theme
                if($_GET['sp'] == 1){
                    // Update the vote
                    ...
                    if(!empty($_POST['polloption'])){
                        ...
                        foreach($_POST['polloption'] as $key => $val){
                            $option_info = $model->table('circle_thpolloption')->where([
                                'pollop_id' => $key,
                                'theme_id' => $this->t_id
                            ])->find();
                            if(!empty($option_info)){
                                ...
                                $model->table('circle_thpolloption')->where(['pollop_id' => $key])->update($update);
                            } else {
                                if($val == ''){
                                    continue;
                                }
    ...
    public function ajax_likeyesOp() {
        ...
        if(empty($like_info)){
            // 插入话题赞表
            Model()->table('circle_like')->insert([
                'theme_id' => $this->t_id,
                'member_id' => $_SESSION['member_id'],
                'circle_id' => $this->c_id
            ]);
    ...
    public function replyOp() {
        ...
        $this->themeInfo();
        if($_GET['answer_id'] != ''){
            $reply_info = Model()->table('circle_threply')->where([
                'theme_id' => $this->t_id,
                'reply_id' => intval($_GET['answer_id'])
            ])->find();
            if(!empty($reply_info)){
                Tpl::output('answer', $reply_info);
            }
        }
        // 附件信息
        $where = [];
    ...
    public function edit_replyOp() {
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                ["input" => $_POST["replycontent"], "require" => "true", "message" => L('circle_reply_not_null')],
            ];
            $error = $obj_validate->validate();
            if($error != ''){
                showDialog($error);
                ...
                $update['is_closed'] = 0;
                $rs = $model->table('circle_threply')->where([
                    'theme_id' => $this->t_id,
                    'reply_id' => $this->r_id
                ])->update($update);
                if($rs){
                    ...
                    $model->table('circle_affix')->where([
                        ...
                        'member_id' => $_SESSION['member_id'],
                        'reply_id' => 0
                    ])->update(['theme_id' => $this->t_id, 'reply_id' => $this->r_id, 'circle_id' => $this->c_id]);
        ...
        $where['member_id'] = $_SESSION['member_id'];
        $where['affix_type'] = 2;
        $where['reply_id'] = ['in', [0, $this->r_id]];
        $where['theme_id'] = ['in', [0, $this->t_id]];
        $affix_list = Model()->table('circle_affix')->where($where)->select();
        Tpl::output('affix_list', $affix_list);
        // 商品信息
        $where = [];
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if($_POST['answer_id'] != ''){
                    $reply_info = Model()->table('circle_threply')->where([
                        'theme_id' => $this->t_id,
                        'reply_id' => intval($_POST['answer_id'])
                    ])->find();
                    if(!empty($reply_info)){
                ...
                if($reply_id){
                    if($_GET['type'] == 'adv'){
                        ...
                        $model->table('circle_affix')->where([
                            ...
                            'member_id' => $_SESSION['member_id'],
                            'reply_id' => 0
                        ])->update(['theme_id' => $this->t_id, 'reply_id' => $reply_id, 'circle_id' => $this->c_id]);
                    }
                    // 话题被回复数增加 最后发言人发言时间
                    $update = [];
                    $update['theme_commentcount'] = ['exp', 'theme_commentcount+1'];
                    ...
                    $model->table('circle_member')->where([
                        'member_id' => $_SESSION['member_id'],
                        'circle_id' => $this->c_id
                    ])->update(['cm_comcount' => ['exp', 'cm_comcount+1'], 'cm_lastspeaktime' => time()]);
                    // set cookie of SEC
                    if(intval(C('circle_intervaltime')) > 0){
                        setNcCookie('circle_intervaltime', true, intval(C('circle_intervaltime')));
                    }
    ...
    public function del_replyOp() {
        ...
        // 删除回复
        $model->table('circle_threply')->where([
            'theme_id' => $this->t_id,
            'reply_id' => $this->r_id,
            'member_id' => $_SESSION['member_id']
        ])->delete();
