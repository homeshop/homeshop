<?php namespace App\Circle\Http\Controllers\P;

use App\Circle\Http\Controllers\BaseCirclePersonalControl;
 ...
 * Personal Center
 */
class  CenterControl extends BaseCirclePersonalControl {
    ...
    public function indexOp() {
        $model = Model();
        $theme_list = $model->table('circle_theme')->where(['member_id' => $this->m_id])->page(10)->order('theme_id desc')->select();
        if(!empty($theme_list)){
            ...
            // like
            $like_list = $model->table('circle_like')->where(['theme_id' => ['in', $themeid_array]])->select();
    ...
    public function likeingOp() {
        $model = Model();
        $like_array = $model->table('circle_like')->field('circle_id,theme_id')->where(['member_id' => $this->m_id])->order('theme_id desc')->page(10)->select();
        if(!empty($like_array)){
            ...
            $circleid_array = array_unique($circleid_array);
            // theme
            $theme_list = $model->table('circle_theme')->where(['theme_id' => ['in', $themeid_array]])->select();
            // affix
            $affix_list = $model->table('circle_affix')->where([
                'affix_type' => 1,
                'theme_id' => ['in', $themeid_array]
    ...
    public function my_groupOp() {
        $model = Model();
        $circlemember_array = $model->table('circle_member')->where(['member_id' => $this->m_id])->select();
        if(!empty($circlemember_array)){
            ...
            $circle_list = $model->table('circle')->where(['circle_id' => ['in', $circleid_array]])->select();
    ...
    public function my_recycledOp() {
        $model = Model();
        $recycle_list = $model->table('circle_recycle')->where(['member_id' => $_SESSION['member_id']])->order('recycle_id desc')->page(10)->select();
    ...
    public function clr_recycledOp() {
        Model()->table('circle_recycle')->where(['member_id' => $_SESSION['member_id']])->delete();
        ...
        switch($menu_type) {
            case 'theme':
                $menu_array = [
                    1 => [
                        'menu_key' => 'theme',
                        'menu_name' => L('p_center_published_theme'),
                        'menu_url' => 'index.php?act=p_center'
                    ],
                    2 => [
                        'menu_key' => 'likeing',
                        'menu_name' => L('p_center_liked_theme'),
                        'menu_url' => 'index.php?act=p_center&op=likeing'
                    ],
                ];
                break;
            case 'group':
                $menu_array = [
                    1 => [
                        'menu_key' => 'group',
                        'menu_name' => L('p_center_my_circle'),
                        'menu_url' => 'index.php?act=p_center&op=my_group'
                    ],
                ];
                break;
            case 'inform':
                $menu_array = [
                    1 => [
                        'menu_key' => 'inform',
                        'menu_name' => L('p_center_my_inform'),
                        'menu_url' => 'index.php?act=p_center&op=my_inform'
                    ],
                ];
                break;
            case 'recycled':
                $menu_array = [
                    1 => [
                        'menu_key' => 'recycled',
                        'menu_name' => L('p_center_my_recycled'),
                        'menu_url' => 'index.php?act=p_center&op=my_recycled'
                    ],
                ];
                break;
        }
