<?php namespace App\Circle\Http\Controllers\Theme;
 ...
 * Theme Inform
 */
class  InformControl extends BaseCircleControl {
    ...
    public function indexOp() {
        ...
        if(chksubmit()){
            $circle_info = $model->table('circle')->field('circle_name')->where(['circle_id' => $this->c_id])->find();
            if(!empty($circle_info)){
                echo '<script>DialogManager.close("inform");</script>';
            }
            $theme_info = $model->table('circle_theme')->field('theme_name')->where(['theme_id' => $t_id])->find();
            if(!empty($theme_info)){
                echo '<script>DialogManager.close("inform");</script>';
            }
            $insert = [];
            ...
            $update = [
                'new_informcount' => ['exp', 'new_informcount+1']
            ];
