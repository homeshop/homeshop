...
class  GroupControl extends BaseCircleThemeControl {
    ...
    public function indexOp() {
        ...
        $todaythcount = $model->table('circle_theme')->where([
            'theme_addtime' => ['egt', $dayBegin],
            'circle_id' => $this->c_id
        ])->count();
    ...
    public function applyOp() {
        ...
        // 圈子信息
        $this->circleInfo();
        if(in_array($this->identity, [1, 2, 3, 4])){
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                ["input" => $_POST["apply_content"], "require" => "true", "message" => L('circle_apply_content_null')],
                ["input" => $_POST["intro"], "require" => "true", "message" => L('circle_introduction_not_null')],
            ];
            ...
            if($error != ''){
                ...
                $model->table('circle_member')->pk(['member_id', 'circle_id'])->insert($insert, true);
                if(intval($this->circle_info['circle_joinaudit']) == 0){
                    // Update the number of members
                    $update = [
                        'circle_mcount' => ['exp', 'circle_mcount+1']
                    ];
                    ...
                    $update = [
                        'new_verifycount' => ['exp', 'new_verifycount+1']
                    ];
                    $model->table('circle')->where(['circle_id' => $this->c_id])->update($update);
    ...
    public function quitOp() {
        ...
        if(in_array($this->identity, [2, 3])){
            ...
            $update = [];
            $update['circle_mcount'] = ['exp', 'circle_mcount-1'];
            // Whether to apply for management
            $rs = Model()->table('circle_mapply')->where([
                'circle_id' => $this->c_id,
                'member_id' => $_SESSION['member_id']
            ])->find();
            if($rs){
                Model()->table('circle_mapply')->where([
                    'circle_id' => $this->c_id,
                    'member_id' => $_SESSION['member_id']
                ])->delete();
                $update['new_mapplycount'] = ['exp', 'new_mapplycount-1'];
            }
            // 更新圈子成员数
            Model()->table('circle')->where(['circle_id' => $this->c_id])->update($update);
        }
    ...
    public function group_membereditOp() {
        ...
        if(chksubmit()){
            ...
            // 会员信息
            $this->memberInfo();
            if(!in_array($this->identity, [1, 2, 3, 6])){
    ...
    public function group_goodsOp() {
        ...
        // 成员商品列表
        $model = Model();
        $cmid_list = $model->table('circle_member')->field('member_id')->where([
            'circle_id' => $this->c_id,
            'cm_state' => 1
        ])->select();
        $cmid_list = array_under_reset($cmid_list, 'member_id');
        $cmid_array = array_keys($cmid_list);
        $count = $model->table('sns_sharegoods')->where(['share_memberid' => ['in', $cmid_array]])->count();
        $goods_list = $model->table('sns_sharegoods,sns_goods')->join('left')->on('sns_sharegoods.share_goodsid=sns_goods.snsgoods_goodsid')->where([
            'sns_sharegoods.share_memberid' => [
                'in',
                $cmid_array
            ],
            'share_isshare|share_islike' => 1
        ])->order('share_id desc')->page(20, $count)->select();
        if(!empty($goods_list)){
            ...
            $goods_list = array_under_reset($goods_list, 'share_id');
            $shareid_array = array_keys($goods_list);
            Tpl::output('show_page', $model->showpage('2'));
            Tpl::output('goods_list', $goods_list);
            $pic_list = $model->cls()->table('sns_albumpic')->where(['item_id' => ['in', $shareid_array]])->select();
    ...
    public function manage_applyOp() {
        ...
        $this->circleInfo();
        // Verify membership
        $cm_info = $model->table('circle_member')->where([
            'circle_id' => $this->c_id,
            'cm_state' => 1,
            'is_identity' => 3
        ])->find();
        if(empty($cm_info) || $this->circle_info['mapply_open'] == 0 || $this->circle_info['mapply_ml'] > $cm_info['cm_level'] || $cm_info['is_identity'] == 1){
            if(chksubmit()){
                ...
                echo '<script>showError("' . L('circle_apply_error') . '");DialogManager.close("manage_apply");</script>';
                exit;
            }
        }
        // Ban repeated application
        $mapply_info = $model->table('circle_mapply')->where([
            'circle_id' => $this->c_id,
            'member_id' => $_SESSION['member_id']
        ])->find();
        if(!empty($mapply_info)){
            if(chksubmit()){
                ...
                echo '<script>showError("' . L('circle_repeat_apply_error') . '");DialogManager.close("manage_apply");</script>';
                exit;
            }
        }
        if(chksubmit()){
            ...
            $model->table('circle')->where(['circle_id' => $this->c_id])->update([
                'new_mapplycount' => [
                    'exp',
                    'new_mapplycount+1'
                ]
