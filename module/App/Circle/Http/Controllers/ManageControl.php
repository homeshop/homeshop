...
class  ManageControl extends BaseCircleManageControl {
    ...
    public function indexOp() {
        ...
        if(chksubmit()){
            ...
            if($_POST['c_mapply'] == 1){
                ...
                $model->table('circle_mapply')->where(['circle_id' => $this->c_id])->delete();
            }
            $model->table('circle')->where(['circle_id' => $this->c_id])->update($update);
            showDialog(L('nc_common_op_succ'), 'reload', 'succ');
        }
        $circle_info = $model->table('circle')->where(['circle_id' => $this->c_id])->find();
        Tpl::output('circle_info', $circle_info);
        // member level
        $ml_info = $model->table('circle_ml')->where(['circle_id' => $this->c_id])->find();
        if(empty($ml_info)){
    ...
    public function member_manageOp() {
        ...
        $where['cm_state'] = 1;
        if($_GET['mname'] != ''){
            $where['member_name'] = ['like', '%' . $_GET['mname'] . '%'];
        }
    ...
    public function delmemberOp() {
        ...
        if(chksubmit()){
            ...
            // 更新圈子成员数量
            $count = Model()->table('circle_member')->where(['circle_id' => $this->c_id, 'cm_state' => 1])->count();
            Model()->table('circle')->where(['circle_id' => $this->c_id])->update(['circle_mcount' => $count]);
            showDialog(L('nc_common_op_succ'), 'reload', 'succ');
        }
    ...
    public function applyingOp() {
        ...
        $this->memberJoinCircle();
        // 成员列表
        $cm_list = Model()->table('circle_member')->where([
            'circle_id' => $this->c_id,
            'cm_state' => 0
        ])->order('is_identity asc,cm_jointime desc')->select();
        Tpl::output('cm_list', $cm_list);
    ...
    public function applying_manageOp() {
        ...
        $count = $model->table('circle_member')->where(['circle_id' => $this->c_id, 'cm_state' => 0])->count();
        $model->table('circle')->where(['circle_id' => $this->c_id])->update(['new_verifycount' => $count]);
        showDialog(L('nc_common_op_succ'), 'reload', 'succ');
    }
    ...
    public function classOp() {
        ...
        $this->memberJoinCircle();
        $model = Model();
        $thclass_list = $model->table('circle_thclass')->where(['circle_id' => $this->c_id])->order('thclass_sort asc')->select();
    ...
    public function class_addOp() {
        ...
        if(chksubmit()){
            // 超过10不能继续添加
            $count = Model()->table('circle_thclass')->where(['circle_id' => $this->c_id])->count();
            if($count >= 10){
            ...
            $obj_validate->validateparam = [
                ["input" => $_POST["name"], "require" => "true", "message" => L('circle_tclass_name_not_null')],
                [
                    "input" => $_POST["sort"],
                    "require" => "true",
                    'validator' => 'Number',
                    "message" => L('circle_sort_error')
                ],
            ];
    ...
    public function class_editOp() {
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                $update['circle_id'] = $this->c_id;
                Model()->table('circle_thclass')->where([
                    'thclass_id' => intval($_POST['thc_id']),
                    'circle_id' => $this->c_id
                ])->update($update);
                showDialog(L('nc_common_op_succ'), 'reload', 'succ', 'CUR_DIALOG.close();');
            }
        ...
        $thclass_info = Model()->table('circle_thclass')->where([
            'circle_id' => $this->c_id,
            'thclass_id' => $thc_id
        ])->find();
    ...
    public function class_delOp() {
        ...
        $where = [];
        $where['circle_id'] = $this->c_id;
        $where['thclass_id'] = ['in', $thcid_array];
    ...
    public function friendshipOp() {
        ...
        // 会员加入圈子列表
        $this->memberJoinCircle();
        $fs_list = Model()->table('circle_fs')->where(['circle_id' => $this->c_id])->order('friendship_sort asc')->select();
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                [
                    ...
                    "require" => "true",
                    'validator' => 'Number',
                    "message" => L('fcircle_please_choose')
                ],
                [
                    "input" => $_POST["sort"],
                    "require" => "true",
    ...
    public function search_circleOp() {
        ...
        $model = Model();
        // 查询已加友情圈子
        $circle_list = $model->table('circle_fs')->where(['circle_id' => $this->c_id])->select();
        if(!empty($circle_list)){
            $circle_list = array_under_reset($circle_list, 'friendship_id');
            $circle_array = array_keys($circle_list);
        }
        $where = [];
        $where['circle_name'] = ['like', '%' . $_GET['name'] . '%'];
        $where['circle_status'] = 1;
        if(!empty($circle_array)){
            $circle_array[] = $this->c_id;
            $where['circle_id'] = ['not in', $circle_array];
        } else {
            $where['circle_id'] = ['neq', $this->c_id];
        }
    ...
    public function friendship_editOp() {
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                Model()->table('circle_fs')->where([
                    'circle_id' => $this->c_id,
                    'friendship_id' => intval($_GET['fs_id'])
                ])->update($update);
                showDialog(L('nc_common_op_succ'), 'reload', 'succ', 'CUR_DIALOG.close();');
            }
        }
        ...
        $friendship_info = Model()->table('circle_fs')->where([
            'circle_id' => $this->c_id,
            'friendship_id' => $fs_id
        ])->find();
    ...
    public function friendship_delOp() {
        ...
        $where = [];
        $where['circle_id'] = $this->c_id;
        $where['friendship_id'] = ['in', $fs_array];
    ...
    public function ajaxOp() {
        ...
        Model()->table('circle_theme')->where(['theme_id' => intval($_GET['id'])])->update($update);
        echo 'true';
        exit;
    }
    ...
    public function edit_themeOp() {
        ...
        if(chksubmit()){
            ...
            $rs = $model->table('circle_theme')->where(['theme_id' => $t_id])->update($update);
            if($rs){
                $has_goods = 0; // 存在商品标记
                $has_affix = 0;// 存在附件标记
                // 删除原有商品
                $goods_list = Model()->table('circle_thg')->where(['theme_id' => $t_id, 'reply_id' => 0])->delete();
                // 插入话题商品
                ...
                // 更新话题信息
                $affixe_count = $model->table('circle_affix')->where(['affix_type' => 1, 'theme_id' => $t_id])->count();
                if($affixe_count > 0){
                    $has_affix = 1;
                }
                if($has_goods || $has_affix){
                    ...
                    $model->table('circle_theme')->where(['theme_id' => $t_id])->update($update);
                }
                // Special theme
                if($_GET['sp'] == 1){
                    // Update the vote
                    ...
                    $model->table('circle_thpoll')->where(['theme_id' => $t_id])->update($update);
        ...
        // 话题商品
        $goods_list = $model->table('circle_thg')->where(['theme_id' => $t_id, 'reply_id' => 0])->select();
        $goods_list = tidyThemeGoods($goods_list, 'themegoods_id');
        Tpl::output('goods_list', $goods_list);
        // 话题附件
        $affix_list = $model->table('circle_affix')->where(['affix_type' => 1, 'theme_id' => $t_id])->select();
        ...
        if($this->theme_info['theme_special'] == 1){
            $poll_info = $model->table('circle_thpoll')->where(['theme_id' => $t_id])->find();
            Tpl::output('poll_info', $poll_info);
            $option_list = $model->table('circle_thpolloption')->where(['theme_id' => $t_id])->order('pollop_sort asc')->select();
    ...
    public function delimgOp() {
        ...
        // 验证是否为该圈子附件
        $theme_info = Model()->table('circle_theme')->where([
            'circle_id' => $this->c_id,
            'theme_id' => $affix_info['theme_id']
        ])->find();
        if(empty($theme_info)){
            echo false;
            exit;
    ...
    public function del_themeOp() {
        ...
        // 验证话题
        $t_id = intval($_GET['t_id']);
        $theme_info = $model->table('circle_theme')->where(['theme_id' => $t_id, 'circle_id' => $this->c_id])->find();
        if(empty($theme_info)){
            showDialog(L('wrong_argument'));
        }
        // 删除附件
        $affix_list = $model->table('circle_affix')->where(['theme_id' => $t_id])->select();
        if(!empty($affix_list)){
            foreach($affix_list as $val){
                @unlink(themeImagePath($val['affix_filename']));
                @unlink(themeImagePath($val['affix_filethumb']));
            }
            $model->table('circle_affix')->where(['theme_id' => $t_id])->delete();
        }
        // 删除商品
        $model->table('circle_thg')->where(['theme_id' => $t_id])->delete();
        // 删除赞表相关
        $model->table('circle_like')->where(['theme_id' => $t_id])->delete();
        // 删除回复
        $model->table('circle_threply')->where(['theme_id' => $t_id])->delete();
        ...
        // 删除话题
        $model->table('circle_theme')->where(['theme_id' => $t_id])->delete();
        // Experience
    ...
    public function del_replyOp() {
        ...
        // 删除附件
        $affix_list = $model->table('circle_affix')->where([
            'affix_type' => 2,
            'theme_id' => $t_id,
            'reply_id' => $r_id
        ])->select();
        ...
        // 删除商品
        $model->table('circle_thg')->where(['theme_id' => $t_id, 'reply_id' => $r_id])->delete();
        // The recycle bin add delete records
        $param = [];
        $param['theme_id'] = $t_id;
        $param['reply_id'] = $r_id;
        ...
        Model('circle_recycle')->saveRecycle($param);
        // 删除回复
        $rs = $model->table('circle_threply')->where(['theme_id' => $t_id, 'reply_id' => $r_id])->delete();
        // 更新话题回复数
        $model->table('circle_theme')->where(['theme_id' => $t_id])->update([
            'theme_commentcount' => [
                'exp',
                'theme_commentcount-1'
            ]
        ]);
        // Experience
    ...
    public function ajax_nospeakOp() {
        ...
        if(in_array($m_info['is_identity'], [1, 2])){
