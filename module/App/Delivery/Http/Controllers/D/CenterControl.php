<?php namespace App\Delivery\Http\Controllers\D;
 ...
 * 物流自提服务站首页
 */
class  CenterControl extends BaseDeliveryCenterControl {
    ...
    public function indexOp() {
        ...
        if($_GET['search_name'] != ''){
            $where['order_sn|shipping_code|reciver_mobphone'] = ['like', '%' . $_GET['search_name'] . '%'];
    ...
    public function informationOp() {
        ...
        $delivery_info = $model_dp->getDeliveryPointInfo(['dlyp_id' => $_SESSION['dlyp_id']]);
    ...
    public function change_passwordOp() {
        if(chksubmit()){
            ...
            $model_dp->editDeliveryPoint(['dlyp_passwd' => md5($_POST['password'])], $where);
    ...
    public function arrive_pointOp() {
        ...
        Model('delivery_order')->editDeliveryOrderArrive($update, [
            'order_id' => $order_id,
            'dlyp_id' => $_SESSION['dlyp_id']
        ]);
        // 更新订单扩展表数据
        Model('order')->editOrderCommon($update, ['order_id' => $order_id]);
        // 发送短信提醒
        QueueClient::push('sendPickupcode', ['pickup_code' => $pickup_code, 'order_id' => $order_id]);
    ...
    public function pickup_parcelOp() {
        if(chksubmit()){
            ...
            $model_do = Model('delivery_order');
            $dorder_info = $model_do->getDeliveryOrderInfo([
                'order_id' => $order_id,
                'dlyp_id' => $_SESSION['dlyp_id'],
                'dlyo_pickup_code' => $pickup_code
            ]);
            if(empty($dorder_info)){
                showDialog('提货码错误', '', 'error', 'DialogManager.close("pickup_parcel")');
            }
            $result = $model_do->editDeliveryOrderPickup([], [
                'order_id' => $order_id,
                'dlyp_id' => $_SESSION['dlyp_id'],
                'dlyo_pickup_code' => $pickup_code
            ]);
            if($result){
                // 更新订单状态
                $order_info = Model('order')->getOrderInfo(['order_id' => $order_id]);
