...
class StoreControl extends MobileHomeControl {
    ...
    public function store_infoOp() {
        ...
        // 店铺头像
        $store_info['store_avatar'] = $store_online_info['store_avatar'] ? UPLOAD_SITE_URL . '/' . ATTACH_STORE . '/' . $store_online_info['store_avatar'] : UPLOAD_SITE_URL . '/' . ATTACH_COMMON . DS . C('default_store_avatar');
        // 商品数
        ...
        if($store_info['is_own_shop']){
            ...
            $store_info['store_credit_text'] = sprintf('描述: %0.1f, 服务: %0.1f, 物流: %0.1f', $store_online_info['store_credit']['store_desccredit']['credit'], $store_online_info['store_credit']['store_servicecredit']['credit'], $store_online_info['store_credit']['store_deliverycredit']['credit']);
        }
        // 页头背景图
    ...
    public function store_goodsOp() {
        ...
        $condition['is_book'] = 0;
        if($stc_id > 0){
            $condition['goods_stcids'] = ['like', '%,' . $stc_id . ',%'];
        }
        //促销类型
        if($param['prom_type']){
    ...
    public function store_creditOp() {
        ...
        output_data(['store_credit' => $store_online_info['store_credit']]);
    }
    ...
    public function store_goods_rankOp() {
        ...
        $goods_list = $this->_goods_list_extend($goods_list);
        output_data(['goods_list' => $goods_list]);
    }
    ...
    public function store_introOp() {
        ...
        $store_info['store_collect'] = (int)$store_online_info['store_collect'];
        //店铺所属分类
        $store_class = Model('store_class')->getStoreClassInfo(['sc_id' => $store_info['sc_id']]);
        ...
        output_data(['store_info' => $store_info]);
    }
    ...
    public function store_promotionOp() {
        ...
        if(in_array('xianshi', $fields_arr)){
            ...
            $where['store_id'] = $store_id;
            $where['state'] = 1;
            $where['start_time'] = ['elt', TIMESTAMP];
            $where['end_time'] = ['egt', TIMESTAMP];
        ...
        output_data(['promotion' => $promotion_arr]);
    }
}
