...
class AutoControl extends MobileMemberControl {
    ...
    public function checkAuthOp() {
        ...
        if(isset($_GET['code'])){
            ...
            $model_member = Model('member');
            $member_info = $model_member->getMemberInfo(['weixin_unionid' => $res5['unionid']]);
            if(!empty($member_info)){
    ...
    private function register($user_info) {
        ...
        if(!empty($unionid)){
            ...
            $model_member = Model('member');
            $member_info = $model_member->getMemberInfo(['member_name' => $member_name]);
            ...
            if($avatar){
                $model_member->editMember(['member_id' => $result], ['member_avatar' => "avatar_$result.jpg"]);
            }
