...
class SpecialControl extends SystemControl {
    ...
    public function get_special_xmlOp() {
        ...
        if(!empty($list) && is_array($list)){
            $fields_array = ['special_id', 'special_desc'];
            foreach($list as $k => $v){
                $out_array = getFlexigridArray([], $fields_array, $v);
                $out_array['special_desc'] = '<span nc_type="edit_special_desc" column_id="' . $v['special_id'] . '" title="可编辑" class="editable tooltip w270">' . $v['special_desc'] . '</span>';
                $operation = '';
                $operation .= '<a class="btn red" href="javascript:fg_operation_del(' . $v['special_id'] . ');"><i class="fa fa-trash-o"></i>删除</a>';
                $operation .= '<a class="btn blue" href="' . urlAdminMobile('mb_special', 'special_edit', ['special_id' => $v['special_id']]) . '"><i class="fa fa-pencil-square-o"></i>编辑</a>';
    ...
    public function update_item_sortOp() {
        ...
        if(!empty($item_id_string)){
            ...
            foreach($item_id_array as $item_id){
                $result = $model_mb_special->editMbSpecialItemByID(['item_sort' => $index], $item_id, $special_id);
                $index++;
            }
        }
        $data = [];
    ...
    private function show_menu($menu_key = '') {
        ...
        if($menu_key == 'index_edit'){
            $menu_array[] = ['menu_key' => 'index_edit', 'menu_name' => '首页', 'menu_url' => 'javascript:;'];
            $menu_array[] = [
                'menu_key' => 'special_list',
                'menu_name' => '专题',
                'menu_url' => urlAdminMobile('mb_special', 'special_list')
            ];
        } else {
            $menu_array[] = [
                'menu_key' => 'index_edit',
                'menu_name' => '首页',
                'menu_url' => urlAdminMobile('mb_special', 'index')
            ];
            $menu_array[] = ['menu_key' => 'special_list', 'menu_name' => '专题', 'menu_url' => 'javascript:;'];
        }
        if($menu_key == 'index_edit'){
