...
class GoodsControl extends MobileHomeControl {
    ...
    public function goods_listOp() {
        ...
        if(!is_null($goods_list)){
            ...
            if($price_from && $price_from){
                ...
                $condition['goods_promotion_price'] = ['egt', $price_from];
            } elseif($price_to) {
                $condition['goods_promotion_price'] = ['elt', $price_to];
            }
            if($_GET['gift'] == 1){
            ...
            if($_tmp){
                $condition['goods_promotion_type'] = ['in', $_tmp];
            }
            unset($_tmp);
            //虚拟商品
            if($_GET['virtual'] == 1){
    ...
    private function _goods_list_extend($goods_list) {
        ...
        $sole_array = Model('p_sole')->getSoleGoodsList(['goods_id' => ['in', $goodsid_array]]);
    ...
    public function goods_detailOp() {
        ...
        $goods_list = $model_goods->getGoodsContract([0 => $goods_detail['goods_info']]);
        ...
        if($member_id = $this->getMemberIdIfExists()){
            ...
            QueueClient::push('addViewedGoods', ['goods_id' => $goods_id, 'member_id' => $member_id]);
            if(!$goods_detail['goods_info']['is_virtual']){
                ...
                $condition['voucher_t_store_id'] = ['in', $store_info['store_id']];
            ...
            if($promotion_type == 'groupbuy'){
                ...
                if($upper_limit > 0){
                    ...
                    $order_goods_list = $model_order->getOrderGoodsList([
                        'goods_id' => $goods_id,
                        'buyer_id' => $buyer_id,
                        'goods_type' => 2
                    ]);
                    if($order_goods_list){
                        ...
                        $groupbuy_info = $model_groupbuy->getGroupbuyInfo(['groupbuy_id' => $promotions_id]);
                        if($groupbuy_info){
                            $IsHaveBuy = 1;
                        } else {
                            $IsHaveBuy = 0;
        ...
        // 评价列表
        $goods_eval_list = Model("evaluate_goods")->getEvaluateGoodsList(['geval_goodsid' => $goods_id], null, '3');
    ...
    private function _goods_detail_extend($goods_detail) {
        ...
        //商品链接
        $goods_detail['goods_info']['goods_url'] = urlShop('goods', 'index', ['goods_id' => $goods_detail['goods_info']['goods_id']]);
        //整理数据
    ...
    public function goods_evaluateOp() {
        ...
        switch($type) {
            case '1':
                $condition['geval_scores'] = ['in', '5,4'];
                break;
            case '2':
                $condition['geval_scores'] = ['in', '3,2'];
                break;
            case '3':
                $condition['geval_scores'] = ['in', '1'];
                break;
            case '4':
                $condition['geval_image|geval_image_again'] = ['neq', ''];
                break;
            case '5':
                $condition['geval_content_again'] = ['neq', ''];
                break;
        }
        //查询商品评分信息
        ...
        output_data(['goods_eval_list' => $goods_eval_list], mobile_page($page_count));
    }
    ...
    public function _calc($area_id, $goods_id) {
        $goods_info = Model('goods')->getGoodsInfo(['goods_id' => $goods_id], 'transport_id,store_id,goods_freight,goods_storage');
        ...
        return [
            'content' => $freight_total,
            'if_store_cn' => $if_store === false ? '无货' : ($goods_info['goods_storage'] > 0 ? '有货' : '无货'),
            'if_store' => $if_store === false ? false : true,
            'area_name' => $area_name ? $area_name : '全国'
        ];
    }
    ...
    public function store_o2o_addrOp() {
        ...
        output_data(['addr_list' => $map_new_list]);
    }
    
    public function auto_completeOp() {
        $data = Model('search')->autoComplete(['term' => $_GET['term']]);
        foreach($data as $k => $v){
            $data[ $k ] = $v['value'];
        }
