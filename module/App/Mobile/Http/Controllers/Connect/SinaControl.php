<?php namespace App\Mobile\Http\Controllers\Connect;
 ...
 * 新浪微博登录 v3-b13
 */
class SinaControl extends MobileHomeControl {
    ...
    public function registerOp() {
        ...
        if(chksubmit()){
            ...
            $model_member->editMember(['member_id' => $_SESSION['member_id']], $update_info);
            ...
            $check_member_name = $model_member->getMemberInfo(['member_name' => trim($user_array['member_name'])]);
            $result = 0;
            ...
            if($result){
                ...
                if($avatar){
                    ...
                    $model_member->editMember(['member_id' => $result], $update_info);
                }
                $member_info = $model_member->getMemberInfo(['member_name' => $user_array['member_name']]);
    ...
    public function bindsinaOp() {
        ...
        $sinauser_info = $c->show_user_by_id($_SESSION['slast_key']['uid']);//根据ID获取用户等基本信息
        $sina_str = serialize($sinauser_info);
        $edit_state = $model_member->editMember(['member_id' => $_SESSION['member_id']], [
