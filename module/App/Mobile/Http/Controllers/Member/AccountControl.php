 ...
 * 我的钱
 */
class AccountControl extends MobileMemberControl {
    ...
    public function bind_mobile_step1Op() {
        ...
        //发送频率验证
        $member_common_info = $model_member->getMemberCommonInfo(['member_id' => $this->member_info['member_id']]);
        if(!empty($member_common_info['send_acode_time'])){
            if(date('Ymd', $member_common_info['send_acode_time']) != date('Ymd', TIMESTAMP)){
                ...
                $update = $model_member->editMemberCommon($data, ['member_id' => $this->member_info['member_id']]);
            } else {
        ...
        try {
            ...
            $tpl_info = Model('mail_templates')->getTplInfo(['code' => 'authenticate']);
            $param = [];
            ...
            if($result){
                ...
                $update = $model_member->editMemberCommon($update_data, ['member_id' => $this->member_info['member_id']]);
                if(!$update){
                    output_error('系统发生错误');
                }
                $data = [];
                $data['member_mobile'] = $_POST['mobile'];
                $update = $model_member->editMember(['member_id' => $this->member_info['member_id']], $data);
                if(!$update){
                    output_error('系统发生错误');
                }
                output_data(['sms_time' => DEFAULT_CONNECT_SMS_TIME]);
            } else {
    ...
    public function bind_mobile_step2Op() {
        ...
        if($member_common_info['auth_code'] != $_POST['auth_code']){
            $data = [];
            $update_data['auth_code_check_times'] = ['exp', 'auth_code_check_times+1'];
            $update = $model_member->editMemberCommon($update_data, ['member_id' => $this->member_info['member_id']]);
            if(!$update){
                output_error('系统发生错误');
            }
        ...
        $update = $model_member->editMember(['member_id' => $this->member_info['member_id']], ['member_mobile_bind' => 1]);
        if(!$update){
            output_error('系统发生错误');
        }
        output_data('1');
    ...
    public function _modify_pwd_check_vcode() {
        ...
        //更改密码授权
        $update = $model_member->editMemberCommon(['auth_modify_pwd_time' => TIMESTAMP], ['member_id' => $this->member_info['member_id']]);
        if(!$update){
            output_error('系统发生错误');
        }
        output_data('1');
    ...
    public function modify_password_step5Op() {
        ...
        $this->_modify_pwd_limit_check();
        $model_member = Model('member');
        $update = $model_member->editMember(['member_id' => $this->member_info['member_id']], ['member_passwd' => md5($_POST['password'])]);
        if(!$update){
            output_error('密码修改失败');
        }
        $update = $model_member->editMemberCommon(['auth_modify_pwd_time' => '0'], ['member_id' => $this->member_info['member_id']]);
        if(!$update){
            output_error('系统发生错误');
        }
        output_data('1');
    ...
    public function modify_paypwd_step5Op() {
        ...
        $this->_modify_pwd_limit_check();
        $model_member = Model('member');
        $update = $model_member->editMember(['member_id' => $this->member_info['member_id']], ['member_paypwd' => md5($_POST['password'])]);
        if(!$update){
            output_error('密码修改失败');
        }
        $update = $model_member->editMemberCommon(['auth_modify_pwd_time' => '0'], ['member_id' => $this->member_info['member_id']]);
