...
class OrderControl extends MobileMemberControl {
    ...
    public function order_listOp() {
        ...
        if($_POST['state_type'] != ''){
            $condition['order_state'] = str_replace(['state_new', 'state_send', 'state_noeval'], [
                ORDER_STATE_NEW,
                ORDER_STATE_SEND,
                ORDER_STATE_SUCCESS
            ], $_POST['state_type']);
        ...
        if(preg_match('/^\d{10,20}$/', $_POST['order_key'])){
            ...
            $condition['order_id'] = ['in', $this->_getOrderIdByKeyword($_POST['order_key'])];
        }
        $order_list_array = $model_order->getNormalOrderList($condition, $this->page, '*', 'order_id desc', '', ['order_goods']);
    ...
    public function get_current_deliverOp() {
        ...
        if(empty($content)){
            ...
            foreach($content as $k => $v){
                ...
                output_data(['deliver_info' => $content[0]]);
            }
            output_error('物流信息查询失败');
        }
    }
