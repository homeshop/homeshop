 ...
 * 会员退货
 */
class ReturnControl extends MobileMemberControl {
    ...
    public function get_return_listOp() {
        ...
        if(!empty($list) && is_array($list)){
            ...
            $spec_list = [];
            $goods_list = [];
            $goods_list = $model_order->getOrderGoodsList(['rec_id' => ['in', $rec_id_list]]);
        ...
        output_data(['return_list' => $refund_list], mobile_page($page_count));
    }
    ...
    public function get_return_infoOp() {
        ...
        if(!empty($refund_info) && is_array($refund_info)){
            ...
            output_data(['return_info' => $refund, 'pic_list' => $pic_list, 'detail_array' => $detail_array]);
        } else {
            output_error('参数错误');
        }
    }
