 ...
 * 我的地址
 */
class AddressControl extends MobileMemberControl {
    ...
    public function address_listOp() {
        $address_list = $this->model_address->getAddressList(['member_id' => $this->member_info['member_id']]);
        output_data(['address_list' => $address_list]);
    }
    ...
    public function address_editOp() {
        ...
        $address_info = $this->_address_valid();
        if($_POST['is_default']){
            $this->model_address->editAddress(['is_default' => 0], [
                'member_id' => $this->member_info['member_id'],
                'is_default' => 1
            ]);
        }
        $result = $this->model_address->editAddress($address_info, [
    ...
    private function _address_valid() {
        ...
        $obj_validate->validateparam = [
            ...
            ["input" => $_POST["area_info"], "require" => "true", "message" => '地区不能为空'],
            ["input" => $_POST["address"], "require" => "true", "message" => '地址不能为空'],
            ["input" => $_POST['tel_phone'] . $_POST['mob_phone'], 'require' => 'true', 'message' => '联系方式不能为空']
        ];
