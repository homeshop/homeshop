<?php namespace App\Mobile\Http\Controllers\Member\Vr;
...
class OrderControl extends MobileMemberControl {
    ...
    public function order_listOp() {
        ...
        if(preg_match('/^\d{10,20}$/', $_POST['order_key'])){
            ...
            $condition['goods_name'] = ['like', '%' . $_POST['order_key'] . '%'];
        }
        if($_POST['state_type'] != ''){
            $condition['order_state'] = str_replace(['state_new', 'state_pay'], [
                ORDER_STATE_NEW,
                ORDER_STATE_PAY
            ], $_POST['state_type']);
        }
    ...
    public function resendOp() {
        ...
        $model_vr_order->editOrder(['vr_send_times' => ['exp', 'vr_send_times+1']], ['order_id' => $order_id]);
        output_data('1');
    }
}
