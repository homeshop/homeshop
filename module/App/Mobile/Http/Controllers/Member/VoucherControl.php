...
class VoucherControl extends MobileMemberControl {
    ...
    public function voucher_listOp() {
        ...
        output_data(['voucher_list' => $voucher_list], mobile_page($page_count));
    }
    ...
    public function voucher_pwexOp() {
        ...
        // 查询代金券
        $model_voucher = Model('voucher');
        $voucher_info = $model_voucher->getVoucherInfo(['voucher_pwd' => md5($pwd_code)]);
        if(!$voucher_info){
        ...
        if($result){
            // 更新代金券模板
            $update_arr = [];
            $update_arr['voucher_t_giveout'] = ['exp', 'voucher_t_giveout+1'];
            $model_voucher->editVoucherTemplate(['voucher_t_id' => $voucher_info['voucher_t_id']], $update_arr);
            output_data('1');
        } else {
