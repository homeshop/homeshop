 ...
 * 会员退款
 */
class RefundControl extends MobileMemberControl {
    ...
    public function refund_all_formOp() {
        ...
        if(!empty($order_info) && is_array($order_info)){
            ...
            output_data(['order' => $order, 'goods_list' => $goods_list, 'gift_list' => $gift_list]);
        } else {
            output_error('参数错误');
        }
    }
    ...
    public function refund_formOp() {
        ...
        if($refund_state == 1 && $goods_id > 0){
            ...
            output_data(['order' => $order, 'goods' => $goods, 'reason_list' => $new_reason_list]);
        } else {
            output_error('参数错误');
        }
    }
    ...
    public function refund_postOp() {
        ...
        if($refund_state == 1 && $goods_id > 0 && empty($refund)){
            ...
            $reason_list = $model_refund->getReasonList([], '', '', 'reason_id,reason_info');//退款退货原因
    ...
    public function upload_picOp() {
        ...
        if($result){
            ...
            output_data(['file_name' => $file_name, 'pic' => $pic]);
        } else {
            output_error('图片上传失败');
        }
    }
    ...
    public function get_refund_listOp() {
        ...
        output_data(['refund_list' => $refund_list], mobile_page($page_count));
    }
    ...
    public function get_refund_infoOp() {
        ...
        if(!empty($refund_info) && is_array($refund_info)){
            ...
            $detail_info = $model_refund->getDetailInfo(['refund_id' => $refund_info['refund_id']]);
            $detail_array = [];
            ...
            output_data(['refund' => $refund, 'pic_list' => $pic_list, 'detail_array' => $detail_array]);
        } else {
            output_error('参数错误');
        }
    }
