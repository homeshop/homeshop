...
class CartControl extends MobileMemberControl {
    ...
    public function cart_listOp() {
        ...
        foreach($cart_list as $cart){
            ...
            $cart['xianshi_info'] = $cart['xianshi_info'] ? $cart['xianshi_info'] : [];
            $cart['groupbuy_info'] = $cart['groupbuy_info'] ? $cart['groupbuy_info'] : [];
        ...
        output_data([
            'cart_list' => array_values($store_cart_list),
    ...
    public function cart_list_oldOp() {
        ...
        output_data(['cart_list' => $cart_list, 'sum' => ncPriceFormat($sum)]);
    }
    ...
    public function cart_edit_quantityOp() {
        ...
        $model_cart = Model('cart');
        $cart_info = $model_cart->getCartInfo(['cart_id' => $cart_id, 'buyer_id' => $this->member_info['member_id']]);
        //检查是否为本人购物车
        ...
        $update = $model_cart->editCart($data, ['cart_id' => $cart_id]);
        if($update){
            $return = [];
    ...
    public function cart_batchaddOp() {
        ...
        $cartlist_arr = $cartlist_str ? explode('|', $cartlist_str) : [];
        if(!$cartlist_arr){
            output_error('参数错误');
        }
        $cartlist_new = [];
