 ...
 * 支付
 */
class PaymentControl extends MobileMemberControl {
    ...
    public function __construct() {
        ...
        if($_GET['op'] != 'payment_list' && !$_POST['payment_code']){
            $payment_code = 'alipay';
            if(in_array($_GET['op'], ['wx_app_pay', 'wx_app_pay3', 'wx_app_vr_pay', 'wx_app_vr_pay3'], true)){
                $payment_code = 'wxpay';
            } else {
                if(in_array($_GET['op'], ['alipay_native_pay', 'alipay_native_vr_pay'], true)){
    ...
    public function pay_newOp() {
        ...
        if(in_array($_GET['payment_code'], ['alipay', 'wxpay_jsapi'])){
            $model_mb_payment = Model('mb_payment');
            $condition = [];
     ...
     * 获取订单支付信息
     */
    private function _get_real_order_info($pay_sn, $rcb_pd_pay = []) {
        ...
        $update = Model('order')->editOrder(['api_pay_time' => TIMESTAMP], ['order_id' => ['in', $pay_order_id_list]]);
        if(!$update){
            return ['error' => '更新订单信息发生错误，请重新支付'];
        }
        //如果是开始支付尾款，则把支付单表重置了未支付状态，因为支付接口通知时需要判断这个状态
        if($result['data']['if_buyer_repay']){
            $update = Model('order')->editOrderPay(['api_pay_state' => 0], ['pay_id' => $result['data']['pay_id']]);
            if(!$update){
                return ['error' => '订单支付失败'];
            }
     ...
     * 获取虚拟订单支付信息
     */
    private function _get_vr_order_info($pay_sn, $rcb_pd_pay = []) {
        $logic_payment = Logic('payment');
        //取得订单信息
        ...
        $result['data']['api_pay_amount'] = ncPriceFormat($pay_amount);
        $update = Model('order')->editOrder(['api_pay_time' => TIMESTAMP], ['order_id' => $result['data']['order_id']]);
        if(!$update){
            return ['error' => '更新订单信息发生错误，请重新支付'];
        }
        //计算本次需要在线支付的订单总金额
    ...
    public function payment_listOp() {
        ...
        output_data(['payment_list' => $payment_array]);
    }
    ...
    public function alipay_native_payOp() {
        ...
        $prestr = 'partner="' . $payment_api->param['partner'] . '"&seller_id="' . $payment_api->param['seller_id'] . '"&out_trade_no="' . $payment_api->param['out_trade_no'] . '"&subject="' . $payment_api->param['subject'] . '"&body="r"&total_fee="' . $payment_api->param['total_fee'] . '"&notify_url="' . $payment_api->param['notify_url'] . '"&service="mobile.securitypay.pay"&payment_type="1"&_input_charset="utf-8"&it_b_pay="1"';
        $mysign = $payment_api->mySign($prestr);
        output_data(['signStr' => $prestr . '&sign_type="RSA"&sign="' . urlencode($mysign) . '"']);
    }
    ...
    public function alipay_native_vr_payOp() {
        ...
        $payment_api = new $this->payment_code();
        $payment_api->init($this->payment_config, $pay_info['data']);
        $prestr = 'partner="' . $payment_api->param['partner'] . '"&seller_id="' . $payment_api->param['seller_id'] . '"&out_trade_no="' . $payment_api->param['out_trade_no'] . '"&subject="' . $payment_api->param['subject'] . '"&body="v"&total_fee="' . $payment_api->param['total_fee'] . '"&notify_url="' . $payment_api->param['notify_url'] . '"&service="mobile.securitypay.pay"&payment_type="1"&_input_charset="utf-8"&it_b_pay="1"';
        $mysign = $payment_api->mySign($prestr);
        output_data(['signStr' => $prestr . '&sign_type="RSA"&sign="' . urlencode($mysign) . '"']);
    }
}
