 ...
 * 会员聊天
 */
class ChatControl extends MobileMemberControl {
    ...
    public function get_node_infoOp() {
        $output_data = [
            'node_chat' => C('node_chat'),
            'node_site_url' => NODE_SITE_URL,
            'resource_site_url' => RESOURCE_SITE_URL
        ];
    ...
    public function get_user_listOp() {
        ...
        if(intval($_POST['recent']) != 1){
            $member_list = $model_chat->getFriendList(['friend_frommid' => $member_id], $n, $member_list);
        }
        $add_time = date("Y-m-d");
        $add_time30 = strtotime($add_time) - 60 * 60 * 24 * 30;
        $member_list = $model_chat->getRecentList([
            'f_id' => $member_id,
            'add_time' => ['egt', $add_time30]
        ], 10, $member_list);
        $member_list = $model_chat->getRecentFromList([
            't_id' => $member_id,
            'add_time' => ['egt', $add_time30]
        ], 10, $member_list);
        $member_info = [];
        ...
        output_data(['node_info' => $node_info, 'member_info' => $member_info, 'list' => $member_list]);
    }
    ...
    public function get_infoOp() {
        ...
        $member = [];
        $model_chat = Model('web_chat');
        $types = ['member_id', 'member_name', 'store_id', 'member'];
        $key = $_POST['t'];
    ...
    public function get_chat_logOp() {
        ...
        if(trim($key) != '' && array_key_exists($key, $time_from)){
            ...
            output_data(['list' => $list], mobile_page($total_page));
        }
    }
}
