 ...
 * 我的好友
 */
class SnsfriendControl extends MobileMemberControl {
    ...
    public function member_listOp() {
        ...
        $condition['member_id'] = ['neq', $this->member_info['member_id']];
        $condition['member_name'] = ['like', '%' . trim($_POST['m_name']) . '%'];//会员名称
    ...
    public function friend_listOp() {
        ...
        $friend_list = $model_chat->getFriendList(['friend_frommid' => $member_id], $this->page);
        $page_count = $model_chat->gettotalpage();
        output_data(['friend_list' => $friend_list], mobile_page($page_count));
    }
    ...
    public function friend_addOp() {
        ...
        $count = $model_snsfriend->countFriend(['friend_tomid' => $m_id, 'friend_frommid' => $self_info['member_id']]);
        ...
        $friend_info = $model_snsfriend->getFriendRow([
            'friend_frommid' => $m_id,
        ...
        if($result){
            if(!empty($friend_info)){//更新对方关注状态
                $model_snsfriend->editFriend(['friend_followstate' => '2'], ['friend_id' => $friend_info['friend_id']]);
            }
            output_data('1');
        } else {
            output_error('添加好友失败');
    ...
    public function friend_delOp() {
        ...
        if($result){
            //更新对方的关注状态
            $model_snsfriend->editFriend(['friend_followstate' => '1'], [
                'friend_frommid' => $m_id,
                'friend_tomid' => $this->member_info['member_id']
            ]);
            output_data('1');
        } else {
            output_error('删除好友失败');
