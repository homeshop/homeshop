 ...
 * 商家注销
 */
class StoreControl extends MobileSellerControl {
    ...
    public function store_infoOp() {
        ...
        output_data(['store_info' => $this->store_info]);
    }
    ...
    public function store_editOp() {
        ...
        $result = Model('store')->editStore($param, ['store_id' => $this->store_info['store_id']]);
        if(!$result){
            output_error('编辑失败');
        }
        output_data('1');
    ...
    public function store_statisticsOp() {
        ...
        // 月销量 月订单
        $condition = [];
        $condition['order_add_time'] = ['gt', $start_time];
        ...
        $condition['stattime'] = ['gt', $start_time];
        $condition['type'] = 'sum';
