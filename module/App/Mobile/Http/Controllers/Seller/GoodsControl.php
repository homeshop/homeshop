 ...
 * 商家注销
 */
class GoodsControl extends MobileSellerControl {
    ...
    public function goods_addOp() {
        ...
        unset($_POST['key']);
        $result = $logic_goods->saveGoods($_POST, $this->seller_info['store_id'], $this->store_info['store_name'], $this->store_info['store_state'], $this->seller_info['seller_id'], $this->seller_info['seller_name'], $this->store_info['bind_all_gc']);
        if(!$result['state']){
            output_error($result['msg']);
        }
        output_data(['common_id' => $result['data']]);
    }
    ...
    public function goods_listOp() {
        ...
        if(trim($keyword) != ''){
            switch($search_type) {
                case 0:
                    $condition['goods_name'] = ['like', '%' . trim($keyword) . '%'];
                    break;
                case 1:
                    $condition['goods_serial'] = ['like', '%' . trim($keyword) . '%'];
                    break;
                case 2:
    ...
    public function goods_infoOp() {
        ...
        $where = ['goods_commonid' => $common_id, 'store_id' => $this->store_info['store_id']];
        ...
        if(is_array($goods_array) && !empty($goods_array)){
            ...
            // 取得已选择了哪些商品的属性
            $attr_checked_l = $model_type->typeRelatedList('goods_attr_index', ['goods_id' => intval($goods_array[0]['goods_id'])], 'attr_id,attr_value_id');
    ...
    public function goods_editOp() {
        $logic_goods = Logic('goods');
        unset($_POST['key']);
        $result = $logic_goods->updateGoods($_POST, $this->seller_info['store_id'], $this->store_info['store_name'], $this->store_info['store_state'], $this->seller_info['seller_id'], $this->seller_info['seller_name'], $this->store_info['bind_all_gc']);
        if(!$result['state']){
            output_error($result['msg']);
        }
        output_data(['common_id' => $result['data']]);
