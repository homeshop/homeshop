 ...
 * 商家销售统计
 */
class StatControl extends MobileSellerControl {
    ...
    private function ordersstat($data_type) {
        ...
        $where['order_add_time'] = ['between', [$this->search_arr['stime'], $this->search_arr['etime']]];
    ...
    public function goodssales($data_type) {
        ...
        foreach((array)$statlist as $k => $v){
            $y_data[ $k ] = ['name' => strval($v['goods_name']), 'y' => intval($v[ $data_type ])];
        }
        output_data(['x' => $x_data, 'y' => $y_data]);
    }
    ...
    public function goodsflowOp() {
        ...
        $where = [];
        $where['store_id'] = $this->store_info['store_id'];
        $where['stattime'] = ['between', [$this->search_arr['stime'], $this->search_arr['etime']]];
        $where['type'] = 'goods';
        ...
        if($statlist_tmp){
            ...
            //查询相应商品
            $goods_list_tmp = $model_stat->statByGoods(['goods_id' => ['in', $goodsid_arr]], 'goods_name,goods_id');
            ...
            foreach((array)$statlist_tmp as $k => $v){
                ...
                $y_data[ $k ] = [
                    'name' => strval($goods_list[ $v['goods_id'] ]['goods_name']),
                    'y' => floatval($v['allnum'])
                ];
            }
        }
    ...
    public function areasales($data_type) {
        ...
        foreach((array)$statlist as $k => $v){
            ...
            if($province_id){
                //数据
                $y_data[] = ['name' => strval($province_array[ $province_id ]), 'y' => $v[ $data_type ]];
                //横轴
                //$x_data[] = strval($province_array[$province_id]);
            } else {
                //数据
                $y_data[] = ['name' => '未知', 'y' => $v[ $data_type ]];
                //横轴
                //$x_data[] = '未知';
            }
        }
