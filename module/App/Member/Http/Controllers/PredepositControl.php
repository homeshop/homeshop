...
class  PredepositControl extends BaseMemberControl {
    ...
    public function rcb_log_listOp() {
        $model = Model();
        $list = $model->table('rcb_log')->where([
    ...
    public function pd_cash_addOp() {
        if(chksubmit()){
            ...
            $validate_arr[] = [
                ...
                'operator' => '>=',
                "to" => '0.01',
                "message" => Language::get('predeposit_cash_add_pricemin_error')
            ];
            $validate_arr[] = [
                "input" => $_POST["pdc_bank_name"],
                "require" => "true",
                "message" => Language::get('predeposit_cash_add_shoukuanbanknull_error')
            ];
            $validate_arr[] = [
                "input" => $_POST["pdc_bank_no"],
                "require" => "true",
                "message" => Language::get('predeposit_cash_add_shoukuanaccountnull_error')
            ];
            $validate_arr[] = [
                "input" => $_POST["pdc_bank_user"],
                "require" => "true",
                "message" => Language::get('predeposit_cash_add_shoukuannamenull_error')
            ];
            $validate_arr[] = ["input" => $_POST["password"], "require" => "true", "message" => '请输入支付密码'];
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        $menu_array = [
            ['menu_key' => 'loglist', 'menu_name' => '账户余额', 'menu_url' => 'index.php?act=predeposit&op=pd_log_list'],
            ['menu_key' => 'recharge_list', 'menu_name' => '充值明细', 'menu_url' => 'index.php?act=predeposit&op=index'],
            ['menu_key' => 'cashlist', 'menu_name' => '余额提现', 'menu_url' => 'index.php?act=predeposit&op=pd_cash_list'],
            [
        ...
        switch($menu_type) {
            case 'rechargeinfo':
                $menu_array[] = ['menu_key' => 'rechargeinfo', 'menu_name' => '充值详细', 'menu_url' => ''];
                break;
            case 'recharge_add':
                $menu_array[] = ['menu_key' => 'recharge_add', 'menu_name' => '在线充值', 'menu_url' => ''];
                break;
            case 'rechargecard_add':
                $menu_array[] = [
                    'menu_key' => 'rechargecard_add',
                    'menu_name' => '充值卡充值',
                    'menu_url' => 'javascript:;'
                ];
                break;
            case 'cashadd':
                $menu_array[] = [
                    'menu_key' => 'cashadd',
                    'menu_name' => '提现申请',
                    'menu_url' => 'index.php?act=predeposit&op=pd_cash_add'
                ];
                break;
            case 'cashinfo':
                $menu_array[] = ['menu_key' => 'cashinfo', 'menu_name' => '提现详细', 'menu_url' => ''];
                break;
            case 'log':
            default:
                break;
