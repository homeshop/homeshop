 ...
 * 手机短信登录
 */
class  SmsControl extends BaseLoginControl {
    ...
    public function registerOp() {
        ...
        if(chksubmit() && strlen($phone) == 11 && strlen($captcha) == 6){
            ...
            $member = $model_member->getMemberInfo(['member_name' => $member_name]);//检查重名
            if(!empty($member)){
                showDialog('用户名已被注册', '', 'error');
            }
            $member = $model_member->getMemberInfo(['member_mobile' => $phone]);//检查手机号是否已被注册
            if(!empty($member)){
    ...
    public function find_passwordOp() {
        if(checkSeccode($_POST['nchash'], $_POST['captcha'])){
            ...
            if(!empty($member)){
                ...
                $model_member->editMember(['member_id' => $member['member_id']], ['member_passwd' => $new_password]);
                $model_member->createSession($member);//自动登录
