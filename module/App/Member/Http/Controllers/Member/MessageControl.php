...
class  MessageControl extends BaseMemberControl {
    ...
    public function messageOp() {
        ...
        $page->setEachNum(10);
        $page->setStyle('admin');
        $message_array = $model_message->listMessage([
            'message_type' => '2',
            'to_member_id_common' => $_SESSION['member_id'],
            'no_message_state' => '2'
        ], $page);
    ...
    public function sendmsgOp() {
        ...
        $friend_list = $friend_model->listFriend(['friend_frommid' => "{$_SESSION['member_id']}"]);
    ...
    public function savemsgOp() {
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                [
                    "input" => $_POST["to_member_name"],
                    "require" => "true",
                    "message" => Language::get('home_message_receiver_null')
                ],
                [
                    "input" => $_POST["msg_content"],
                    "require" => "true",
                    "message" => Language::get('home_message_content_null')
                ],
            ];
            ...
            $member_list = $member_model->getMemberList(['member_name' => ['in', $membername_arr]]);
    ...
    public function showmsgcommonOp() {
        ...
        $referer_url = getReferer();
        if(!in_array($drop_type, ['msg_list']) || $message_id <= 0){
            showMessage(Language::get('wrong_argument'), $referer_url, 'html', 'error');
        }
        //查询站内信
        $param = [];
        ...
        if($message_info['message_parent_id'] > 0){
            //查询该站内信的父站内信
            $parent_array = $model_message->getRowMessage([
                'message_id' => "{$message_info['message_parent_id']}",
                'message_type' => '0',
                'no_message_state' => '2'
            ]);
            //查询该站内信的回复站内信
            $reply_array = $model_message->listMessage([
                'message_parent_id' => "{$message_info['message_parent_id']}",
                'message_type' => '0',
                'no_message_state' => '2'
            ]);
        } else {//此信息为父站内信
        ...
        if(!empty($messageid_arr)){
            ...
            $model_message->updateCommonMessage(['message_open' => '1'], ['message_id_in' => "$messageid_str"]);
        }
        //更新未读站内信数量cookie值
    ...
    public function showmsgbatchOp() {
        ...
        $drop_type = trim($_GET['drop_type']);
        $referer_url = getReferer();
        if(!in_array($drop_type, ['msg_system', 'msg_seller']) || $message_id <= 0){
            showMessage(Language::get('wrong_argument'), $referer_url, 'html', 'error');
        }
        //查询站内信
        $param = [];
        ...
        if($drop_type == 'msg_seller'){
            ...
            $store_info = $model_store->getStoreInfo(['member_id' => "{$message_info['from_member_id']}"]);
        ...
        $model_message->updateCommonMessage(['read_member_id' => $tmp_readid_str], ['message_id' => "{$message_id}"]);
        //更新未读站内信数量cookie值
        $cookie_name = 'msgnewnum' . $_SESSION['member_id'];
        $countnum = $model_message->countNewMessage($_SESSION['member_id']);
        setNcCookie($cookie_name, $countnum, 2 * 3600);//保存2小时
        ...
        if($_POST['form_submit'] == 'ok'){
            ...
            $obj_validate->validateparam = [
                [
                    "input" => $_POST["msg_content"],
                    "require" => "true",
                    "message" => Language::get('home_message_reply_content_null')
                ]
            ];
            $error = $obj_validate->validate();
            if($error != ''){
            ...
            if($insert_state){
                ...
                $model_message->updateCommonMessage($update_arr, ['message_id' => "{$insert_arr['message_parent_id']}"]);
            }
            if($_GET['inajax'] == 1){
    ...
    public function dropcommonmsgOp() {
        ...
        $drop_type = trim($_GET['drop_type']);
        if(!in_array($drop_type, ['msg_private', 'msg_list', 'sns_msg']) || empty($message_id)){
        ...
        $model_message = Model('message');
        $param = ['message_id_in' => $messageid_str];
    ...
    public function dropbatchmsgOp() {
        $message_id = trim($_GET['message_id']);
        $drop_type = trim($_GET['drop_type']);
        if(!in_array($drop_type, ['msg_system', 'msg_seller']) || empty($message_id)){
    ...
    public function settingOp() {
        ...
        if(chksubmit()){
            $insert = [
                ...
                [
                    ...
                    'is_receive' => intval($_POST['order_payment_success'])
                ],
                // 商品出库提醒
                [
                    'mmt_code' => 'order_deliver_success',
                    'member_id' => $_SESSION['member_id'],
                    'is_receive' => intval($_POST['order_deliver_success'])
                ],
                // 预定订单尾款支付提醒
                [
                ...
                [
                    ...
                    'member_id' => $_SESSION['member_id'],
                    'is_receive' => intval($_POST['predeposit_change'])
                ],
                // 充值卡余额变动提醒
                [
                    'mmt_code' => 'recharge_card_balance_change',
                    'member_id' => $_SESSION['member_id'],
                    'is_receive' => intval($_POST['recharge_card_balance_change'])
                ],
                // 代金券使用提醒
                [
                    'mmt_code' => 'voucher_use',
                    'member_id' => $_SESSION['member_id'],
                    'is_receive' => intval($_POST['voucher_use'])
                ],
                // 红包使用提醒
                [
                    'mmt_code' => 'rpt_use',
                    'member_id' => $_SESSION['member_id'],
                    'is_receive' => intval($_POST['rpt_use'])
                ],
                // 退款退货提醒
                [
                    'mmt_code' => 'refund_return_notice',
                    'member_id' => $_SESSION['member_id'],
                    'is_receive' => intval($_POST['refund_return_notice'])
                ],
                // 到货通知提醒
                [
                    'mmt_code' => 'arrival_notice',
                    'member_id' => $_SESSION['member_id'],
                    'is_receive' => intval($_POST['arrival_notice'])
                ],
                // 商品咨询回复提醒
                [
                    'mmt_code' => 'consult_goods_reply',
                    'member_id' => $_SESSION['member_id'],
                    'is_receive' => intval($_POST['consult_goods_reply'])
                ],
                // 平台客服回复提醒
                [
                    'mmt_code' => 'consult_mall_reply',
                    'member_id' => $_SESSION['member_id'],
                    'is_receive' => intval($_POST['consult_mall_reply'])
                ],
                // 代金券即将到期
                [
                    'mmt_code' => 'voucher_will_expire',
                    'member_id' => $_SESSION['member_id'],
                    'is_receive' => intval($_POST['voucher_will_expire'])
                ],
                // 兑换码即将到期提醒
                [
                    'mmt_code' => 'vr_code_will_expire',
                    'member_id' => $_SESSION['member_id'],
                    'is_receive' => intval($_POST['vr_code_will_expire'])
                ],
            ];
        ...
        // 新消息数量
        $this->showReceivedNewNum();
        $setting_list = $model_membermsgsetting->getMemberMsgSettingList(['member_id' => $_SESSION['member_id']]);
        $setting_array = [];
    ...
    private function receivedCommonNewNum() {
        $model_message = Model('message');
        $countnum = $model_message->countMessage([
            'message_type' => '2',
            'to_member_id_common' => $_SESSION['member_id'],
            'no_message_state' => '2',
            'message_open_common' => '0'
        ]);
        return $countnum;
    }
    ...
    private function profile_menu($menu_key = '') {
        $menu_array = [
            [
                'menu_key' => 'message',
                'menu_name' => Language::get('home_message_received_message'),
                'menu_url' => 'index.php?act=member_message&op=message'
            ],
            [
                'menu_key' => 'private',
                'menu_name' => Language::get('home_message_private_message'),
                'menu_url' => 'index.php?act=member_message&op=privatemsg'
            ],
            [
                'menu_key' => 'system',
                'menu_name' => Language::get('home_message_system_message'),
                'menu_url' => 'index.php?act=member_message&op=systemmsg'
            ],
            ['menu_key' => 'ggmsg', 'menu_name' => '系统公告', 'menu_url' => 'index.php?act=member_message&op=ggmsg'],
            [
                'menu_key' => 'close',
                'menu_name' => Language::get('home_message_close'),
                'menu_url' => 'index.php?act=member_message&op=personalmsg'
            ],
            ['menu_key' => 'setting', 'menu_name' => '接收设置', 'menu_url' => urlMember('member_message', 'setting')]
        ];
        if($menu_key == 'sendmsg'){
            $menu_array[] = [
                'menu_key' => 'sendmsg',
                'menu_name' => Language::get('home_message_send_message'),
                'menu_url' => 'index.php?act=member_message&op=sendmsg'
            ];
        } elseif($menu_key == 'showmsg') {
            $menu_array[] = [
                'menu_key' => 'showmsg',
                'menu_name' => Language::get('home_message_view_message'),
                'menu_url' => '#'
            ];
        }
