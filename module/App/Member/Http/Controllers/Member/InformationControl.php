 ...
 * 用户中心
 */
class  InformationControl extends BaseMemberControl {
    ...
    public function memberOp() {
        ...
        if(chksubmit()){
            ...
            $update = $model_member->editMember(['member_id' => $_SESSION['member_id']], $member_array);
        ...
        if($this->member_info['member_privacy'] != ''){
            ...
            $this->member_info['member_privacy'] = [];
        }
    ...
    public function moreOp() {
        ...
        $model = Model();
        if(chksubmit()){
            $model->table('sns_mtagmember')->where(['member_id' => $_SESSION['member_id']])->delete();
            if(!empty($_POST['mid'])){
                $insert_array = [];
                foreach($_POST['mid'] as $val){
                    $insert_array[] = ['mtag_id' => $val, 'member_id' => $_SESSION['member_id'], 'recommend' => 0];
                }
                $model->table('sns_mtagmember')->pk(['mtag_id', 'member_id'])->insertAll($insert_array, true);
            }
        ...
        // 用户已添加标签列表。
        $mtm_array = $model->table('sns_mtagmember')->where(['member_id' => $_SESSION['member_id']])->select();
        $mtag_list = [];
        $mtm_list = [];
    ...
    public function cutOp() {
        if(chksubmit()){
            ...
            @unlink($src);
            Model('member')->editMember(['member_id' => $_SESSION['member_id']], ['member_avatar' => 'avatar_' . $_SESSION['member_id'] . '.jpg']);
        ...
        switch($menu_type) {
            case 'member':
                $menu_array = [
                    1 => [
                        'menu_key' => 'member',
                        'menu_name' => Language::get('home_member_base_infomation'),
                        'menu_url' => 'index.php?act=member_information&op=member'
                    ],
                    2 => [
                        'menu_key' => 'more',
                        'menu_name' => Language::get('home_member_more'),
                        'menu_url' => 'index.php?act=member_information&op=more'
                    ],
                    5 => [
                        'menu_key' => 'avatar',
                        'menu_name' => Language::get('home_member_modify_avatar'),
                        'menu_url' => 'index.php?act=member_information&op=avatar'
                    ]
                ];
                break;
        }
