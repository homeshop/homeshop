 ...
 * 收货地址
 */
class  AddressControl extends BaseMemberControl {
    ...
    public function addressOp() {
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                ...
                ["input" => $_POST["region"], "require" => "true", "message" => $lang['member_address_area_null']],
                ["input" => $_POST["address"], "require" => "true", "message" => $lang['member_address_address_null']],
                [
            ...
            if($_POST['is_default']){
                $address_class->editAddress(['is_default' => 0], [
                    'member_id' => $_SESSION['member_id'],
                    'is_default' => 1
                ]);
            }
            if(intval($_POST['id']) > 0){
                $rs = $address_class->editAddress($data, [
                ...
                $count = $address_class->getAddressCount(['member_id' => $_SESSION['member_id']]);
                if($count >= 20){
        ...
        if($del_id > 0){
            $rs = $address_class->delAddress(['address_id' => $del_id, 'member_id' => $_SESSION['member_id']]);
            if($rs){
        ...
        $address_list = $address_class->getAddressList(['member_id' => $_SESSION['member_id']]);
    ...
    public function delivery_addOp() {
        if(chksubmit()){
            $info = Model('delivery_point')->getDeliveryPointOpenInfo(['dlyp_id' => intval($_POST['dlyp_id'])]);
            if(empty($info)){
            ...
            if(intval($_POST['address_id'])){
                $result = Model('address')->editAddress($data, ['address_id' => intval($_POST['address_id'])]);
            } else {
                $count = Model('address')->getAddressCount(['member_id' => $_SESSION['member_id']]);
                if($count >= 20){
                    showDialog('最多允许添加20个有效地址', '', 'error');
                }
            ...
            if(intval($_GET['id']) > 0){
                ...
                $condition = ['address_id' => intval($_GET['id']), 'member_id' => $_SESSION['member_id']];
                $address_info = $model_addr->getAddressInfo($condition);
                //取出省级ID
                $area_info = Model('area')->getAreaInfo(['area_id' => $address_info['city_id']]);
            ...
            case 'address':
                $menu_array = [
                    1 => [
                        'menu_key' => 'address',
                        'menu_name' => '地址列表',
                        'menu_url' => 'index.php?act=member_adderss&op=address'
                    ]
                ];
                break;
        }
