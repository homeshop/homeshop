 ...
 * SNS首页
 */
class  SnsfriendControl extends BaseMemberControl {
    ...
    public function findOp() {
        if(chksubmit()){
            ...
            // 查看推荐
            $model = Model();
            $mtag_list = $model->table('sns_membertag')->where(['mtag_recommend' => 1])->order('mtag_sort asc')->select();
            if(!empty($mtag_list)){
                // 查询已关注好友 ，不显示已关注好友
                $friend_array = $model->table('sns_friend')->field('friend_tomid')->where(['friend_frommid' => $_SESSION['member_id']])->select();
                ...
                // 查询会员
                $where['sns_mtagmember.mtag_id'] = ['in', $mtagid_array]; //查询条件
                $where['sns_mtagmember.member_id'] = ['not in', $friendid_array];
                $tagmember_list = $model->table('sns_mtagmember,member')->field('sns_mtagmember.*,member.member_avatar,member.member_name')->join('left')->on('sns_mtagmember.member_id=member.member_id')->where($where)->order('sns_mtagmember.recommend desc, sns_mtagmember.member_id asc')->limit(count($mtagid_array) * 20)->select();
                // 整理
    ...
    public function findlistOp() {
        if(trim($_POST['searchname']) != ''){
            ...
            $model = Model();
            //查询关注会员id
            $followlist = $model->table('sns_friend')->field('friend_tomid, friend_followstate')->where(['friend_frommid' => $_SESSION['member_id']])->select();
            unset($condition_arr);
            $followlist_new = [];
            if(!empty($followlist)){
            ...
            $where['member_id'] = ['neq', $_SESSION['member_id']];
            $where['member_name'] = ['like', '%' . trim($_POST['searchname']) . '%'];// 会员名称
            // 省份
            ...
            if(intval($_POST['age']) > 0){
                switch(intval($_POST['age'])) {
                    case 1:
                        ...
                        $e_time = date('Y-m-d');
                        $where['member_birthday'] = ['BETWEEN', $s_time . ',' . $e_time];
                        break;
                    case 2:
                    ...
                    case 5:
                        ...
                        $where['member_birthday'] = ['elt', $e_time];
                        break;
                }
            }
    ...
    public function addfollowOp() {
        ...
        $condition_arr['member_id'] = ['in', [$mid, $_SESSION['member_id']]];
        ...
        if($result){
            ...
            $return = json_encode(['state' => $insert_arr['friend_followstate']]);
        } else {
            $return = 'false';
        }
        echo !empty($_GET['callback']) ? $_GET['callback'] . '(' . $return . ')' : $return;
        exit;
    }
    ...
    public function delfollowOp() {
        ...
        //取消关注
        $friend_model = Model('sns_friend');
        $result = $friend_model->delFriend(['friend_frommid' => "{$_SESSION['member_id']}", 'friend_tomid' => "$mid"]);
        if($result){
            //更新对方的关注状态
    ...
    public function batch_addfollowOp() {
        ...
        // 将被关注会员列表
        $pm_array = $model->table('member')->where(['member_id' => ['in', $ids]])->select();
        // 查询是否已经关注
        ...
        // 查询对方是否关注我
        $bgz_array = $model->table('sns_friend')->where([
            'friend_frommid' => ['in', $ids],
            'friend_tomid' => $_SESSION['member_id']
        ])->select();
        ...
        if(!empty($pm_array)){
            ...
            foreach($pm_array as $val){
                ...
                if(isset($bgz_array[ $val['member_id'] ])){
                    ...
                    $model->table('sns_friend')->update([
                        'friend_followstate' => 2,
                        'friend_id' => $bgz_array[ $val['member_id'] ]['friend_id']
                    ]);
                } else {
    ...
    public function followOp() {
        ...
        $page->setEachNum(15);
        $page->setStyle('admin');
        $follow_list = $friend_model->listFriend(['friend_frommid' => "{$_SESSION['member_id']}"], '*', $page, 'detail');
    ...
    public function fanOp() {
        ...
        $page->setEachNum(15);
        $page->setStyle('admin');
        $fan_list = $friend_model->listFriend(['friend_tomid' => "{$_SESSION['member_id']}"], '*', $page, 'fromdetail');
        if(!empty($fan_list)){
    ...
    private function profile_menu($menu_key) {
        $menu_array = [
            1 => [
                'menu_key' => 'find',
                'menu_name' => Language::get('snsfriend_findmember'),
                'menu_url' => 'index.php?act=member_snsfriend&op=find'
            ],
            2 => [
                'menu_key' => 'follow',
                'menu_name' => Language::get('snsfriend_follow'),
                'menu_url' => 'index.php?act=member_snsfriend&op=follow'
            ],
            3 => [
                'menu_key' => 'fan',
                'menu_name' => Language::get('snsfriend_fans'),
                'menu_url' => 'index.php?act=member_snsfriend&op=fan'
            ]
        ];
