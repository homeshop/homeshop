 ...
 * 分享绑定
 */
class  SharemanageControl extends BaseMemberControl {
    ...
    public function checkbindOp() {
        ...
        $result = '';
        if(empty($app_key)){
            $result = json_encode(['done' => false, 'msg' => Language::get('sharebind_bind_fail')]);
        } else {
            $model = Model('sns_binding');
    ...
    private function profile_menu($menu_key = '', $array = []) {
        ...
        $menu_array = [
            1 => [
                'menu_key' => 'sns_binding',
                'menu_name' => '分享绑定',
                'menu_url' => urlMember('member_sharemanage', 'index')
            ],
        ];
        if(!empty($array)){
            $menu_array[] = $array;
