 ...
 * 会员中心——积分兑换信息
 */
class  PointorderControl extends BaseMemberControl {
    ...
    public function orderlistOp() {
        ...
        if(is_array($order_idarr) && count($order_idarr) > 0){
            $prod_list = $model_pointorder->getPointOrderGoodsList(['point_orderid' => ['in', $order_idarr]]);
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        ...
        switch($menu_type) {
            case 'pointorder':
                $menu_array = [
                    1 => ['menu_key' => 'points', 'menu_name' => '积分明细', 'menu_url' => 'index.php?act=member_points'],
                    2 => [
                        'menu_key' => 'orderlist',
            ...
            case 'pointorderinfo':
                $menu_array = [
                    ...
                    2 => [
                        'menu_key' => 'orderlist',
                        'menu_name' => Language::get('nc_member_path_pointorder_list'),
                        'menu_url' => 'index.php?act=member_pointorder&op=orderlist'
                    ],
                    3 => ['menu_key' => 'orderinfo', 'menu_name' => Language::get('nc_member_path_pointorder_info'), '']
                ];
                break;
        }
