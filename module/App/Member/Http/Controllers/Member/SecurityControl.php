 ...
 * 账户安全
 */
class  SecurityControl extends BaseMemberControl {
    ...
    public function send_bind_emailOp() {
        ...
        $data['send_acode_times'] = ['exp', 'send_acode_times+1'];
        $update = $model_member->editMemberCommon($data, ['member_id' => $_SESSION['member_id']]);
        if(!$update){
            showDialog('系统发生错误，如有疑问请与管理员联系');
        }
        $uid = base64_encode(encrypt($_SESSION['member_id'] . ' ' . $_POST["email"], MD5_KEY));
        $verify_url = urlMember('login', 'bind_email', ['uid' => $uid, 'hash' => md5($seed)]);
    ...
    public function send_auth_codeOp() {
        if(!in_array($_GET['type'], ['email', 'mobile'])){
            exit();
        }
        $model_member = Model('member');
    ...
    public function modify_pwdOp() {
        ...
        $update = $model_member->editMember(['member_id' => $_SESSION['member_id']], ['member_passwd' => md5($_POST['password'])]);
    ...
    public function send_modify_mobileOp() {
        ...
        $verify_code = rand(100, 999) . rand(100, 999);
        $model_tpl = Model('mail_templates');
        $tpl_info = $model_tpl->getTplInfo(['code' => 'modify_mobile']);
        $param = [];
        $param['site_name'] = C('site_name');
        $param['send_time'] = date('Y-m-d H:i', TIMESTAMP);
        $param['verify_code'] = $verify_code;
            ...
            case 'index':
                $menu_array = [
                    [
                        'menu_key' => 'index',
                        'menu_name' => '账户安全',
                        'menu_url' => 'index.php?act=member_security&op=index'
                    ],
                ];
                break;
            case 'modify_pwd':
                $menu_array = [
                    ...
                    [
                        'menu_key' => 'modify_pwd',
                        'menu_name' => '修改登录密码',
                        'menu_url' => 'index.php?act=member_security&op=auth&type=modify_pwd'
                    ],
                ];
                break;
            case 'modify_email':
                $menu_array = [
                    ...
                    [
                        'menu_key' => 'modify_email',
                        'menu_name' => '邮箱验证',
                        'menu_url' => 'index.php?act=member_security&op=auth&type=modify_email'
                    ],
                ];
                break;
            case 'modify_mobile':
                $menu_array = [
                    ...
                    [
                        'menu_key' => 'modify_mobile',
                        'menu_name' => '手机验证',
                        'menu_url' => 'index.php?act=member_security&op=auth&type=modify_mobile'
                    ],
                ];
                break;
            case 'modify_paypwd':
                $menu_array = [
                    ...
                    [
                        'menu_key' => 'modify_paypwd',
                        'menu_name' => '设置支付密码',
                        'menu_url' => 'index.php?act=member_security&op=auth&type=modify_paypwd'
                    ],
                ];
                break;
            case 'pd_cash':
                $menu_array = [
                    [
                        'menu_key' => 'loglist',
                        'menu_name' => '账户余额',
                        'menu_url' => 'index.php?act=predeposit&op=pd_log_list'
                    ],
                    [
                        'menu_key' => 'recharge_list',
                        'menu_name' => '充值明细',
                        'menu_url' => 'index.php?act=predeposit&op=index'
                    ],
                    [
                        'menu_key' => 'cashlist',
                        'menu_name' => '余额提现',
                        'menu_url' => 'index.php?act=predeposit&op=pd_cash_list'
                    ],
                    [
                        'menu_key' => 'pd_cash',
                        'menu_name' => '提现申请',
                        'menu_url' => 'index.php?act=member_security&op=auth&type=pd_cash'
                    ],
                ];
                break;
        }
