 ...
 * 三方账户登录
 */
class  BindControl extends BaseMemberControl {
    ...
    public function qqunbindOp() {
        ...
        if($_POST['is_editpw'] == 'yes'){
            ...
            $obj_validate->validateparam = [
                [
                    ...
                    "min" => 6,
                    "max" => 20,
                    "message" => Language::get('member_qqconnect_password_null')
                ],
                [
        ...
        $edit_state = $model_member->editMember(['member_id' => $_SESSION['member_id']], $update_arr);
        if(!$edit_state){
            showMessage(Language::get('member_qqconnect_password_modify_fail'), 'html', 'error');
        }
        session_unset();
        session_destroy();
        showMessage(Language::get('member_qqconnect_unbind_success'), urlLogin('login', 'index', ['ref_url' => urlMember('member_bind', 'qqbind')]));
    }
    ...
    public function sinaunbindOp() {
        ...
        if($_POST['is_editpw'] == 'yes'){
            ...
            $obj_validate->validateparam = [
                [
                    ...
                    "min" => 6,
                    "max" => 20,
                    "message" => Language::get('member_sconnect_password_null')
                ],
                [
                    "input" => $_POST["confirm_password"],
                    "require" => "true",
        ...
        session_unset();
        session_destroy();
        showMessage(Language::get('member_sconnect_unbind_success'), urlLogin('login', 'index', ['ref_url' => urlMember('member_bind', 'sinabind')]));
    }
    ...
    public function weixinunbindOp() {
        ...
        session_unset();
        session_destroy();
        showMessage('微信解绑成功', urlLogin('login', 'index', ['ref_url' => urlMember('member_bind', 'weixinbind')]));
    }
     ...
     * @return
     */
    private function profile_menu($menu_key = '', $array = []) {
        Language::read('member_layout');
        $lang = Language::getLangContent();
        $menu_array = [];
        $menu_array = [
            1 => ['menu_key' => 'qq_bind', 'menu_name' => 'QQ绑定', 'menu_url' => 'index.php?act=member_bind&op=qqbind'],
            2 => [
                'menu_key' => 'sina_bind',
                'menu_name' => '新浪绑定',
                'menu_url' => 'index.php?act=member_bind&op=sinabind'
            ],
            3 => [
                'menu_key' => 'weixin_bind',
                'menu_name' => '微信绑定',
                'menu_url' => 'index.php?act=member_bind&op=weixinbind'
            ],
        ];
        if(!empty($array)){
            $menu_array[] = $array;
