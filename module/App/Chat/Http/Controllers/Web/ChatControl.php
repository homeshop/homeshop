<?php namespace App\Chat\Http\Controllers\Web;

use App\Chat\Http\Controllers\BaseControl;
use App\Chat\Http\Controllers\Language;
use App\Chat\Http\Controllers\Tpl;
...
class  ChatControl extends BaseControl {
    ...
    public function get_user_listOp() {
        ...
        $member_list = $model_chat->getFriendList(['friend_frommid' => $f_id], $n, $member_list);
        $add_time = date("Y-m-d");
    ...
    public function get_seller_listOp() {
        ...
        if(empty($_SESSION['seller_list'])){
            $member_list = $model_chat->getSellerList(['store_id' => $store_id], $n, $member_list);
        ...
        if($s_id > 0){
            ...
            if(!empty($list) && is_array($list)){
                foreach($list as $k => $v){
                    ...
                    $v['url'] = urlShop('goods', 'index', ['goods_id' => $v['goods_id']]);
                    $v['pic'] = thumb($v, 60);
                    $list[ $k ] = $v;
                }
            }
