<?php namespace App\Admin\Http\Controllers\Vr;

use App\Admin\Http\Controllers\SystemControl;
use App\Admin\Http\Controllers\Language;
use App\Admin\Http\Controllers\Tpl;
...
class  RefundControl extends SystemControl {
    ...
    public function wxpayOp() {
        $result = ['state' => 'false', 'msg' => '参数错误，微信退款失败'];
        ...
        if(!empty($detail_array) && in_array($detail_array['refund_code'], ['wxpay', 'wx_jsapi', 'wx_saoma'])){
            ...
            if($refund_amount > 0){
                ...
                if(!empty($data) && $data['return_code'] == 'SUCCESS'){//请求结果
                    if($data['result_code'] == 'SUCCESS'){//业务结果
                        ...
                        $model_refund->editDetail(['refund_id' => $refund_id], $detail_array);
                        $result['state'] = 'true';
        ...
        if(!empty($detail_array) && $detail_array['refund_code'] == 'alipay'){
            ...
            if($refund_amount > 0){
                ...
                if($b_date != date('Ymd')){
                    ...
                    $model_refund->editDetail(['refund_id' => $refund_id], ['batch_no' => $batch_no]);
                }
    ...
    public function get_detailOp() {
        $result = ['state' => 'false', 'msg' => '退款正在处理中或已失败，稍后查询'];
        $refund_id = intval($_GET['refund_id']);
        $model_refund = Model('vr_refund');
        $condition = [];
        $condition['refund_id'] = $refund_id;
        ...
        if($detail_array['pay_time'] > 0){
            $result = ['state' => 'true', 'msg' => '成功退款:' . ncPriceFormat($detail_array['pay_amount'])];
        }
        exit(json_encode($result));
    }
}
