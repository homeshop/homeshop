...
class  CommonControl extends SystemControl {
        ...
        if(chksubmit()){
            ...
            if(!empty($_FILES['_pic']['tmp_name'])){
                ...
                if($result){
                    exit(json_encode([
                        'status' => 1,
                        'url' => UPLOAD_SITE_URL . '/' . $_GET['uploadpath'] . '/' . $upload->thumb_image
                    ]));
                } else {
    ...
    public function common_operationsOp() {
        $type = $_GET['type'];
        $value = $_GET['value'];
        if(!in_array($type, ['add', 'del']) || empty($value)){
            echo false;
            exit;
        }
        ...
        if($type == 'add'){
            ...
            $quicklink = array_diff($quicklink, [$value]);
        }
    ...
    private function get_pending_matters() {
        ...
        // 预存款提现
        $statistics['cashlist'] = Model('predeposit')->getPdCashCount(['pdc_payment_state' => 0]);
        // 店铺申请数
        $statistics['store_joinin'] = Model('store_joinin')->getStoreJoininCount(['joinin_state' => ['in', [10, 11]]]);
        //店铺续签申请
        $statistics['store_reopen_applay'] = Model('store_reopen')->getStoreReopenCount(['re_state' => 1]);
        // 经营类目申请
        $statistics['store_bind_class_applay'] = Model('store_bind_class')->getStoreBindClassCount(['state' => 0]);
        // 即将到期
        $statistics['store_expire'] = $model_store->getStoreCount([
            'store_state' => 1,
            'store_end_time' => ['between', [TIMESTAMP, TIMESTAMP + 864000]]
        ]);
        // 已经到期
        $statistics['store_expired'] = $model_store->getStoreCount([
            'store_state' => 1,
            'store_end_time' => ['between', [1, TIMESTAMP]]
        ]);
        // 等待审核
        $statistics['product_verify'] = $model_goods->getGoodsCommonWaitVerifyCount([]);
        // 举报
        $statistics['inform'] = Model('inform')->getInformCount(['inform_state' => 1]);
        // 品牌申请
        $statistics['brand_apply'] = Model('brand')->getBrandCount(['brand_apply' => '0']);
        // 退款
        $statistics['refund'] = $model_refund_return->getRefundReturn(['refund_type' => 1, 'refund_state' => 2]);
        // 退货
        $statistics['return'] = $model_refund_return->getRefundReturn(['refund_type' => 2, 'refund_state' => 2]);
        // 虚拟订单退款
        $statistics['vr_refund'] = $model_vr_refund->getRefundCount(['admin_state' => 1]);
        // 投诉
        $statistics['complain_new'] = $model_complain->getComplainCount(['complain_state' => 10]);
        // 待仲裁
        $statistics['complain_handle'] = $model_complain->getComplainCount(['complain_state' => 40]);
        // 抢购数量
        $statistics['groupbuy_verify'] = Model('groupbuy')->getGroupbuyCount(['state' => 10]);
        // 积分订单
        $statistics['points_order'] = Model('pointorder')->getPointOrderCount(['point_orderstate' => 20]);
        //待审核账单
        $condition = [];
        ...
        // 平台客服
        $statistics['mall_consult'] = Model('mall_consult')->getMallConsultCount(['is_reply' => 0]);
        // 服务站
        $statistics['delivery_point'] = Model('delivery_point')->getDeliveryPointWaitVerifyCount([]);
        /**
         * 资讯
         */
        if(C('cms_isuse')){
            // 文章审核
            $statistics['cms_article_verify'] = Model('cms_article')->getCmsArticleCount(['article_state' => 2]);
            // 画报审核
            $statistics['cms_picture_verify'] = Model('cms_picture')->getCmsPictureCount(['picture_state' => 2]);
        }
        /**
         * 圈子
         */
