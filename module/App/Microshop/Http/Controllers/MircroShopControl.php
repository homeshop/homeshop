...
class  MircroShopControl {
    ...
    public function __construct() {
         ...
         * seo
         */
        Tpl::output('html_title', Language::get('nc_microshop') . '-' . C('site_name') . ' - Powered by HomeSHOP');
    ...
    protected function share_app_publish($type, $publish_info = []) {
        $param = [];
        switch($type) {
            case 'comment':
    ...
    protected function get_sidebar_list($member_id) {
        //随心看
        $model_microshop_goods = Model('micro_goods');
        $sidebar_goods_list = $model_microshop_goods->getList(['commend_member_id' => $member_id], null, 'commend_time desc', '*', 9);
        Tpl::output('sidebar_goods_list', $sidebar_goods_list);
        //个人秀
        $model_microshop_personal = Model('micro_personal');
        $sidebar_personal_list = $model_microshop_personal->getList(['commend_member_id' => $member_id], null, 'commend_time desc', '*', 9);
    ...
    protected function get_member_detail_info($member_info) {
        ...
        //粉丝数
        $fan_count = $model->table('sns_friend')->where(['friend_tomid' => $member_id])->count();
        $member_info['fan_count'] = $fan_count;
        //关注数
        $attention_count = $model->table('sns_friend')->where(['friend_frommid' => $member_id])->count();
        $member_info['attention_count'] = $attention_count;
        //兴趣标签
        $mtag_list = $model->table('sns_membertag,sns_mtagmember')->field('mtag_name')->on('sns_membertag.mtag_id = sns_mtagmember.mtag_id')->join('inner')->where(['sns_mtagmember.member_id' => $member_id])->select();
        $tagname_array = [];
        if(!empty($mtag_list)){
    ...
    protected function get_microshop_adv($type = 'index') {
        ...
        $adv_list = $model->getList(['adv_type' => $type], null, 'adv_sort asc');
