<?php namespace App\Microshop\Http\Controllers\Admin\Goods;

use App\Microshop\Http\Controllers\Admin\SystemControl;
use App\Microshop\Http\Controllers\Admin\Language;
use App\Microshop\Http\Controllers\Admin\Tpl;
...
class ClassControl extends SystemControl {
    ...
    public function goodsclass_saveOp() {
        ...
        $validate_array = [
            [
                ...
                "min" => "1",
                "max" => "10",
                'message' => Language::get('class_name_error')
            ],
            [
            ...
            [
                'input' => $_POST['class_parent_id'],
                'require' => 'true',
                'validator' => 'Number',
                'message' => Language::get('parent_id_error')
            ]
        ];
        ...
        if(isset($_POST['class_id']) && intval($_POST['class_id']) > 0){
            $result = $model_class->modify($param, ['class_id' => $_POST['class_id']]);
        } else {
    ...
    public function goodsclass_sort_updateOp() {
        ...
        if($new_sort > 255){
            ...
            $model_class = Model("micro_goods_class");
            $result = $model_class->modify(['class_sort' => $new_sort], ['class_id' => $_GET['id']]);
            if($result){
    ...
    public function goodsclass_name_updateOp() {
        ...
        $error = $obj_validate->validate();
        if($error != ''){
            echo json_encode(['result' => false, 'message' => Language::get('class_name_error')]);
            die;
        } else {
            $model_class = Model("micro_goods_class");
            $result = $model_class->modify(['class_name' => $new_name], ['class_id' => $class_id]);
            if($result){
                echo json_encode(['result' => true, 'message' => 'class_add_success']);
                die;
            } else {
    ...
    public function goodsclass_dropOp() {
        ...
        $model_microshop_goods_class = Model('micro_goods_class');
        $condition = [];
        $condition['class_parent_id'] = ['in', $class_id];
        ...
        $condition = [];
        $condition['class_id'] = ['in', $class_id];
        //删除分类图片
    ...
    public function goodsclass_bindingOp() {
        ...
        $class_binding_list = $model_goods_relation->getList(['class_id' => $class_id]);
    ...
    public function goodsclass_binding_saveOp() {
        ...
        foreach($shop_class_array as $val){
            if(!empty($val)){
                $param[] = ['class_id' => $class_id, 'shop_class_id' => $val];
            }
        }
        $model_goods_relation = Model('micro_goods_relation');
        $model_goods_relation->drop(['class_id' => $class_id]);
    ...
    public function goodsclass_defaultOp() {
        ...
        $model_goods_class->modify(['class_default' => 0], true);
        $result = $model_goods_class->modify(['class_default' => 1], ['class_id' => $class_id]);
        if($result){
    ...
    private function show_menu_goods_class($menu_key) {
        $menu_array = [
            'goods_class_list' => [
                ...
                'menu_url' => 'index.php?act=goods_class&op=goodsclass_list'
            ],
            'goods_class_add' => [
                'menu_type' => 'link',
                'menu_name' => Language::get('nc_new'),
                'menu_url' => 'index.php?act=goods_class&op=goodsclass_add'
            ],
        ];
        if($menu_key == 'goods_class_edit'){
            $menu_array['goods_class_edit'] = [
                'menu_type' => 'link',
        ...
        if($menu_key == 'goods_class_binding'){
            $menu_array['goods_class_binding'] = [
                'menu_type' => 'link',
                'menu_name' => Language::get('microshop_goods_class_binding'),
                'menu_url' => '###'
            ];
        }
