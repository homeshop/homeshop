...
class CommentControl extends SystemControl {
    ...
    public function comment_manage_xmlOp() {
        ...
        if($_REQUEST['advanced']){
            ...
            if(strlen($q = trim((string)$_REQUEST['comment_message']))){
                $condition['comment_message'] = ['like', '%' . $q . '%'];
            }
        } else {
        ...
        foreach($list as $val){
            ...
            $o = '<a class="btn red confirm-del-on-click" href="index.php?act=comment&op=comment_drop&comment_id=' . $val['comment_id'] . '"><i class="fa fa-trash"></i>删除</a>';
            $o .= '<a class="btn green" target="_blank" href="' . MICROSHOP_SITE_URL . DS . 'index.php?act=' . $channel['key'] . '&op=detail&' . $channel['key'] . '_id=' . $val['comment_object_id'] . '"><i class="fa fa-list-alt"></i>查看</a>';
            $i = [];
            $i['operation'] = $o;
            $i['comment_id'] = $val['comment_id'];
            $i['member_name'] = '<a href="' . MICROSHOP_SITE_URL . DS . 'index.php?act=home&member_id=' . $val['comment_member_id'] . '" target="_blank">' . $val['member_name'] . '</a>';
    ...
    public function comment_dropOp() {
        ...
        $condition = [];
        $condition['comment_id'] = ['in', trim($_REQUEST['comment_id'])];
    ...
    private function get_channel_array() {
        $channel_array = [];
        $channel_array[ self::GOODS_FLAG ] = ['name' => Language::get('nc_microshop_goods'), 'key' => 'goods'];
        $channel_array[ self::PERSONAL_FLAG ] = ['name' => Language::get('nc_microshop_personal'), 'key' => 'personal'];
        $channel_array[ self::STORE_FLAG ] = ['name' => Language::get('nc_microshop_store'), 'key' => 'store'];
