...
class PersonalControl extends SystemControl {
    ...
    public function personal_manage_xmlOp() {
        ...
        $list = (array)$model_personal->getListWithUserInfo($condition, $_REQUEST['rp'], 'microshop_sort, commend_time desc', $field);
        $data = [];
        $data['now_page'] = $model_personal->shownowpage();
        $data['total_num'] = $model_personal->gettotalnum();
        foreach($list as $val){
            $o = '<a class="btn red confirm-del-on-click" href="index.php?act=personal&op=personal_drop&personal_id=' . $val['personal_id'] . '"><i class="fa fa-trash-o"></i>删除</a>';
...
EOB;
            ...
            $i['imgs'] = $imgs;
            $i['member_name'] = '<a href="' . MICROSHOP_SITE_URL . DS . 'index.php?act=home&member_id=' . $val['commend_member_id'] . '" target="_blank">' . $val['member_name'] . '</a>';
            $i['commend_message'] = $val['commend_message'];
            $i['commend_time_text'] = date('Y-m-d', $val['commend_time']);
            $i['microshop_commend'] = $val['microshop_commend'] == '1' ? '<span class="yes"><i class="fa fa-check-circle"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
    ...
    public function personal_dropOp() {
        ...
        $condition = [];
        $condition['personal_id'] = ['in', trim($_REQUEST['personal_id'])];
        //删除随心看图片
    ...
    public function personal_sort_updateOp() {
        ...
        if($new_sort > 255){
            ...
            $result = $model_class->modify(['microshop_sort' => $new_sort], ['personal_id' => $_GET['id']]);
            if($result){
