    ...
    public function goods_manage_xmlOp() {
        ...
        if($_REQUEST['advanced']){
            ...
            if(strlen($q = trim((string)$_REQUEST['commend_goods_name']))){
                $condition['commend_goods_name'] = ['like', '%' . $q . '%'];
            }
        ...
        $list = (array)$model_microshop_goods->getListWithUserInfo($condition, $_REQUEST['rp'], 'commend_time desc', $field);
        $data = [];
        $data['now_page'] = $model_microshop_goods->shownowpage();
        $data['total_num'] = $model_microshop_goods->gettotalnum();
        foreach($list as $val){
            $o = '<a class="btn red confirm-del-on-click" href="index.php?act=goods&op=goods_drop&commend_id=' . $val['commend_id'] . '"><i class="fa fa-trash-o"></i>删除</a>';
            ...
            $o .= '<li><a target="_blank" href="' . MICROSHOP_SITE_URL . DS . 'index.php?act=goods&op=detail&goods_id=' . $val['commend_id'] . '">查看商品</a></li>';
            $o .= '</ul></span>';
            $i = [];
            $i['operation'] = $o;
            ...
            $i['commend_state'] = $val['microshop_commend'] == 1 ? '<span class="yes"><i class="fa fa-check-circle"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
            $i['member_name'] = '<a target="_blank" href="' . MICROSHOP_SITE_URL . DS . 'index.php?act=home&member_id=' . $val['commend_member_id'] . '">' . $val['member_name'] . '</a>';
...
EOB;
    ...
    public function goods_dropOp() {
        ...
        $condition = [];
        $condition['commend_id'] = ['in', trim($_REQUEST['commend_id'])];
        //删除随心看图片
    ...
    public function goods_sort_updateOp() {
        ...
        if($new_sort > 255){
            ...
            $result = $model_class->modify(['microshop_sort' => $new_sort], ['commend_id' => $_GET['id']]);
            if($result){
