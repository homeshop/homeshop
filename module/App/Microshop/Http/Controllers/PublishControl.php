...
class  PublishControl extends MircroShopControl {
    ...
    public function goods_favoritesOp() {
        ...
        if(!empty($favorites_list) && is_array($favorites_list)){
            ...
            $goods_list = $model_goods->getGoodsList(['goods_id' => ['in', $goods_id_string]]);
        }
        Tpl::output('list', $goods_list);
    ...
    private function get_commend_goods_list() {
        ...
        $commend_goods_list = $model_microshop_goods->getList(['commend_member_id' => $_SESSION['member_id']]);
        $commend_goods_array = [];
    ...
    public function goods_saveOp() {
        ...
        $goods_relation = $model_goods_relation->getOne(['shop_class_id' => $goods_info['gc_id']]);
        $commend_goods_info = [];
        ...
        if(empty($goods_relation)){
            ...
            $default_class = $model_goods_class->getOne(['class_default' => 1]);
            if(!empty($default_class)){
                //默认分类
                $commend_goods_info['class_id'] = $default_class['class_id'];
            } else {
                $condition = [];
                $condition['class_parent_id'] = ['gt', 0];
