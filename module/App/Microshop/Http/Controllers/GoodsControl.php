...
class  GoodsControl extends MircroShopControl {
    ...
    public function listOp() {
        ...
        if(isset($_GET['keyword'])){
            $condition['commend_goods_name'] = ['like', '%' . $_GET['keyword'] . '%'];
        }
        //分类条件
        if($goods_class_menu_id > 0){
            ...
            if(!empty($class_id_string)){
                $condition['class_id'] = ['in', $class_id_string];
            }
        }
    ...
    public function detailOp() {
        ...
        $goods_image_list = $model_goods->getGoodsImageList(['goods_commonid' => $detail['commend_goods_commonid']]);
