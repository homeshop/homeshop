 ...
 * 商户中心
 */
class  CenterControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        // 昨日销量
        $where['order_add_time'] = [
            'between',
            [strtotime(date('Y-m-d', (time() - 3600 * 24))), strtotime(date('Y-m-d', time())) - 1]
        ];
        $daily_sales = Model('stat')->getoneByStatorder($where, $field);
        Tpl::output('daily_sales', $daily_sales);
        // 月销量
        $where['order_add_time'] = ['gt', strtotime(date('Y-m', time()))];
    ...
    public function statisticsOp() {
        ...
        // 全部商品数
        $goodscount = $model_goods->getGoodsCommonCount(['store_id' => $_SESSION['store_id']]);
        // 出售中的商品
        $goods_online = $model_goods->getGoodsCommonOnlineCount(['store_id' => $_SESSION['store_id']]);
        if(C('goods_verify')){
            // 等待审核的商品
            $goods_waitverify = $model_goods->getGoodsCommonWaitVerifyCount(['store_id' => $_SESSION['store_id']]);
            // 审核失败的商品
            $goods_verifyfail = $model_goods->getGoodsCommonVerifyFailCount(['store_id' => $_SESSION['store_id']]);
        }
        // 仓库待上架的商品
        $goods_offline = $model_goods->getGoodsCommonOfflineCount(['store_id' => $_SESSION['store_id']]);
        // 违规下架的商品
        $goods_lockup = $model_goods->getGoodsCommonLockUpCount(['store_id' => $_SESSION['store_id']]);
        // 等待回复商品咨询
        if(C('dbdriver') == 'mysqli'){
            $consult = Model('consult')->getConsultCount([
            ...
            $consult = Model('consult')->getConsultCount([
                'store_id' => $_SESSION['store_id'],
                'consult_reply' => ['exp', 'consult_reply IS NULL']
            ]);
        }
        
        // 商品图片数量
        $imagecount = Model('album')->getAlbumPicCount(['store_id' => $_SESSION['store_id']]);
        ...
        $condition['complain_state'] = [['gt', 10], ['lt', 90], 'and'];
    ...
    private function _update_quicklink() {
        ...
        $update_array = ['seller_quicklink' => $quicklink];
