...
class  MenuControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        $data_wxconfig = $model_wxconfig->where(['user_id' => $account_id])->find();
        if(empty($find_data)){
        ...
        if(strtoupper($_SERVER['REQUEST_METHOD']) == 'POST'){
            ...
            if(empty($data_wxconfig)){
                $data = ['appid' => $_POST['appid'], 'appsecret' => $_POST['appsecret'], 'user_id' => $account_id];
                ...
                $data = ['appid' => $_POST['appid'], 'appsecret' => $_POST['appsecret']];
                $edit_id = $model_wxconfig->where(['user_id' => $account_id])->update($data);
                if($edit_id){
                    showMessage("修改成功！");
                    return;
                } else {
        ...
        $gcategories_list = [];
        $gcategories_list = $model_menu->where(['store_id' => $account_id])->select();
        ...
        if($gcategories_list){
            ...
            foreach($cate_ids as $id){
                $sorted_gcategories[] = array_merge($gcategories[$id], ['layer' => $tree->getLayer($id)]);
            }
        }
    ...
    function addOp() {
        if(strtoupper($_SERVER['REQUEST_METHOD']) != 'POST'){
            ...
            $gcategory = ['parent_id' => $pid, 'if_show' => 1];//'sort_order' => 255,
    ...
    function editOp() {
        ...
        if(strtoupper($_SERVER['REQUEST_METHOD']) != 'POST'){
            ...
            /* 是否存在 */
            $model_menu = Model('wechat_menu');
            $gcategory = $model_menu->where(['cate_id' => $id])->find();
            if(!$gcategory){
                echo "分类不存在！";
                return;
            }
            ...
            $model_menu = Model('wechat_menu');
            $edit_id = $model_menu->where(['cate_id' => $id])->update($data);
    ...
    function dropOp() {
        ...
        $ids = explode(',', $id);
        $model_menu = Model('wechat_menu');
        $model_menu->where(['cate_id' => ['in', $ids]])->delete();
    ...
    function creat_menuOp() {
        $account_id = intval($_SESSION['member_id']);
        $model_wxconfig = Model('wxconfig');
        $wechat = $model_wxconfig->where(['user_id' => $account_id])->find();
    ...
    public function getmenu() {
        ...
        $keyword = [];
        $model_menu = Model('wechat_menu');
        $topmemu = $model_menu->where(['store_id' => $account_id, 'if_show' => 1, 'parent_id' => 0])->select();
     ...
     */
        ...
        foreach($topmemu as $key){
            $nextmenu = $model_menu->where([
                'store_id' => $account_id,
                'if_show' => 1,
                'parent_id' => $key['cate_id']
            ])->select();
            ...
            if(count($nextmenu) != 0)//没有下级栏目
            {
                foreach($nextmenu as $key2){
                    if($key2['type'] == 1){
                        $kk[] = ['type' => 'view', 'name' => $key2['cate_name'], 'url' => $key2['keyvalue']];
                    } else {
                        $kk[] = ['type' => 'click', 'name' => $key2['cate_name'], 'key' => $key2['keyvalue']];
                    }
                }
                $keyword['button'][] = ['name' => $key['cate_name'], 'sub_button' => $kk];
                $kk = '';
            } else {
                if($key['type'] == 1){
                    $keyword['button'][] = [
    ...
    function _get_options($except = null) {
        ...
        $account_id = intval($_SESSION['member_id']);
        $model_menu = Model('wechat_menu');
        $gcategories = $model_menu->where(['store_id' => $account_id])->select();
