 ...
 * 买家相册
 */
class  AlbumControl extends BaseSNSControl {
    ...
    public function album_addOp() {
         ...
         */
        $model = Model();
        $class_count = $model->table('sns_albumclass')->where(['member_id' => $this->master_id])->count();
    ...
    public function album_add_saveOp() {
        if(chksubmit()){
             ...
             */
            $model = Model();
            $class_count = $model->table('sns_albumclass')->where(['member_id' => $_SESSION['member_id']])->count();
            if($class_count >= 10){
    ...
    public function album_edit_saveOp() {
        ...
        $model = Model();
        // 更新
        $re = $model->table('sns_albumclass')->where(['ac_id' => intval($_POST['id'])])->update($update);
        if($re){
    ...
    public function album_delOp() {
         ...
         * 删除分类
         */
        $return = $model->table('sns_albumclass')->where([
            'ac_id' => $id,
        ...
        $where['member_id'] = $_SESSION['member_id'];
        $class_info = $model->table('sns_albumclass')->where($where)->find();
        $return = $model->where(['ac_id' => $id])->update(['ac_id' => $class_info['ac_id']]);
        if($return){
    ...
    public function change_album_coverOp() {
        ...
        $return = $model->table('sns_albumclass')->where(['ac_id' => $pic_info['ac_id']])->update($update);
        if($return){
    ...
    public function album_pic_delOp() {
        ...
        foreach($id as $v){
            ...
            $ap_info = $model->table('sns_albumpic')->where([
                'ap_id' => $v,
                'member_id' => $_SESSION['member_id']
            ])->find();
            if(empty($ap_info)){
            ...
            $model->table('sns_albumpic')->where(['ap_id' => $ap_info['ap_id']])->delete();
        }
    ...
    public function swfuploadOp() {
        ...
        $model = Model();
        // 验证图片数量
        $count = $model->table('sns_albumpic')->where(['member_id' => $member_id])->count();
