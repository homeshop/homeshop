 ...
 * 图片空间操作
 */
class  CircleControl extends BaseSNSControl {
    ...
    public function themeOp() {
        $model = Model();
        $theme_list = $model->table('circle_theme')->where(['member_id' => $this->master_id])->page(10)->order('theme_id desc')->select();
    ...
    public function circleOp() {
        $model = Model();
        $cm_list = $model->table('circle_member')->where(['member_id' => $this->master_id])->order('cm_jointime desc')->select();
        if(!empty($cm_list)){
    ...
    private function profile_menu($menu_key = '') {
        ...
        $menu_array = [
            1 => [
                'menu_key' => 'theme',
                'menu_name' => $theme_menuname,
                'menu_url' => 'index.php?act=sns_circle&op=theme&mid='.$this->master_id
            ],
            2 => [
                'menu_key' => 'circle',
                'menu_name' => $circle_menuname,
                'menu_url' => 'index.php?act=sns_circle&op=circle&mid='.$this->master_id
            ],
        ];
