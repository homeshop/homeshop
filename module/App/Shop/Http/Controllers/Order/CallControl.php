<?php namespace App\Shop\Http\Controllers\Order;
 ...
 * 卖家实物订单管理
 ***/
class  CallControl extends BaseSellerControl {
    ...
    public function getajaxorderOp() {
        ...
        if($order_list){
            ...
            foreach($order_list as $key => $order_info){
                ...
                //更改打印状态
                $model = Model();
                $model->table('order')->where(['order_id' => $order_info['order_id']])->update(['is_print' => 1]);
                //end
        ...
        $data = ['order_list' => $order_list, 'show_page' => $model_order->showpage(),];
        ...
        exit(json_encode([
            ...
            'print_order_arr' => $print_order_arr
        ]));
    ...
    public function show_orderOp() {
        ...
        if($order_info['order_state'] == ORDER_STATE_CANCEL){
            $order_info['close_info'] = $model_order->getOrderLogInfo(['order_id' => $order_info['order_id']], 'log_id desc');
        }
            ...
            case 'list':
                $menu_array = [
                    [
                        ...
                        'menu_url' => 'index.php?act=order_call'
                    ],
