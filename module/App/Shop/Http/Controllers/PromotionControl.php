...
class  PromotionControl extends BaseHomeControl {
    ...
    public function indexOp() {
        ...
        $condition = [];
        $condition['state'] = 1;
        $condition['start_time'] = ['elt', TIMESTAMP];
        ...
        $condition = ['goods_id' => ['in', array_keys($xs_goods_list)]];
        ...
        foreach($goods_list as $k => $goods_info){
            ...
            if($xs_goods_list[$goods_info['goods_id']]['gc_id_1'] != $goods_info['gc_id_1']){
                ...
                $model_xianshi_goods->editXianshiGoods(['gc_id_1' => $goods_info['gc_id_1']], ['xianshi_goods_id' => $xs_goods_list[$goods_info['goods_id']]['xianshi_goods_id']]);
            }
        }
        
        //查询商品评分信息
        $goodsevallist = Model("evaluate_goods")->getEvaluateGoodsList([
            'geval_goodsid' => [
                'in',
                array_keys($xs_goods_list)
            ]
        ]);
        $eval_list = [];
