...
use App\Shop\Http\Controllers\BaseStoreControl;
...
class  StoreControl extends BaseStoreControl {
    ...
    private function getGoodsMore($goods_list1, $goods_list2 = []) {
        ...
        if(!empty($goods_list)){
            ...
            // 商品多图
            $goodsimage_more = Model('goods')->getGoodsImageList([
                'goods_commonid' => [
                    'in',
                    $commonid_array
                ]
        ...
        return [1 => $goods_list1, 2 => $goods_list2];
    }
    
    public function show_articleOp() {
        //判断是否为导航页面
        $model_store_navigation = Model('store_navigation');
        $store_navigation_info = $model_store_navigation->getStoreNavigationInfo(['sn_id' => intval($_GET['sn_id'])]);
    ...
    public function goods_allOp() {
        ...
        if(trim($_GET['inkeyword']) != ''){
            $condition['goods_name'] = ['like', '%'.trim($_GET['inkeyword']).'%'];
        }
        ...
        if(intval($_GET['stc_id']) > 0){
            $condition['goods_stcids'] = ['like', '%,'.intval($_GET['stc_id']).',%'];
        }
        
        $this->_getGoodsList($condition, $order);
        
        $stc_class = Model('store_goods_class');
        $stc_info = $stc_class->getStoreGoodsClassInfo(['stc_id' => intval($_GET['stc_id'])]);
    ...
    function cou_goodsOp() {
        ...
        $tablePre = C('tablepre');
        $condition[] = ['exp', "goods_id in (select sku_id from {$tablePre}p_cou_sku where cou_id = {$couId})",];
    ...
    function mansong_goodsOp() {
        $this->_getGoodsList(['store_id' => $this->store_info['store_id']]);
        Tpl::output('page', 'index');
    ...
    function ajax_store_trend_countOp() {
        $count = Model('store_sns_tracelog')->getStoreSnsTracelogCount(['strace_storeid' => $this->store_info['store_id']]);
    ...
    public function ajax_flowstat_recordOp() {
        ...
        if($store_id <= 0 || $_SESSION['store_id'] == $store_id){
            echo json_encode(['done' => true, 'msg' => 'done']);
            die;
        }
        //确定统计分表名称
        ...
        $model = Model('stat');
        //查询店铺流量统计数据是否存在
        $store_exist = $model->getoneByFlowstat($flow_tablename, [
            'stattime' => $stattime,
            'store_id' => $store_id,
            'type' => 'sum'
        ]);
        if($_GET['act_param'] == 'goods' && $_GET['op_param'] == 'index'){//统计商品页面流量
            ...
            if($goods_id <= 0){
                echo json_encode(['done' => false, 'msg' => 'done']);
                die;
            }
            $goods_exist = $model->getoneByFlowstat($flow_tablename, [
                'stattime' => $stattime,
                'goods_id' => $goods_id,
                'type' => 'goods'
            ]);
        ...
        $insert_arr = [];
        if($store_exist){
            $model->table($flow_tablename)->where([
                'stattime' => $stattime,
                'store_id' => $store_id,
                'type' => 'sum'
            ])->setInc('clicknum', 1);
