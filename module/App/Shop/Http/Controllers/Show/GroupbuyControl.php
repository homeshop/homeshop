...
class  GroupbuyControl extends BaseHomeControl {
    ...
    public function __construct() {
        ...
        $nav_link = [
            ...
            1 => ['title' => Language::get('nc_groupbuy')]
        ];
    ...
    public function indexOp() {
        ...
        // 线上抢购
        $groupbuy = $model_groupbuy->getGroupbuyOnlineList(['recommended' => 1, 'is_vr' => 0,], 9);
        Tpl::output('groupbuy', $groupbuy);
        
        // 虚拟抢购
        $vr_groupbuy = $model_groupbuy->getGroupbuyOnlineList(['recommended' => 1, 'is_vr' => 1,], 9);
        ...
        foreach(range(1, 4) as $i){
            $a = C('live_pic'.$i);
            if($a){
                $picArr[] = [$a, C('live_color'.$i), C('live_link'.$i)];
            }
        }
    ...
    private function _show_vr_groupbuy_list($function_name) {
        ...
        if(($price_id = intval($_GET['groupbuy_price'])) > 0 && isset($this->groupbuy_price[$price_id])){
            ...
            $condition['groupbuy_price'] = ['between', [$p['range_start'], $p['range_end']]];
        }
    ...
    public function groupbuy_detailOp() {
        ...
        // 浏览数加1
        $update_array = [];
        $update_array['views'] = ['exp', 'views+1'];
        $model_groupbuy->editGroupbuy($update_array, ['groupbuy_id' => $group_id]);
        ...
        Model('seo')->type('group_content')->param(['name' => $groupbuy_info['groupbuy_name']])->show();
    ...
    public function groupbuy_evaluateOp() {
        ...
        if($goods_commonid > 0){
            ...
            if(!empty($goods_list)){
                ...
                $model_evaluate = Model('evaluate_goods');
                $where = [];
                $where['geval_goodsid'] = ['in', $goodsid_array];
