...
class  GoodsControl extends BaseGoodsControl {
    ...
    public function indexOp() {
        ...
        if(empty($_cache)){
            // 查询SNS中该商品的信息
            $snsgoodsinfo = Model('sns_goods')->getSNSGoodsInfo(['snsgoods_goodsid' => $goods_info['goods_id']], 'snsgoods_likenum,snsgoods_sharenum');
            $_cache = [];
        ...
        if($goods_info['is_book'] == 1){
            if($goods_info['book_down_time'] < TIMESTAMP){
                ...
                Tpl::output('remain', ['day' => $remain_day, 'hour' => $remain_hour, 'minute' => $remain_minute]);
            }
        }
        //处理商品消费者保障服务信息
        $goods_list = $model_goods->getGoodsContract([0 => $goods_info]);
        ...
        $nav_link_list[] = ['title' => $goods_info['goods_name']];
    ...
    public function addbrowseOp() {
        ...
        QueueClient::push('addViewedGoods', [
            'goods_id' => $goods_id,
    ...
    public function comments_listOp() {
        ...
        $nav_link_list[] = [
            ...
            'link' => urlShop('goods', 'index', ['goods_id' => $goods_id])
        ];
        $nav_link_list[] = ['title' => '商品评价'];
        Tpl::output('nav_link_list', $nav_link_list);
    ...
    public function salelogOp() {
        ...
        if($_GET['vr']){
            ...
            $sales = $model_order->getOrderAndOrderGoodsSalesRecordList(['goods_id' => $goods_id], '*', 10);
        } else {
            $model_order = Model('order');
            $sales = $model_order->getOrderAndOrderGoodsSalesRecordList(['order_goods.goods_id' => $goods_id], 'order_goods.*, orders.buyer_name, orders.add_time', 10);
        }
        Tpl::output('show_page', $model_order->showpage());
        Tpl::output('sales', $sales);
        
        Tpl::output('order_type', [1 => '原价', 2 => '抢购', 3 => '折扣', 4 => '套装', 5 => '赠品', 8 => '原价', 9 => '换购']);
        Tpl::output('order_vr_type', [0 => '原价', 1 => '抢购']);
    ...
    public function save_consultOp() {
        ...
        if($_SESSION['member_id']){
            //查询会员信息
            $member_model = Model('member');
            $member_info = $member_model->getMemberInfo(['member_id' => $_SESSION['member_id']]);
    ...
    public function calcOp() {
        ...
        echo $_GET['callback'].'('.json_encode(['total' => $freight_total]).')';
    }
    ...
    public function arrival_notice_submitOp() {
        ...
        $js = "ajax_form('arrival_notice', '".$title."', '".urlShop('goods', 'arrival_notice_succ', ['type' => $type])."', 480);";
        showDialog('', '', 'js', $js);
    }
    ...
    public function show_chainOp() {
        ...
        $stock_list = $model_chain_stock->getChainStockList([
            'goods_id' => $goods_id,
            'stock' => ['gt', 0]
        ], 'chain_id');
        if(!empty($stock_list)){
            ...
            $chain_array = $model_chain->getChainList(['chain_id' => ['in', $chainid_array]]);
            $chain_list = [];
