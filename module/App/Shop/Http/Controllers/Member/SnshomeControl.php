 ...
 * SNS我的空间
 */
class  SnshomeControl extends BaseSNSControl {
    ...
    public function indexOp() {
        ...
        $goodslist = $model->table('sns_sharegoods,sns_goods')->on('sns_sharegoods.share_goodsid = sns_goods.snsgoods_goodsid')->join('inner')->where($where)->order('share_addtime desc')->limit(3)->select();
        ...
        // 我的图片
        $pic_list = $model->table('sns_albumpic')->where(['member_id' => $this->master_id])->order('ap_id desc')->limit(3)->select();
        ...
        if(!empty($storelist)){
            ...
            $goodslist = $goods_model->getGoodsOnlineList([
                'store_id' => [
                    'in',
                    $storeid_arr
                ]
            ], 'goods_id,goods_name,goods_image,store_id');
            if(!empty($goodslist)){
                foreach($goodslist as $k => $v){
                    $v['goodsurl'] = urlShop('goods', 'index', ['goods_id' => $v['goods_id']]);
    ...
    public function shareglistOp() {
        ...
        if($_GET['type'] != 'like' && !empty($goodslist)){
            ...
            $pic_array = Model()->table('sns_albumpic')->field('count(item_id) as count,item_id,min(ap_cover) as ap_cover')->where([
                'ap_type' => 1,
                'item_id' => ['in', $shareid_array]
            ])->group('item_id')->select();
            if(!empty($pic_array)){
                $pic_list = [];
    ...
    public function goodsinfoOp() {
        ...
        $sharegoods_info['snsgoods_goodsurl'] = urlShop('goods', 'index', ['goods_id' => $sharegoods_info['snsgoods_goodsid']]);
        ...
        if($_GET['type'] != 'like'){
            // 买下秀图片
            $pic_list = $model->table('sns_albumpic')->where([
        ...
        // 更多分享/喜欢商品
        $sharegoods_list = $model->table('sns_sharegoods,sns_goods')->join('inner')->on('sns_sharegoods.share_goodsid=sns_goods.snsgoods_goodsid')->where($where)->limit(9)->select();
    ...
    public function storelistOp() {
        ...
        if(!empty($storelist)){
            ...
            $goods_model = Model('goods');
            $goodslist = $goods_model->getGoodsOnlineList([
                'store_id' => ['in', $storeid_arr],
                'goods_commend' => 1
            ], 'goods_id,store_id,goods_name,goods_image');
            if(!empty($goodslist)){
                foreach($goodslist as $k => $v){
                    $v['goodsurl'] = urlShop('goods', 'index', ['goods_id' => $v['goods_id']]);
                    $storelist_new[$v['store_id']]['goods'][] = $v;
            ...
            foreach($storeid_arr as $val){
                $storelist_new[$val]['goods_count'] = $goods_model->getGoodsCommonCount(['store_id' => $val]);
            }
        }
        //信息输出
        Tpl::output('storelist', $storelist_new);
    ...
    public function add_shareOp() {
        ...
        $sharegoods_info = $model->table('sns_goods')->where(['snsgoods_goodsid' => intval($_GET['gid'])])->find();
        Tpl::output('sharegoods_info', $sharegoods_info);
        Tpl::output('sid', $sid);
    ...
    public function image_uploadOp() {
         ...
         */
        $model = Model();
        $default_class = $model->table('sns_albumclass')->where([
        ...
        // 验证图片数量
        $count = $model->table('sns_albumpic')->where(['member_id' => $_SESSION['member_id']])->count();
        ...
        if($ap_id > 0){
            $pic_info = $model->table('sns_albumpic')->where(['ap_id' => $ap_id])->find();
            if(!empty($pic_info)){
    ...
    public function del_sharepicOp() {
        ...
        $data = [];
        if($ap_id > 0){
            Model()->table('sns_albumpic')->where([
                'ap_id' => $ap_id,
                'member_id' => $_SESSION['member_id']
            ])->delete();
            $data['type'] = 'true';
