 ...
 * 投诉
 */
class  ComplainControl extends BaseMemberControl {
    ...
    public function complain_newOp() {
        ...
        if(intval($order_list[$order_id]['extend_complain'][$goods_id]) == 1){//退款投诉
            $complain_subject = Model()->table('complain_subject')->where(['complain_subject_id' => 1])->select();//投诉主题
    ...
    public function complain_cancelOp() {
        ...
        if(intval($complain_info['complain_state']) === 10){
            ...
            $model_complain = Model('complain');
            $model_complain->dropComplain(['complain_id' => $complain_id]);
    ...
    private function profile_menu($menu_key = '') {
        $menu_array = [
            [
                'menu_key' => 'complain_accuser_list',
                'menu_name' => Language::get('complain_manage_title'),
                'menu_url' => 'index.php?act=member_complain'
            ]
        ];
