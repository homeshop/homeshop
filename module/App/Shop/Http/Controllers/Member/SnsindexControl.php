<?php namespace App\Shop\Http\Controllers\Member;
 ...
 * SNS首页
 */
class  SnsindexControl extends BaseMemberControl {
    ...
    public function indexOp() {
        ...
        $visitme_list = $visitor_model->getVisitorList(['v_ownermid' => "{$_SESSION['member_id']}", 'limit' => 9]);
        ...
        //查询我访问过的人
        $visitother_list = $visitor_model->getVisitorList(['v_mid' => "{$_SESSION['member_id']}", 'limit' => 9]);
    ...
    public function addtraceOp() {
        ...
        $validate_arr[] = [
            "input" => $_POST["content"],
            "require" => "true",
            "message" => Language::get('sns_sharemood_content_null')
        ];
        $validate_arr[] = [
            "input" => $_POST["content"],
            "validator" => 'Length',
            "min" => 0,
            "max" => 140,
            "message" => Language::get('sns_content_beyond')
        ];
        //发帖数超过最大次数出现验证码
        if(intval(cookie('weibonum')) >= self::MAX_RECORDNUM){
            $validate_arr[] = [
                "input" => $_POST["captcha"],
                "require" => "true",
                "message" => Language::get('wrong_null')
            ];
        }
        ...
        $member_info = $member_model->getMemberInfo(['member_id' => $_SESSION['member_id'], 'member_state' => 1]);
    ...
    public function sharegoodsOp() {
        ...
        if($_POST['form_submit'] == 'ok'){
            $obj_validate = new Validate();
            $validate_arr[] = [
                "input" => $_POST["choosegoodsid"],
                "require" => "true",
                "message" => Language::get('sns_sharegoods_choose')
            ];
            $validate_arr[] = [
                "input" => $_POST["comment"],
                "validator" => 'Length',
            ...
            //判断该商品是否已经存在分享或者喜欢记录
            $sharegoods_info = $sharegoods_model->getSharegoodsInfo([
                'share_memberid' => "{$_SESSION['member_id']}",
                'share_goodsid' => "{$goods_info['goods_id']}"
            ]);
            $result = false;
            if(empty($sharegoods_info)){
                ...
                $result = $sharegoods_model->editSharegoods($update_arr, ['share_id' => "{$sharegoods_info['share_id']}"]);
                unset($update_arr);
            }
            if($result){
                //商品缓存数据更新
                ...
                $snsgoods_info = $snsgoods_model->getGoodsInfo(['snsgoods_goodsid' => "{$goods_info['goods_id']}"]);
                if(empty($snsgoods_info)){
                    ...
                    $snsgoods_model->editGoods($update_arr, ['snsgoods_goodsid' => "{$goods_info['goods_id']}"]);
                }
                //添加分享动态
                $tracelog_model = Model('sns_tracelog');
                $insert_arr = [];
                ...
                $content_str .= "<div class=\"fd-media\">
                    <div class=\"goodsimg\"><a target=\"_blank\" href=\"".urlShop('goods', 'index', ['goods_id' => $goods_info['goods_id']])."\"><img src=\"".thumb($goods_info, 240)."\" onload=\"javascript:DrawImage(this,120,120);\" alt=\"{$goods_info['goods_name']}\"></a></div>
                    <div class=\"goodsinfo\">
                        <dl>
                            <dt><a target=\"_blank\" href=\"".urlShop('goods', 'index', ['goods_id' => $goods_info['goods_id']])."\">".$goods_info['goods_name']."</a></dt>
                ...
                if(C('share_isuse') == 1){
                    ...
                    $params['url'] = urlShop('goods', 'index', ['goods_id' => $goods_info['goods_id']]);
            ...
            // 查询收藏商品
            $favorites_list = Model()->table('favorites')->field('fav_id')->where([
                'member_id' => $_SESSION['member_id'],
                'fav_type' => 'goods'
            ])->select();
            $favorites_goodsid = [];
    ...
    public function sharestoreOp() {
        ...
        if($_POST['form_submit'] == 'ok'){
            $obj_validate = new Validate();
            $validate_arr[] = [
                "input" => $_POST["choosestoreid"],
                "require" => "true",
                "message" => Language::get('sns_sharestore_choose')
            ];
            $validate_arr[] = [
                "input" => $_POST["comment"],
                "validator" => 'Length',
            ...
            //判断该商品是否已经分享过
            $sharestore_info = $sharestore_model->getSharestoreInfo([
                'share_memberid' => "{$_SESSION['member_id']}",
                'share_storeid' => "{$store_info['store_id']}"
            ]);
            $result = false;
            if(empty($sharestore_info)){
                ...
                $result = $sharestore_model->editSharestore($update_arr, ['share_id' => "{$sharestore_info['share_id']}"]);
                unset($update_arr);
            }
            if($result){
                //添加分享动态
                ...
                $store_info['store_url'] = urlShop('show_store', 'index', ['store_id' => $store_info['store_id']]);
                $content_str .= "<div class=\"fd-media\">
                ...
                if(C('share_isuse') == 1){
                    ...
                    $params['url'] = urlShop('show_store', 'index', ['store_id' => $store_info['store_id']]);
    ...
    public function deltraceOp() {
        ...
        if($result){
            //修改该动态的转帖信息
            $tracelog_model->tracelogEdit(['trace_originalstate' => '1'], ['trace_originalid' => "$id"]);
            //删除对应的评论
    ...
    public function tracelistOp() {
        ...
        $friend_list = $friend_model->listFriend(['friend_frommid' => "{$_SESSION['member_id']}"], '*', '', 'simple');
        $mutualfollowid_arr = [];
        $followid_arr = [];
    ...
    public function editprivacyOp() {
        ...
        $privacy = in_array($_GET['privacy'], [0, 1, 2]) ? $_GET['privacy'] : 0;
        $result = $sharegoods_model->editSharegoods(['share_privacy' => "$privacy"], $condition);
        if($result){
    ...
    public function delgoodsOp() {
        ...
        if($result){
            ...
            if($_GET['type'] == 'like'){
                $snsgoods_model = Model('sns_goods');
                $snsgoods_info = $snsgoods_model->getGoodsInfo(['snsgoods_goodsid' => "{$sharegoods_info['share_goodsid']}"]);
                if(!empty($snsgoods_info)){
                    $update_arr = [];
                    ...
                    $snsgoods_model->editGoods($update_arr, ['snsgoods_goodsid' => "{$snsgoods_info['snsgoods_goodsid']}"]);
                }
            }
            $js = "location.reload();";
            showDialog(Language::get('nc_common_del_succ'), '', 'succ', $js);
    ...
    public function storeprivacyOp() {
        ...
        $condition['share_memberid'] = "{$_SESSION['member_id']}";
        $privacy = in_array($_GET['privacy'], [0, 1, 2]) ? $_GET['privacy'] : 0;
        $result = $sharestore_model->editSharestore(['share_privacy' => "$privacy"], $condition);
        if($result){
            $privacy_item = $privacy + 1;
            $js = "var obj = $(\"#recordone_{$id}\").find(\"[nc_type='privacytab']\"); $(obj).find('span').removeClass('selected');$(obj).find('li:nth-child(".$privacy_item.")').find('span').addClass('selected');";
            showDialog(Language::get('sns_setting_succ'), '', 'succ', $js);
    ...
    public function addcommentOp() {
        ...
        $validate_arr[] = [
            "input" => $_POST["commentcontent"],
            "require" => "true",
            "message" => Language::get('sns_comment_null')
        ];
        $validate_arr[] = [
            "input" => $_POST["commentcontent"],
            "validator" => 'Length',
        ...
        if($originaltype == 1){
            //查询分享和喜欢商品信息
            $sharegoods_model = Model('sns_sharegoods');
            $sharegoods_info = $sharegoods_model->getSharegoodsInfo(['share_id' => "{$originalid}"]);
            if(empty($sharegoods_info)){
            ...
            //查询原帖信息
            $tracelog_model = Model('sns_tracelog');
            $tracelog_info = $tracelog_model->getTracelogRow([
                'trace_id' => "{$originalid}",
                'trace_state' => '0'
            ]);
        ...
        if($result){
            if($originaltype == 1){
                //更新商品的评论数
                $update_arr = [];
                $update_arr['share_commentcount'] = ['exp', 'share_commentcount+1'];
                $sharegoods_model->editSharegoods($update_arr, ['share_id' => "{$originalid}"]);
            } else {
                //更新动态统计信息
                $update_arr = [];
                $update_arr['trace_commentcount'] = ['exp', 'trace_commentcount+1'];
                if(intval($tracelog_info['trace_originalid']) == 0){
                    $update_arr['trace_orgcommentcount'] = ['exp', 'trace_orgcommentcount+1'];
                }
                $tracelog_model->tracelogEdit($update_arr, ['trace_id' => "$originalid"]);
                unset($update_arr);
                //更新所有转帖的原帖评论次数
                if(intval($tracelog_info['trace_originalid']) == 0){
                    $tracelog_model->tracelogEdit(['trace_orgcommentcount' => $tracelog_info['trace_orgcommentcount'] + 1], ['trace_originalid' => "$originalid"]);
                }
            }
            //建立cookie
    ...
    public function delcommentOp() {
        ...
        $comment_model = Model('sns_comment');
        //查询评论信息
        $comment_info = $comment_model->getCommentRow([
            'comment_id' => "$id",
            'comment_memberid' => "{$_SESSION['member_id']}"
        ]);
        ...
        if($result){
            if($comment_info['comment_originaltype'] == 1){
                ...
                $update_arr = [];
                $update_arr['share_commentcount'] = ['exp', 'share_commentcount-1'];
                $sharegoods_model->editSharegoods($update_arr, ['share_id' => "{$comment_info['comment_originalid']}"]);
            } else {
                //更新动态统计信息
                $tracelog_model = Model('sns_tracelog');
                $update_arr = [];
                $update_arr['trace_commentcount'] = ['exp', 'trace_commentcount-1'];
                $tracelog_model->tracelogEdit($update_arr, ['trace_id' => "{$comment_info['comment_originalid']}"]);
            }
    ...
    public function editlikeOp() {
        ...
        $validate_arr[] = [
            "input" => $_GET["id"],
            "require" => "true",
            "message" => Language::get('sns_likegoods_choose')
        ];
        $obj_validate->validateparam = $validate_arr;
        $error = $obj_validate->validate();
        if($error != ''){
    ...
    public function addforwardOp() {
        ...
        $validate_arr[] = [
            ...
            "operator" => ' > ',
            'to' => 0,
            "message" => Language::get('sns_forward_fail')
        ];
        $validate_arr[] = [
            "input" => $_POST["forwardcontent"],
            "validator" => 'Length',
            "min" => 0,
            "max" => 140,
            "message" => Language::get('sns_content_beyond')
        ...
        //查询原帖信息
        $tracelog_model = Model('sns_tracelog');
        $tracelog_info = $tracelog_model->getTracelogRow(['trace_id' => "{$originalid}", 'trace_state' => "0"]);
        if(empty($tracelog_info)){
        ...
        if($result){
            ...
            $tracelog_model = Model('sns_tracelog');
            $update_arr = [];
            $update_arr['trace_copycount'] = ['exp', 'trace_copycount+1'];
            $update_arr['trace_orgcopycount'] = ['exp', 'trace_orgcopycount+1'];
            $condition = [];
            //原始贴和被转帖都增加转帖次数
            ...
            $tracelog_model->tracelogEdit(['trace_orgcopycount' => $tracelog_info['trace_orgcopycount'] + 1], $condition);
            if($_GET['irefresh']){
                //建立cookie
