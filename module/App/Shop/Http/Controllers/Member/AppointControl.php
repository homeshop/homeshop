 ...
 * 买家 预约/到货通知
 */
class  AppointControl extends BaseMemberControl {
    ...
    public function indexOp() {
        ...
        $appoint_list = $model_arrtivalnotice->getArrivalNoticeList(['member_id' => $_SESSION['member_id']], '*', '', '15');
    ...
    public function del_appointOp() {
        $id = intval($_GET['id']);
        $model_arrtivalnotice = Model('arrival_notice');
        $model_arrtivalnotice->delArrivalNotice(['member_id' => $_SESSION['member_id'], 'an_id' => $id]);
    ...
    private function profile_menu($menu_key = '') {
        $menu_array = [
            [
                ...
                'menu_name' => '预约/到货通知',
                'menu_url' => 'index.php?act=member_appoint'
            ]
        ];
