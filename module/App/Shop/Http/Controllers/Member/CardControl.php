...
class  CardControl extends BaseHomeControl {
    ...
    public function indexOp() {
        ...
        $model = Model();
        $member_info = $model->table('member')->field('member_id, member_name, member_truename, member_sex, member_email, member_qq, member_ww, member_areainfo, member_birthday, member_privacy, member_exppoints')->where(['member_id' => $uid])->find();
        ...
        $data['attention_count'] = $model->table('sns_friend')->where(['friend_frommid' => $uid])->count();
        
        // Number of fans
        $data['fans_count'] = $model->table('sns_friend')->where(['friend_tomid' => $uid])->count();
