 ...
 * 买家 我的实物订单
 */
class  OrderControl extends BaseMemberControl {
    ...
    public function indexOp() {
        ...
        if(preg_match('/^\d{10,20}$/', $_GET['keyword'])){
            ...
            $condition['order_id'] = ['in', $this->_getOrderIdByKeyword($_GET['keyword'])];
        }
        ...
        foreach($order_list as $order_id => $order_info){
            ...
            $_flag = ($order_info['order_state'] == ORDER_STATE_NEW && $order_info['order_type'] == 1) || ($order_info['order_state'] == ORDER_STATE_NEW && $order_info['order_type'] == 3 && $order_info['payment_code'] == 'online');
            if($_flag){
    ...
    public function search_deliverOp() {
        ...
        if(empty($order_info) || !in_array($order_info['order_state'], [ORDER_STATE_SEND, ORDER_STATE_SUCCESS])){
    ...
    public function change_stateOp() {
        ...
        if($_GET['state_type'] == 'order_cancel'){
            ...
            if($_GET['state_type'] == 'order_receive'){
                $result = $this->_order_receive($order_info, $_POST);
            } else {
                if(in_array($_GET['state_type'], ['order_delete', 'order_drop', 'order_restore'])){
    ...
    private function _order_recycle($order_info, $get) {
        $model_order = Model('order');
        $logic_order = Logic('order');
        $state_type = str_replace(['order_delete', 'order_drop', 'order_restore'], [
            'delete',
            'drop',
            'restore'
        ], $_GET['state_type']);
    ...
    private function profile_menu($menu_key = '') {
        ...
        $menu_array = [
            ...
            [
                'menu_key' => 'member_order_recycle',
                'menu_name' => '回收站',
                'menu_url' => 'index.php?act=member_order&recycle=1'
            ],
        ];
    ...
    private function _getOrderIdByKeyword($keyword) {
        ...
        ], 'order_id', 100, null, '', null, 'order_id');
