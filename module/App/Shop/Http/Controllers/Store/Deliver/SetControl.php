<?php namespace App\Shop\Http\Controllers\Store\Deliver;
 ...
 * 发货设置
 */
class  SetControl extends BaseSellerControl {
    ...
    public function daddress_addOp() {
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                [
                    "input" => $_POST["seller_name"],
                    "require" => "true",
                    "message" => $lang['store_daddress_receiver_null']
                ],
                [
                    "input" => $_POST["area_id"],
                    "require" => "true",
                    "validator" => "Number",
                    "message" => $lang['store_daddress_wrong_area']
                ],
                [
                    "input" => $_POST["city_id"],
                    "require" => "true",
                ...
                ["input" => $_POST["region"], "require" => "true", "message" => $lang['store_daddress_area_null']],
                [
                    "input" => $_POST["address"],
                    "require" => "true",
                    "message" => $lang['store_daddress_address_null']
                ],
                [
                    "input" => $_POST['telphone'],
                    'require' => 'true',
                    'message' => $lang['store_daddress_phone_and_mobile']
                ]
            ];
    ...
    public function daddress_default_setOp() {
        ...
        $condition = [];
        $condition['store_id'] = $_SESSION['store_id'];
        $update = Model('daddress')->editAddress(['is_default' => 0], $condition);
        $condition['address_id'] = $address_id;
        $update = Model('daddress')->editAddress(['is_default' => 1], $condition);
    }
    ...
    public function expressOp() {
        ...
        if(chksubmit()){
            ...
            if(!$model->getby_store_id($_SESSION['store_id'])){
                ...
                $result = $model->where(['store_id' => $_SESSION['store_id']])->update($data);
            }
            if($result){
    ...
    public function free_freightOp() {
        ...
        if(chksubmit()){
            ...
            $model_store->editStore(['store_free_price' => $store_free_price], ['store_id' => $_SESSION['store_id']]);
    ...
    public function deliver_regionOp() {
        if(chksubmit()){
            Model('store')->editStore(['deliver_region' => $_POST['area_ids'].'|'.$_POST['region']], ['store_id' => $_SESSION['store_id']]);
            showDialog(L('nc_common_save_succ'), '', 'succ');
        }
        $deliver_region = ['', ''];
    ...
    public function print_setOp() {
        $model = Model();
        $store_info = $model->table('store')->where(['store_id' => $_SESSION['store_id']])->find();
        if(empty($store_info)){
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                [
                    "input" => $_POST['store_printdesc'],
                    "require" => "true",
                    "validator" => "Length",
                    "min" => 1,
                    "max" => 200,
                    "message" => Language::get('store_printsetup_desc_error')
                ]
            ];
            $error = $obj_validate->validate();
            if($error != ''){
            ...
            $rs = $model->table('store')->where(['store_id' => $_SESSION['store_id']])->update($update_arr);
            if($rs){
        ...
        switch($menu_type) {
            case 'daddress':
                $menu_array = [
                    [
                        'menu_key' => 'daddress',
                        'menu_name' => Language::get('store_deliver_daddress_list'),
                        'menu_url' => 'index.php?act=store_deliver_set&op=daddress_list'
                    ],
                    [
                        'menu_key' => 'express',
                        'menu_name' => Language::get('store_deliver_default_express'),
                        'menu_url' => 'index.php?act=store_deliver_set&op=express'
                    ],
                    [
                        'menu_key' => 'free_freight',
                        'menu_name' => '免运费额度',
                        'menu_url' => 'index.php?act=store_deliver_set&op=free_freight'
                    ],
                    [
                        'menu_key' => 'deliver_region',
                        'menu_name' => '默认配送地区',
                        'menu_url' => 'index.php?act=store_deliver_set&op=deliver_region'
                    ],
                    [
                        'menu_key' => 'print_set',
                        'menu_name' => '发货单打印设置',
                        'menu_url' => 'index.php?act=store_deliver_set&op=print_set'
                    ]
                ];
                break;
        }
