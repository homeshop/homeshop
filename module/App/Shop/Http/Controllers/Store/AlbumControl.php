...
class  AlbumControl extends BaseSellerControl {
    ...
    public function album_cateOp() {
         ...
         * 验证是否存在默认相册
         */
        $return = $model_album->checkAlbum([
            'album_aclass.store_id' => $_SESSION['store_id'],
            'is_default' => '1'
        ]);
        if(!$return){
            $album_arr = [];
    ...
    public function album_editOp() {
        ...
        $model_album = Model('album');
        $param = [];
        $param['field'] = ['aclass_id', 'store_id'];
        $param['value'] = [intval($_GET['id']), $_SESSION['store_id']];
    ...
    public function album_edit_saveOp() {
        ...
        $return = $model_album->checkAlbum([
            'album_aclass.store_id' => $_SESSION['store_id'],
            'album_aclass.aclass_id' => intval($_POST['id'])
        ]);
        if($return){
            /**
             * 更新
    ...
    public function album_delOp() {
        ...
        $return = $model_album->checkAlbum([
            'album_aclass.store_id' => $_SESSION['store_id'],
            'album_aclass.aclass_id' => intval($_GET['id']),
            'is_default' => '0'
        ]);
        if(!$return){
         ...
         */
        $param = [];
        $param['field'] = ['is_default', 'store_id'];
        $param['value'] = ['1', $_SESSION['store_id']];
        $class_info = $model_album->getOneClass($param);
        $param = [];
        $param['aclass_id'] = $class_info['aclass_id'];
        $return = $model_album->updatePic($param, ['aclass_id' => intval($_GET['id'])]);
        if($return){
    ...
    public function pic_listOp() {
         ...
         */
        $page = new Page();
        if(in_array($_GET['item'], ['goods_image'])){
            $page->setEachNum(12);
        } else {
            $page->setEachNum(14);
        }
        ...
        if(!empty($_GET) && $_GET['id'] != '0'){
             ...
             */
            $cparam = [];
            $cparam['field'] = ['aclass_id', 'store_id'];
            $cparam['value'] = [intval($_GET['id']), $_SESSION['store_id']];
    ...
    public function change_album_coverOp() {
         ...
         */
        $param = [];
        $param['field'] = ['apic_id', 'store_id'];
        $param['value'] = [intval($_GET['id']), $_SESSION['store_id']];
        $pic_info = $model_album->getOnePicById($param);
        $return = $model_album->checkAlbum([
            'album_aclass.store_id' => $_SESSION['store_id'],
            'album_aclass.aclass_id' => $pic_info['aclass_id']
        ]);
        if($return){
            $re = $model_album->updateClass(['aclass_cover' => $pic_info['apic_cover']], $pic_info['aclass_id']);
            if($re){
    ...
    public function change_pic_nameOp() {
        ...
        $return = $model_album->updatePic(['apic_name' => $_POST['name']], ['apic_id' => intval($_POST['id'])]);
        if($return){
            echo 'true';
        } else {
            echo 'false';
    ...
    public function album_pic_infoOp() {
         ...
         */
        $return = $model_album->checkAlbum([
            'album_pic.store_id' => $_SESSION['store_id'],
            'album_pic.apic_id' => intval($_GET['id'])
        ]);
        if(!$return){
            showMessage(Language::get('album_parameter_error'), '', 'html', 'error');
        }
        ...
        $param = [];
        $param['field'] = ['aclass_id', 'store_id'];
        $param['value'] = [intval($_GET['class_id']), $_SESSION['store_id']];
        $class_info = $model_album->getOneClass($param);
        Tpl::output('class_info', $class_info);
    ...
    public function album_pic_delOp() {
        ...
        $return = $model_album->checkAlbum(['album_pic.store_id' => $_SESSION['store_id'], 'in_apic_id' => $id]);
        if(!$return){
    ...
    public function album_pic_moveOp() {
        ...
        if(chksubmit()){
            ...
            if($class_cover != ''){
                ...
                foreach($list_pic as $val){
                    if(str_ireplace('.', '_small.', $val['apic_cover']) == $class_cover){
                        $model_album->updateClass(['aclass_cover' => ''], $class_id);
                        break;
                    }
                }
            }
            ...
            $return = $model_album->updatePic($param, ['in_apic_id' => "'".$_REQUEST['id']."'"]);
            if($return){
    ...
    public function replace_image_uploadOp() {
        ...
        $param = [];
        $param['field'] = ['apic_id', 'store_id'];
        $param['value'] = [$id, $_SESSION['store_id']];
        ...
        $return = $model_album->updatePic($param, ['apic_id' => $id]);
        
        echo json_encode(['state' => 'true', 'id' => $id]);
        exit();
    }
    ...
    public function store_watermarkOp() {
        ...
        if(empty($store_wm_info)){
             ...
             * 新建店铺水印设置信息
             */
            $model_store_wm->addStoreWM(['wm_text_font' => 'default', 'store_id' => $_SESSION['store_id']]);
            $store_wm_info = $model_store_wm->getOneStoreWMByStoreId($_SESSION['store_id']);
        }
    ...
    public function image_uploadOp() {
        ...
        if($album_limit > 0){
            $album_count = Model('album')->getCount(['store_id' => $store_id]);
        ...
        switch($menu_type) {
            case 'album':
                $menu_array = [
                    1 => [
                        'menu_key' => 'album',
                        'menu_name' => Language::get('nc_member_path_my_album'),
                        'menu_url' => 'index.php?act=store_album'
                    ],
                    2 => [
                        'menu_key' => 'watermark',
                        'menu_name' => Language::get('nc_member_path_watermark'),
            ...
            case 'album_pic':
                $menu_array = [
                    ...
                    3 => [
                        'menu_key' => 'pic_list',
                        'menu_name' => Language::get('nc_member_path_album_pic_list'),
                        'menu_url' => 'index.php?act=store_album&op=album_pic_list&id='.intval($_GET['id'])
                    ],
                    2 => [
                        'menu_key' => 'watermark',
                        'menu_name' => Language::get('nc_member_path_watermark'),
            ...
            case 'album_pic_info':
                $menu_array = [
                    ...
                    3 => [
                        'menu_key' => 'pic_info',
                        'menu_name' => Language::get('nc_member_path_album_pic_info'),
                        'menu_url' => 'index.php?act=store_album&op=album_pic_info&id='.intval($_GET['id']).'&class_id='.intval($_GET['class_id'])
                    ],
                    2 => [
                        'menu_key' => 'watermark',
                        'menu_name' => Language::get('nc_member_path_watermark'),
    ...
    public function ajax_check_class_nameOp() {
        ...
        $model_album = Model('album');
        $param = [];
        $param['field'] = ['aclass_name', 'store_id'];
        $param['value'] = [$ac_name, $_SESSION['store_id']];
        $class_info = $model_album->getOneClass($param);
        if(!empty($class_info)){
            echo 'false';
            die;
