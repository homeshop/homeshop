 ...
 * 用户中心-限时折扣
 */
class  XianshiControl extends BaseSellerControl {
    ...
    public function xianshi_listOp() {
        ...
        if(!empty($_GET['xianshi_name'])){
            $condition['xianshi_name'] = ['like', '%'.$_GET['xianshi_name'].'%'];
        }
        if(!empty($_GET['state'])){
    ...
    public function xianshi_saveOp() {
        ...
        if($result){
            ...
            // 添加计划任务
            Model('cron')->addCron(['exetime' => $param['end_time'], 'exeid' => $result, 'type' => 7], true);
    ...
    public function xianshi_edit_saveOp() {
        ...
        $param['xianshi_explain'] = $_POST['xianshi_explain'];
        $param['lower_limit'] = $lower_limit;
        $result = $model_xianshi->editXianshi($param, ['xianshi_id' => $xianshi_id]);
        $result1 = $model_xianshi_goods->editXianshiGoods($param, ['xianshi_id' => $xianshi_id]);
        if($result && $result1){
    ...
    public function xianshi_delOp() {
        ...
        $model_xianshi = Model('p_xianshi');
        $result = $model_xianshi->delXianshi(['xianshi_id' => $xianshi_id]);
    ...
    public function xianshi_quota_add_saveOp() {
        ...
        if(empty($current_xianshi_quota)){
            ...
            $param = [];
            $param['end_time'] = ['exp', 'end_time + '.$add_time];
            $model_xianshi_quota->editXianshiQuota($param, ['quota_id' => $current_xianshi_quota['quota_id']]);
        }
    ...
    public function goods_selectOp() {
        ...
        $condition = [];
        $condition['store_id'] = $_SESSION['store_id'];
        $condition['goods_name'] = ['like', '%'.$_GET['goods_name'].'%'];
    ...
    public function xianshi_goods_addOp() {
        ...
        //检查商品是否已经参加同时段活动
        $condition = [];
        $condition['end_time'] = ['gt', $xianshi_info['start_time']];
        ...
        if($xianshi_goods_info){
            ...
            Model('cron')->addCron([
                'type' => 2,
                'exeid' => $goods_info['goods_id'],
                'exetime' => $param['start_time']
            ]);
        } else {
            $data['result'] = false;
            $data['message'] = L('param_error');
    ...
    public function xianshi_goods_deleteOp() {
        ...
        if(!$model_xianshi_goods->delXianshiGoods(['xianshi_goods_id' => $xianshi_goods_id])){
            $data['result'] = false;
    ...
    private function profile_menu($menu_key = '') {
        $menu_array = [
            1 => [
                'menu_key' => 'xianshi_list',
                'menu_name' => Language::get('promotion_active_list'),
                'menu_url' => 'index.php?act=store_promotion_xianshi&op=xianshi_list'
            ],
        ];
        switch($menu_key) {
            case 'xianshi_add':
                $menu_array[] = [
                    'menu_key' => 'xianshi_add',
                    'menu_name' => Language::get('promotion_join_active'),
                    'menu_url' => 'index.php?act=store_promotion_xianshi&op=xianshi_add'
                ];
                break;
            case 'xianshi_edit':
                $menu_array[] = [
                    'menu_key' => 'xianshi_edit',
                    'menu_name' => '编辑活动',
                    'menu_url' => 'javascript:;'
                ];
                break;
            case 'xianshi_quota_add':
                $menu_array[] = [
                    'menu_key' => 'xianshi_quota_add',
                    'menu_name' => Language::get('promotion_buy_product'),
                    'menu_url' => 'index.php?act=store_promotion_xianshi&op=xianshi_quota_add'
                ];
                break;
            case 'xianshi_manage':
                $menu_array[] = [
                    'menu_key' => 'xianshi_manage',
                    'menu_name' => Language::get('promotion_goods_manage'),
                    'menu_url' => 'index.php?act=store_promotion_xianshi&op=xianshi_manage&xianshi_id='.$_GET['xianshi_id']
                ];
                break;
        }
