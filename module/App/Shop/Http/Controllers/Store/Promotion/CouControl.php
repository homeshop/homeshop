 ...
 * 用户中心-加价购
 */
class  CouControl extends BaseSellerControl {
    ...
    public function cou_quota_add_saveOp() {
        ...
        if(empty($current_cou_quota)){
            ...
            $param = [];
            $param['tend'] = ['exp', 'tend + '.$add_time];
            $couQuotaModel->editCouQuota($param, ['id' => $current_cou_quota['id'],]);
        }
    ...
    public function cou_add_saveOp() {
        ...
        if($result){
            ...
            // 添加计划任务
            Model('cron')->addCron(['exetime' => $param['tend'], 'exeid' => $result, 'type' => 8], true);
    ...
    public function cou_edit_saveOp() {
        ...
        $couModel->editCou(['name' => $cou_name,], ['id' => $couId,]);
    ...
    public function cou_skuOp() {
        ...
        $where[] = [
            'exp',
            'goods_id not in (select sku_id from '.C('tablepre').'p_cou_sku where cou_id <> '.$couId.' and tstart < '.$cou_info['tend'].' and tend > '.$cou_info['tstart'].')',
        ];
    ...
    private function profile_menu($menu_key = '') {
        ...
        switch($menu_key) {
            case 'cou_quota_add':
                ...
                break;
            case 'cou_add':
                $menu_array[] = ['menu_key' => 'cou_add', 'menu_name' => '添加活动', 'menu_url' => 'javascript:;'];
                break;
            case 'cou_edit':
                $menu_array[] = ['menu_key' => 'cou_edit', 'menu_name' => '编辑活动', 'menu_url' => 'javascript:;'];
                break;
        }
