 ...
 * F码商品管理
 */
class  FcodeControl extends BaseSellerControl {
    ...
    public function fcode_listOp() {
        ...
        if($hasList){
            $goods_list = $model_fcode->getFCodeGoodsList(['store_id' => $_SESSION['store_id']], '*', null);
            if(!empty($goods_list)){
                $gcid_array = [];  // 商品分类id
    ...
    public function choosed_goodsOp() {
        ...
        if(!checkPlatformStore()){
            // 验证套餐时候过期
            $fcode_info = $model_fcode->getFCodeQuotaInfo([
                'store_id' => $_SESSION['store_id'],
                'fcq_endtime' => ['gt', TIMESTAMP]
            ]);
            if(empty($fcode_info)){
                if($_GET['inajax']){
        ...
        if(chksubmit()){
            ...
            if($rs){
                // 生成F码
                QueueClient::push('createGoodsFCode', [
                    'goods_id' => $gid,
    ...
    public function download_f_code_excelOp() {
        ...
        //header
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '号码'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '使用状态'];
        $data = Model('goods_fcode')->getGoodsFCodeList(['goods_id' => $goods_id]);
        foreach($data as $k => $v){
            $tmp = [];
            $tmp[] = ['data' => $v['fc_code']];
            $tmp[] = ['data' => $v['fc_state'] ? '已使用' : '未使用'];
            $excel_data[] = $tmp;
        }
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        ...
        switch($menu_type) {
            case 'fcode_goods_list':
                $menu_array = [
                    1 => [
                        'menu_key' => 'fcode_goods_list',
                        'menu_name' => '商品列表',
                        'menu_url' => urlShop('store_promotion_fcode', 'fcode_goods_list')
                    ]
                ];
                break;
            case 'fcode_quota_add':
                $menu_array = [
                    ...
                    2 => [
                        'menu_key' => 'fcode_quota_add',
                        'menu_name' => '购买套餐',
                        'menu_url' => urlShop('store_promotion_fcode', 'fcode_quota_add')
                    ]
                ];
                break;
            case 'fcode_renew':
                $menu_array = [
                    ...
                    2 => [
                        'menu_key' => 'fcode_renew',
                        'menu_name' => '套餐续费',
                        'menu_url' => urlShop('store_promotion_fcode', 'fcode_renew')
                    ]
                ];
                break;
        }
