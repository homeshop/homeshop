 ...
 * 预定商品管理
 */
class  BookControl extends BaseSellerControl {
    ...
    public function book_listOp() {
        ...
        if($hasList){
            $goods_list = $model_book->getAllGoodsList(['store_id' => $_SESSION['store_id']], '*', null);
            if(!empty($goods_list)){
                $gcid_array = [];  // 商品分类id
    ...
    public function choosed_goodsOp() {
        ...
        if(!checkPlatformStore()){
            // 验证套餐时候过期
            $book_info = $model_book->getBookQuotaInfo([
                'store_id' => $_SESSION['store_id'],
                'bkq_endtime' => ['gt', TIMESTAMP]
            ]);
            if(empty($book_info)){
                if($_GET['inajax']){
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        ...
        switch($menu_type) {
            case 'book_goods_list':
                $menu_array = [
                    1 => [
                        'menu_key' => 'book_goods_list',
                        'menu_name' => '商品列表',
                        'menu_url' => urlShop('store_promotion_book', 'book_goods_list')
                    ]
                ];
                break;
            case 'book_quota_add':
                $menu_array = [
                    ...
                    2 => [
                        'menu_key' => 'book_quota_add',
                        'menu_name' => '购买套餐',
                        'menu_url' => urlShop('store_promotion_book', 'book_quota_add')
                    ]
                ];
                break;
            case 'book_renew':
                $menu_array = [
                    ...
                    2 => [
                        'menu_key' => 'book_renew',
                        'menu_name' => '套餐续费',
                        'menu_url' => urlShop('store_promotion_book', 'book_renew')
                    ]
                ];
                break;
        }
