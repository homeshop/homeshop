 ...
 * 手机专享管理
 */
class  SoleControl extends BaseSellerControl {
    ...
    public function sole_goods_listOp() {
        ...
        $where['sole_quota_endtime'] = ['lt', TIMESTAMP];
        ...
        if($hasList){
            // 查询已选择商品
            $solegoods_list = $model_sole->getSoleGoodsList(['store_id' => $_SESSION['store_id']]);
    ...
    public function choosed_goodsOp() {
        ...
        $solegoods_info = Model('p_sole')->getSoleGoodsInfo([
    ...
    public function choosed_goods_saveOp() {
        ...
        if(!checkPlatformStore()){
            // 验证套餐时候过期
            $sole_info = $model_sole->getSoleQuotaInfo([
                'store_id' => $_SESSION['store_id'],
                'sole_quota_endtime' => ['gt', TIMESTAMP]
            ], 'sole_quota_id');
            if(empty($sole_info)){
        ...
        $solegoods_info = $model_sole->getSoleGoodsInfo([
            'store_id' => $_SESSION['store_id'],
        ...
        if(empty($solegoods_info)){
            ...
            $model_sole->editSoleGoods($param, ['sole_goods_id' => $solegoods_info['sole_goods_id']]);
    ...
    public function del_choosed_goodsOp() {
        ...
        $result = Model('p_sole')->delSoleGoods(['goods_id' => $gid, 'store_id' => $_SESSION['store_id']]);
        if($result){
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        ...
        switch($menu_type) {
            case 'sole_goods_list':
                $menu_array = [
                    1 => [
                        'menu_key' => 'sole_goods_list',
                        'menu_name' => '商品列表',
                        'menu_url' => urlShop('store_promotion_sole', 'sole_goods_list')
                    ]
                ];
                break;
            case 'sole_quota_add':
                $menu_array = [
                    ...
                    2 => [
                        'menu_key' => 'sole_quota_add',
                        'menu_name' => '购买套餐',
                        'menu_url' => urlShop('store_promotion_sole', 'sole_quota_add')
                    ]
                ];
                break;
            case 'sole_renew':
                $menu_array = [
                    ...
                    2 => [
                        'menu_key' => 'sole_renew',
                        'menu_name' => '套餐续费',
                        'menu_url' => urlShop('store_promotion_sole', 'sole_renew')
                    ]
                ];
                break;
        }
