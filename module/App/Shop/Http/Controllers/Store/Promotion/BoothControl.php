 ...
 * 推荐展位管理
 */
class  BoothControl extends BaseSellerControl {
    ...
    public function booth_goods_listOp() {
        ...
        $where['booth_quota_endtime'] = ['lt', TIMESTAMP];
        ...
        if($hasList){
            // 查询已选择商品
            $boothgoods_list = $model_booth->getBoothGoodsList(['store_id' => $_SESSION['store_id']], 'goods_id');
    ...
    public function choosed_goodsOp() {
        ...
        if(!checkPlatformStore()){
            // 验证套餐时候过期
            $booth_info = $model_booth->getBoothQuotaInfo([
                'store_id' => $_SESSION['store_id'],
                'booth_quota_endtime' => ['gt', TIMESTAMP]
            ], 'booth_quota_id');
            if(empty($booth_info)){
                $data = ['result' => 'false', 'msg' => '套餐过期请重新购买套餐'];
                $this->_echoJson($data);
            }
        }
        
        // 验证已添加商品数量，及选择商品是否已经被添加过
        $bootgoods_info = $model_booth->getBoothGoodsList(['store_id' => $_SESSION['store_id']], 'goods_id');
        // 已添加商品总数
        if(count($bootgoods_info) >= C('promotion_booth_goods_sum')){
            $data = ['result' => 'false', 'msg' => '只能添加'.C('promotion_booth_goods_sum').'个商品'];
            $this->_echoJson($data);
        }
        // 商品是否已经被添加
        $bootgoods_info = array_under_reset($bootgoods_info, 'goods_id');
        if(isset($bootgoods_info[$gid])){
            $data = ['result' => 'false', 'msg' => '商品已经添加，请选择其他商品'];
            $this->_echoJson($data);
        }
        ...
        $goods_info['url'] = urlShop('goods', 'index', ['goods_id' => $goods_info['goods_id']]);
    ...
    public function del_choosed_goodsOp() {
        ...
        $result = Model('p_booth')->delBoothGoods(['goods_id' => $gid, 'store_id' => $_SESSION['store_id']]);
        if($result){
            $this->recordSellerLog('删除推荐展位商品，商品id：'.$gid);
            $data = ['result' => 'true'];
        } else {
            $data = ['result' => 'false', 'msg' => '删除失败'];
        }
        $this->_echoJson($data);
    }
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        ...
        switch($menu_type) {
            case 'booth_goods_list':
                $menu_array = [
                    1 => [
                        'menu_key' => 'booth_goods_list',
                        'menu_name' => '商品列表',
                        'menu_url' => urlShop('store_promotion_booth', 'booth_goods_list')
                    ]
                ];
                break;
            case 'booth_quota_add':
                $menu_array = [
                    ...
                    2 => [
                        'menu_key' => 'booth_quota_add',
                        'menu_name' => '购买套餐',
                        'menu_url' => urlShop('store_promotion_booth', 'booth_quota_add')
                    ]
                ];
                break;
            case 'booth_renew':
                $menu_array = [
                    ...
                    2 => [
                        'menu_key' => 'booth_renew',
                        'menu_name' => '套餐续费',
                        'menu_url' => urlShop('store_promotion_booth', 'booth_renew')
                    ]
                ];
                break;
        }
