 ...
 * 商户中心-满就送
 */
class  MansongControl extends BaseSellerControl {
    ...
    public function mansong_listOp() {
        ...
        if(!empty($_GET['mansong_name'])){
            $condition['mansong_name'] = ['like', '%'.$_GET['mansong_name'].'%'];
        }
        if(!empty($_GET['state'])){
    ...
    public function mansong_quota_add_saveOp() {
        ...
        if(empty($current_mansong_quota)){
            ...
            $model_mansong_quota->editMansongQuota($param, ['quota_id' => $current_mansong_quota['quota_id']]);
        }
    ...
    private function profile_menu($menu_key = '') {
        $menu_array = [
            1 => [
                'menu_key' => 'mansong_list',
                'menu_name' => Language::get('promotion_active_list'),
                'menu_url' => urlShop('store_promotion_mansong', 'mansong_list')
            ],
        ];
        switch($menu_key) {
            case 'mansong_add':
                $menu_array[] = [
                    'menu_key' => 'mansong_add',
                    'menu_name' => Language::get('promotion_join_active'),
                    'menu_url' => urlShop('store_promotion_mansong', 'mansong_add')
                ];
                break;
            case 'mansong_quota_add':
                $menu_array[] = [
                    'menu_key' => 'mansong_quota_add',
                    'menu_name' => Language::get('promotion_buy_product'),
                    'menu_url' => urlShop('store_promotion_mansong', 'mansong_quota_add')
                ];
                break;
            case 'mansong_detail':
                $menu_array[] = [
                    'menu_key' => 'mansong_detail',
                    'menu_name' => Language::get('mansong_active_content'),
                    'menu_url' => urlShop('store_promotion_mansong', 'mansong_detail', ['mansong_id' => $_GET['mansong_id']])
                ];
                break;
        }
