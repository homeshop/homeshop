 ...
 * 预定商品管理
 */
class  ComboControl extends BaseSellerControl {
    ...
    public function combo_listOp() {
        ...
        if($hasList){
            ...
            $combo_list = $model_combo_goods->getComboGoodsList(['store_id' => $_SESSION['store_id']], 'distinct goods_id', 10, ' goods_id desc');
            if(!empty($combo_list)){
                $goodsid_array = [];
                foreach($combo_list as $val){
                    $goodsid_array[] = $val['goods_id'];
                }
                $goods_list = Model('goods')->getGoodsList(['goods_id' => ['in', $goodsid_array]]);
    ...
    public function choosed_goodsOp() {
        ...
        if(!checkPlatformStore()){
            // 验证套餐时候过期
            $combo_info = $model_combo->getComboQuotaInfo([
                'store_id' => $_SESSION['store_id'],
                'cq_endtime' => ['gt', TIMESTAMP]
            ]);
            if(empty($combo_info)){
                if($_GET['inajax']){
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        ...
        switch($menu_type) {
            case 'combo_goods_list':
                $menu_array = [
                    1 => [
                        'menu_key' => 'combo_goods_list',
                        'menu_name' => '商品列表',
                        'menu_url' => urlShop('store_promotion_combo', 'combo_goods_list')
                    ]
                ];
                break;
            case 'combo_goods_add':
                $menu_array = [
                    ...
                    2 => ['menu_key' => 'combo_goods_add', 'menu_name' => '设置组合', 'menu_url' => 'javascript:;']
                ];
                break;
            case 'combo_quota_add':
                $menu_array = [
                    ...
                    2 => [
                        'menu_key' => 'combo_quota_add',
                        'menu_name' => '购买套餐',
                        'menu_url' => urlShop('store_promotion_combo', 'combo_quota_add')
                    ]
                ];
                break;
            case 'combo_renew':
                $menu_array = [
                    ...
                    2 => [
                        'menu_key' => 'combo_renew',
                        'menu_name' => '套餐续费',
                        'menu_url' => urlShop('store_promotion_combo', 'combo_renew')
                    ]
                ];
                break;
        }
