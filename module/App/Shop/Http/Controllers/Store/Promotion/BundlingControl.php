<?php namespace App\Shop\Http\Controllers\Store\Promotion;
 ...
 * 用户中心-优惠套装
 */
class  BundlingControl extends BaseSellerControl {
    ...
    public function bundling_listOp() {
        ...
        $where['bl_quota_endtime'] = ['lt', TIMESTAMP];
        ...
        if(checkPlatformStore()){
            ...
            if(!empty($bundling_quota)){
                // 计算已经发布活动、剩余活动数量
                $bundling_published = $model_bundling->getBundlingCount(['store_id' => $_SESSION['store_id']]);
        ...
        if($hasList){
            ...
            if($_GET['bundling_name'] != ''){
                $where['bl_name'] = ['like', '%'.trim($_GET['bundling_name']).'%'];
            }
            ...
            if(!empty($bundling_list)){
                ...
                $model_bundling->cls();
                $bgoods_array = $model_bundling->getBundlingGoodsList([
                    'bl_id' => [
                        'in',
                        $blid_array
                    ]
                ], 'min(bl_id) as bl_id,min(goods_id) as goods_id,min(goods_image) as goods_image,count(*) as count', 'bl_appoint desc', 'bl_id');
            ...
            // 状态数组
            $state_array = [0 => Language::get('bundling_status_0'), 1 => Language::get('bundling_status_1')];
    ...
    public function bundling_renewOp() {
        if(chksubmit()){
            ...
            if($return){
                ...
                // 添加任务队列
                Model('cron')->addCron([
                    'exetime' => $update['bl_quota_endtime'],
                    'exeid' => $_SESSION['store_id'],
                    'type' => 3
                ], true);
    ...
    public function bundling_addOp() {
        ...
        if(intval(C('promotion_bundling_sum')) != 0 && !isset($_REQUEST['bundling_id'])){
            $count = $model_bundling->getBundlingCount(['store_id' => $_SESSION['store_id']]);
        ...
        if(chksubmit()){
            ...
            $model_bundling->delBundlingGoods(['bl_id' => intval($_POST['bundling_id'])]);
        ...
        if(intval($_GET['bundling_id']) > 0){
            $bundling_info = $model_bundling->getBundlingInfo([
            ...
            $b_goods_list = $model_bundling->getBundlingGoodsList(['bl_id' => intval($_GET['bundling_id'])]);
            if(!empty($b_goods_list)){
                $goodsid_array = [];
                foreach($b_goods_list as $val){
                    $goodsid_array[] = $val['goods_id'];
                }
                $goods_list = Model('goods')->getGoodsList([
                    'goods_id' => [
                        'in',
                        $goodsid_array
                    ]
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        ...
        switch($menu_type) {
            ...
            case 'bundling_quota_list':
                $menu_array = [
                    1 => [
                        'menu_key' => 'bundling_list',
                        'menu_name' => Language::get('bundling_list'),
                        'menu_url' => urlShop('store_promotion_bundling', 'bundling_list')
                    ]
                ];
                break;
            case 'bundling_quota_add':
                $menu_array = [
                    ...
                    2 => [
                        'menu_key' => 'bundling_quota_add',
                        'menu_name' => Language::get('bundling_quota_add'),
                        'menu_url' => urlShop('store_promotion_bundling', 'bundling_quota_add')
                    ]
                ];
                break;
            case 'bundling_renew':
                $menu_array = [
                    ...
                    2 => [
                        'menu_key' => 'bundling_renew',
                        'menu_name' => '套餐续费',
                        'menu_url' => urlShop('store_promotion_bundling', 'bundling_renew')
                    ]
                ];
                break;
            case 'bundling_add':
                $menu_array = [
                    ...
                    2 => [
                        'menu_key' => 'bundling_add',
                        'menu_name' => Language::get('bundling_add'),
                        'menu_url' => urlShop('store_promotion_bundling', 'bundling_add')
                    ]
                ];
                break;
            case 'bundling_edit':
                $menu_array = [
                    ...
                    2 => [
                        'menu_key' => 'bundling_edit',
                        'menu_name' => Language::get('bundling_edit'),
                        'menu_url' => urlShop('store_promotion_bundling', 'bundling_edit')
                    ]
                ];
                break;
        }
