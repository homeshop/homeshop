...
class  DecorationControl extends BaseSellerControl {
    ...
    public function decoration_settingOp() {
        ...
        $store_decoration_info = $model_store_decoration->getStoreDecorationInfo(['store_id' => $_SESSION['store_id']]);
    ...
    public function decoration_setting_saveOp() {
        ...
        $result = $model_store->editStore($update, ['store_id' => $_SESSION['store_id']]);
        if($result){
        ...
        if($result){
            ...
            Model('store')->editStore([
                'store_decoration_image_count' => [
                    'exp',
                    'store_decoration_image_count+1'
                ]
            ], ['store_id' => $_SESSION['store_id']]);
    ...
    private function _validate_background_setting_input($decoration_id, $setting) {
        ...
        if(!empty($setting['background_image'])){
            ...
            if($setting['background_image_url'] == ''){
                return ['error', '请选择正确的背景图片'];
            }
        } else {
            $setting['background_image'] = '';
        }
        if(!in_array($setting['background_image_repeat'], ['no-repeat', 'repeat', 'repeat-x', 'repeat-y'])){
    ...
    public function decoration_block_addOp() {
        ...
        if($block_id){
            ob_start();
            Tpl::output('block', ['block_id' => $block_id]);
    ...
    public function decoration_block_sortOp() {
        ...
        foreach($sort_array as $value){
            ...
            $model_store_decoration->editStoreDecorationBlock(['block_sort' => $sort], $condition);
            $sort = $sort + 1;
        }
    ...
    public function goods_searchOp() {
        ...
        $condition = [];
        $condition['store_id'] = $_SESSION['store_id'];
        $condition['goods_name'] = ['like', '%'.$_GET['keyword'].'%'];
    ...
    private function _update_module_goods_info($decoration_id, $store_id) {
        ...
        if(!empty($block_list_goods) && is_array($block_list_goods)){
            foreach($block_list_goods as $block){
                ...
                $model_store_decoration->editStoreDecorationBlock($update, ['block_id' => $block['block_id']]);
            }
        }
    }
    ...
    private function profile_menu($menu_key = '') {
        $menu_array = [
            1 => [
                'menu_key' => 'decoration_setting',
                'menu_name' => '店铺装修',
                'menu_url' => urlShop('store_decoration', 'decoration_setting')
            ],
            2 => [
                'menu_key' => 'decoration_album',
                'menu_name' => '装修图库',
                'menu_url' => urlShop('store_decoration', 'decoration_album')
            ],
        ];
