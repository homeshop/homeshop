...
class  ContractControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        //查询店铺保障项目开启情况
        $c_list = $model_contract->getContractList(['ct_storeid' => $_SESSION['store_id']]);
        if($c_list){
    ...
    private function applyadd_platformstore($item_info, $c_info) {
        ...
        try {
            ...
            if($c_info){
                ...
                $result = $model_contract->editContract(['ct_id' => $c_info['ct_id']], $update_arr);
            } else {
                $insert_arr = [];
    ...
    public function applypayOp() {
        ...
        if(!in_array($c_info['ct_auditstate'], [
            $this->contract_auditstate_arr['auditpass']['sign'],
            $this->contract_auditstate_arr['costfailure']['sign']
        ])
        ){
            showDialog('申请信息错误', '', 'error');
        }
        ...
        $where['cta_auditstate'] = [
            'in',
            [$this->join_auditstate_arr['auditpass']['sign'], $this->join_auditstate_arr['costfailure']['sign']]
        ];
        ...
        if(chksubmit()){
            try {
                ...
                $result = $model_contract->editContractApply(['cta_id' => $apply_info['cta_id']], $update_arr);
                if(!$result){
            ...
            case 'log':
                $menu_array = [
                    1 => [
                        'menu_key' => 'index',
                        'menu_name' => '消费者保障服务',
                        'menu_url' => 'index.php?act=store_contract&op=index'
                    ],
                    2 => ['menu_key' => 'contractlog', 'menu_name' => '保障服务详情', 'menu_url' => '']
                ];
                break;
            case 'cost':
                $menu_array = [
                    ...
                    2 => ['menu_key' => 'cost', 'menu_name' => '保证金日志', 'menu_url' => '']
                ];
                break;
            case 'costpay':
                $menu_array = [
                    ...
                    2 => ['menu_key' => 'applypay', 'menu_name' => '支付保证金', 'menu_url' => '']
                ];
                break;
            default:
                $menu_array = [
