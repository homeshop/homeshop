 ...
 * 商家中心抢购管理
 */
class  GroupbuyControl extends BaseSellerControl {
    ...
    public function groupbuy_quota_add_saveOp() {
        ...
        if(empty($current_groupbuy_quota)){
            ...
            $model_groupbuy_quota->editGroupbuyQuota($param, ['quota_id' => $current_groupbuy_quota['quota_id']]);
        }
    ...
    public function groupbuy_listOp() {
        ...
        $condition['groupbuy_name'] = ['like', '%'.$_GET['groupbuy_name'].'%'];
    ...
    public function groupbuy_goods_infoOp() {
        ...
        $data['goods_href'] = urlShop('goods', 'index', ['goods_id' => $goods_info['goods_id']]);
    ...
    public function check_groupbuy_goodsOp() {
        ...
        //检查商品是否已经参加同时段活动
        $condition = [];
        $condition['end_time'] = ['gt', $start_time];
    ...
    public function groupbuy_add_vrOp() {
        ...
        $classlist = $model_vr_groupbuy_class->getVrGroupbuyClassList(['parent_class_id' => 0]);
    ...
    private function profile_menu($menu_key = '') {
        $menu_array = [
            1 => [
                'menu_key' => 'groupbuy_list',
                'menu_name' => L('nc_member_path_group_list'),
                'menu_url' => urlShop('store_groupbuy', 'groupbuy_list')
            ]
        ];
        switch($menu_key) {
            case 'groupbuy_add':
                $menu_array[] = [
                    'menu_key' => 'groupbuy_add',
                    'menu_name' => L('nc_member_path_new_group'),
                    'menu_url' => 'index.php?act=store_groupbuy&op=groupbuy_add'
                ];
                break;
            case 'groupbuy_add_vr':
                $menu_array[] = [
                    'menu_key' => 'groupbuy_add_vr',
                    'menu_name' => '新增虚拟抢购',
                    'menu_url' => 'index.php?act=store_groupbuy&op=groupbuy_add_vr'
                ];
                break;
            case 'groupbuy_quota_add':
                $menu_array[] = [
                    'menu_key' => 'groupbuy_quota_add',
                    'menu_name' => '购买套餐',
                    'menu_url' => urlShop('store_groupbuy', 'groupbuy_quota_add')
                ];
                break;
            case 'groupbuy_edit':
                $menu_array[] = [
                    'menu_key' => 'groupbuy_edit',
                    'menu_name' => L('nc_member_path_edit_group'),
                    'menu_url' => 'index.php?act=store_groupbuy'
                ];
                break;
            case 'cancel':
                $menu_array[] = ['menu_key' => 'groupbuy_cancel', 'menu_name' => L('nc_member_path_cancel_group')];
                break;
        }
