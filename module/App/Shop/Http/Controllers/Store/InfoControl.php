...
class  InfoControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        // 店铺分类信息
        $store_class_info = $model_store_class->getStoreClassInfo(['sc_id' => $store_info['sc_id']]);
        ...
        $joinin_detail = $model_store_joinin->getOne(['member_id' => $store_info['member_id']]);
    ...
    public function bind_classOp() {
        ...
        $store_bind_class_list = $model_store_bind_class->getStoreBindClassList(['store_id' => $_SESSION['store_id']], null);
        $goods_class = Model('goods_class')->getGoodsClassIndexedListAll();
        for($i = 0, $j = count($store_bind_class_list); $i < $j; $i++){
            $store_bind_class_list[$i]['class_1_name'] = $goods_class[$store_bind_class_list[$i]['class_1']]['gc_name'];
            $store_bind_class_list[$i]['class_2_name'] = $goods_class[$store_bind_class_list[$i]['class_2']]['gc_name'];
    ...
    public function reopenOp() {
        ...
        $reopen_list = $model_store_reopen->getStoreReopenList(['re_store_id' => $_SESSION['store_id']]);
    ...
    public function reopen_uploadOp() {
        ...
        $data['re_state'] = 1;
        $model_store_reopen = Model('store_reopen');
        $update = $model_store_reopen->editStoreReopen($data, ['re_id' => $_POST['re_id'], 're_state' => 0]);
        if($update){
    ...
    private function profile_menu($menu_type, $menu_key = '', $array = []) {
        ...
        switch($menu_type) {
            case 'index':
                $menu_array[] = [
                    'menu_key' => 'bind_class',
                    'menu_name' => $lang['nc_member_path_bind_class'],
                    'menu_url' => 'index.php?act=store_info&op=bind_class'
                ];
                if(!checkPlatformStore()){
                    $menu_array[] = [
                        'menu_key' => 'index',
                        'menu_name' => $lang['nc_member_path_store_info'],
                        'menu_url' => 'index.php?act=store_info&op=index'
                    ];
                    $menu_array[] = [
                        'menu_key' => 'reopen',
                        'menu_name' => $lang['nc_member_path_store_reopen'],
                        'menu_url' => 'index.php?act=store_info&op=reopen'
                    ];
                }
                break;
            case 'bind_class':
                $menu_array = [
                    [
                        'menu_key' => 'index',
                        'menu_name' => $lang['nc_member_path_bind_class'],
                        'menu_url' => 'index.php?act=store_bind_class&op=index'
                    ],
                ];
                break;
            case 'add':
                $menu_array = [
                    ...
                    [
                        'menu_key' => 'add',
                        'menu_name' => $lang['nc_member_path_bind_class_add'],
                        'menu_url' => 'index.php?act=store_bind_class&op=add'
                    ]
                ];
                break;
        }
