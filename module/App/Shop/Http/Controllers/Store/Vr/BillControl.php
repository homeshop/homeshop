...
class  BillControl extends BaseSellerControl {
    ...
    public function show_billOp() {
        ...
        if(preg_match('/^\d{8,20}$/', $_GET['query_order_no'])){
            //取order_id
            $order_info = $model_order->getOrderInfo(['order_sn' => $_GET['query_order_no']], 'order_id');
        ...
        if($if_start_date || $if_end_date){
            $condition_time = ['time', [$start_unixtime, $end_unixtime]];
        } else {
            $condition_time = ['between', "{$bill_info['ob_start_date']},{$bill_info['ob_end_date']}"];
        }
    ...
    public function export_orderOp() {
        ...
        $export_data = [];
        $export_data[0] = ['兑换码', '消费时间', '订单号', '消费金额', '佣金金额', '买家', '买家编号', '商品'];
        if($_GET['type'] == 'timeout'){
            ...
            case 'show':
                $menu_array = [
                    [
                        'menu_key' => '',
                        'menu_name' => '已使用',
                        'menu_url' => 'index.php?act=store_vr_bill&op=show_bill&ob_id='.$_GET['ob_id']
                    ],
                    [
                        'menu_key' => 'timeout',
                        'menu_name' => '已过期',
                        'menu_url' => 'index.php?act=store_vr_bill&op=show_bill&type=timeout&ob_id='.$_GET['ob_id']
                    ]
                ];
                break;
        }
