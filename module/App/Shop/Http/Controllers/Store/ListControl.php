 ...
 * 店铺列表
 */
class  ListControl extends BaseHomeControl {
    ...
    public function indexOp() {
        ...
        if(C('fullindexer.open') && !empty($keyword)){
            ...
            if($keyword != ''){
                $condition['store_name|store_zy'] = ['like', '%'.$keyword.'%'];
            }
        ...
        if($_GET['cate_id'] > 0){
            $child = array_merge((array)$class_list[$_GET['cate_id']]['child'], [$_GET['cate_id']]);
            $condition['sc_id'] = ['in', $child];
        }
        
        $condition['store_state'] = 1;
        
        if(!in_array($_GET['order'], ['desc', 'asc'])){
            unset($_GET['order']);
        }
        if(!in_array($_GET['key'], ['store_sales', 'store_credit'])){
            unset($_GET['key']);
        }
    ...
    private function full_search($search_txt) {
        ...
        if($search_txt != ''){
            $condition['store.store_id'] = ['in', $matchs_id];
        }
        return $condition;
    }
