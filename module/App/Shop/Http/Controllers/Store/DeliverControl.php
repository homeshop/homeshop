...
class  DeliverControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        if(!in_array($_GET['state'], ['deliverno', 'delivering', 'delivered'])){
            $_GET['state'] = 'deliverno';
        }
        $order_state = str_replace(['deliverno', 'delivering', 'delivered'], [
            ORDER_STATE_PAY,
            ORDER_STATE_SEND,
            ORDER_STATE_SUCCESS
        ], $_GET['state']);
        ...
        $order_list = $model_order->getOrderList($condition, 5, '*', 'order_id desc', '', [
            'order_goods',
            'order_common',
            'member'
        ]);
    ...
    public function sendOp() {
        ...
        $condition['store_id'] = $_SESSION['store_id'];
        $order_info = $model_order->getOrderInfo($condition, ['order_common', 'order_goods']);
        $if_allow_send = intval($order_info['lock_state']) || !in_array($order_info['order_state'], [
                ORDER_STATE_PAY,
                ORDER_STATE_SEND
            ]);
        if($if_allow_send){
        ...
        if($order_info['extend_order_common']['daddress_id'] > 0){
            $daddress_info = $model_daddress->getAddressInfo(['address_id' => $order_info['extend_order_common']['daddress_id']]);
        } else {
            //取默认地址
            $daddress_info = $model_daddress->getAddressList(['store_id' => $_SESSION['store_id']], '*', 'is_default desc', 1);
    ...
    public function send_address_selectOp() {
        ...
        $address_list = Model('daddress')->getAddressList(['store_id' => $_SESSION['store_id']]);
    ...
    public function delay_receiveOp() {
        ...
        if(chksubmit()){
            ...
            if(!in_array($delay_date, [5, 10, 15])){
                showDialog(Language::get('wrong_argument'), '', 'error', empty($_GET['inajax']) ? '' : 'CUR_DIALOG.close();');
            }
            $update = $model_order->editOrder([
                'delay_time' => [
                    'exp',
                    'delay_time+'.$delay_date * 3600 * 24
                ]
            ], $condition);
            if($update){
                //新的最晚收货时间
    ...
    public function waybill_printOp() {
        ...
        $order_info = $model_order->getOrderInfo(['order_id' => intval($_GET['order_id'])], ['order_common']);
        
        $store_waybill_list = $model_store_waybill->getStoreWaybillList(['store_id' => $order_info['store_id']], 'is_default desc');
        ...
        $waybill_info = $model_waybill->getWaybillInfo(['waybill_id' => $store_waybill_info['waybill_id']]);
        ...
        switch($menu_type) {
            case 'deliver':
                $menu_array = [
                    [
                        'menu_key' => 'deliverno',
                        'menu_name' => Language::get('nc_member_path_deliverno'),
                        'menu_url' => 'index.php?act=store_deliver&op=index&state=deliverno'
                    ],
                    [
                        'menu_key' => 'delivering',
                        'menu_name' => Language::get('nc_member_path_delivering'),
                        'menu_url' => 'index.php?act=store_deliver&op=index&state=delivering'
                    ],
                    [
                        'menu_key' => 'delivered',
                        'menu_name' => Language::get('nc_member_path_delivered'),
                        'menu_url' => 'index.php?act=store_deliver&op=index&state=delivered'
                    ],
                ];
                break;
            case 'search':
                $menu_array = [
                    1 => [
                        'menu_key' => 'nodeliver',
                        'menu_name' => Language::get('nc_member_path_deliverno'),
                        'menu_url' => 'index.php?act=store_deliver&op=index&state=nodeliver'
                    ],
                    2 => [
                        'menu_key' => 'delivering',
                        'menu_name' => Language::get('nc_member_path_delivering'),
                    ...
                    4 => [
                        'menu_key' => 'search',
                        'menu_name' => Language::get('nc_member_path_deliver_info'),
                        'menu_url' => '###'
                    ],
                ];
                break;
