...
class  SupplierControl extends BaseSellerControl {
    ...
    public function sup_listOp() {
        ...
        if($_GET['sup_name'] != ''){
            $condition['sup_name'] = ['like', "%{$_GET['sup_name']}%"];
        }
    ...
    public function sup_addOp() {
        $model_sup = Model('store_supplier');
        if($_GET['sup_id'] != ''){
            $sup_info = $model_sup->getStoreSupplierInfo([
                'sup_id' => $_GET['sup_id'],
                'sup_store_id' => $_SESSION['store_id']
            ]);
            if(empty($sup_info)){
    ...
    public function sup_delOp() {
        ...
        if($sup_id > 0){
            $model_sup->delStoreSupplier(['sup_id' => $sup_id, 'sup_store_id' => $_SESSION['store_id']]);
            ...
            case 'sup_list':
                $menu_array = [
                    [
                        'menu_key' => 'sup_list',
                        'menu_name' => '供货商',
                        'menu_url' => 'index.php?act=store_supplier&op=sup_list'
                    ]
                ];
                break;
        }
