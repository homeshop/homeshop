<?php namespace App\Shop\Http\Controllers\Store\Account;
 ...
 * 卖家账号组管理
 */
class  GroupControl extends BaseSellerControl {
    ...
    public function group_addOp() {
        // 店铺消息模板列表
        $smt_list = Model('store_msg_tpl')->getStoreMsgTplList([], 'smt_code,smt_name');
    ...
    public function group_editOp() {
        ...
        $model_seller_group = Model('seller_group');
        $seller_group_info = $model_seller_group->getSellerGroupInfo(['group_id' => $group_id]);
    ...
    private function _get_goods_class_list($group_id = null) {
        ...
        if(checkPlatformStoreBindingAllGoodsClass()){
            ...
            for($i = 0, $j = count($bind_class); $i < $j; $i++){
                ...
                if(empty($cur['class_2'])){
                    continue;
                }
                if(!isset($class_list[$cur['class_1']]['class2'])){
                    $class_list[$cur['class_1']]['class2'] = [];
                }
                ...
                if(empty($cur['class_3'])){
                    continue;
                }
                if(!isset($tmp_2[$cur['class_2']]['class3'])){
                    $tmp_2[$cur['class_2']]['class3'] = [];
                }
        ...
        if(!empty($group_id)){
            ...
            //最低级ID列表
            $gc_list_useing = $model_seller_group_bclass->getSellerGroupBclasList(['group_id' => $group_id], '', '', 'gc_id', 'gc_id');
            Tpl::output('gc_id_use_list', array_keys($gc_list_useing));
            //处理哪些二级分类需要选中
            $gc_list_useing = $model_seller_group_bclass->getSellerGroupBclasList([
                'group_id' => $group_id,
                'class_2' => [neq, 0]
            ], '', '', 'count(bid) as ccount,class_2', 'class_2', 'class_2');
            Tpl::output('class_2_use_list', $gc_list_useing);
        } else {
            Tpl::output('gc_id_use_list', []);
        }
    }
    
    private function _get_goods_class_save($group_id = null) {
        if(!is_array($_POST['cate']) || isset($_POST['gc_select_all'])){
            $_POST['cate'] = [];
        }
        $input = [];
        ...
        $model_seller_group_bclass = Model('seller_group_bclass');
        $a = $model_seller_group_bclass->delSellerGroupBclass(['group_id' => $group_id]);
