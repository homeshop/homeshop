 ...
 * 商家入住
 */
class  JoininControl extends BaseHomeControl {
    ...
    public function __construct() {
        ...
        $count = Model('store')->getStoreCount([]);
        ...
        $seller_info = $model_seller->getSellerInfo(['member_id' => $_SESSION['member_id']]);
    ...
    private function check_joinin_state() {
        ...
        $joinin_detail = $model_store_joinin->getOne(['member_id' => $_SESSION['member_id']]);
        if(!empty($joinin_detail)){
            ...
            switch(intval($joinin_detail['joinin_state'])) {
                ...
                case STORE_JOIN_STATE_VERIFY_SUCCESS:
                    if(!in_array($_GET['op'], ['pay', 'pay_save'])){
                        $this->payOp();
                    }
                    break;
                case STORE_JOIN_STATE_VERIFY_FAIL:
                    if(!in_array($_GET['op'], ['step1', 'step2', 'step3', 'step4', 'ajax_upload_image'])){
    ...
    public function step2Op() {
        if(!empty($_POST)){
            ...
            $model_store_joinin = Model('store_joinin');
            $joinin_info = $model_store_joinin->getOne(['member_id' => $_SESSION['member_id']]);
            if(empty($joinin_info)){
                $param['member_id'] = $_SESSION['member_id'];
                $model_store_joinin->save($param);
            } else {
                $model_store_joinin->modify($param, ['member_id' => $_SESSION['member_id']]);
            }
        }
        Tpl::output('step', '2');
    ...
    private function step2_save_valid($param) {
        ...
        $obj_validate->validateparam = [
            [
                "input" => $param['company_name'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "50",
                "message" => "公司名称不能为空且必须小于50个字"
            ],
            [
                "input" => $param['company_address'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "50",
                "message" => "公司地址不能为空且必须小于50个字"
            ],
            [
                ...
                "min" => "1",
                "max" => "50",
                "message" => "公司详细地址不能为空且必须小于50个字"
            ],
            [
                "input" => $param['company_phone'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "20",
            ...
            [
                "input" => $input['company_employee_count'],
                "require" => "true",
                "validator" => "Number",
                "员工总数不能为空且必须是数字"
            ],
            [
                "input" => $input['company_registered_capital'],
                "require" => "true",
                "validator" => "Number",
                "注册资金不能为空且必须是数字"
            ],
            [
                "input" => $param['contacts_name'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "20",
                "message" => "联系人姓名不能为空且必须小于20个字"
            ],
            [
                "input" => $param['contacts_phone'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "20",
            ...
            [
                "input" => $param['contacts_email'],
                "require" => "true",
                "validator" => "email",
                "message" => "电子邮箱不能为空"
            ],
            [
                "input" => $param['business_licence_number'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "20",
                "message" => "营业执照号不能为空且必须小于20个字"
            ],
            [
                "input" => $param['business_licence_address'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "50",
                "message" => "营业执照所在地不能为空且必须小于50个字"
            ],
            ["input" => $param['business_licence_start'], "require" => "true", "message" => "营业执照有效期不能为空"],
            ["input" => $param['business_licence_end'], "require" => "true", "message" => "营业执照有效期不能为空"],
            [
                "input" => $param['business_sphere'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "500",
                "message" => "法定经营范围不能为空且必须小于50个字"
            ],
            ["input" => $param['business_licence_number_elc'], "require" => "true", "message" => "营业执照电子版不能为空"],
            [
                "input" => $param['organization_code'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "20",
                "message" => "组织机构代码不能为空且必须小于20个字"
            ],
            ["input" => $param['organization_code_electronic'], "require" => "true", "message" => "组织机构代码电子版不能为空"],
        ];
    ...
    public function step3Op() {
        ...
        $store_class = $model_store->getStoreClassList([], '', false);
    ...
    private function step3_save_valid($param) {
        ...
        $obj_validate->validateparam = [
            [
                "input" => $param['bank_account_name'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "50",
                "message" => "银行开户名不能为空且必须小于50个字"
            ],
            [
                "input" => $param['bank_account_number'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "20",
                "message" => "银行账号不能为空且必须小于20个字"
            ],
            [
                "input" => $param['bank_name'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "50",
                "message" => "开户银行支行名称不能为空且必须小于50个字"
            ],
            [
                "input" => $param['bank_code'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "20",
                "message" => "支行联行号不能为空且必须小于20个字"
            ],
            ["input" => $input['bank_address'], "require" => "true", "开户行所在地不能为空"],
            ["input" => $input['bank_licence_electronic'], "require" => "true", "开户银行许可证电子版不能为空"],
            [
                "input" => $param['settlement_bank_account_name'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "50",
            ...
            [
                "input" => $param['settlement_bank_account_number'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "20",
            ...
            [
                "input" => $param['settlement_bank_name'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "50",
            ...
            [
                "input" => $param['settlement_bank_code'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "20",
            ...
            ["input" => $input['settlement_bank_address'], "require" => "true", "开户行所在地不能为空"],
            [
                "input" => $param['tax_registration_certificate'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "20",
                "message" => "税务登记证号不能为空且必须小于20个字"
            ],
            [
                "input" => $param['taxpayer_id'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "20",
            ...
            ["input" => $param['tax_registration_certif_elc'], "require" => "true", "message" => "税务登记证号电子版不能为空"],
        ];
        $error = $obj_validate->validate();
        if($error != ''){
            showMessage($error);
    ...
    public function step4Op() {
        ...
        if(!empty($grade_list[$_POST['sg_id']])){
            ...
            $param['sg_info'] = serialize(['sg_price' => $grade_list[$_POST['sg_id']]['sg_price']]);
        }
        
        //取最新店铺分类信息
        $store_class_info = Model('store_class')->getStoreClassInfo(['sc_id' => intval($_POST['sc_id'])]);
        if($store_class_info){
    ...
    private function step4_save_valid($param) {
        ...
        $obj_validate->validateparam = [
            [
                "input" => $param['store_name'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "50",
                "message" => "店铺名称不能为空且必须小于50个字"
            ],
            ["input" => $param['sg_id'], "require" => "true", "message" => "店铺等级不能为空"],
            ["input" => $param['sc_id'], "require" => "true", "message" => "店铺分类不能为空"],
        ];
        $error = $obj_validate->validate();
        if($error != ''){
            showMessage($error);
        ...
        if(!empty($file['tmp_name'])){
            ...
            if($result){
                echo json_encode([
                    ...
                    'pic_url' => UPLOAD_SITE_URL.DS.ATTACH_PATH.DS.'store_joinin'.DS.$upload->file_name
                ]);
            } else {
                echo json_encode(['state' => false, 'message' => $upload->error]);
            }
        }
    }
    ...
    public function checknameOp() {
        ...
        $store_info = $model_store->getStoreInfo(['store_name' => $store_name]);
