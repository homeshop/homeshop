...
class  BillControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        $ext_info = $model_store_ext->getStoreExtendInfo(['store_id' => $_SESSION['store_id']]);
    ...
    public function show_billOp() {
        ...
        if($if_start_date || $if_end_date){
            $order_condition['finnshed_time'] = ['time', [$start_unixtime, $end_unixtime]];
        } else {
        ...
        if($_GET['type'] == 'refund'){
            ...
            if(preg_match('/^\d{8,20}$/', $_GET['query_order_no'])){
                $order_info = $model_order->getOrderInfo(['order_sn' => $_GET['query_order_no']]);
                if($order_info){
    ...
    public function confirm_billOp() {
        ...
        $update = $model_bill->editOrderBill(['ob_state' => BILL_STATE_STORE_COFIRM], $condition);
        if($update){
    ...
    public function export_orderOp() {
        ...
        $model_bill = Model('bill');
        $bill_info = $model_bill->getOrderBillInfo([
            'ob_id' => intval($_GET['ob_id']),
            'ob_store_id' => $_SESSION['store_id']
        ]);
        if(!$bill_info){
            showMessage('参数错误', '', 'html', 'error');
        }
        ...
        $end_unixtime = $if_end_date ? strtotime($_GET['query_end_date']) : null;
        if($if_start_date || $if_end_date){
            $condition['finnshed_time'] = ['time', [$start_unixtime, $end_unixtime]];
        } else {
        ...
        if(!is_numeric($_GET['curpage'])){
            ...
            if($count > self::EXPORT_SIZE){
                ...
                //如果数量小，直接下载
                $data = $model_order->getOrderList($condition, '', '*', 'order_id desc', self::EXPORT_SIZE, ['order_goods']);
            }
        } else {
            //下载
            $limit1 = ($_GET['curpage'] - 1) * self::EXPORT_SIZE;
            $limit2 = self::EXPORT_SIZE;
            $data = $model_order->getOrderList($condition, '', '*', 'order_id desc', "{$limit1},{$limit2}", ['order_goods']);
        }
        ...
        $export_data = [];
        $export_data[0] = ['订单编号', '下单时间', '成交时间', '订单金额', '运费', '佣金', '平台红包', '买家', '买家编号', '商品'];
        $order_totals = 0;
    ...
    public function export_bookOp() {
        ...
        $export_data = [];
        $export_data[0] = ['订单编号', '下单时间', '取消时间', '订单金额', '运费', '未退定金', '商家', '商家编号', '买家', '买家编号'];
        $order_amount = 0;
        $deposit_amount = 0;
        $k = 0;
        foreach($data as $v){
    ...
    public function export_refund_orderOp() {
        ...
        $end_unixtime = $if_end_date ? strtotime($_GET['query_end_date']) : null;
        if($if_start_date || $if_end_date){
            $condition['admin_time'] = ['time', [$start_unixtime, $end_unixtime]];
        } else {
        ...
        $export_data = [];
        $export_data[0] = ['退单编号', '订单编号', '退单金额', '退单佣金', '退还红包', '类型', '退款时间', '买家', '买家编号'];
        $refund_amount = 0;
        $commis_totals = 0;
        $rpt_amount = 0;
        $k = 0;
            ...
            case 'list':
                $menu_array = [
                    1 => [
                        'menu_key' => 'list',
                        'menu_name' => '实物订单结算',
                        'menu_url' => 'index.php?act=bill&op=list'
                    ],
                ];
                break;
            case 'show':
                $menu_array = [
                    [
                        'menu_key' => 'order_list',
                        'menu_name' => '订单列表',
                        'menu_url' => 'index.php?act=store_bill&op=show_bill&ob_id='.$_GET['ob_id']
                    ],
                    [
                        'menu_key' => 'refund_list',
                        'menu_name' => '退款订单',
                        'menu_url' => 'index.php?act=store_bill&op=show_bill&type=refund&ob_id='.$_GET['ob_id']
                    ],
                    [
                        'menu_key' => 'cost_list',
                        'menu_name' => '促销费用',
                        'menu_url' => 'index.php?act=store_bill&op=show_bill&type=cost&ob_id='.$_GET['ob_id']
                    ]
                ];
                if(floatval($this->_bill_info['ob_order_book_totals']) > 0){
                    array_push($menu_array, [
                        'menu_key' => 'book_list',
                        'menu_name' => '未退定金',
                        'menu_url' => 'index.php?act=store_bill&op=show_bill&type=book&ob_id='.$_GET['ob_id']
                    ]);
                }
                break;
        }
