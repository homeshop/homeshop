...
class  SettingControl extends BaseSellerControl {
    ...
    public function store_settingOp() {
        ...
        if(chksubmit()){
            ...
            if(!empty($store_domain) && $store_domain != $store_info['store_domain']){
                ...
                $store = $model_class->getStoreInfo(['store_domain' => $store_domain]);
                //二级域名存在,则提示错误
                ...
                if($subdomain_times > $store_domain_times){//可继续修改
                    ...
                    $model_class->editStore($param, ['store_id' => $store_id]);
                }
    ...
    public function store_slideOp() {
        ...
        if($_POST['form_submit'] == 'ok'){
            ...
            $model_store->editStore($update, ['store_id' => $_SESSION['store_id']]);
            
            // 删除upload表中数据
            $model_upload->delByWhere(['upload_type' => 3, 'item_id' => $_SESSION['store_id']]);
            showDialog(Language::get('nc_common_save_succ'), 'index.php?act=store_setting&op=store_slide', 'succ');
        }
        
        // 删除upload中的无用数据
        $upload_info = $model_upload->getUploadList([
            'upload_type' => 3,
            'item_id' => $_SESSION['store_id']
        ], 'file_name');
    ...
    public function dorp_imgOp() {
        ...
        echo json_encode(['succeed' => Language::get('nc_common_save_succ', 'UTF-8')]);
        die;
    }
    ...
    public function set_themeOp() {
        ...
        if(!empty($style) && file_exists(BASE_TPL_PATH.DS.'/store/style/'.$style.'/images/preview.jpg')){
            $store_class = Model('store');
            $rs = $store_class->editStore(['store_theme' => $style], ['store_id' => $_SESSION['store_id']]);
    ...
    protected function getStoreMbSliders() {
        ...
        if(!$mbSliders){
            $mbSliders = array_fill(1, self::MAX_MB_SLIDERS, ['img' => '', 'type' => 1, 'link' => '',]);
        }
    ...
    protected function setStoreMbSliders(array $mbSliders) {
        return Model('store')->editStore(['mb_sliders' => serialize($mbSliders),], ['store_id' => $_SESSION['store_id'],]);
    }
    ...
    public function store_mb_slidersOp() {
        try {
            ...
            echo json_encode(['uploadedUrl' => UPLOAD_SITE_URL.DS.ATTACH_STORE.DS.$newImg,]);
    ...
    public function store_mobileOp() {
        ...
        if(chksubmit()){
            ...
            if($skuToValid){
                $validSkus = (array)Model()->table('goods')->field('goods_id')->where([
                    'goods_id' => [
                        'in',
                        $skuToValid
                    ],
            ...
            Model('store')->editStore($update_array, ['store_id' => $_SESSION['store_id'],]);
    ...
    private function profile_menu($menu_key = '') {
        ...
        $menu_array = [
            ...
            4 => [
                'menu_key' => 'store_slide',
                'menu_name' => Language::get('nc_member_path_store_slide'),
                'menu_url' => 'index.php?act=store_setting&op=store_slide'
            ],
            5 => [
                'menu_key' => 'store_theme',
                'menu_name' => '店铺主题',
                'menu_url' => 'index.php?act=store_setting&op=theme'
            ],
