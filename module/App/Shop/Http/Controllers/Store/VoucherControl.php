...
class  VoucherControl extends BaseSellerControl {
    ...
    public function __construct() {
        ...
        //申请记录状态
        $this->applystate_arr = [
            'new' => [1, Language::get('voucher_applystate_new')],
            'verify' => [2, Language::get('voucher_applystate_verify')],
            'cancel' => [3, Language::get('voucher_applystate_cancel')]
        ];
        //套餐状态
        $this->quotastate_arr = [
            'activity' => [1, Language::get('voucher_quotastate_activity')],
            'cancel' => [2, Language::get('voucher_quotastate_cancel')],
            'expire' => [3, Language::get('voucher_quotastate_expire')]
        ];
        //代金券模板状态
        $this->templatestate_arr = [
            'usable' => [1, Language::get('voucher_templatestate_usable')],
            'disabled' => [2, Language::get('voucher_templatestate_disabled')]
        ];
    ...
    public function templatelistOp() {
        ...
        if(trim($_GET['txt_keyword'])){
            $param['voucher_t_title'] = ['like', '%'.trim($_GET['txt_keyword']).'%'];
        }
        ...
        if($_GET['txt_startdate']){
            $param['voucher_t_end_date'] = ['egt', strtotime($_GET['txt_startdate'])];
        }
        if($_GET['txt_enddate']){
            $param['voucher_t_start_date'] = ['elt', strtotime($_GET['txt_enddate'])];
        }
     ...
     * @param array $data
     */
    private function createExcel($data = [], $t_info) {
        ...
        $excel_obj->setStyle([
            'id' => 's_title',
            'Font' => ['FontName' => '宋体', 'Size' => '12', 'Bold' => '1']
        ]);
        //header
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '代金券编码'];
        if($t_info['voucher_t_gettype'] == 2){
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '卡密'];
        }
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '代金券名称'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '有效期'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '面额'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '订单限额'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '所属会员'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '领取时间'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '使用状态'];
        //代金券模板名称
        $voucher_t_title = '';
        //data
        $model_voucher = Model('voucher');
        ...
        foreach((array)$data as $k => $info){
            ...
            $tmp = [];
            $tmp[] = ['data' => $info['voucher_code']];
            if($t_info['voucher_t_gettype'] == 2){
                $tmp[] = ['data' => $info['voucher_pwd']];
            }
            $tmp[] = ['data' => $info['voucher_title']];
            $info['expirydatetext'] = date('Y-m-d', $info['voucher_start_date']).'~'.date('Y-m-d', $info['voucher_end_date']);
            $tmp[] = ['data' => $info['expirydatetext']];
            $tmp[] = ['data' => $info['voucher_price']];
            $tmp[] = ['data' => $info['voucher_limit']];
            $tmp[] = ['data' => ($info['voucher_owner_name'] ? $info['voucher_owner_name'] : '')];
            if($info['voucher_owner_id'] > 0){
                $info['voucher_active_date'] = @date('Y-m-d H:i:s', $info['voucher_active_date']);
            } else {
                $info['voucher_active_date'] = '';
            }
            $tmp[] = ['data' => $info['voucher_active_date']];
            $info['voucher_state_text'] = $voucherstate_arr[$info['voucher_state']];;
            $tmp[] = ['data' => $info['voucher_state_text']];
            $excel_data[] = $tmp;
        }
    ...
    public function quotaaddOp() {
        if(chksubmit()){
            ...
            if(empty($current_quota)){
                ...
                $param = [];
                $param['quota_endtime'] = ['exp', 'quota_endtime + '.$add_time];
                $reault = $model->table('voucher_quota')->where(['quota_id' => $current_quota['quota_id']])->update($param);
            }
    ...
    public function templateaddOp() {
        ...
        if($isOwnShop = checkPlatformStore()){
            ...
            //查询该套餐下代金券模板列表
            $count = $model->table('voucher_template')->where([
                'voucher_t_quotaid' => $quotainfo['quota_id'],
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                [
                    "input" => $_POST['txt_template_title'],
                    "require" => "true",
                    "validator" => "Length",
                    "min" => "1",
                    "max" => "50",
                    "message" => Language::get('voucher_template_title_error')
                ],
                [
                    "input" => $_POST['txt_template_total'],
                    "require" => "true",
                    "validator" => "Number",
                    "min" => "1",
                    "message" => Language::get('voucher_template_total_error')
                ],
                [
                    "input" => $_POST['select_template_price'],
                    "require" => "true",
                    "validator" => "Number",
                    "message" => Language::get('voucher_template_price_error')
                ],
                [
                    "input" => $_POST['txt_template_limit'],
                    "require" => "true",
                    "validator" => "Double",
                    "message" => Language::get('voucher_template_limit_error')
                ],
                [
                    "input" => $_POST['txt_template_describe'],
                    "require" => "true",
                    "validator" => "Length",
                    "min" => "1",
                    "max" => "255",
                    "message" => Language::get('voucher_template_describe_error')
                ],
                ["input" => $_POST['gettype_sel'], "require" => "true", "message" => '请选择领取方式'],
            ];
    ...
    public function templateeditOp() {
        ...
        $param['voucher_t_giveout'] = ['elt', '0'];
        $param['voucher_t_end_date'] = ['gt', time()];
        ...
        if($isOwnShop = checkPlatformStore()){
            ...
            //查询套餐信息
            $quotainfo = $model->table('voucher_quota')->where([
                'quota_id' => $t_info['voucher_t_quotaid'],
                'quota_storeid' => $_SESSION['store_id']
            ])->find();
            if(empty($quotainfo)){
                showMessage(Language::get('voucher_template_quotanull'), 'index.php?act=store_voucher&op=quotaadd', 'html', 'error');
            }
        ...
        if(chksubmit()){
            ...
            if($error){
                ...
                $rs = $model->table('voucher_template')->where(['voucher_t_id' => $t_info['voucher_t_id']])->update($update_arr);
                if($rs){
                    //生成卡密代金券
                    if($gettype == 'pwd'){
    ...
    public function templatedelOp() {
        ...
        $param['voucher_t_id'] = $t_id;
        $param['voucher_t_store_id'] = $_SESSION['store_id'];
        $param['voucher_t_giveout'] = ['elt', '0'];//会员没领取过代金券才可删除
        $t_info = $model->table('voucher_template')->where($param)->find();
        if(empty($t_info)){
            showMessage(Language::get('wrong_argument'), 'index.php?act=store_voucher&op=templatelist', 'html', 'error');
        }
        $rs = $model->table('voucher_template')->where(['voucher_t_id' => $t_info['voucher_t_id']])->delete();
        if($rs){
            //删除自定义的图片
    ...
    private function check_voucher_template_expire($voucher_template_id = '') {
        ...
        if(empty($voucher_template_id)){
            $where_array['voucher_t_end_date'] = ['lt', time()];
        } else {
            $where_array['voucher_t_id'] = $voucher_template_id;
        }
        $where_array['voucher_t_state'] = $this->templatestate_arr['usable'][0];
        $model = Model();
        $model->table('voucher_template')->where($where_array)->update(['voucher_t_state' => $this->templatestate_arr['disabled'][0]]);
    }
        ...
        switch($menu_type) {
            case 'voucher':
                $menu_array = [
                    1 => [
                        ...
                        'menu_name' => Language::get('nc_member_path_store_voucher'),
                        'menu_url' => 'index.php?act=store_voucher&op=templatelist'
                    ],
                ];
                break;
            case 'quota_add':
                $menu_array = [
                    ...
                    4 => [
                        'menu_key' => 'quotaadd',
                        'menu_name' => Language::get('voucher_applyadd'),
                        'menu_url' => 'index.php?act=store_voucher&op=quotaadd'
                    ]
                ];
                break;
            case 'template':
                $menu_array = [
                    ...
                    2 => [
                        'menu_key' => 'templateadd',
                        'menu_name' => Language::get('voucher_templateadd'),
                        'menu_url' => 'index.php?act=store_voucher&op=templateadd'
                    ],
                ];
                break;
            case 'templateedit':
                $menu_array = [
                    ...
                    2 => [
                        ...
                        'menu_name' => Language::get('voucher_templateedit'),
                        'menu_url' => ''
                    ],
                ];
                break;
            case 'templateinfo':
                $menu_array = [
                    ...
                    2 => [
                        'menu_key' => 'templateinfo',
                        'menu_name' => Language::get('voucher_templateinfo'),
                        'menu_url' => ''
                    ],
                ];
                break;
