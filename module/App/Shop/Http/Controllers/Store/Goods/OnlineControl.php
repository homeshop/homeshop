<?php namespace App\Shop\Http\Controllers\Store\Goods;
...
class  OnlineControl extends BaseSellerControl {
    ...
    public function goods_listOp() {
        ...
        $distribution_info = Model('store_distribution')->where(['distri_store_id' => $store_info['store_id']])->find();
    ...
    public function edit_goodsOp() {
        ...
        // 自定义属性
        $custom_list = Model('type_custom')->getTypeCustomList(['type_id' => $goods_class['type_id']]);
        ...
        if(is_array($goods_array) && !empty($goods_array)){
            ...
            // 取得已选择了哪些商品的属性
            $attr_checked_l = $model_type->typeRelatedList('goods_attr_index', ['goods_id' => intval($goods_array[0]['goods_id'])], 'attr_value_id');
    ...
    public function edit_save_goodsOp() {
        ...
        $result = $logic_goods->updateGoods($_POST, $_SESSION['store_id'], $_SESSION['store_name'], $this->store_info['store_state'], $_SESSION['seller_id'], $_SESSION['seller_name'], $_SESSION['bind_all_gc']);
    ...
    public function edit_imageOp() {
        ...
        $image_list = $model_goods->getGoodsImageList(['goods_commonid' => $common_id]);
        $image_list = array_under_reset($image_list, 'color_id', 2);
        
        $img_array = $model_goods->getGoodsList(['goods_commonid' => $common_id], 'color_id,min(goods_image) as goods_image', 'color_id');
        // 整理，更具id查询颜色名称
        if(!empty($img_array)){
        ...
        $value_array = $model_spec->getSpecValueList([
            'sp_value_id' => ['in', $colorid_array],
            'store_id' => $_SESSION['store_id']
        ], 'sp_value_id,sp_value_name');
        if(empty($value_array)){
            $value_array[] = ['sp_value_id' => '0', 'sp_value_name' => '无颜色'];
        }
    ...
    public function edit_jingleOp() {
        if(chksubmit()){
            ...
            $commonid_array = explode(',', $common_id);
            $where = ['goods_commonid' => ['in', $commonid_array], 'store_id' => $_SESSION['store_id']];
            $update = ['goods_jingle' => trim($_POST['g_jingle'])];
    ...
    public function add_giftOp() {
        ...
        // 商品列表
        $goods_array = $model_goods->getGoodsList(['goods_commonid' => $common_id], '*');
        Tpl::output('goods_array', $goods_array);
        
        // 赠品列表
        $gift_list = $model_gift->getGoodsGiftList(['goods_commonid' => $common_id]);
        $gift_array = [];
        if(!empty($gift_list)){
        ...
        $menu_promotion = ['lock' => $goodscommon_info['goods_lock'] == 1 ? true : false, 'gift' => true];
    ...
    public function save_giftOp() {
        ...
        // 验证商品是否存在
        $model_gift = Model('goods_gift');
        $goods_list = $model_gift->getAllowGiftGoodsList([
        ...
        // 删除该商品原有赠品
        $model_gift->delGoodsGift(['goods_commonid' => $commonid]);
        // 重置商品礼品标记
        $model_goods->editGoods(['have_gift' => 0], ['goods_commonid' => $commonid]);
        // 商品id
        $goodsid_array = [];
        ...
        if(!empty($update_goodsid)){
            $model_goods->editGoodsById(['have_gift' => 1], $update_goodsid);
        }
    ...
    public function search_goodsOp() {
        ...
        $where['store_id'] = $_SESSION['store_id'];
        if($_GET['name']){
            $where['goods_name'] = ['like', '%'.$_GET['name'].'%'];
        }
        $model_goods = Model('goods');
    ...
    public function get_goods_list_ajaxOp() {
        ...
        ], 'goods_id,goods_spec,store_id,goods_price,goods_serial,goods_storage_alarm,goods_storage,goods_image,invite_rate');
        if(empty($goods_list)){
            echo 'false';
            exit();
        }
        ...
        foreach($goods_list as $key => $val){
            ...
            $goods_list[$key]['url'] = urlShop('goods', 'index', ['goods_id' => $val['goods_id']]);
        }
     ...
     * @return
     */
    private function profile_menu($menu_type, $menu_key, $allow_promotion = []) {
        $menu_array = [];
        switch($menu_type) {
            case 'goods_list':
                $menu_array = [
            ...
            case 'edit_detail':
                if($allow_promotion['lock'] === false){
                    $menu_array = [
                        [
                            ...
                            'menu_name' => '编辑商品',
                            'menu_url' => urlShop('store_goods_online', 'edit_goods', [
                        ...
                        [
                            'menu_key' => 'edit_image',
                            'menu_name' => '编辑图片',
                            'menu_url' => urlShop('store_goods_online', 'edit_image', [
                                'commonid' => $_GET['commonid'],
                ...
                if($allow_promotion['gift'] == true){
                    $menu_array[] = [
                        'menu_key' => 'add_gift',
                        'menu_name' => '赠送赠品',
                        'menu_url' => urlShop('store_goods_online', 'add_gift', [
                            'commonid' => $_GET['commonid'],
                            'ref_url' => ($_GET['ref_url'] ? $_GET['ref_url'] : getReferer())
                        ])
                    ];
            ...
            case 'edit_class':
                $menu_array = [
                    [
                        'menu_key' => 'edit_class',
                        'menu_name' => '选择分类',
                        'menu_url' => urlShop('store_goods_online', 'edit_class', [
                            'commonid' => $_GET['commonid'],
                            'ref_url' => $_GET['ref_url']
                        ])
                    ],
