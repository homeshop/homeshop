 ...
 * 会员中心——我是卖家
 */
class  ClassControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        if($_GET['type'] == 'ok'){
            if(intval($_GET['class_id']) != 0){
                $class_info = $model_class->getStoreGoodsClassInfo(['stc_id' => intval($_GET['class_id'])]);
                Tpl::output('class_info', $class_info);
            }
            if(intval($_GET['top_class_id']) != 0){
                Tpl::output('class_info', ['stc_parent_id' => intval($_GET['top_class_id'])]);
            }
            $goods_class = $model_class->getStoreGoodsClassList([
            ...
            $goods_class = $model_class->getTreeClassList(['store_id' => $_SESSION['store_id']], 2);
            $str = '';
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        ...
        switch($menu_type) {
            case 'store_goods_class':
                $menu_array = [
                    1 => [
                        'menu_key' => 'store_goods_class',
                        'menu_name' => '店铺分类',
                        'menu_url' => 'index.php?act=store_goods_class&op=store_goods_class'
                    ]
                ];
                break;
        }
