...
use App\Shop\Http\Controllers\BaseStoreSnsControl;
 ...
 * 店铺动态
 */
class  SnshomeControl extends BaseStoreSnsControl {
    ...
    public function indexOp() {
        ...
        if($_GET['type'] != ''){
            switch(trim($_GET['type'])) {
                case 'promotion':
                    $where['strace_type'] = ['in', [4, 5, 6, 7, 8]];
                    break;
                case 'new':
                    $where['strace_type'] = 3;
                    break;
        ...
        // 最多收藏的会员
        $favorites = Model('favorites')->getStoreFavoritesList(['fav_id' => $sid], '*', 0, 'fav_time desc', 8);
        if(!empty($favorites)){
            $memberid_array = [];
            foreach($favorites as $val){
                $memberid_array[] = $val['member_id'];
            }
            $favorites_list = Model('member')->getMemberList([
                'member_id' => [
                    'in',
                    $memberid_array
                ]
    ...
    public function commenttopOp() {
        ...
        if($stid > 0){
            ...
            $where = ['strace_id' => $stid, 'scomm_state' => 1];
            ...
            // 更新评论数量
            Model('store_sns_tracelog')->editStoreSnsTracelog(['strace_comment' => $countnum], ['strace_id' => $stid]);
        }
    ...
    public function commentlistOp() {
        ...
        if($stid > 0){
            ...
            // 更新评论数量
            $commentlist = Model('store_sns_tracelog')->editStoreSnsTracelog(['strace_comment' => $countnum], ['strace_id' => $stid]);
        }
    ...
    public function addcommentOp() {
        ...
        //      //查询会员信息
        $model = Model();
        $member_info = $model->table('member')->where([
            'member_state' => 1,
        ...
        if($result){
            ...
            $update = ['strace_comment' => ['exp', 'strace_comment+1']];
    ...
    public function addforwardOp() {
        ...
        //查询原帖信息
        $model_stracelog = Model('store_sns_tracelog');
        $stracelog_info = $model_stracelog->getStoreSnsTracelogInfo(['strace_id' => $stid]);
        ...
        if($result){
            //更新动态转发次数
            $where = ['strace_id' => $stid];
            $update = ['strace_spread' => ['exp', 'strace_spread+1']];
    ...
    public function deltraceOp() {
        ...
        //删除动态
        $result = Model('store_sns_tracelog')->delStoreSnsTracelog([
            'strace_id' => $stid,
            'strace_storeid' => $_SESSION['store_id']
        ]);
        if($result){
            //删除对应的评论
            Model('store_sns_comment')->delStoreSnsComment(['strace_id' => $stid]);
    ...
    public function delcommentOp() {
        ...
        $where = [
            'strace_id' => $stid,
            'scomm_id' => $scid,
            'scomm_memberid' => $_SESSION['member_id']
        ];    // where条件
        ...
        if($result){
            // 更新动态统计信息
            $where = ['strace_id' => $scomment_info['strace_id']];
            $update = ['strace_comment' => ['exp', 'strace_comment-1']];
            Model('store_sns_tracelog')->editStoreSnsTracelog($update, $where);
    ...
    public function straceinfoOp() {
        ...
        $model_stracelog = Model('store_sns_tracelog');
        $strace_info = $model_stracelog->getStoreSnsTracelogInfo(['strace_id' => $stid]);
    ...
    private function checkLoginStatus() {
        if($_SESSION['is_login'] != 1){
            @header("location: ".urlLogin('login', 'index', ['ref_url' => urlShop('member_snsnome')]));
        }
