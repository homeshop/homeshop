...
class  PaymentControl extends BaseHomeControl {
    ...
    public function real_orderOp() {
        ...
        $result = Model('order')->editOrder(['api_pay_time' => TIMESTAMP], [
            'order_id' => [
                'in',
                $pay_order_id_list
            ]
        ...
        if($order_pay_info['data']['if_buyer_repay']){
            $update = Model('order')->editOrderPay(['api_pay_state' => 0], ['pay_id' => $order_pay_info['data']['pay_id']]);
            if(!$update){
    ...
    public function vr_orderOp() {
        ...
        $result = Model('vr_order')->editOrder(['api_pay_time' => TIMESTAMP], ['order_id' => $order_info['order_id']]);
        if(!$result){
            showMessage('更新订单信息发生错误，请重新支付', $url, 'html', 'error');
        }
    ...
    private function _api_pay($order_info, $payment_info) {
        ...
        if($payment_info['payment_code'] == 'chinabank'){
            ...
            if(array_key_exists('order_list', $order_info)){
                ...
                Tpl::output('order_list', [$order_info]);
    ...
    public function notifyOp() {
        ...
        if($order_type == 'real_order'){
            ...
            //订单存在被系统自动取消的可能性
            if(!in_array($result['data']['order_state'], [ORDER_STATE_NEW, ORDER_STATE_CANCEL])){
                exit($success);
            }
        ...
        if($result['state']){
            ...
            QueueClient::push('addConsume', [
                'member_id' => $log_buyer_id,
                'member_name' => $log_buyer_name,
                'consume_amount' => ncPriceFormat($api_pay_amount),
    ...
    public function returnOp() {
        ...
        if($order_type == 'real_order'){
            ...
            if(!in_array($result['data']['order_state'], [ORDER_STATE_NEW])){
                $payment_state = 'success';
            }
    ...
    public function query_stateOp() {
        if($_GET['pay_id'] && intval($_GET['pay_id']) > 0){
            $info = Model('order')->getOrderPayInfo([
                'pay_id' => intval($_GET['pay_id']),
                'buyer_id' => intval($_GET['buyer_id'])
            ]);
            exit(json_encode([
                'state' => ($info['api_pay_state'] == '1'),
                'pay_sn' => $info['pay_sn'],
                'type' => 'r'
            ]));
        } elseif(intval($_GET['order_id']) > 0) {
            $info = Model('vr_order')->getOrderInfo([
