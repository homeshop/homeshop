<?php namespace App\Shop\Http\Controllers\Taobao;
 ...
 * 淘宝导入插件  v5
 */
class  ImportControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        if(!$_POST){
            ...
            $store_goods_class = $model_store_class->getClassTree([
            ...
            $gc_sub_list = $gc->getGoodsClassList(['gc_parent_id' => intval($_POST['gc_id'])]);
            ...
            // 商品数量
            $goods_num = $model_store_goods->getGoodsCommonCount(['store_id' => $_SESSION['store_id']]);
            
            /**
             * 商品数,空间使用，使用期限判断
             */
            $model_store = Model('store');
            $store_info = $model_store->getStoreInfo(['store_id' => $_SESSION['store_id']]);
    ...
    private function parse_taobao_csv($csv_string) {
        ...
        $fields_cols = []; // 每个字段所在CSV中的列序号，从0开始算
        ...
        while($pos < strlen($csv_string)) {
            ...
            if($status == 0 && !in_array($t, [ORD_SPACE, ORD_TAB, ORD_N, ORD_R])){
                $status = 1;
                $title_pos = $pos;
            }
            ...
            if($status == 1){
                ...
                if($field_status == 1 && $t == ORD_QUOTE && in_array($next, [
                        ORD_N,
                        ORD_R,
                        ORD_TAB
                    ])
                ...
                if($field_status == 2 && in_array($t, [ORD_N, ORD_R, ORD_TAB])) // 换行 或 \t
                {
                    $records[$line][$field] = addslashes(substr($csv_string, $start_pos, $pos - $start_pos));
                    $field++;
                    if($field == $csv_col_num){
