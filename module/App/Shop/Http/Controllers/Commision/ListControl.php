<?php namespace App\Shop\Http\Controllers\Commision;
...
class  ListControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        if(trim($_POST['order_sn']) != ''){
            $condition['order_sn'] = ['like', '%'.trim($_POST['order_sn']).'%'];
        ...
        foreach($CLists as $key => $val){
            $goods_info = $model_goods->getGoodsInfo(['goods_id' => $val['goods_id']]);
    ...
    public function getStoreId($goods_id) {
        ...
        $model_goods = Model('goods');
        $goods_info = $model_goods->getGoodsInfo(['goods_id' => $goods_id]);
