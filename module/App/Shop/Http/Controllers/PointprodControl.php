...
class  PointprodControl extends BasePointShopControl {
    ...
    public function plistOp() {
        ...
        if(count($level_filter) > 0){
            if(isset($level_filter['search']) && isset($level_filter['isable'])){
                $where['pgoods_limitmgrade'] = [
                    ['eq', $level_filter['search']],
                    ['elt', $level_filter['isable']],
                    'and'
                ];
            } elseif(isset($level_filter['search'])) {
                $where['pgoods_limitmgrade'] = $level_filter['search'];
            } elseif(isset($level_filter['isable'])) {
                $where['pgoods_limitmgrade'] = ['elt', $level_filter['isable']];
            }
        }
        ...
        if(count($points_filter) > 0){
            ...
            if(count($points_filter) > 1){
                ...
                $where['pgoods_points'] = ['between', [$points_filter[0], $points_filter[1]]];
            } else {
                if($points_filter['min']){
                    $where['pgoods_points'] = ['egt', $points_filter['min']];
                } elseif($points_filter['max']) {
                    $where['pgoods_points'] = ['elt', $points_filter['max']];
                } elseif($points_filter['isable']) {
                    $where['pgoods_points'] = ['elt', $points_filter['isable']];
                }
            }
        }
        //排序
        ...
        $nav_link = [
            ...
            1 => ['title' => '积分中心', 'link' => urlShop('pointshop', 'index')],
            2 => ['title' => '兑换礼品列表']
        ];
    ...
    public function pinfoOp() {
        ...
        $model_pointprod = Model('pointprod');
        //查询兑换礼品详细
        $prodinfo = $model_pointprod->getOnlinePointProdInfo(['pgoods_id' => $pid]);
        if(empty($prodinfo)){
        ...
        $tm_tm_visite_pgoods = $tm_tm_visite_pgoods ? explode(',', $tm_tm_visite_pgoods) : [];
        ...
        $where = [];
        $where['point_orderstate'] = ['neq', $pointorderstate_arr['canceled'][0]];
        ...
        if($orderprod_list){
            ...
            $memberlist_tmp = Model('member')->getMemberList([
                'member_id' => [
                    'in',
                    $buyerid_arr
                ]
            ], 'member_id,member_avatar');
            $memberlist = [];
            if($memberlist_tmp){
