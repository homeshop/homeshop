    ...
    public function groupbuy_list_xmlOp() {
        ...
        if($_REQUEST['advanced']){
            if(strlen($q = trim((string)$_REQUEST['groupbuy_name']))){
                $condition['groupbuy_name'] = ['like', '%' . $q . '%'];
            }
            ...
            switch($_REQUEST['qtype']) {
                case 'groupbuy_name':
                    $condition['groupbuy_name'] = ['like', '%' . $_REQUEST['query'] . '%'];
                    break;
                case 'goods_name':
                    $condition['goods_name'] = ['like', '%' . $_REQUEST['query'] . '%'];
                    break;
                case 'store_name':
                    $condition['store_name'] = ['like', '%' . $_REQUEST['query'] . '%'];
                    break;
            }
        }
        ...
        foreach($groupbuy_list as $val){
            ...
            $o = '<a class="btn red confirm-on-click" href="javascript:;" data-href="' . urlAdminShop('groupbuy', 'groupbuy_del', [
            ...
            if($val['reviewable']){
                $o .= '<li><a class="confirm-on-click" href="javascript:;" data-href="' . urlAdminShop('groupbuy', 'groupbuy_review_pass', [
                        'groupbuy_id' => $val['groupbuy_id'],
                    ]) . '">批准活动</a></li>';
                $o .= '<li><a class="confirm-on-click" href="javascript:;" data-href="' . urlAdminShop('groupbuy', 'groupbuy_review_fail', [
                        'groupbuy_id' => $val['groupbuy_id'],
                    ]) . '">拒绝活动</a></li>';
            }
            
            if($val['recommended'] == '1'){
                $o .= '<li><a href="javascript:;" data-href="' . urlAdminShop('groupbuy', 'groupbuy_rec', [
                        'groupbuy_id' => $val['groupbuy_id'],
                        'rec' => 0,
                    ]) . '">取消推荐</a></li>';
            } else {
            ...
            if($val['cancelable']){
                $o .= '<li><a class="confirm-on-click" href="javascript:;" data-href="' . urlAdminShop('groupbuy', 'groupbuy_cancel', [
                        'groupbuy_id' => $val['groupbuy_id'],
                    ]) . '">取消活动</a></li>';
            }
            ...
            $i['goods_name'] = '<a target="_blank" href="' . SHOP_SITE_URL . "/index.php?act=goods&goods_id=" . $val['goods_id'] . '">' . $val['goods_name'] . '</a>';
            
            $i['store_name'] = '<a target="_blank" href="' . urlShop('show_store', 'index', ['store_id' => $val['store_id']]) . '">' . $val['store_name'] . '</a>';
...
EOB;
            ...
            $i['recommended'] = $val['recommended'] == '1' ? '<span class="yes"><i class="fa fa-check-circle"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
    ...
    public function groupbuy_review_passOp() {
        ...
        if($result){
            ...
            // 添加队列
            $groupbuy_info = $model_groupbuy->getGroupbuyInfo(['groupbuy_id' => $groupbuy_id]);
            Model('cron')->addCron([
                'exetime' => $groupbuy_info['start_time'],
                'exeid' => $groupbuy_info['goods_commonid'],
                'type' => 5
            ]);
            Model('cron')->addCron([
                'exetime' => $groupbuy_info['end_time'],
                'exeid' => $groupbuy_info['goods_commonid'],
                'type' => 6
            ]);
    ...
    public function groupbuy_delOp() {
        ...
        $model_groupbuy = Model('groupbuy');
        $result = $model_groupbuy->delGroupbuy(['groupbuy_id' => $groupbuy_id]);
        if($result){
    ...
    public function class_saveOp() {
        ...
        if(empty($class_id)){
            ...
            //编辑
            if($model_groupbuy_class->updates($param, ['class_id' => $class_id])){
    ...
    public function price_list_xmlOp() {
        ...
        foreach($groupbuy_price_list as $val){
            $o = '<a class="confirm-del-on-click btn red" href="index.php?act=groupbuy&op=price_drop&range_id=' . $val['range_id'] . '"><i class="fa fa-trash-o"></i>删除</a>';
            $o .= '<a class="btn blue" href="index.php?act=groupbuy&op=price_edit&range_id=' . $val['range_id'] . '"><i class="fa fa-pencil-square-o"></i>编辑</a>';
    ...
    public function price_saveOp() {
        ...
        if(empty($range_id)){
            ...
            //编辑
            if($model->updates($param, ['range_id' => $range_id])){
                dkcache('groupbuy_price');
    ...
    private function show_menu($menu_key) {
        $menu_array = [
            'groupbuy_list' => [
                'menu_type' => 'link',
                'menu_name' => '抢购活动',
                'menu_url' => 'index.php?act=groupbuy&op=groupbuy_list'
            ],
            'groupbuy_quota' => [
                'menu_type' => 'link',
                'menu_name' => '套餐管理',
                'menu_url' => 'index.php?act=groupbuy&op=groupbuy_quota'
            ],
            'class_list' => [
                'menu_type' => 'link',
            ...
            'groupbuy_setting' => [
                'menu_type' => 'link',
                'menu_name' => '设置',
                'menu_url' => urlAdminShop('groupbuy', 'groupbuy_setting')
            ],
            'slider' => [
                'menu_type' => 'link',
                'menu_name' => '幻灯片管理',
                'menu_url' => urlAdminShop('groupbuy', 'slider')
            ],
        ];
        switch($menu_key) {
            case 'class_add':
