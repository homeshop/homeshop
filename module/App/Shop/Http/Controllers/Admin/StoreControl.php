...
class  StoreControl extends SystemControl {
    ...
    private $_links = [
        ['url' => 'act=store&op=store', 'text' => '管理'],
        ['url' => 'act=store&op=store_joinin', 'text' => '开店申请'],
        ['url' => 'act=store&op=reopen_list', 'text' => '续签申请'],
        ['url' => 'act=store&op=store_bind_class_applay_list', 'text' => '经营类目申请'],
        ['url' => 'act=store&op=bill_cycle', 'text' => '结算周期设置']
    ];
    ...
    public function get_xmlOp() {
        ...
        if($_GET['member_name'] != ''){
            $condition['member_name'] = ['like', '%' . $_GET['member_name'] . '%'];
        }
        if($_GET['seller_name'] != ''){
            $condition['seller_name'] = ['like', '%' . $_GET['seller_name'] . '%'];
        }
        if($_GET['grade_id'] != ''){
        ...
        $class_list = $model_store_class->getStoreClassList([], '', false);
        $class_array = [];
        if(!empty($class_list)){
        ...
        foreach($store_list as $value){
            ...
            if(str_cut($store_state, 6) == 'expire' && cookie('remindRenewal' . $value['store_id']) == null){
                $operation .= "<li><a class='expire' href=" . urlAdminShop('store', 'remind_renewal', ['store_id' => $value['store_id']]) . ">提醒商家续费</a></li>";
            }
            $operation .= "</ul></span>";
            $param['operation'] = $operation;
            $param['store_id'] = $value['store_id'];
            $store_name = "<a class='" . $store_state . "' href='" . urlShop('show_store', 'index', ['store_id' => $value['store_id']]) . "' target='blank'>";
            if($store_state == 'expired'){
                $store_name .= "<i class='fa fa-clock-o' title='该店铺已过期，可从编辑菜单提醒续费'></i>";
            } else {
                if($store_state == 'expire'){
    ...
    public function get_bill_cycle_xmlOp() {
        ...
        $store_ext_list = Model('store_extend')->getStoreExendList(['store_id' => ['in', $store_id_list]]);
        $store_bill_cycle = [];
        if($store_ext_list){
    ...
    public function export_csvOp() {
        ...
        if($_GET['id'] != ''){
            ...
            $condition['store_id'] = ['in', $id_array];
        }
        if($_GET['store_name'] != ''){
            $condition['store_name'] = ['like', '%' . $_GET['store_name'] . '%'];
        }
        ...
        if($_REQUEST['query'] != ''){
            $condition[ $_REQUEST['qtype'] ] = ['like', '%' . $_REQUEST['query'] . '%'];
        }
        $order = '';
        $param = [
            'store_id',
        ...
        if(in_array($_REQUEST['sortname'], $param) && in_array($_REQUEST['sortorder'], ['asc', 'desc'])){
    ...
    public function store_editOp() {
        ...
        if(chksubmit()){
            ...
            if($update_array['store_state'] == 0){
                ...
                $model_goods->editProducesOffline(['store_id' => $_POST['store_id']]);
            ...
            $result = $model_store->editStore($update_array, ['store_id' => $_POST['store_id']]);
            if($result){
                $url = [
                    [
        ...
        //店铺分类
        $model_store_class = Model('store_class');
        $parent_list = $model_store_class->getStoreClassList([], '', false);
        //店铺等级
        $model_grade = Model('store_grade');
        $grade_list = $model_grade->getGradeList();
        Tpl::output('grade_list', $grade_list);
        ...
        $joinin_detail = Model('store_joinin')->getOne(['member_id' => $store_array['member_id']]);
    ...
    public function bill_cycyle_editOp() {
        ...
        //保存
        if(chksubmit()){
            $result = $model_store_ext->editStoreExtend(['bill_cycle' => intval($_POST['bill_cycle'])], ['store_id' => $_POST['store_id']]);
            if($result){
        ...
        $store_ext = $model_store_ext->getStoreExtendInfo(['store_id' => $_GET['store_id']]);
    ...
    public function edit_save_joininOp() {
        if(chksubmit()){
            ...
            $result = Model('store_joinin')->editStoreJoinin(['member_id' => $member_id], $param);
            if($result){
    ...
    private function upload_image($file) {
        ...
        $upload->set('allow_type', ['jpg', 'jpeg', 'gif', 'png']);
    ...
    public function store_bind_classOp() {
        ...
        $store_bind_class_list = $model_store_bind_class->getStoreBindClassList([
            'store_id' => $store_id,
            'state' => ['in', [1, 2]]
        ], null);
    ...
    public function store_bind_class_delOp() {
        ...
        $store_bind_class_info = $model_store_bind_class->getStoreBindClassInfo(['bid' => $bid]);
        if(empty($store_bind_class_info)){
            $data['result'] = false;
        ...
        $condition['gc_id'] = ['in', rtrim($gc_id, ',')];
        $model_goods->editProducesLockUp($update, $condition);
        
        $result = $model_store_bind_class->delStoreBindClass(['bid' => $bid]);
    ...
    public function store_bind_class_updateOp() {
        ...
        if($new_commis_rate < 0 || $new_commis_rate >= 100){
            ...
            $update = ['commis_rate' => $new_commis_rate];
            $condition = ['bid' => $bid];
            $model_store_bind_class = Model('store_bind_class');
            ...
            if($result){
                ...
                echo json_encode(['result' => false, 'message' => L('nc_common_op_fail')]);
                die;
            }
        }
    }
    ...
    public function haoshop_addOp() {
        if(chksubmit()){
            ...
            try {
                $memberId = model('member')->addMember([
            ...
            //插入店铺扩展表
            $model = Model();
            $model->table('store_extend')->insert(['store_id' => $storeId]);
    ...
    private function checkSellerName($sellerName, $storeId = 0) {
        ...
        $count = (int)Model('store_joinin')->getStoreJoininCount([
        ...
        $seller = Model('seller')->getSellerInfo([
            'seller_name' => $sellerName,
        ]);
    ...
    private function checkMemberName($memberName) {
        ...
        return !Model('member')->getMemberCount([
            'member_name' => $memberName,
        ]);
    }
    ...
    public function get_joinin_xmlOp() {
        ...
        foreach($store_list as $value){
            $param = [];
            if(in_array(intval($value['joinin_state']), [STORE_JOIN_STATE_NEW, STORE_JOIN_STATE_PAY])){
    ...
    public function get_bind_class_applay_xmlOp() {
        ...
        $order = '';
        $param = ['bid', 'store_id', 'commis_rate', 'class_1', 'class_2', 'class_3', 'state'];
        if(in_array($_POST['sortname'], $param) && in_array($_POST['sortorder'], ['asc', 'desc'])){
            $order = $_POST['sortname'] . ' ' . $_POST['sortorder'];
        }
        ...
        $store_list = Model('store')->getStoreList(['store_id' => ['in', $storeid_array]]);
        $store_array = [];
        foreach($store_list as $value){
    ...
    public function store_bind_class_applay_checkOp() {
        ...
        $condition['state'] = 0;
        $update = $model_store_bind_class->editStoreBindClass(['state' => 1], $condition);
        if($update){
    ...
    public function reopen_checkOp() {
        ...
        if($id > 0){
            ...
            if($update){
                //更新店铺有效期
                Model('store')->editStore(['store_end_time' => $data['re_end_time']], ['store_id' => $reopen_info['re_store_id']]);
                $msg = '审核通过店铺续签申请，店铺ID：' . $reopen_info['re_store_id'] . '，续签时间段：' . date('Y-m-d', $data['re_start_time']) . ' - ' . date('Y-m-d', $data['re_end_time']);
                $this->log($msg, 1);
                exit(json_encode(['state' => true, 'msg' => '审核成功']));
            } else {
                exit(json_encode(['state' => false, 'msg' => '审核失败']));
            }
        } else {
            exit(json_encode(['state' => false, 'msg' => '审核失败']));
        }
    ...
    public function store_joinin_detailOp() {
        $model_store_joinin = Model('store_joinin');
        $joinin_detail = $model_store_joinin->getOne(['member_id' => $_GET['member_id']]);
        $joinin_detail_title = '查看';
        if(in_array(intval($joinin_detail['joinin_state']), [STORE_JOIN_STATE_NEW, STORE_JOIN_STATE_PAY])){
    ...
    public function store_joinin_verifyOp() {
        $model_store_joinin = Model('store_joinin');
        $joinin_detail = $model_store_joinin->getOne(['member_id' => $_POST['member_id']]);
    ...
    private function store_joinin_verify_pass($joinin_detail) {
        ...
        $model_store_joinin = Model('store_joinin');
        $model_store_joinin->modify($param, ['member_id' => $_POST['member_id']]);
    ...
    private function store_joinin_verify_open($joinin_detail) {
        ...
        //验证商家用户名是否已经存在
        if($model_seller->isSellerExist(['seller_name' => $joinin_detail['seller_name']])){
        ...
        if($_POST['verify_type'] === 'pass'){
            ...
            if($state){
                ...
                $model = Model();
                //插入店铺扩展表
                $model->table('store_extend')->insert(['store_id' => $store_id]);
    ...
    public function ckeck_store_nameOp() {
        ...
        $where['store_id'] = ['neq', $_GET['store_id']];
