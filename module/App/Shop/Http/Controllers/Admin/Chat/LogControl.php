<?php namespace App\Shop\Http\Controllers\Admin\Chat;
 ...
 * 聊天记录查询
 */
class  LogControl extends SystemControl {
    ...
    public function get_xmlOp() {
        ...
        foreach($log_list as $value){
            $param = [];
            $param['operation'] = "<a class='btn green' href='javascript:void(0);' onclick='ajax_form(\"login\",\"消息内容\",\"" . urlAdminShop('chat_log', 'chat_info', ['id' => $value['m_id']]) . "\",640)' class='url'><i class='fa fa-list-alt'></i>查看</a>";
    ...
    public function msg_logOp() {
        if(!empty($_GET['msg'])){
            ...
            $condition['t_msg'] = ['like', '%' . trim($_GET['msg']) . '%'];
    ...
    public function chat_infoOp() {
        $id = $_GET['id'];
        $chat_info = Model('web_chat')->getLogInfo(['m_id' => $id]);
