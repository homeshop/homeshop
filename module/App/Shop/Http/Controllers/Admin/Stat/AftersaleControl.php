 ...
 * 售后分析
 */
class  AftersaleControl extends SystemControl {
    
    private $links = [
        ['url' => 'act=stat_aftersale&op=refund', 'lang' => 'stat_refund'],
        ['url' => 'act=stat_aftersale&op=evalstore', 'lang' => 'stat_evalstore'],
    ];
    ...
    public function refundlistOp() {
        ...
        $where['add_time'] = ['between', $searchtime_arr];
        if(!empty($_GET['id']) && is_array($_GET['id'])){
            $where['refund_id'] = ['in', $_GET['id']];
        }
        if($this->search_arr['exporttype'] == 'excel'){
            $refundlist_tmp = $model_refund->getRefundReturnList($where, 0);
        }
        $statheader = [];
        $statheader[] = ['text' => '订单编号', 'key' => 'order_sn'];
        $statheader[] = ['text' => '退款编号', 'key' => 'refund_sn'];
        $statheader[] = ['text' => '店铺名', 'key' => 'store_name', 'class' => 'alignleft'];
        $statheader[] = ['text' => '商品名称', 'key' => 'goods_name', 'class' => 'alignleft'];
        $statheader[] = ['text' => '买家会员名', 'key' => 'buyer_name'];
        $statheader[] = ['text' => '申请时间', 'key' => 'add_time'];
        $statheader[] = ['text' => '退款金额', 'key' => 'refund_amount'];
        $statheader[] = ['text' => '商家审核', 'key' => 'seller_state'];
        $statheader[] = ['text' => '平台确认', 'key' => 'refund_state'];
    ...
    public function get_refund_highchartsOp() {
        ...
        if($this->search_arr['search_type'] == 'day'){
            ...
            if(C('dbdriver') == 'oracle'){
                $_group = 'HOUR(FROM_UNIXTIME(add_time))';
            }
        }
        if($this->search_arr['search_type'] == 'week'){
            ...
            if(C('dbdriver') == 'oracle'){
                $_group = 'WEEKDAY(FROM_UNIXTIME(add_time))+1';
            }
        }
        if($this->search_arr['search_type'] == 'month'){
            ...
            if(C('dbdriver') == 'oracle'){
                $_group = 'day(FROM_UNIXTIME(add_time))';
            }
        }
        $where = [];
        $where['add_time'] = ['between', $searchtime_arr];
    ...
    public function get_refundlist_xmlOp() {
        ...
        $order_type = ['add_time', 'refund_amount'];
        $sort_type = ['asc', 'desc'];
    ...
    public function evalstoreOp() {
        ...
        if(trim($this->search_arr['storename'])){
            $where['seval_storename'] = ['like', "%" . trim($this->search_arr['storename']) . "%"];
        }
        if(!empty($_GET['id']) && is_array($_GET['id'])){
            $where['seval_storeid'] = ['in', $_GET['id']];
        }
        ...
        if($this->search_arr['exporttype'] == 'excel'){
            ...
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '描述相符度'];
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '服务态度'];
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '发货速度'];
            //data
            foreach((array)$statlist as $k => $v){
                $excel_data[ $k + 1 ][] = ['data' => $v['seval_storename']];
                $excel_data[ $k + 1 ][] = ['data' => $v['avgdesccredit']];
                $excel_data[ $k + 1 ][] = ['data' => $v['avgservicecredit']];
                $excel_data[ $k + 1 ][] = ['data' => $v['avgdeliverycredit']];
            }
            $excel_data = $excel_obj->charset($excel_data, CHARSET);
            $excel_obj->addArray($excel_data);
    ...
    public function get_evalstore_xmlOp() {
        ...
        $order_type = ['seval_storename', 'avgdesccredit', 'avgservicecredit', 'avgdeliverycredit'];
        $sort_type = ['asc', 'desc'];
        $sortname = trim($this->search_arr['sortname']);
        if(!in_array($sortname, $order_type)){
        ...
        $list = $model->statByStoreAndEvaluatestore($where, $field, [$page, $countnum], 0, $orderby, 'seval_storeid');
        $statlist = [];
        if(!empty($list) && is_array($list)){
            foreach($list as $k => $v){
