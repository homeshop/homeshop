...
class  TradeControl extends SystemControl {
    ...
    private $links = [
        ['url' => 'act=stat_trade&op=income', 'lang' => 'stat_sale_income'],
        ['url' => 'act=stat_trade&op=predeposit', 'lang' => 'stat_predeposit'],
        ['url' => 'act=stat_trade&op=sale', 'lang' => 'stat_sale']
    ];
    ...
    public function __construct() {
        ...
        //处理搜索时间
        if(in_array($_REQUEST['op'], ['sale_trend', 'get_sale_xml', 'get_plat_sale'])){
    ...
    public function incomeOp() {
        ...
        $where = [];
        if($_GET['store_name']){
            $where['ob_store_name'] = ['like', "%{$_GET['store_name']}%"];
        }
        if(trim($_GET['query_start_date']) && trim($_GET['query_end_date'])){
            ...
            $where['ob_end_date'] = ['between', "$sdate,$edate"];
        } elseif(trim($_GET['query_start_date'])) {
            $sdate = strtotime($_GET['query_start_date']);
            $where['ob_end_date'] = ['egt', $sdate];
        } elseif(trim($_GET['query_end_date'])) {
            $edate = strtotime($_GET['query_end_date']);
            $where['ob_end_date'] = ['elt', $edate];
        }
        if($_GET['exporttype'] == 'excel'){
            //获取全部店铺结账数据
            $field = [
                'min(ob_id) ob_id, min(ob_store_id) ob_store_id, min(ob_store_name) ob_store_name, SUM(ob_order_totals) as ob_order_totals',
                'SUM(ob_commis_totals) as ob_commis_totals',
                'SUM(ob_order_return_totals) as ob_order_return_totals',
                'SUM(ob_commis_return_totals) as ob_commis_return_totals',
                'SUM(ob_store_cost_totals) as ob_store_cost_totals',
                'SUM(ob_result_totals) as ob_result_totals'
            ];
            ...
            //header
            $excel_data[0][0] = ['styleid' => 's_title', 'data' => '店铺名称'];
            //$excel_data[0][1] = array('styleid'=>'s_title','data'=>'商家账号');
            $excel_data[0][1] = ['styleid' => 's_title', 'data' => '订单金额'];
            $excel_data[0][2] = ['styleid' => 's_title', 'data' => '收取佣金'];
            $excel_data[0][3] = ['styleid' => 's_title', 'data' => '退单金额'];
            $excel_data[0][4] = ['styleid' => 's_title', 'data' => '退回佣金'];
            $excel_data[0][5] = ['styleid' => 's_title', 'data' => '店铺费用'];
            $excel_data[0][6] = ['styleid' => 's_title', 'data' => '结算金额'];
            //data
            foreach($bill_list as $k => $v){
                $excel_data[ $k + 1 ][0] = ['data' => $v['ob_store_name']];
                //$excel_data[$k+1][1] = array('data'=>$v['member_name']);
                $excel_data[ $k + 1 ][1] = ['data' => $v['ob_order_totals']];
                $excel_data[ $k + 1 ][2] = ['data' => $v['ob_commis_totals']];
                $excel_data[ $k + 1 ][3] = ['data' => $v['ob_order_return_totals']];
                $excel_data[ $k + 1 ][4] = ['data' => $v['ob_commis_return_totals']];
                $excel_data[ $k + 1 ][5] = ['data' => $v['ob_store_cost_totals']];
                $excel_data[ $k + 1 ][6] = ['data' => $v['ob_result_totals']];
            }
    ...
    public function get_plat_incomeOp() {
        ...
        $field = [
            'SUM(ob_order_totals) as ob_order_totals',
            'SUM(ob_commis_totals) as ob_commis_totals',
            'SUM(ob_order_return_totals) as ob_order_return_totals',
            'SUM(ob_commis_return_totals) as ob_commis_return_totals',
            'SUM(ob_store_cost_totals) as ob_store_cost_totals',
    ...
    public function get_income_xmlOp() {
        ...
        if(!empty($list) && is_array($list)){
            ...
            foreach($list as $k => $v){
                ...
                $out_array['ob_store_name'] = '<a href="' . urlShop('show_store', 'index', ['store_id' => $v['ob_store_id']]) . '" target="_blank">' . $v['ob_store_name'] . '</a>';
    ...
    public function predepositOp() {
        ...
        if($_GET['exporttype'] == 'excel'){
            ...
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '创建时间'];
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '可用金额（元）'];
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '冻结金额（元）'];
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '管理员名称'];
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '类型'];
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '描述'];
            //data
            foreach($log_list as $k => $v){
                $excel_data[ $k + 1 ][] = ['data' => $v['lg_member_name']];
                $excel_data[ $k + 1 ][] = ['data' => date('Y-m-d H:i:s', $v['lg_add_time'])];
                $excel_data[ $k + 1 ][] = ['data' => $v['lg_av_amount']];
                $excel_data[ $k + 1 ][] = ['data' => $v['lg_freeze_amount']];
                $excel_data[ $k + 1 ][] = ['data' => $v['lg_admin_name']];
                switch($v['lg_type']) {
                    case 'recharge':
                        $excel_data[ $k + 1 ][] = ['data' => '充值'];
                        break;
                    case 'order_pay':
                        $excel_data[ $k + 1 ][] = ['data' => '消费'];
                        break;
                    case 'cash_pay':
                        $excel_data[ $k + 1 ][] = ['data' => '提现'];
                        break;
                    case 'refund':
                        $excel_data[ $k + 1 ][] = ['data' => '退款'];
                        break;
                }
                $excel_data[ $k + 1 ][] = ['data' => $v['lg_desc']];
            }
            $excel_data = $excel_obj->charset($excel_data, CHARSET);
            $excel_obj->addArray($excel_data);
    ...
    public function get_predeposit_highchartsOp() {
        ...
        if($_REQUEST['search_type'] == 'day'){
            ...
            if(C('dbdriver') == 'oracle'){
                $_group = 'DAY(FROM_UNIXTIME(lg_add_time)),HOUR(FROM_UNIXTIME(lg_add_time))';
            }
    ...
    public function get_plat_predepositOp() {
        ...
        $recharge_amount = $model->getPredepositInfo([
            'lg_type' => 'recharge',
            'lg_add_time' => ['between', [$stime, $etime]]
        ], 'sum(lg_av_amount) as allnum');
        $order_amount = $model->getPredepositInfo([
            'lg_type' => 'order_pay',
            'lg_add_time' => ['between', [$stime, $etime]]
        ], 'sum(lg_av_amount) as allnum');
        $cash_amount = $model->getPredepositInfo([
            'lg_type' => 'cash_pay',
            'lg_add_time' => ['between', [$stime, $etime]]
        ], 'sum(lg_freeze_amount) as allnum');
    ...
    public function get_predeposit_xmlOp() {
        ...
        $statlist = [];
        if(!empty($list) && is_array($list)){
            $format_array = ['lg_av_amount', 'lg_freeze_amount'];
            foreach($list as $k => $v){
                $out_array = getFlexigridArray([], $order_type, $v, $format_array);
    ...
    public function saleOp() {
        ...
        $where = [];
        $where['order_add_time'] = ['between', [$curr_stime, $etime]];
        if(trim($_GET['order_type']) != ''){
            $where['order_state'] = trim($_GET['order_type']);
        }
        if(trim($_GET['store_name']) != ''){
            $where['store_name'] = ['like', '%' . trim($_GET['store_name']) . '%'];
        }
        if(!empty($_GET['id']) && is_array($_GET['id'])){
            $where['order_id'] = ['in', $_GET['id']];
        }
        //导出Excel
        if($_GET['exporttype'] == 'excel'){
            ...
            //统计数据标题
            $statlist = [];
            $statlist['headertitle'] = ['订单号', '买家', '店铺名称', '下单时间', '订单总额', '订单状态'];
            ...
            foreach((array)$statlist['data'] as $k => $v){
                $excel_data[ $k + 1 ][] = ['data' => $v['order_sn']];
                $excel_data[ $k + 1 ][] = ['data' => $v['buyer_name']];
                $excel_data[ $k + 1 ][] = ['data' => $v['store_name']];
                $excel_data[ $k + 1 ][] = ['data' => date('Y-m-d H:i:s', $v['order_add_time'])];
                $excel_data[ $k + 1 ][] = ['data' => number_format(($v['order_amount']), 2)];
                $excel_data[ $k + 1 ][] = ['data' => $v['order_statetext']];
            }
            $excel_data = $excel_obj->charset($excel_data, CHARSET);
            $excel_obj->addArray($excel_data);
    ...
    public function get_sale_xmlOp() {
        ...
        $order_type = ['order_sn', 'buyer_name', 'store_name', 'order_add_time', 'order_amount', 'order_state'];
        $sort_type = ['asc', 'desc'];
        $sortname = trim($this->search_arr['sortname']);
        if(!in_array($sortname, $order_type)){
            $sortname = 'order_id';
        ...
        if(!empty($list) && is_array($list)){
            foreach($list as $k => $v){
                $out_array = getFlexigridArray([], $order_type, $v);
    ...
    public function sale_trendOp() {
        ...
        if($this->search_arr['search_type'] == 'month'){
            ...
            $where['order_add_time'] = ['between', [$searchtime_arr[0], $searchtime_arr[1]]];
