...
class  StoreControl extends SystemControl {
    ...
    private $links = [
        ['url' => 'act=stat_store&op=newstore', 'lang' => 'stat_newstore'],
        ['url' => 'act=stat_store&op=hotrank', 'lang' => 'stat_storehotrank'],
        ['url' => 'act=stat_store&op=storesales', 'lang' => 'stat_storesales'],
        ['url' => 'act=stat_store&op=degree', 'lang' => 'stat_storedegree'],
        ['url' => 'act=stat_store&op=storearea', 'lang' => 'stat_storearea'],
    ];
    ...
    public function showstoreOp() {
        ...
        if(!empty($_GET['t'])){
            ...
            $where['store_time'] = ['between', $searchtime_arr];
        }
        //商品分类
        ...
        if($_GET['exporttype'] == 'excel'){
            ...
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '店主账号'];
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '店主商家账号'];
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '所属等级'];
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '有效期至'];
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '开店时间'];
            //data
            foreach($store_list as $k => $v){
                $excel_data[ $k + 1 ][] = ['data' => $v['store_name']];
                $excel_data[ $k + 1 ][] = ['data' => $v['member_name']];
                $excel_data[ $k + 1 ][] = ['data' => $v['seller_name']];
                if(empty($search_grade_list[ $v['grade_id'] ])){
                    $excel_data[ $k + 1 ][] = ['data' => '平台店铺'];
                } else {
                    $excel_data[ $k + 1 ][] = ['data' => $search_grade_list[ $v['grade_id'] ]];
                }
                $excel_data[ $k + 1 ][] = ['data' => $v['store_end_time'] ? date('Y-m-d', $v['store_end_time']) : '无限制'];
                $excel_data[ $k + 1 ][] = ['data' => date('Y-m-d', $v['store_time'])];
            }
            $excel_data = $excel_obj->charset($excel_data, CHARSET);
            $excel_obj->addArray($excel_data);
        ...
        $this->links[] = ['url' => 'act=stat_store&op=showstore', 'lang' => 'stat_storelist'];
    ...
    public function get_store_xmlOp() {
        ...
        if(!empty($list) && is_array($list)){
            $fields_array = ['store_name', 'member_name', 'seller_name', 'grade_id', 'store_end_time', 'store_time'];
            foreach($list as $k => $v){
    ...
    public function storesales_listOp() {
        ...
        //店铺名称
        $where['store_name'] = ['like', "%{$_GET['search_sname']}%"];
        if(!empty($_GET['id']) && is_array($_GET['id'])){
            $where['store_id'] = ['in', $_GET['id']];
        }
        //查询总条数
        ...
        //列表header
        $statheader = [];
        $statheader[] = ['text' => '店铺名称', 'key' => 'store_name'];
    ...
    public function get_storesales_xmlOp() {
        ...
        $field = " store_id,min(store_name) as store_name,SUM(order_amount) as orderamount, COUNT(*) as ordernum, COUNT(DISTINCT buyer_id) as membernum";
        //排序
        $order_type = ['store_name', 'membernum', 'ordernum', 'orderamount'];
        $sort_type = ['asc', 'desc'];
        ...
        $list = $model->statByStatorder($where, $field, [$page, $countnum], 0, $orderby, 'store_id');
        $statlist = [];
        if(!empty($list) && is_array($list)){
            foreach($list as $k => $v){
    ...
    public function storesales_trendsOp() {
        ...
        $stat_arr = ['orderamount' => [], 'ordernum' => [], 'membernum' => []];
        $statlist = [];
    ...
    public function storeareaOp() {
        ...
        if(trim($_GET['search_time'])){
            $where['store_time'] = ['elt', strtotime($_GET['search_time'])];
        }
        $search_sclass = intval($_REQUEST['search_sclass']);
        if($search_sclass > 0){
            $where['sc_id'] = $search_sclass;
        ...
        foreach((array)$province_array as $k => $v){
            if($statlist[ $k ]){
                $stat_arr[] = [
                    'cha' => $k,
                    'name' => $v,
                    'des' => "，店铺量：{$statlist[$k]['storenum']}",
                    'level' => $statlist[ $k ]['level']
                ];
            } else {
                $stat_arr[] = ['cha' => $k, 'name' => $v, 'des' => '，无订单数据', 'level' => 4];
            }
        }
