...
class  GoodsControl extends SystemControl {
    ...
    private $links = [
        ['url' => 'act=stat_goods&op=pricerange', 'lang' => 'stat_goods_pricerange'],
        ['url' => 'act=stat_goods&op=hotgoods', 'lang' => 'stat_hotgoods'],
        ['url' => 'act=stat_goods&op=goods_sale', 'lang' => 'stat_goods_sale'],
    ];
    ...
    public function hotgoods_listOp() {
        ...
        for($i = 1; $i <= 50; $i++){
            //数据
            $stat_arr['series'][0]['data'][] = ['name' => '', 'y' => 0];
            //横轴
        ...
        foreach((array)$statlist as $k => $v){
            switch($_GET['type']) {
                case 'goodsnum':
                    $stat_arr['series'][0]['data'][ $k ] = [
                        'name' => strval($v['goods_name']),
                        'y' => intval($v[ $_GET['type'] ])
                    ];
                    break;
                case 'orderamount':
                    $stat_arr['series'][0]['data'][ $k ] = [
                        'name' => strval($v['goods_name']),
                        'y' => floatval($v[ $_GET['type'] ])
                    ];
                    break;
            }
            $statlist[ $k ]['sort'] = $k + 1;
    ...
    public function goods_saleOp() {
        ...
        if(trim($_GET['goods_name'])){
            $where['goods_name'] = ['like', '%' . trim($_GET['goods_name']) . '%'];
        }
        ...
        if($_GET['exporttype'] == 'excel'){
            ...
            //header
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '商品名称'];
            $excel_data[0][] = ['styleid' => 's_title', 'data' => 'SPU'];
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '店铺名称'];
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '下单商品件数'];
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '下单单量'];
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '下单金额'];
            //data
            foreach($goods_list as $k => $v){
                $excel_data[ $k + 1 ][] = ['data' => $v['goods_name']];
                $excel_data[ $k + 1 ][] = ['data' => $v['goods_commonid']];
                $excel_data[ $k + 1 ][] = ['data' => $v['store_name']];
                $excel_data[ $k + 1 ][] = ['data' => $v['goodsnum']];
                $excel_data[ $k + 1 ][] = ['data' => $v['ordernum']];
                $excel_data[ $k + 1 ][] = ['data' => $v['goodsamount']];
            }
            ...
            //查询品牌
            $brand_list = Model('brand')->getBrandList(['brand_apply' => 1]);
    ...
    public function get_goods_xmlOp() {
        ...
        $field = 'goods_id,min(goods_name) as goods_name,min(store_id) as store_id,min(store_name) as store_name,min(goods_commonid) as goods_commonid,SUM(goods_num) as goodsnum,COUNT(DISTINCT order_id) as ordernum,SUM(goods_pay_price) as goodsamount';
        //排序
        $order_type = ['goods_name', 'store_name', 'goods_commonid', 'goodsnum', 'ordernum', 'goodsamount'];
        $sort_type = ['asc', 'desc'];
