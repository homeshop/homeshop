 ...
 * 营销分析
 */
class  MarketingControl extends SystemControl {
    
    private $links = [
        ['url' => 'act=stat_marketing&op=promotion', 'lang' => 'stat_promotion'],
        ['url' => 'act=stat_marketing&op=group', 'lang' => 'stat_group'],
    ];
    ...
    public function __construct() {
        ...
        //处理搜索时间
        if(!isset($this->search_arr['op']) || in_array($this->search_arr['op'], [
                'promotion',
                'group',
                'get_groupgoods_xml',
                'groupgoods',
    ...
    public function promotionOp() {
        ...
        //优惠类型数组
        $goodstype_arr = [2 => '抢购', 3 => '限时折扣', 4 => '优惠套装'];
        $statlist = [];
        $statcount = ['ordernum' => 0, 'goodsnum' => 0, 'orderamount' => 0.00];
        $stat_arr = [];
        $stat_json = ['ordernum' => '', 'goodsnum' => '', 'orderamount' => ''];
        if($statlist_tmp){
            ...
            foreach((array)$statlist_tmp as $k => $v){
                ...
                $stat_arr['ordernum'][] = [
                    'p_name' => $goodstype_arr[ $v['goods_type'] ],
                    'allnum' => $v['ordernumratio']
                ];
                $stat_arr['goodsnum'][] = [
                    'p_name' => $goodstype_arr[ $v['goods_type'] ],
                    'allnum' => $v['goodsnumratio']
                ];
                $stat_arr['orderamount'][] = [
                    'p_name' => $goodstype_arr[ $v['goods_type'] ],
                    'allnum' => $v['orderamountratio']
                ];
            }
            ...
            $stat_json['ordernum'] = getStatData_Pie([
                'title' => '下单量',
                'name' => '下单量(%)',
                'label_show' => false,
                'series' => $stat_arr['ordernum']
            ]);
            $stat_json['goodsnum'] = getStatData_Pie([
                'title' => '下单商品数',
                'name' => '下商品数(%)',
                'label_show' => false,
                'series' => $stat_arr['goodsnum']
            ]);
            $stat_json['orderamount'] = getStatData_Pie([
                'title' => '下单金额',
                'name' => '下单金额(%)',
                'label_show' => false,
                'series' => $stat_arr['orderamount']
            ]);
        }
    ...
    public function get_grouplist_xmlOp() {
        ...
        $where['start_time'] = ['exp', "(end_time >= {$searchtime_arr[0]} and start_time <= {$searchtime_arr[1]})"];
        $where['state'] = ['in', [10, 20, 30]];
        
        $keyword_type = ['groupbuy_name', 'goods_name'];
        if(trim($_POST['query']) != '' && in_array($_POST['qtype'], $keyword_type)){
            $type = $_POST['qtype'];
            $where[ $type ] = ['like', '%' . $_POST['query'] . '%'];
        }
        $order_type = ['start_time', 'end_time', 'goods_price', 'groupbuy_price', 'views'];
        $sort_type = ['asc', 'desc'];
        ...
        if($grouplist_tmp){
            ...
            $where['promotions_id'] = ['in', array_keys($grouplist)];
        ...
        if(!empty($grouplist) && is_array($grouplist)){
            ...
            foreach($grouplist as $k => $v){
                ...
                $out_array['groupbuy_name'] = '<a href="' . urlShop('show_groupbuy', 'groupbuy_detail', ['group_id' => $v['groupbuy_id']]) . '" target="_blank">' . $v['groupbuy_name'] . '</a>';
                $out_array['start_time'] = date('Y-m-d', $v['start_time']);
                $out_array['end_time'] = date('Y-m-d', $v['end_time']);
                $out_array['goods_name'] = '<a href="' . urlShop('goods', 'index', ['goods_id' => $v['goods_id']]) . '" target="_blank">' . $v['goods_name'] . '</a>';
    ...
    public function groupgoodsOp() {
        ...
        $statheader = [];
        $statheader[] = ['text' => '商品名称', 'key' => 'goods_name', 'class' => 'alignleft'];
        $statheader[] = ['text' => '下单商品数', 'key' => 'goodsnum', 'isorder' => 1];
        $statheader[] = ['text' => '下单金额', 'key' => 'goodsamount', 'isorder' => 1];
        $statheader[] = ['text' => '取消商品数', 'key' => 'cancelgoodsnum', 'isorder' => 1];
        $statheader[] = ['text' => '取消金额', 'key' => 'cancelgoodsamount', 'isorder' => 1];
        $statheader[] = ['text' => '完成商品数', 'key' => 'finishgoodsnum', 'isorder' => 1];
        $statheader[] = ['text' => '完成金额', 'key' => 'finishgoodsamount', 'isorder' => 1];
        foreach((array)$statlist_tmp as $k => $v){
            $tmp = $v;
            foreach($statheader as $h_k => $h_v){
                $tmp[ $h_v['key'] ] = $v[ $h_v['key'] ];
                if($h_v['key'] == 'goods_name'){
                    $tmp[ $h_v['key'] ] = '<a href="' . urlShop('goods', 'index', ['goods_id' => $v['goods_id']]) . '" target="_blank">' . $v['goods_name'] . '</a>';
                }
            }
            $statlist[] = $tmp;
        }
    ...
    public function get_groupgoods_xmlOp() {
        ...
        $list = $model->statByStatordergoods($where, $field, [$page, $countnum], 0, $orderby, 'goods_id');
        
        $statlist = [];
        if(!empty($list) && is_array($list)){
            $format_array = ['goodsamount', 'cancelgoodsamount', 'finishgoodsamount'];
            foreach($list as $k => $v){
