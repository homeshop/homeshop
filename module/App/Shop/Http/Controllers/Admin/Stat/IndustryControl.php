 ...
 * 行业分析
 */
class  IndustryControl extends SystemControl {
    
    private $links = [
        ['url' => 'act=stat_industry&op=scale', 'lang' => 'stat_industryscale'],
        ['url' => 'act=stat_industry&op=rank', 'lang' => 'stat_industryrank'],
        ['url' => 'act=stat_industry&op=price', 'lang' => 'stat_industryprice'],
        ['url' => 'act=stat_industry&op=general', 'lang' => 'stat_industrygeneral'],
    ];
    ...
    public function __construct() {
        ...
        //处理搜索时间
        if(!isset($this->search_arr['op']) || in_array($this->search_arr['op'], ['scale', 'rank', 'price'])){
        ...
        if(!isset($this->search_arr['op']) || in_array($this->search_arr['op'], ['scale', 'general'])){//仅显示前两级分类
            $show_depth = 2;
        }
    ...
    public function scale_listOp() {
        ...
        if($gc_childarr){
            ...
            foreach($stat_list as $k => $v){
                //数据
                $stat_arr['series'][0]['data'][] = ['name' => strval($v['gc_name']), 'y' => $v['y']];
                //横轴
    ...
    public function rankOp() {
        ...
        for($i = 1; $i <= 50; $i++){
            //数据
            $goods_stat_arr['series'][0]['data'][] = ['name' => '', 'y' => 0];
            //横轴
        ...
        foreach((array)$goods_list as $k => $v){
            $goods_stat_arr['series'][0]['data'][ $k ] = [
                'name' => strval($v['goods_name']),
                'y' => floatval($v['goodsnum'])
            ];
        }
        //得到统计图数据
        ...
        for($i = 1; $i <= 30; $i++){
            //数据
            $store_stat_arr['series'][0]['data'][] = ['name' => '', 'y' => 0];
            //横轴
        ...
        foreach((array)$store_list as $k => $v){
            $store_stat_arr['series'][0]['data'][ $k ] = [
                'name' => strval($v['store_name']),
                'y' => floatval($v['ordernum'])
            ];
        }
        //得到统计图数据
    ...
    public function general_listOp() {
        ...
        if(!empty($gc_childarr) && is_array($gc_childarr)){
            ...
            $goods_list_tmp = $model->statByGoods(['is_virtual' => 0], $field, 0, 0, '', 'gc_id_' . $gc_childdepth);
            ...
            if($this->search_arr['exporttype'] == 'excel'){
                //列表header
                $statheader = [];
                $statheader[] = ['text' => '类目名称', 'key' => 'gc_name'];
                $statheader[] = ['text' => '平均价格（元）', 'key' => 'priceavg', 'isorder' => 1];
                $statheader[] = ['text' => '有销量商品数', 'key' => 'ordergcount', 'isorder' => 1];
                $statheader[] = ['text' => '销量', 'key' => 'ordergnum', 'isorder' => 1];
                $statheader[] = ['text' => '销售额（元）', 'key' => 'orderamount', 'isorder' => 1];
                $statheader[] = ['text' => '商品总数', 'key' => 'goodscount', 'isorder' => 1];
                $statheader[] = ['text' => '无销量商品数', 'key' => 'unordergcount', 'isorder' => 1];
                //导出Excel
