...
class  OwnshopControl extends SystemControl {
    ...
    public function delOp() {
        ...
        if(empty($store_info)){
            exit(json_encode(['state' => false, 'msg' => '自营店铺不存在']));
        }
        
        if(!$store_info['is_own_shop']){
            exit(json_encode(['state' => false, 'msg' => '不能在此删除非自营店铺']));
        }
        ...
        if($count > 0){
            exit(json_encode(['state' => false, 'msg' => '已经发布商品的自营店铺不能被删除']));
        }
    ...
    public function editOp() {
        ...
        $storeArray = $storeModel->where(['store_id' => $_GET['id']])->find();
        ...
        if(chksubmit()){
            ...
            $storeModel->editStore($saveArray, [
                'store_id' => $_GET['id'],
            ]);
    ...
    public function bind_classOp() {
        ...
        $store_bind_class_list = $model_store_bind_class->getStoreBindClassList(['store_id' => $store_id], 30);
    ...
    public function bind_class_addOp() {
        ...
        if(empty($class_2)){
            //如果没选 二级
            $class_2_list = $model_goods_class->getGoodsClassList(['gc_parent_id' => $class_1]);
            if(!empty($class_2_list)){
                foreach($class_2_list as $class_2_info){
                    $class_3_list = $model_goods_class->getGoodsClassList(['gc_parent_id' => $class_2_info['gc_id']]);
            ...
            if(empty($class_3)){
                ...
                $class_3_list = $model_goods_class->getGoodsClassList(['gc_parent_id' => $class_2]);
                if(!empty($class_3_list)){
                    foreach($class_3_list as $class_3_info){
                        $param['class_3'] = $class_3_info['gc_id'];
                        // 检查类目是否已经存在
    ...
    public function get_xmlOp() {
        ...
        $order = '';
        $param = ['store_id', 'store_name', 'member_name', 'seller_name'];
        ...
        $stores_with_goods = model('goods')->where([
            'store_id' => [
                'in',
                $store_ids
            ]
