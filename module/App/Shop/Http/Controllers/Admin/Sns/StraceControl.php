<?php namespace App\Shop\Http\Controllers\Admin\Sns;
 ...
 * SNS动态
 */
class  StraceControl extends SystemControl {
    ...
    public function get_xmlOp() {
        ...
        foreach($strace_list as $value){
            ...
            $operation .= "<li><a href='javascript:void(0);' onclick='ajax_form(\"login\",\"动态内容\",\"" . urlAdminShop('sns_strace', 'strace_info', ['id' => $value['strace_id']]) . "\",640)'>动态内容</a></li>";
            $operation .= "<li><a href='index.php?act=sns_strace&op=scomm_list&st_id=" . $value['strace_id'] . "'>查看评论</a></li>";
            $operation .= "<li><a href='javascript:void(0);' onclick='ajaxget(\"" . urlAdminShop('sns_strace', 'strace_edit', [
                    'id' => $value['strace_id'],
                    'value' => ($value['strace_state'] == 1 ? 0 : 1)
                ]) . "\")'>" . ($value['strace_state'] == 1 ? '屏蔽动态' : '显示动态') . "</a></li>";
            $operation .= "</ul></span>";
    ...
    public function strace_infoOp() {
        $model_stracelog = Model('store_sns_tracelog');
        $id = $_GET['id'];
        $strace_info = $model_stracelog->getStoreSnsTracelogInfo(['strace_id' => $id]);
    ...
    public function strace_delOp() {
        ...
        // 实例化模型
        $rs = Model('store_sns_tracelog')->delStoreSnsTracelog(['strace_id' => $st_id]);
        if($rs){
            // 删除评论
            Model('store_sns_comment')->delStoreSnsComment(['strace_id' => $st_id]);
    ...
    public function scomm_listOp() {
        ...
        if($id <= 0){
            ...
            $strace_info = Model('store_sns_tracelog')->getStoreSnsTracelogInfo(['strace_id' => $id]);
    ...
    public function get_scomm_xmlOp() {
        ...
        $order = '';
        $param = ['scomm_id', 'scomm_content', 'scomm_membername', 'scomm_memberid', 'scomm_state', 'scomm_time'];
        if(in_array($_POST['sortname'], $param) && in_array($_POST['sortorder'], ['asc', 'desc'])){
            $order = $_POST['sortname'] . ' ' . $_POST['sortorder'];
        }
        $page = $_POST['rp'];
        ...
        foreach($scomm_list as $value){
            ...
            $operation .= "<span class='btn'><em><i class='fa fa-cog'></i>" . L('nc_set') . " <i class='arrow'></i></em><ul>";
            $operation .= "<li><a href='javascript:void(0);' onclick='ajax_form(\"login\",\"评论内容\",\"" . urlAdminShop('sns_strace', 'scomm_info', ['id' => $value['scomm_id']]) . "\",640)'>评论内容</a></li>";
            $operation .= "<li><a href='javascript:void(0);' onclick='ajaxget(\"" . urlAdminShop('sns_strace', 'scomm_edit', [
                    'id' => $value['scomm_id'],
                    'value' => ($value['scomm_state'] == 1 ? 0 : 1)
                ]) . "\")'>" . ($value['scomm_state'] == 1 ? '屏蔽评论' : '显示评论') . "</a></li>";
            $operation .= "</ul></span>";
            $param['operation'] = $operation;
    ...
    public function scomm_infoOp() {
        $model_scomment = Model('store_sns_comment');
        $id = $_GET['id'];
        $scomm_info = $model_scomment->getStoreSnsCommentInfo(['scomm_id' => $id]);
    ...
    public function scomm_delOp() {
        ...
        // 实例化模型
        $rs = Model('store_sns_comment')->delStoreSnsComment(['scomm_id' => $id]);
        if($rs){
    ...
    public function scomm_editOp() {
        ...
        // 实例化模型
        $rs = Model('store_sns_comment')->editStoreSnsComment(['scomm_state' => $scomm_state], ['scomm_id' => $id]);
        if($rs){
