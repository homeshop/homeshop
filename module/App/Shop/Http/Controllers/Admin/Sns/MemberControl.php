...
class  MemberControl extends SystemControl {
    ...
    public function member_tagOp() {
        ...
        if(chksubmit()){
            switch($_POST['submit_type']) {
                case 'del':
                    $result = $model->table('sns_membertag')->where([
                        'mtag_id' => [
                            'in',
                            implode(',', $_POST['id'])
                        ]
    ...
    public function tag_addOp() {
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                [
                    "input" => $_POST["membertag_name"],
                    "require" => "true",
                    "message" => Language::get('sns_member_tag_name_not_null')
                ],
                [
                    "input" => $_POST["membertag_sort"],
                    "require" => "true",
                    'validator' => 'Number',
                    "message" => Language::get('sns_member_tag_sort_is_number')
                ],
            ];
    ...
    public function tag_editOp() {
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                $result = $model->table('sns_membertag')->where(['mtag_id' => intval($_POST['id'])])->update($update);
        ...
        $mtag_info = $model->table('sns_membertag')->where(['mtag_id' => $mtag_id])->find();
    ...
    public function tag_delOp() {
        ...
        $model = Model();
        $result = $model->table('sns_membertag')->where(['mtag_id' => $mtag_id])->delete();
    ...
    public function tag_recommendOp() {
        ...
        Model('sns_membertag')->editSnsMemberTag(['mtag_id' => $mtag_id], ['mtag_recommend' => intval($_GET['recommend'])]);
    ...
    public function get_xmlOp() {
        ...
        $order = '';
        $param = ['mtag_id', 'mtag_name', 'mtag_sort', 'mtag_recommend'];
    ...
    public function tag_mrecommendOp() {
        ...
        Model('sns_mtagmember')->editSnsMTagMember([
            'mtag_id' => $mtag_id,
            'member_id' => $member_id
    ...
    public function get_tm_xmlOp() {
        ...
        $member_list = Model('member')->getMemberList(['member_id' => ['in', $memberid_array]]);
        $member_array = [];
        ...
        foreach($tag_list as $value){
            $param = [];
            $param['operation'] = "<a class='btn red' href='index.php?act=sns_member&op=mtag_del&id=" . $value['mtag_id'] . "&member_id=" . $value['member_id'] . "'><i class='fa fa-list-alt'></i>删除</a><a class='btn green' href='javascript:void(0);' onclick='ajaxget(\"" . urlAdminShop('sns_member', 'tag_mrecommend', [
                    'id' => $value['mtag_id'],
                    'member_id' => $value['member_id'],
                    'recommend' => ($value['recommend'] == 1 ? 0 : 1)
                ]) . "\")'><i class='fa " . ($value['recommend'] == 1 ? 'fa-thumbs-o-down' : 'fa-thumbs-o-up') . "'></i>" . ($value['recommend'] == 1 ? '取消推荐' : '推荐会员') . "</a>";
