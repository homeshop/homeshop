...
class  VoucherControl extends SystemControl {
    ...
    public function __construct() {
        ...
        $this->applystate_arr = [
            'new' => [1, Language::get('admin_voucher_applystate_new')],
            'verify' => [2, Language::get('admin_voucher_applystate_verify')],
            'cancel' => [3, Language::get('admin_voucher_applystate_cancel')]
        ];
        $this->quotastate_arr = [
            'activity' => [1, Language::get('admin_voucher_quotastate_activity')],
            'cancel' => [2, Language::get('admin_voucher_quotastate_cancel')],
            'expire' => [3, Language::get('admin_voucher_quotastate_expire')]
        ];
        //代金券模板状态
        $this->templatestate_arr = [
            'usable' => [1, Language::get('admin_voucher_templatestate_usable')],
            'disabled' => [2, Language::get('admin_voucher_templatestate_disabled')]
        ];
    ...
    public function pricelist_xmlOp() {
        $model = Model();
        $list = (array)$model->table('voucher_price')->order('voucher_price asc')->page($_REQUEST['rp'])->select();
        ...
        foreach($list as $val){
            $o = '<a class="confirm-del-on-click btn red" href="' . urlAdminShop('voucher', 'pricedrop', [
                    'voucher_price_id' => $val['voucher_price_id'],
                ]) . '"><i class="fa fa-trash"></i>删除</a>';
            $o .= '<a class="btn green" href="' . urlAdminShop('voucher', 'priceedit', [
    ...
    public function priceaddOp() {
        if(chksubmit()){
            ...
            $model = Model();
            $voucherprice_info = $model->table('voucher_price')->where(['voucher_price' => $voucher_price])->find();
    ...
    public function priceeditOp() {
        ...
        if(chksubmit()){
            ...
            $voucher_price = intval($_POST['voucher_price']);
            $voucher_points = intval($_POST['voucher_points']);
            $voucherprice_info = $model->table('voucher_price')->where([
                'voucher_price' => $voucher_price,
                'voucher_price_id' => ['neq', $id]
            ])->find();
            if(!empty($voucherprice_info)){
                $error .= Language::get('admin_voucher_price_exist');
            }
            if($error != ''){
                ...
                $rs = $model->table('voucher_price')->where(['voucher_price_id' => $id])->update($update_arr);
                if($rs){
            ...
            $voucherprice_info = $model->table('voucher_price')->where(['voucher_price_id' => $id])->find();
            if(empty($voucherprice_info)){
                showMessage(Language::get('param_error'), 'index.php?act=voucher&op=pricelist');
            }
    ...
    public function pricedropOp() {
        ...
        $model = Model();
        $rs = $model->table('voucher_price')->where(['voucher_price_id' => ['in', $voucher_price_id]])->delete();
        if($rs){
    ...
    public function quotalistOp() {
        ...
        //更新过期套餐的状态
        $time = time();
        $model->table('voucher_quota')->where([
            'quota_endtime' => ['lt', $time],
            'quota_state' => "{$this->quotastate_arr['activity'][0]}"
        ])->update(['quota_state' => $this->quotastate_arr['expire'][0]]);
    ...
    public function quotalist_xmlOp() {
        ...
        if(strlen($q = trim($_REQUEST['query']))){
            switch($_REQUEST['qtype']) {
                case 'quota_storename':
                    $condition['quota_storename'] = ['like', '%' . $q . '%'];
                    break;
            }
        }
        
        $model = Model();
        $list = (array)$model->table('voucher_quota')->where($condition)->order('quota_id desc')->page($_REQUEST['rp'])->select();
        ...
        foreach($list as $val){
            ...
            $i['quota_storename'] = '<a target="_blank" href="' . urlShop('show_store', 'index', [
    ...
    public function templatelist_xmlOp() {
        ...
        if($_REQUEST['advanced']){
            if(strlen($q = trim((string)$_REQUEST['voucher_t_title']))){
                $condition['voucher_t_title'] = ['like', '%' . $q . '%'];
            }
            if(strlen($q = trim((string)$_REQUEST['voucher_t_storename']))){
                $condition['voucher_t_storename'] = ['like', '%' . $q . '%'];
            }
            ...
            if(trim($_GET['sdate']) && trim($_GET['edate'])){
                ...
                $condition['voucher_t_add_date'] = ['between', "$sdate,$edate"];
            } elseif(trim($_GET['sdate'])) {
                $sdate = strtotime($_GET['sdate']);
                $condition['voucher_t_add_date'] = ['egt', $sdate];
            } elseif(trim($_GET['edate'])) {
                $edate = strtotime($_GET['edate']);
                $condition['voucher_t_add_date'] = ['elt', $edate];
            }
        ...
        foreach($list as $val){
            $o = '<a class="btn blue" href="' . urlAdminShop('voucher', 'templateedit', [
            ...
            $i['voucher_t_storename'] = '<a target="_blank" href="' . urlShop('show_store', 'index', [
            ...
            $i['recommend'] = $val['voucher_t_recommend'] == '1' ? '<span class="yes"><i class="fa fa-check-circle"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
    ...
    public function ajaxOp() {
        ...
        switch($_GET['branch']) {
            case 'voucher_t_recommend':
                $model_voucher->editVoucherTemplate(['voucher_t_id' => intval($_GET['id'])], [$_GET['column'] => intval($_GET['value'])]);
                $logtext = '';
