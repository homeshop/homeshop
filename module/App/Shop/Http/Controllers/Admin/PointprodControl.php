...
class  PointprodControl extends SystemControl {
    ...
    public function pointprod_xmlOp() {
        ...
        $prod_list = (array)$pointprod_model->getPointProdList($condition, '*', $sort, 0, $_REQUEST['rp']);
        ...
        foreach($prod_list as $val){
            $o = '<a class="btn red confirm-del-on-click" href="javascript:;" data-href="' . urlAdminShop('pointprod', 'prod_drop', [
                    'pg_id' => $val['pgoods_id'],
                ]) . '"><i class="fa fa-trash-o"></i>删除</a>';
            
            $o .= '<span class="btn"><em><i class="fa fa-cog"></i>设置<i class="arrow"></i></em><ul>';
            
            $o .= '<li><a href="' . urlAdminShop('pointprod', 'prod_edit', [
                    'pg_id' => $val['pgoods_id'],
                ]) . '">编辑礼品</a></li>';
            ...
            $i['pgoods_name'] = '<a target="_blank" href="' . urlShop('pointprod', 'pinfo', ['id' => $val['pgoods_id']]) . '">' . $val['pgoods_name'] . '<i class="fa fa-external-link " title="新窗口打开"></i></a>';
...
EOB;
            ...
            $i['pgoods_show_onoff'] = $val['pgoods_show'] == '1' ? '<span class="yes"><i class="fa fa-check-circle"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
            
            $i['pgoods_commend_onoff'] = $val['pgoods_commend'] == '1' ? '<span class="yes"><i class="fa fa-check-circle"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
    ...
    public function prod_addOp() {
        ...
        if(chksubmit()){
            ...
            $validate_arr[] = [
                ...
                "require" => "true",
                "message" => L('admin_pointprod_add_goodsname_error')
            ];
            $validate_arr[] = [
                "input" => $_POST["goodsprice"],
                "require" => "true",
                "validator" => "DoublePositive",
                "message" => L('admin_pointprod_add_goodsprice_number_error')
            ];
            $validate_arr[] = [
                'input' => $_POST['goodspoints'],
                'require' => 'true',
                'validator' => 'IntegerPositive',
                'message' => L('admin_pointprod_add_goodspoint_number_error')
            ];
            $validate_arr[] = [
                'input' => $_POST['goodsserial'],
                'require' => 'true',
                'message' => L('admin_pointprod_add_goodsserial_null_error')
            ];
            $validate_arr[] = [
                'input' => $_POST['goodsstorage'],
                'require' => 'true',
                'validator' => 'IntegerPositive',
                'message' => L('admin_pointprod_add_storage_number_error')
            ];
            $validate_arr[] = [
                'input' => $_POST['sort'],
                'require' => 'true',
                'validator' => 'IntegerPositive',
                'message' => L('admin_pointprod_add_sort_number_error')
            ];
            if($_POST['islimit'] == 1){
                $validate_arr[] = [
            ...
            if($_POST['islimittime']){
                $validate_arr[] = [
                    ...
                    'require' => 'true',
                    'message' => L('admin_pointprod_add_limittime_null_error')
                ];
                $validate_arr[] = [
                    'input' => $_POST['endtime'],
                    'require' => 'true',
            ...
            if($state){
                ...
                $upload_model->updatebywhere(['item_id' => $state], [
                    'upload_type' => 6,
                    'item_id' => '0',
    ...
    public function prod_editOp() {
        ...
        $model_pointprod = Model('pointprod');
        //查询礼品记录是否存在
        $prod_info = $model_pointprod->getPointProdInfo(['pgoods_id' => $pg_id]);
        if(!$prod_info){
        ...
        if(chksubmit()){
            ...
            $state = $model_pointprod->editPointProd($goods_array, ['pgoods_id' => $prod_info['pgoods_id']]);
            if($state){
                //礼品代表图片数据入库
                if($indeximg_succ){
                    //删除原有图片
                    $upload_list = $upload_model->getUploadList([
                        'upload_type' => 5,
                ...
                $upload_model->updatebywhere(['item_id' => $prod_info['pgoods_id']], [
                    'upload_type' => 6,
                    'item_id' => '0',
                    'upload_id_in' => "{$file_idstr}"
                ]);
    ...
    public function ajaxOp() {
        ...
        $model_pointprod->editPointProd($update_array, ['pgoods_id' => $id]);
        echo 'true';
        exit;
    }
    ...
    public function pointorder_list_xmlOp() {
        ...
        if($_REQUEST['advanced']){
            if(strlen($q = trim((string)$_REQUEST['point_ordersn']))){
                $condition['point_ordersn'] = ['like', '%' . $q . '%'];
            }
        ...
        foreach($list as $val){
            $o = '<a class="btn green" href="' . urlAdminShop('pointprod', 'order_info', [
                    'order_id' => $val['point_orderid'],
                ]) . '"><i class="fa fa-list-alt"></i>查看</a>';
            
            if($val['point_orderallowship'] || $val['point_orderalloweditship'] || $val['point_orderallowcancel'] || $val['point_orderallowdelete']){
                $o .= '<span class="btn"><em><i class="fa fa-cog"></i>设置<i class="arrow"></i></em><ul>';
                
                if($val['point_orderallowship']){
                    // 发货（已确认付款，待发货）
                    $o .= '<li><a href="' . urlAdminShop('pointprod', 'order_ship', [
                ...
                if($val['point_orderallowdelete']){
                    // 删除订单
                    $o .= '<li><a class="confirm-on-click" href="' . urlAdminShop('pointprod', 'order_drop', [
                            'order_id' => $val['point_orderid'],
                        ]) . '">删除订单</a></li>';
                }
    ...
    public function order_shipOp() {
        ...
        $where['point_orderstate'] = [
            'in',
            [$pointorderstate_arr['waitship'][0], $pointorderstate_arr['waitreceiving'][0]]
        ];//待发货和已经发货状态
        ...
        if(chksubmit()){
            ...
            $validate_arr[] = [
                ...
                "require" => "true",
                "message" => L('admin_pointorder_ship_code_nullerror')
            ];
            $obj_validate->validateparam = $validate_arr;
            $error = $obj_validate->validate();
            if($error != ''){
    ...
    public function order_infoOp() {
        ...
        //查询订单信息
        $model_pointorder = Model('pointorder');
        $order_info = $model_pointorder->getPointOrderInfo(['point_orderid' => $order_id]);
        if(!$order_info){
            showDialog(L('admin_pointorderd_record_error'), 'index.php?act=pointprod&op=pointorder_list', 'error');
        }
        ...
        //查询兑换订单收货人地址
        $orderaddress_info = $model_pointorder->getPointOrderAddressInfo(['point_orderid' => $order_id]);
        Tpl::output('orderaddress_info', $orderaddress_info);
        
        //兑换商品信息
        $prod_list = $model_pointorder->getPointOrderGoodsList(['point_orderid' => $order_id]);
