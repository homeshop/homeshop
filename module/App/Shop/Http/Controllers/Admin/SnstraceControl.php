...
class  SnstraceControl extends SystemControl {
    ...
    public function get_xmlOp() {
        ...
        foreach((array)$help_list as $value){
            ...
            $operation .= "<li><a href='javascript:void(0);' onclick='ajax_form(\"login\",\"动态内容\",\"" . urlAdminShop('snstrace', 'traceinfo', ['id' => $value['trace_id']]) . "\",640)'>动态内容</a></li>";
            $operation .= "<li><a href='index.php?act=snstrace&op=commentlist&tid=" . $value['trace_id'] . "'>查看评论</a></li>";
            $operation .= "<li><a href='javascript:void(0);' onclick='ajaxget(\"" . urlAdminShop('snstrace', 'traceedit', [
                    'id' => $value['trace_id'],
                    'value' => ($value['trace_state'] == 1 ? 0 : 1)
                ]) . "\")'>" . ($value['trace_state'] == 1 ? '显示动态' : '屏蔽动态') . "</a></li>";
            $operation .= "</ul></span>";
    ...
    public function traceinfoOp() {
        $tid = intval($_GET['id']);
        $trace_info = Model('sns_tracelog')->getTracelogRow(['trace_id' => $tid]);
    ...
    public function tracedelOp() {
        ...
        //删除动态
        $tracelog_model = Model('sns_tracelog');
        $result = $tracelog_model->delTracelog(['trace_id' => $tid]);
        if($result){
            //删除动态下的评论
            ...
            //更新转帖的原帖删除状态为已经删除
            $tracelog_model->tracelogEdit(['trace_originalstate' => '1'], ['trace_originalid' => $tid]);
    ...
    public function traceeditOp() {
        ...
        $result = $tracelog_model->tracelogEdit($update_arr, ['trace_id' => $tid]);
        if($result){
            //更新转帖的原帖删除状态为已经删除或者为显示
            $update_arr = [];
            $update_arr['trace_originalstate'] = $value;
            $tracelog_model->tracelogEdit($update_arr, ['trace_originalid' => $tid]);
    ...
    public function commentlistOp() {
        ...
        if($id <= 0){
            ...
            $trace_info = Model('sns_tracelog')->getTracelogRow(['trace_id' => $id]);
    ...
    public function get_comment_xmlOp() {
        ...
        foreach((array)$help_list as $value){
            ...
            $operation .= "<span class='btn'><em><i class='fa fa-cog'></i>" . L('nc_set') . " <i class='arrow'></i></em><ul>";
            $operation .= "<li><a href='javascript:void(0);' onclick='ajax_form(\"login\",\"评论内容\",\"" . urlAdminShop('snstrace', 'comminfo', ['id' => $value['comment_id']]) . "\",640)'>评论内容</a></li>";
            $operation .= "<li><a href='javascript:void(0);' onclick='ajaxget(\"" . urlAdminShop('snstrace', 'commentedit', [
                    'id' => $value['comment_id'],
                    'value' => ($value['comment_state'] == 1 ? 0 : 1)
                ]) . "\")'>" . ($value['comment_state'] == 1 ? '屏蔽评论' : '显示评论') . "</a></li>";
            $operation .= "</ul></span>";
            $param['operation'] = $operation;
    ...
    public function comminfoOp() {
        ...
        $id = $_GET['id'];
        $comm_info = $model_comment->getCommentRow(['comment_id' => $id]);
    ...
    public function commentdelOp() {
        $cid = intval($_GET['id']);
        if($cid <= 0){
            exit(json_encode(['state' => false, 'msg' => L('admin_snstrace_pleasechoose_del')]));
        }
        //删除评论
        $comment_model = Model('sns_comment');
        $result = $comment_model->delComment(['comment_id' => $cid]);
        if($result){
    ...
    public function commenteditOp() {
        ...
        $comment_model = Model('sns_comment');
        $value = $_GET['value'] == 1 ? 1 : 0;
        $result = $comment_model->commentEdit(['comment_state' => $value], ['comment_id' => $cid]);
        if($result){
