...
class  WaybillControl extends SystemControl {
    ...
    public function waybill_delOp() {
        ...
        $result = $model_waybill->delWaybill(['waybill_id' => $waybill_id]);
        if($result){
    ...
    public function ajaxOp() {
        switch($_GET['branch']) {
            case 'usable':
                $model_waybill = Model('waybill');
                $where = ['waybill_id' => intval($_GET['id'])];
                $update = ['waybill_usable' => intval($_GET['value'])];
    ...
    private function show_menu($menu_key = '') {
        $menu_array = [
            1 => [
                ...
                'menu_name' => '列表',
                'menu_url' => urlAdminShop('waybill', 'waybill_list')
            ],
        ];
        if($menu_key == 'waybill_edit'){
            $menu_array[] = ['menu_key' => 'waybill_edit', 'menu_name' => '编辑', 'menu_url' => 'javascript:;'];
        }
        if($menu_key == 'waybill_design'){
            $menu_array[] = ['menu_key' => 'waybill_design', 'menu_name' => '设计', 'menu_url' => 'javascript:;'];
        }
