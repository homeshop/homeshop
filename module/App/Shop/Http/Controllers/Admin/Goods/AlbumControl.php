<?php namespace App\Shop\Http\Controllers\Admin\Goods;
 ...
 * 相册管理
 */
class  AlbumControl extends SystemControl {
    ...
    public function get_xmlOp() {
        ...
        $order = '';
        $param = ['aclass_id', 'aclass_name', 'store_id', 'store_name', 'aclass_cover', 'pic_count', 'aclass_des'];
        ...
        // 图片数量
        $count_list = $model->cls()->table('album_pic')->field('count(*) as count, aclass_id')->where([
            'aclass_id' => [
                'in',
                $aclassid_array
            ]
        ])->group('aclass_id')->select();
        $count_array = [];
        ...
        foreach($album_list as $value){
            ...
            $param['store_name'] = "<a href='" . urlShop('show_store', 'index', ['store_id' => $value['store_id']]) . "' target='blank'>" . $store_array[ $value['store_id'] ] . "<i class='fa fa-external-link ' title='新窗口打开'></i></a>";
    ...
    public function aclass_delOp() {
        ...
        $model = Model();
        $pic = $model->table('album_pic')->field('apic_cover')->where(['aclass_id' => $aclass_id])->select();
        if(is_array($pic)){
            foreach($pic as $v){
                $this->del_file($v['apic_cover']);
            }
        }
        $model->table('album_pic')->where(['aclass_id' => $aclass_id])->delete();
        $model->table('album_class')->where(['aclass_id' => $aclass_id])->delete();
        $this->log(L('nc_delete,g_album_one') . '[ID:' . intval($_GET['aclass_id']) . ']', 1);
        exit(json_encode(['state' => true, 'msg' => L('nc_common_del_succ')]));
    }
    ...
    public function del_album_picOp() {
        ...
        Model('album_pic')->where(['apic_id' => $apic_id])->delete();
    ...
    public function del_more_picOp() {
        ...
        $list = $model->where(['apic_id' => ['in', $_POST['delbox']]])->select();
        if(is_array($list)){
            foreach($list as $v){
                $this->del_file($v['apic_cover']);
            }
        }
        $model->where(['apic_id' => ['in', $_POST['delbox']]])->delete();
    ...
    private function del_file($filename) {
        ...
        if(C('oss.open')){
            if($filename != ''){
                oss::del([ATTACH_GOODS . DS . $store_id . DS . $filename]);
            }
        } else {
