...
class  ClassControl extends SystemControl {
    ...
    private $links     = [
        ['url' => 'act=goods_class&op=goods_class', 'lang' => 'nc_manage'],
        ['url' => 'act=goods_class&op=goods_class_import', 'lang' => 'goods_class_index_import'],
        ['url' => 'act=goods_class&op=tag', 'lang' => 'goods_class_index_tag']
    ];
    private $show_type = [
        1 => '颜色',
        2 => 'SPU'
    ...
    public function goods_class_addOp() {
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                ["input" => $_POST["gc_name"], "require" => "true", "message" => $lang['goods_class_add_name_null']],
                [
                    "input" => $_POST["gc_sort"],
                    "require" => "true",
                    'validator' => 'Number',
        ...
        $type_list = $model_type->typeList(['order' => 'type_sort asc'], '', 'type_id,type_name,class_id,class_name');
        $t_list = [];
    ...
    public function goods_class_editOp() {
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                ["input" => $_POST["gc_name"], "require" => "true", "message" => $lang['goods_class_add_name_null']],
                [
                    "input" => $_POST["commis_rate"],
                    "require" => "true",
                    'validator' => 'range',
                    'max' => 100,
                    'min' => 0,
                    "message" => $lang['goods_class_add_commis_rate_error']
                ],
                [
                    "input" => $_POST["gc_sort"],
                    "require" => "true",
            ...
            // 更新分类信息
            $where = ['gc_id' => intval($_POST['gc_id'])];
            $update_array = [];
        ...
        $this->links[] = ['url' => 'act=goods_class&op=goods_class_edit', 'lang' => 'nc_edit'];
    ...
    public function get_xmlOp() {
        ...
        $order = '';
        $param = ['gc_tag_id', 'gc_tag_name', 'gc_tag_value', 'gc_id_1', 'gc_id_2', 'gc_id_3'];
    ...
    public function tag_updateOp() {
        ...
        //需要更新的TAG列表
        $tag_list = $model_class_tag->getTagList([], '', 'gc_tag_id,gc_id_1,gc_id_2,gc_id_3');
    ...
    public function tag_editOp() {
        ...
        if($_POST['form_submit']){
            $return = $model_class_tag->updateTag(['gc_tag_id' => $_POST['id'], 'gc_tag_value' => $_POST['tag_value']]);
            if($return){
        ...
        $id = $_GET['id'];
        $tag_list = $model_class_tag->getTagList(['gc_tag_id' => $id]);
    ...
    public function ajaxOp() {
        switch($_GET['branch']) {
            ...
            case 'gc_name':
                ...
                $condition['gc_id'] = ['neq', intval($_GET['id'])];
                $class_list = $model_class->getGoodsClassList($condition);
                if(empty($class_list)){
                    $where = ['gc_id' => intval($_GET['id'])];
                    $update_array = [];
            ...
            case 'check_class_name':
                ...
                $condition['gc_id'] = ['neq', intval($_GET['gc_id'])];
                $class_list = $model_class->getGoodsClassList($condition);
                if(empty($class_list)){
                    echo 'true';
                    exit;
