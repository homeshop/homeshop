...
class  RecommendControl extends SystemControl {
    ...
    public function addOp() {
        ...
        if($rec_gc_id > 0){
            ...
            if(!empty($rec_list)){
                ...
                ], 'goods_name,goods_id,goods_image');
                if(!empty($goods_list)){
        ...
        Tpl::output('rec_info', is_array($rec_list) ? current($rec_list) : []);
    ...
    public function saveOp() {
        ...
        $model_rec = Model('goods_recommend');
        $del = $model_rec->delGoodsRecommend(['rec_gc_id' => $gc_id]);
        if(!$del){
            showMessage('保存失败');
        }
    ...
    public function get_xmlOp() {
        ...
        if($_POST['query'] != '' && in_array($_POST['qtype'], ['rec_gc_name'])){
    ...
    public function deleteOp() {
        ...
        if(preg_match('/^[\d,]+$/', $_GET['del_id'])){
            ...
            $condition['rec_gc_id'] = ['in', $_GET['del_id']];
        }
    ...
    public function get_goods_listOp() {
        ...
        foreach($goods_list as $v){
            $url = urlShop('goods', 'index', ['goods_id' => $v['goods_id']]);
            $img = thumb($v, 240);
            $html .= <<<EOB
            <li>
