 ...
 * 推荐展位管理
 */
class  BoothControl extends SystemControl {
    ...
    public function goods_list_xmlOp() {
        ...
        foreach($goods_list as $val){
            $o = '<a class="btn red confirm-del-on-click" href="javascript:;" data-href="' . urlAdminShop('promotion_booth', 'del_goods', [
                    'goods_id' => $val['goods_id'],
                ]) . '"><i class="fa fa-trash"></i>删除</a>';
            
            $o .= '<a class="btn green" target="_blank" href="' . urlShop('goods', 'index', [
                    'goods_id' => $val['goods_id'],
...
EOB;
    ...
    public function booth_quota_list_xmlOp() {
        ...
        foreach($list as $val){
            ...
            $i['operation'] = '<a class="btn green" href="' . urlAdminShop('promotion_booth', 'goods_list', [
                    'store_id' => $val['store_id'],
    ...
    public function del_goodsOp() {
        ...
        if($goodsIds){
            ...
            $rs = Model('p_booth')->delBoothGoods([
    ...
    public function booth_settingOp() {
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                [
                    "input" => $_POST["promotion_booth_price"],
                    "require" => "true",
                    'validator' => 'Number',
                    "message" => '请填写展位价格'
                ],
                [
                    "input" => $_POST["promotion_booth_goods_sum"],
                    "require" => "true",
                    'validator' => 'Number',
                    "message" => '不能为空，且不小于1的整数'
                ],
            ];
