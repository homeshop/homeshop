 ...
 * 优惠套装管理
 */
class  BundlingControl extends SystemControl {
    ...
    public function bundling_list_xmlOp() {
        ...
        foreach($bundling_list as $val){
            $o = '<a class="btn red confirm-del-on-click" href="javascript:;" data-href="' . urlAdminShop('promotion_bundling', 'del_bundling', [
                    'bl_id' => $val['bl_id'],
    ...
    public function del_bundlingOp() {
        ...
        $rs = Model('p_bundling')->delBundlingForAdmin(['bl_id' => $bl_id]);
        if($rs){
            $this->jsonOutput();
        } else {
