 ...
 * 限时折扣管理
 */
class  FcodeControl extends SystemControl {
    
    private $links = [
        ['url' => 'act=promotion_fcode&op=index', 'text' => '商品列表'],
        ['url' => 'act=promotion_fcode&op=fcode_quota_list', 'text' => '套餐列表'],
        ['url' => 'act=promotion_fcode&op=fcode_setting', 'text' => '设置']
    ];
    ...
    public function get_goods_xmlOp() {
        ...
        foreach($goods_list as $value){
            ...
            $operation .= "<li><a href='" . urlAdminShop('promotion_fcode', 'download_f_code_excel', ['gid' => $value['goods_id']]) . "' target=\"_blank\">下载Ｆ码</a></li>";
            $operation .= "<li><a href='" . urlShop('goods', 'index', ['goods_id' => $value['goods_id']]) . "' target='_blank'>查看商品</a></li>";
            $operation .= "</ul>";
    ...
    public function get_quota_xmlOp() {
        ...
        $order = '';
        $param = ['store_id', 'store_name', 'fcq_starttime', 'fcq_endtime'];
        if(in_array($_POST['sortname'], $param) && in_array($_POST['sortorder'], ['asc', 'desc'])){
            $order = $_POST['sortname'] . ' ' . $_POST['sortorder'];
        }
        $page = $_POST['rp'];
    ...
    public function fcode_settingOp() {
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                [
                    "input" => $_POST["promotion_fcode_price"],
                    "require" => "true",
                    'validator' => 'Number',
                    "message" => '请填写展位价格'
                ],
