 ...
 * 预售活动管理
 */
class  BookControl extends SystemControl {
    
    private $links = [
        ['url' => 'act=promotion_book&op=index', 'text' => '定金预售'],
        ['url' => 'act=promotion_book&op=presell_goods_list', 'text' => '全款预售'],
        ['url' => 'act=promotion_book&op=book_quota_list', 'text' => '套餐列表'],
        ['url' => 'act=promotion_book&op=book_setting', 'text' => '设置']
    ];
    ...
    public function get_book_goods_xmlOp() {
        ...
        foreach($goods_list as $value){
            $param = [];
            $operation = "<a class='btn red' href='javascript:void(0);' onclick=\"fg_del('" . $value['goods_id'] . "')\"><i class='fa fa-trash-o'></i>删除</a><a class='btn green' href='" . urlShop('goods', 'index', ['goods_id' => $value['goods_id']]) . "' target='_blank'><i class='fa fa-list-alt'></i>查看</a>";
    ...
    public function get_quota_xmlOp() {
        ...
        $order = '';
        $param = ['store_id', 'store_name', 'bkq_starttime', 'bkq_endtime'];
        if(in_array($_POST['sortname'], $param) && in_array($_POST['sortorder'], ['asc', 'desc'])){
            $order = $_POST['sortname'] . ' ' . $_POST['sortorder'];
        }
        $page = $_POST['rp'];
    ...
    public function book_settingOp() {
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                [
                    "input" => $_POST["promotion_book_price"],
                    "require" => "true",
                    'validator' => 'Number',
                    "message" => '请填写展位价格'
                ],
