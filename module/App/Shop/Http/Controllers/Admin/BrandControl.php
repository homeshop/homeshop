...
class  BrandControl extends SystemControl {
    ...
    public function export_csvOp() {
        ...
        if($_GET['id'] != ''){
            ...
            $condition['brand_id'] = ['in', $id_array];
        }
    ...
    public function brand_addOp() {
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                ["input" => $_POST["brand_name"], "require" => "true", "message" => $lang['brand_add_name_null']],
                ["input" => $_POST["brand_initial"], "require" => "true", "message" => '请填写首字母'],
                [
    ...
    public function brand_editOp() {
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                $brand_info = $model_brand->getBrandInfo(['brand_id' => intval($_POST['brand_id'])]);
                $where = [];
        ...
        $brand_info = $model_brand->getBrandInfo(['brand_id' => intval($_GET['brand_id'])]);
        if(empty($brand_info)){
    ...
    public function brand_delOp() {
        ...
        Model('brand')->delBrand(['brand_id' => $brand_id]);
    ...
    public function brand_apply_setOp() {
        $brand_id = intval($_GET['id']);
        if($brand_id <= 0){
            exit(json_encode(['state' => false, 'msg' => '参数错误']));
        }
        ...
        $result = Model('brand')->editBrand(['brand_id' => $brand_id], $update_array);
        if($result){
    ...
    public function ajaxOp() {
        ...
        switch($_GET['branch']) {
            ...
            case 'check_brand_name':
                ...
                $condition['brand_id'] = ['neq', intval($_GET['id'])];
    ...
    public function export_step1Op() {
        ...
        $order = '';
        $param = ['brand_id', 'brand_name', 'brand_initial', 'brand_pic', 'brand_sort', 'brand_recommend', 'show_type'];
    ...
    private function createExcel($data = []) {
        ...
        //header
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_brandid')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_brand')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_brand_cate')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_brand_img')];
        foreach((array)$data as $k => $v){
            $tmp = [];
            $tmp[] = ['data' => $v['brand_id']];
            $tmp[] = ['data' => $v['brand_name']];
            $tmp[] = ['data' => $v['brand_class']];
            $tmp[] = ['data' => $v['brand_pic']];
            $excel_data[] = $tmp;
        }
