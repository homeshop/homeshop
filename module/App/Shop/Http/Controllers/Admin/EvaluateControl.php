...
class  EvaluateControl extends SystemControl {
    ...
    private $links = [
        ['url' => 'act=evaluate&op=evalgoods_list', 'text' => '来自买家的评价'],
        ['url' => 'act=evaluate&op=evalstore_list', 'text' => '店铺动态评价']
    ];
    ...
    public function get_goods_xmlOp() {
        ...
        $condition = [];
        if($_POST['query'] != '' && in_array($_POST['qtype'], [
        ...
        foreach($evalgoods_list as $k => $evalgoods_info){
            ...
            $list['geval_goodsname'] = "<a class='open' title='{$evalgoods_info['geval_goodsname']}' href='" . urlShop('goods', 'index', ['goods_id' => $evalgoods_info['geval_goodsid']]) . "' target='blank'>{$evalgoods_info['geval_goodsname']}</a>";
            $list['geval_storename'] = "<a class='open' title='{$evalgoods_info['geval_storename']}' href='" . urlShop('show_store', 'index', ['store_id' => $evalgoods_info['geval_storeid']]) . "' target='blank'>{$evalgoods_info['geval_storename']}</a>";
    ...
    public function evalgoods_delOp() {
        if(preg_match('/^[\d,]+$/', $_GET['geval_id'])){
            ...
            $model_evaluate_goods = Model('evaluate_goods');
            $result = $model_evaluate_goods->delEvaluateGoods(['geval_id' => ['in', $_GET['geval_id']]]);
            if($result){
    ...
    public function get_store_xmlOp() {
        ...
        $condition = [];
        if($_POST['query'] != '' && in_array($_POST['qtype'], ['seval_membername', 'seval_storename'])){
            $condition[ $_POST['qtype'] ] = ['like', "%{$_POST['query']}%"];
        }
        $sort_fields = [
            'seval_membername',
        ...
        foreach($evalstore_list as $k => $evalstore_info){
            ...
            $list['seval_storename'] = "<a class='open' title='{$evalstore_info['seval_storename']}' href='" . urlShop('show_store', 'index', ['store_id' => $evalstore_info['seval_storeid']]) . "' target='blank'>{$evalstore_info['seval_storename']}</a>";
    ...
    public function evalstore_delOp() {
        if(preg_match('/^[\d,]+$/', $_GET['seval_id'])){
            ...
            $model_evaluate_store = Model('evaluate_store');
            $result = $model_evaluate_store->delEvaluateStore(['seval_id' => ['in', $_GET['seval_id']]]);
            if($result){
