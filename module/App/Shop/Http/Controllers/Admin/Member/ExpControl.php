<?php namespace App\Shop\Http\Controllers\Admin\Member;
 ...
 * 经验值管理
 */
class  ExpControl extends SystemControl {
    ...
    public function expsettingOp() {
        ...
        if(chksubmit()){
            ...
            $result = $model_setting->updateSetting(['exppoints_rule' => serialize($exp_arr)]);
            if($result === true){
        ...
        $list_setting['exppoints_rule'] = $list_setting['exppoints_rule'] ? unserialize($list_setting['exppoints_rule']) : [];
    ...
    public function get_xmlOp() {
        ...
        $order = '';
        $param = ['exp_id', 'exp_memberid', 'exp_membername', 'exp_points', 'exp_addtime'];
    ...
    public function member_gradeOp() {
        $model_setting = Model('setting');
        $list_setting = $model_setting->getListSetting();
        $list_setting['member_grade'] = $list_setting['member_grade'] ? unserialize($list_setting['member_grade']) : [];
        if(chksubmit()){
            $update_arr = [];
            if($_POST['mg']){
                $mg_arr = [];
