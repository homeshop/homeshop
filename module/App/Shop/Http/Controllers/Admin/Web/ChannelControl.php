<?php namespace App\Shop\Http\Controllers\Admin\Web;
...
class  ChannelControl extends SystemControl {
    ...
    private $links = [
        ['url' => 'act=web_channel&op=web_channel', 'text' => '频道列表'],
        ['url' => 'act=web_channel&op=floor_list', 'text' => '模块列表']
    ];
    ...
    public function get_channel_xmlOp() {
        ...
        if($_POST['qtype'] == 'channel_name'){
            $condition[ $_POST['qtype'] ] = ['like', '%' . trim($_POST['query']) . '%'];
        }
        $list = $model_channel->getChannelList($condition, $page);
        $out_list = [];
        if(!empty($list) && is_array($list)){
            $fields_array = ['channel_name', 'channel_style', 'gc_name', 'channel_show'];
            foreach($list as $k => $v){
                $out_array = getFlexigridArray([], $fields_array, $v, $format_array = '');
                ...
                $operation = '';
                if($v['channel_show'] == 1){
                    $operation .= '<a class="btn green" href="' . urlShop('channel', 'index', ['id' => $v['channel_id']]) . '" target="_blank"><i class="fa fa-list-alt"></i>查看</a>';
                } else {
    ...
    public function get_floor_xmlOp() {
        ...
        $out_list = [];
        if(!empty($list) && is_array($list)){
            $fields_array = ['web_name', 'web_page', 'update_time', 'web_show'];
            foreach($list as $k => $v){
                $out_array = getFlexigridArray([], $fields_array, $v, $format_array = '');
        ...
        $data['total_num'] = $model_channel->gettotalnum();
        $data['list'] = $out_list;
        echo Tpl::flexigridXMLfloor($data);
        exit();
    }
    ...
    public function get_channel_flOp() {
        ...
        $condition = [];
        if(!empty($_GET['name'])){
            $condition['web_name'] = ['like', '%' . trim($_GET['name']) . '%'];
        }
    ...
    public function edit_channel_tpOp() {
        ...
        $code_list = $model_web_config->getCodeList(['web_id' => "$web_id"]);
        ...
        $floor_list = $model_web_config->getWebList(['web_id' => "$web_id"]);
    ...
    public function code_updateOp() {
        ...
        if(!empty($code)){
            ...
            $state = $model_web_config->updateCode(['code_id' => $code_id], ['code_info' => $code_info]);
        }
        if($state){
            echo '1';
            exit;
    ...
    public function html_updateOp() {
        $model_web_config = Model('web_config');
        $web_id = intval($_GET["web_id"]);
        $web_list = $model_web_config->getWebList(['web_id' => $web_id]);
    ...
    public function channel_slideOp() {
        ...
        if(!empty($code)){
            ...
            $code_info = $model_web_config->get_str($code_info, $code_type);
            $model_web_config->updateCode(['code_id' => $code_id], ['code_info' => $code_info]);
            Tpl::setDirquna('shop');
