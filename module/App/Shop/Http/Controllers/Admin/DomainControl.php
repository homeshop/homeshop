...
class  DomainControl extends SystemControl {
    ...
    public function get_xmlOp() {
        ...
        $condition = [];
        $condition['store_state'] = ['neq', 2];
        if(C('dbdriver') == 'mysql'){
            $condition['store_domain'] = ['neq', ''];
        } else {
            $condition['store_domain'] = ['exp', 'store_domain IS NOT NULL'];
        }
        if($_POST['query'] != ''){
            $condition[ $_POST['qtype'] ] = ['like', '%' . $_POST['query'] . '%'];
        }
        $order = '';
        $param = ['store_domain', 'store_domain_times', 'store_id', 'store_name', 'seller_name'];
    ...
    public function store_domain_editOp() {
        ...
        if(chksubmit()){
            ...
            if(!empty($store_domain)){
                ...
                $store_info = $model_store->getStoreInfo([
            ...
            $model_store->editStore($param, ['store_id' => $store_id]);
