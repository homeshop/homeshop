...
class  GroupbuyControl extends SystemControl {
    ...
    public function class_addOp() {
        if(chksubmit()){ //添加虚拟抢购分类
            ...
            $validate_array = [
                [
                    'input' => $_POST['class_name'],
                    'require' => 'true',
                    "validator" => "Length",
                    "min" => "1",
                    "max" => "10",
                    'message' => Language::get('groupbuy_class_name_is_not_null')
                ],
                [
                    'input' => $_POST['class_name'],
                    'require' => 'true',
                    ...
                    'min' => 0,
                    'max' => 255,
                    'message' => Language::get('groupbuy_class_sort_is_not_null')
                ],
            ];
        ...
        $list = $model_vr_groupbuy_class->getVrGroupbuyClassList(['parent_class_id' => 0]);
        Tpl::output('list', $list);
    ...
    public function class_editOp() {
        ...
        $class = $model_vr_groupbuy_class->getVrGroupbuyClassInfo(['class_id' => intval($_GET['class_id'])]);
        if(empty($class)){
        ...
        if(chksubmit()){
            ...
            if(!empty($classidArr)){
                ...
                foreach($classidArr as $val){
                    $class = $model->table('vr_groupbuy_class')->where(['class_id' => $val])->find();
                    if($class['parent_class_id'] == 0){
                        $model->table('vr_groupbuy_class')->where(['parent_class_id' => $class['class_id']])->delete();
                    }
                    $model->table('vr_groupbuy_class')->where(['class_id' => $val])->delete();
                }
            }
        }
    ...
    public function update_class_sortOp() {
        ...
        do {
            ...
            $r = Model('vr_groupbuy_class')->editVrGroupbuyClass([
                'class_id' => $id,
            ], [
                'class_sort' => $value,
            ]);
    ...
    public function ajaxOp() {
        ...
        switch($_GET['branch']) {
            case 'class':
                $model_vr_groupbuy_class = Model('vr_groupbuy_class');
                $res = $model_vr_groupbuy_class->editVrGroupbuyClass(['class_id' => $id], [$field => $value]);
                if($res){
                    // 删除虚拟抢购分类缓存
                    Model('groupbuy')->dropCachedData('groupbuy_vr_classes');
...
EOB;
    ...
    public function area_addOp() {
        if(isset($_POST) && !empty($_POST)){
            ...
            $obj_validate = new Validate();
            $validate_array = [
                ['input' => $_POST['area_name'], 'require' => 'true', 'message' => '区域名称不能为空'],
                ['input' => $_POST['first_letter'], 'require' => 'true', 'message' => '首字母不能为空'],
            ];
            $obj_validate->validateparam = $validate_array;
            $error = $obj_validate->validate();
            if($error != ''){
        ...
        if(isset($_GET['area_id'])){
            $model_vr_groupbuy_area = Model('vr_groupbuy_area');
            $area = $model_vr_groupbuy_area->getVrGroupbuyAreaInfo(['area_id' => intval($_GET['area_id'])]);
    ...
    public function area_editOp() {
        ...
        $model = Model();
        $area = $model->table('vr_groupbuy_area')->where(['area_id' => intval($_GET['area_id'])])->find();
        Tpl::output('area', $area);
        
        $parent_area = $model->table('vr_groupbuy_area')->where(['area_id' => $area['parent_area_id']])->find();
    ...
    public function area_viewOp() {
        //获取区域信息
        $model = Model();
        $area_list = $model->table('vr_groupbuy_area')->where(['parent_area_id' => intval($_GET['parent_area_id'])])->limit(false)->select();
        
        Tpl::output('list', $area_list);
        
        $area = $model->table('vr_groupbuy_area')->where(['area_id' => intval($_GET['parent_area_id'])])->find();
    ...
    public function area_streetOp() {
        //获取区域信息
        $model = Model();
        $mall_list = $model->table('vr_groupbuy_area')->where(['parent_area_id' => intval($_GET['parent_area_id'])])->limit(false)->select();
        
        Tpl::output('list', $mall_list);
        
        $mall = $model->table('vr_groupbuy_area')->where(['area_id' => intval($_GET['parent_area_id'])])->find();
    ...
    public function area_dropOp() {
        $model = Model();
        $res = $model->table('vr_groupbuy_area')->where(['area_id' => ['in', trim($_POST['area_id'])]])->delete();
