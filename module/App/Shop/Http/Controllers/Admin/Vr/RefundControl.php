...
class  RefundControl extends SystemControl {
    ...
    private $links = [
        ['url' => 'act=vr_refund&op=refund_manage', 'text' => '待处理'],
        ['url' => 'act=vr_refund&op=refund_all', 'text' => '所有记录'],
    ];
    ...
    public function get_all_xmlOp() {
        ...
        foreach($refund_list as $k => $refund_info){
            ...
            $list['goods_name'] = "<a class='open' title='{$refund_info['goods_name']}' href='" . urlShop('goods', 'index', ['goods_id' => $refund_info['goods_id']]) . "' target='blank'>{$refund_info['goods_name']}</a>";
    ...
    public function editOp() {
        ...
        $order = $model_vr_order->getOrderInfo(['order_id' => $order_id]);
        ...
        if(chksubmit()){
            ...
            if($state){
                ...
                $param['param'] = [
                    'refund_url' => urlShop('member_vr_refund', 'view', ['refund_id' => $refund['refund_id']]),
    ...
    private function _get_condition($condition) {
        ...
        if(!in_array($_GET['qtype_time'], ['add_time', 'admin_time'])){
