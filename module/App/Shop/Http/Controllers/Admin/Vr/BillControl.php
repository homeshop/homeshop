...
class  BillControl extends SystemControl {
    ...
    public function get_bill_info_code_xmlOp() {
        ...
        $condition = [];
        if($_POST['query'] != '' && in_array($_POST['qtype'], ['vr_code', 'buyer_name'])){
            if($_POST['qtype'] == 'vr_code'){
                $condition['vr_code'] = ['like', "%{$_POST['query']}%"];
            } else {
                ...
                if(!empty($order_list)){
                    ...
                    $condition['order_id'] = ['in', $order_id_list];
                } else {
                    $condition['order_id'] = 0;
                }
            }
        }
        $sort_fields = ['vr_indate', 'vr_usetime', 'buyer_id'];
        ...
        if($_GET['query_type'] == 'timeout'){
            ...
            $condition['vr_indate'] = ['between', "{$bill_info['ob_start_date']},{$bill_info['ob_end_date']}"];
        } else {
            //计算已使用兑换码列表
            $condition['vr_state'] = 1;
            $condition['vr_usetime'] = ['between', "{$bill_info['ob_start_date']},{$bill_info['ob_end_date']}"];
        }
    ...
    public function export_billOp() {
        ...
        $export_data = [];
        $export_data[0] = ['账单编号', '开始日期', '结束日期', '消费金额', '佣金金额', '本期应结', '出账日期', '账单状态', '店铺', '店铺ID'];
        $ob_order_totals = 0;
        $ob_commis_totals = 0;
        $ob_result_totals = 0;
        foreach($data as $k => $v){
    ...
    public function export_orderOp() {
        ...
        if(preg_match('/^[\d,]+$/', $_GET['rec_id'])){
            ...
            $condition['rec_id'] = ['in', $_GET['rec_id']];
        }
        if(!is_numeric($_GET['curpage'])){
        ...
        $export_data = [];
        $export_data[0] = ['兑换码', '消费时间', '订单号', '消费金额', '佣金金额', '商家', '商家编号', '买家', '买家编号', '商品'];
        if($_GET['query_type'] == 'timeout'){
    ...
    private function _get_bill_condition($condition) {
        ...
        if($_REQUEST['query'] != '' && in_array($_REQUEST['qtype'], ['ob_no', 'ob_store_name'])){
