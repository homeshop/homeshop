<?php namespace App\Shop\Http\Controllers\Admin\Vr;
...
class  OrderControl extends SystemControl {
        ...
        if(!chksubmit()){
            ...
            if($result['state']){
                ...
                QueueClient::push('addConsume', [
                    ...
                    'consume_remark' => '管理员更改虚拟订单为已收款状态，订单号：' . $order_info['order_sn']
                ]);
            }
            return $result;
        }
    ...
    public function show_orderOp() {
        ...
        //退款信息
        $refund_list = Model('vr_refund')->getRefundList(['order_id' => $order_info['order_id']]);
    ...
    private function createExcel($data = []) {
        ...
        foreach((array)$data as $k => $order_info){
            ...
            $tmp[] = ['data' => $list['buyer_phone']];
            $tmp[] = ['data' => $list['buyer_id']];
            $tmp[] = ['data' => $list['store_name']];
            $tmp[] = ['data' => $list['store_id']];
            $tmp[] = ['data' => $list['goods_name']];
            $tmp[] = ['data' => $list['vr_indate']];
    ...
    private function _get_condition(& $condition) {
        if($_REQUEST['query'] != '' && in_array($_REQUEST['qtype'], ['order_sn', 'store_name', 'buyer_name'])){
            $condition[ $_REQUEST['qtype'] ] = ['like', "%{$_REQUEST['query']}%"];
        }
        if($_GET['keyword'] != '' && in_array($_GET['keyword_type'], ['order_sn', 'store_name', 'buyer_name'])){
            if($_GET['jq_query']){
        ...
        if(in_array($_GET['order_state'], ['0', '10', '20', '40'])){
