<?php namespace App\Shop\Http\Controllers\Admin\Store;
    ...
    public function store_classOp() {
        ...
        if(chksubmit()){
            if(!empty($_POST['check_sc_id']) && is_array($_POST['check_sc_id'])){
                $result = $model_class->delStoreClass(['sc_id' => ['in', $_POST['check_sc_id']]]);
                if($result){
        ...
        $store_class_list = $model_class->getStoreClassList([]);
    ...
    public function store_class_addOp() {
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                ["input" => $_POST["sc_name"], "require" => "true", "message" => $lang['store_class_name_no_null']],
            ];
    ...
    public function store_class_editOp() {
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                $result = $model_class->editStoreClass($update_array, ['sc_id' => intval($_POST['sc_id'])]);
                if($result){
        ...
        $class_array = $model_class->getStoreClassInfo(['sc_id' => intval($_GET['sc_id'])]);
    ...
    public function ajaxOp() {
        ...
        switch($_GET['branch']) {
            ...
            case 'sc_name':
                ...
                $condition['sc_id'] = ['neq', intval($_GET['sc_id'])];
                $class_list = $model_class->getStoreClassList($condition);
                if(empty($class_list)){
                    $update_array['sc_name'] = $_GET['value'];
                    $update = $model_class->editStoreClass($update_array, ['sc_id' => intval($_GET['id'])]);
            ...
            case 'sc_sort':
                ...
                $result = $model_class->editStoreClass($update_array, ['sc_id' => intval($_GET['id'])]);
