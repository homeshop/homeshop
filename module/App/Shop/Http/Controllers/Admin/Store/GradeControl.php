 ...
 * 店铺等级管理
 */
class  GradeControl extends SystemControl {
    ...
    public function store_grade_addOp() {
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                ["input" => $_POST["sg_name"], "require" => "true", "message" => $lang['store_grade_name_no_null']],
                [
            ...
            if($error != ''){
                ...
                //验证等级名称
                if(!$this->checkGradeName(['sg_name' => trim($_POST['sg_name'])])){
                    showMessage($lang['now_store_grade_name_is_there']);
                }
                //验证级别是否存在
                if(!$this->checkGradeSort(['sg_sort' => trim($_POST['sg_sort'])])){
    ...
    public function store_grade_editOp() {
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                //验证等级名称
                if(!$this->checkGradeName(['sg_name' => trim($_POST['sg_name']), 'sg_id' => intval($_POST['sg_id'])])){
                    showMessage($lang['now_store_grade_name_is_there'], 'index.php?act=store_grade&op=store_grade_edit&sg_id=' . intval($_POST['sg_id']));
                }
                //验证级别是否存在
                if(!$this->checkGradeSort(['sg_sort' => trim($_POST['sg_sort']), 'sg_id' => intval($_POST['sg_id'])])){
                ...
                $result = $model_grade->where(['sg_id' => intval($_POST['sg_id'])])->update($update_array);
    ...
    public function isable_delGrade($sg_id) {
        //判断该等级下是否存在店铺，存在的话不能删除
        $model_store = Model('store');
        $store_list = $model_store->getStoreList(['grade_id' => $sg_id]);
