<?php namespace App\Shop\Http\Controllers\Admin\Help;
...
class  StoreControl extends SystemControl {
    ...
    public function get_xmlOp() {
        ...
        $condition = [];
        $condition['help_id'] = ['gt', '99'];//内容列表不显示系统自动添加的数据
        if($_POST['query'] != ''){
            $condition[ $_POST['qtype'] ] = ['like', '%' . $_POST['query'] . '%'];
        }
        $order = '';
        $param = ['help_id', 'help_sort', 'help_title', 'update_time', 'type_id'];
    ...
    public function del_helpOp() {
        ...
        if($id > 0){
            ...
            $state = $model_help->delHelp($condition, [$id]);
