...
class  InformControl extends SystemControl {
    ...
    private $links = [
        ['url' => 'act=inform&op=inform_list', 'text' => '未处理'],
        ['url' => 'act=inform&op=inform_handled_list', 'text' => '已处理'],
        ['url' => 'act=inform&op=inform_subject_type_list', 'text' => '类型设置'],
        ['url' => 'act=inform&op=inform_subject_list', 'text' => '主题设置']
    ];
    ...
    public function get_xmlOp() {
        ...
        if(in_array($_POST['sortname'], [
                'inform_datetime',
                'inform_member_id',
                'inform_goods_id',
                'inform_store_id'
            ]) && in_array($_POST['sortorder'], ['asc', 'desc'])
        ){
        ...
        if($inform_list && is_array($inform_list)){
            ...
            foreach($inform_list as $k => $v){
                ...
                $list['inform_goods_name'] = "<a class='open' title='{$v['inform_goods_name']}' href='" . urlShop('goods', 'index', ['goods_id' => $v['inform_goods_id']]) . "' target='blank'>{$v['inform_goods_name']}</a>";
                $list['inform_pic'] = '';
                ...
                if($_GET['type'] == ''){
                    $list['inform_handle_type'] = str_replace([1, 2, 3], [
                        '无效举报',
                        '恶意举报',
                        '有效举报'
                    ], $v['inform_handle_type']);
    ...
    private function denyMemberInform($member_id) {
        ...
        return $model_member->editMember(['member_id' => $member_id], $param);
    }
    ...
    private function denyGoods($goods_id) {
        ...
        return Model('goods')->editProducesLockUp(['goods_stateremark' => '商品被举报，平台禁售'], ['goods_commonid' => $goods_info['goods_commonid']]);
    }
}
