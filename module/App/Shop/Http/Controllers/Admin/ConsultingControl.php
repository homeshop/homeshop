...
class  ConsultingControl extends SystemControl {
    ...
    private $links = [
        ['url' => 'act=consulting&op=consulting', 'text' => '咨询管理'],
        ['url' => 'act=consulting&op=type_list', 'text' => '类型设置'],
        ['url' => 'act=consulting&op=setting', 'text' => '头部文字设置']
    ];
    ...
    public function get_consulting_xmlOp() {
        ...
        if($_POST['query'] != '' && in_array($_POST['qtype'], ['consult_content', 'member_name'])){
            $condition[ $_POST['qtype'] ] = ['like', "%{$_POST['query']}%"];
        }
        
        if($_GET['keyword'] != '' && in_array($_GET['keyword_type'], ['consult_content', 'member_name'])){
            if($_GET['jq_query']){
        ...
        if(!in_array($_GET['qtype_time'], ['consult_addtime', 'consult_reply_time'])){
        ...
        $sort_fields = ['member_id', 'goods_id', 'consult_id', 'consult_reply_time', 'store_id'];
        ...
        foreach($consult_list as $k => $consult_info){
            ...
            $list['goods_name'] = "<a class='open' title='{$consult_info['goods_name']}' href='" . urlShop('goods', 'index', ['goods_id' => $consult_info['goods_id']]) . "' target='blank'>{$consult_info['goods_name']}</a>";
            ...
            $list['store_name'] = "<a class='open' title='{$consult_info['store_name']}' href='" . urlShop('show_store', 'index', ['store_id' => $consult_info['store_id']]) . "' target='blank'>{$consult_info['store_name']}</a>";
    ...
    public function deleteOp() {
        ...
        if(preg_match('/^[\d,]+$/', $_GET['consult_id'])){
            ...
            $del = $model_consult->delConsult(['consult_id' => ['in', $_GET['consult_id']]]);
            if($del){
    ...
    public function type_listOp() {
        ...
        if(chksubmit()){
            ...
            $result = $model_ct->delConsultType(['ct_id' => ['in', $ctid_array]]);
        ...
        $type_list = $model_ct->getConsultTypeList([], 'ct_id,ct_name,ct_sort');
    ...
    public function type_addOp() {
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                ["input" => $_POST["ct_name"], "require" => "true", "message" => '请填写咨询类型名称'],
                [
                    "input" => $_POST["ct_sort"],
                    "require" => "true",
                    'validator' => 'Number',
    ...
    public function type_editOp() {
        ...
        $ct_info = $model_ct->getConsultTypeInfo(['ct_id' => $ct_id]);
    ...
    public function type_delOp() {
        ...
        $result = Model('consult_type')->delConsultType(['ct_id' => $ct_id]);
        if($result){
