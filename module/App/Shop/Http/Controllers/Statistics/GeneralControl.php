 ...
 * 统计概述
 */
class  GeneralControl extends BaseSellerControl {
    ...
    public function generalOp() {
        ...
        //商品总数、收藏量
        $goods_list = $model->statByGoods([
            'store_id' => $_SESSION['store_id'],
            'is_virtual' => 0
        ], 'COUNT(*) as goodsnum, SUM(goods_collect) as gcollectnum');
        $statnew_arr['goodsnum'] = ($t = $goods_list[0]['goodsnum']) > 0 ? $t : 0;
        $statnew_arr['gcollectnum'] = ($t = $goods_list[0]['gcollectnum']) > 0 ? $t : 0;
        
        //店铺收藏量
        $store_list = $model->getoneByStore(['store_id' => $_SESSION['store_id']], 'store_collect');
        ...
        $where['order_isvalid'] = 1;//计入统计的有效订单
        $where['order_add_time'] = ['between', [$stime, $etime]];
        $where['store_id'] = ['neq', $_SESSION['store_id']];
        if(!checkPlatformStore()){//如果不是平台店铺，则查询店铺经营类目的同行数据
            //查询店铺经营类目
            $store_bindclass = Model('store_bind_class')->getStoreBindClassList(['store_id' => $_SESSION['store_id']]);
            $goodsclassid_arr = [];
    ...
    public function pricesettingOp() {
        ...
        if(chksubmit()){
            ...
            $result = $model->where(['store_id' => $_SESSION['store_id']])->update($update_array);
            if($result === true){
        ...
        $pricerange = $pricerange ? unserialize($pricerange) : [];
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        ...
        switch($menu_type) {
            case 'setting':
                $menu_array = [
                    1 => [
                        'menu_key' => 'pricesetting',
                        'menu_name' => '商品价格区间',
                        'menu_url' => 'index.php?act=statistics_general&op=pricesetting'
                    ],
                    2 => [
                        'menu_key' => 'orderprange',
                        'menu_name' => '订单金额区间',
                        'menu_url' => 'index.php?act=statistics_general&op=orderprange'
                    ]
                ];
                break;
        }
