<?php namespace App\Shop\Http\Controllers\Statistics;
 ...
 * 用户中心店铺统计
 */
class  SaleControl extends BaseSellerControl {
    ...
    public function saleOp() {
        ...
        Tpl::output('searchtime', implode('|', [$curr_stime, $etime]));
    ...
    public function salelistOp() {
        ...
        $statheader[] = ['text' => '买家', 'key' => 'buyer_name'];
        $statheader[] = ['text' => '下单时间', 'key' => 'order_add_time'];
        $statheader[] = ['text' => '订单总额', 'key' => 'order_amount'];
        $statheader[] = ['text' => '订单状态', 'key' => 'order_statetext'];
    ...
    public function areaOp() {
        ...
        $datatype = ['orderamount' => '下单金额', 'ordernum' => '下单量', 'membernum' => '下单会员数'];
        ...
        foreach((array)$statlist_tmp as $t_k => $t_v){
            foreach((array)$t_v as $k => $v){
                ...
                if($province_id){
                    //数据
                    $stat_arr_bar[$t_k]['series'][0]['data'][] = [
                    ...
                    //数据
                    $stat_arr_bar[$t_k]['series'][0]['data'][] = ['name' => '未知', 'y' => $v[$t_k]];
                    //横轴
        ...
        foreach((array)$province_array as $k => $v){
            foreach($datatype as $t_k => $t_v){
                if($statlist[$t_k][$k][$t_k]){
                    ...
                    $des = "，无订单数据";
                    $stat_arr_map[$t_k][] = ['cha' => $k, 'name' => $v, 'des' => $des, 'level' => 4];
                }
            }
        }
        $stat_json_map = [];
    ...
    public function buyingOp() {
        ...
        $pricerange_arr = $pricerange ? unserialize($pricerange) : [];
        if($pricerange_arr){
    ...
    private function profile_menu($menu_key = '') {
        $menu_array = [
            1 => [
                'menu_key' => 'sale',
                'menu_name' => '销售统计',
                'menu_url' => 'index.php?act=statistics_sale&op=sale'
            ],
            2 => [
                'menu_key' => 'area',
                'menu_name' => '区域分布',
                'menu_url' => 'index.php?act=statistics_sale&op=area'
            ],
            3 => [
                'menu_key' => 'buying',
                'menu_name' => '购买分析',
                'menu_url' => 'index.php?act=statistics_sale&op=buying'
            ],
        ];
