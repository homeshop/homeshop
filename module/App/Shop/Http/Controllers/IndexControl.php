...
class IndexControl extends BaseHomeControl {
    ...
    public function indexOp() {
        ...
        //推荐品牌
        $brand_r_list = Model('brand')->getBrandPassedList(['brand_recommend' => 1], 'brand_id,brand_name,brand_pic,brand_xbgpic,brand_tjstore', 0, 'brand_sort asc, brand_id desc', 16);
    ...
    public function viewed_infoOp() {
        ...
        if($_SESSION['is_login'] == '1'){
            ...
            if(C('voucher_allow') == 1){
                $time_to = time();//当前日期
                $info['voucher'] = Model()->table('voucher')->where([
                    'voucher_owner_id' => $member_id,
                    'voucher_state' => 1,
                    'voucher_start_date' => ['elt', $time_to],
                    'voucher_end_date' => ['egt', $time_to]
                ])->count();
            }
            $time_to = strtotime(date('Y-m-d'));//当前日期
            $time_from = date('Y-m-d', ($time_to - 60 * 60 * 24 * 7));//7天前
            $info['consult'] = Model()->table('consult')->where([
                'member_id' => $member_id,
                'consult_reply_time' => [['gt', strtotime($time_from)], ['lt', $time_to + 60 * 60 * 24], 'and']
            ])->count();
        }
