...
class  BrandControl extends BaseHomeControl {
    ...
    public function indexOp() {
        ...
        $nav_link = [
            ...
            1 => ['title' => Language::get('brand_index_all_brand')]
        ];
        //Tpl::output('nav_link_list',$nav_link);
        
        //获得品牌列表
        $model = Model();
        $brand_c_list = $model->table('brand')->where(['brand_apply' => '1'])->order('brand_sort asc')->select();
    ...
    private function _tidyBrand($brand_c_list) {
        ...
        return ['brand_listnew' => $brand_listnew, 'brand_class' => $brand_class, 'brand_r_list' => $brand_r_list];
    }
    ...
    public function listOp() {
        ...
        $brand_info = $model_brand->getBrandInfo(['brand_id' => intval($_GET['brand'])]);
        if(!$brand_info){
         ...
         * 获得推荐品牌
         */
        $brand_r_list = Model('brand')->getBrandPassedList(['brand_recommend' => 1], 'brand_id,brand_name,brand_pic', 0, 'brand_sort asc, brand_id desc', 10);
        Tpl::output('brand_r', $brand_r_list);
        ...
        if(!empty($goods_list)){
            ...
            // 商品多图
            $goodsimage_more = $model_goods->getGoodsImageList(['goods_commonid' => ['in', $commonid_array]]);
            // 店铺
        ...
        $nav_link = [
            ...
            1 => ['title' => $lang['brand_index_all_brand'], 'link' => urlShop('brand', 'index')],
            2 => ['title' => $brand_info['brand_name']]
        ];
        //Tpl::output('nav_link_list',$nav_link);
        /**
         * 页面输出
        ...
        Model('seo')->type('brand_list')->param(['name' => $brand_info['brand_name']])->show();
