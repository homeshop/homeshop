...
class seller_wechatControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        if(chksubmit()){
            if(empty($find_data)){
                ...
                $result = $model_wechat->where(['user_id' => $account_id])->update($data);
                if(result){
                    showMessage('修改成功！');
                    return;
                } else {
    ...
    */
    ...
    public function showfollowOp() {
        ...
        $keyinfo = $model_keyword->where(['user_id' => $account_id, 'isfollow' => 1])->find();
        if(!empty($keyinfo)){
            if($keyinfo['type'] == 2){
    ...
    public function addfollowOp() {
        if(strtoupper($_SERVER['REQUEST_METHOD']) == 'POST'){
            ...
            if(empty($keyinfo)){
                ...
                $model_keyword->where(['user_id' => $account_id, 'isfollow' => 1])->update($keyword);
                $edit_id = $keyinfo[0]['kid'];
                if($edit_id > 0)//if($model_keyword->edit("user_id=".$userid." AND isfollow =1",$keyword)!==false)
                {
                    echo "1";
                    //exit("1");
                } else {
    ...
    function keyword_indexOp() {
        ...
        if(!empty($op)){
            ...
            $model_keyword = Model('keyword');
            $keyinfo = $model_keyword->where(['user_id' => $account_id, 'iskey' => 1])->select();
    ...
    function addmessOp() {
        if(strtoupper($_SERVER['REQUEST_METHOD']) == 'POST'){
            ...
            if(empty($keyinfo)){
                ...
                if($model_keyword->where(['user_id' => $account_id, 'ismess' => 1])->update($keyword))
    ...
    function showmessOp() {
        $account_id = intval($_SESSION['member_id']);
        $model_keyword = Model('keyword');
        $keyinfo = $model_keyword->where(['user_id' => $account_id, 'ismess' => 1])->find();
    ...
    function addkeyword_ajaxOp() {
        ...
        //$account_id = intval($s_account_info['id']);
        $model_keyword = Model('keyword');
        $keyinfo = $model_keyword->where(['iskey' => 1, 'kid' => $kid, 'user_id' => $account_id])->find();
