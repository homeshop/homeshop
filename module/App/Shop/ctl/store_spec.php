...
class store_specControl extends BaseSellerControl {
    ...
    public function add_specOp() {
        ...
        // 规格值信息
        $sp_value_list = $model_spec->getSpecValueList([
    ...
    public function ajax_delspecOp() {
        ...
        $rs = Model('spec')->delSpecValue(['sp_value_id' => $sp_value_id, 'store_id' => $_SESSION['store_id']]);
        if($rs){
            echo 'true';
            exit();
        } else {
    ...
    public function ajax_classOp() {
        ...
        // 分类不为空输出分类信息
        if(!empty($gc_list)){
            $data = ['type' => 'class', 'data' => $gc_list, 'deep' => $deep];
        } else {
            // 查询类型
            $model_type = Model('type');
            $spec_list = $model_type->getSpecByType(['type_id' => $gc_info['type_id']], 'type_id, spec.*');
            
            $data = ['type' => 'spec', 'data' => $spec_list, 'gcid' => $id, 'deep' => $deep];
        }
        ...
        switch($menu_type) {
            case 'spec':
                $menu_array = [
                    [
                        'menu_key' => 'spec',
                        'menu_name' => L('nc_member_path_store_spec'),
                        'menu_url' => 'index.php?act=store_spec'
                    ]
                ];
                break;
        }
