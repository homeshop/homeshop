...
class pointcartControl extends BasePointShopControl {
    ...
    public function indexOp() {
        ...
        $data = $model_pointcart->getPCartListAndAmount(['pmember_id' => $_SESSION['member_id']]);
    ...
    public function addOp() {
        ...
        if($pgid <= 0 || $quantity <= 0){
            echo json_encode(['done' => false, 'msg' => L('pointcart_cart_addcart_fail')]);
            die;
        }
        
        //验证积分礼品是否存在购物车中
        $model_pointcart = Model('pointcart');
        $check_cart = $model_pointcart->getPointCartInfo([
            'pgoods_id' => $pgid,
            ...
            switch($data['error']) {
                case 'ParameterError':
                    echo json_encode([
                        'done' => false,
                        'msg' => $data['msg'],
                        'url' => 'index.php?act=pointprod&op=plist'
                    ]);
                    die;
                    break;
                default:
                    echo json_encode(['done' => false, 'msg' => $data['msg']]);
                    die;
                    break;
            }
        }
    ...
    public function updateOp() {
        ...
        //验证礼品购物车信息是否存在
        $model_pointcart = Model('pointcart');
        $cart_info = $model_pointcart->getPointCartInfo([
            'pcart_id' => $pcart_id,
            'pmember_id' => $_SESSION['member_id']
        ]);
        if(!$cart_info){
        ...
        if(!$data['state']){
            echo json_encode(['msg' => $data['msg'], 'pgoods_choosenum' => $cart_info['pgoods_choosenum']]);
            die;
        }
        $prod_info = $data['data']['prod_info'];
        ...
        if($cart_info['pgoods_choosenum'] != $quantity){
            $cart_state = $model_pointcart->editPointCart([
                'pcart_id' => $pcart_id,
                'pmember_id' => $_SESSION['member_id']
        ...
        if($cart_state){
            ...
            echo json_encode([
                'done' => 'true',
                'subtotal' => $prod_info['pointsamount'],
                'amount' => $amount,
                'quantity' => $quantity
            ]);
            die;
    ...
    public function dropOp() {
        $pcart_id = intval($_GET['pc_id']);
        if($pcart_id <= 0){
            echo json_encode(['done' => false, 'msg' => '删除失败']);
            die;
        }
        $model_pointcart = Model('pointcart');
    ...
    public function step1Op() {
        ...
        //实例化收货地址模型（不显示自提点地址）
        $address_list = Model('address')->getAddressList([
    ...
    public function step2Op() {
        ...
        //创建兑换订单
        $data = Model('pointorder')->createOrder($_POST, $pointprod_arr, [
            'member_id' => $_SESSION['member_id'],
     ...
     * 流程第三步
     */
    public function step3Op($order_arr = []) {
