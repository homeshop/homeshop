...
class store_transportControl extends BaseSellerControl {
    ...
    public function listOp() {
        ...
        $list = $model_transport->getTransportList(['store_id' => $_SESSION['store_id']], 4);
        if(!empty($list) && is_array($list)){
            ...
            $extend = $model_transport->getExtendList(['transport_id' => ['in', array_keys($transport)]]);
            // 整理
            if(!empty($extend)){
                $tmp_extend = [];
    ...
    public function editOp() {
        ...
        $id = intval($_GET['id']);
        $model_transport = Model('transport');
        $transport = $model_transport->getTransportInfo(['id' => $id]);
        $extend = $model_transport->getExtendInfo(['transport_id' => $id]);
    ...
    public function deleteOp() {
        ...
        if($model_transport->delTansport(['id' => $id])){
    ...
    public function cloneOp() {
        ...
        try {
            ...
            if($insert){
                $extend = $model_transport->getExtendList(['transport_id' => $id]);
            ...
            if(!$insert){
                throw new Exception(Language::get('transport_op_fail'));
            }
    ...
    public function saveOp() {
        ...
        if(is_numeric($_POST['transport_id'])){
            ...
            $model_transport->transUpdate($trans_info, ['id' => intval($_POST['transport_id'])]);
    ...
    public function offpay_areaOp() {
        ...
        if(chksubmit()){
            ...
            //内置自营店ID
            $area_info = $model_parea->getAreaInfo(['store_id' => $store_id]);
            $data = [];
            ...
            if(!$area_info){
                ...
                $result = $model_parea->updateArea(['store_id' => $store_id], $data);
            }
            if($result){
                showMessage('保存成功');
            } else {
        ...
        //取出支持货到付款的县ID及上级市ID
        $parea_info = $model_parea->getAreaInfo(['store_id' => $store_id]);
        ...
        //取出支持货到付款县ID的上级市ID
        $city_checked_child_array = [];
        
        foreach($parea_ids as $i) if(isset($areas['parent'][$i])){
        ...
        switch($menu_type) {
            ...
            case 'offpay_area':
                $menu_array = [
                    1 => [
                        'menu_key' => 'transport',
                        'menu_name' => Language::get('nc_member_path_postage'),
                        'menu_url' => 'index.php?act=store_transport'
                    ],
                ];
                //if (checkPlatformStore()) {
                $menu_array[] = [
