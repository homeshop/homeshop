...
class pointredpacketControl extends BasePointShopControl {
    ...
    public function pointredpacketOp() {
        ...
        $where['rpacket_t_end_date'] = ['egt', time()];
        ...
        if(count($points_filter) > 0){
            ...
            if(count($points_filter) > 1){
                ...
                $where['rpacket_t_points'] = ['between', [$points_filter[0], $points_filter[1]]];
            } else {
                if($points_filter['min']){
                    $where['rpacket_t_points'] = ['egt', $points_filter['min']];
                } elseif($points_filter['max']) {
                    $where['rpacket_t_points'] = ['elt', $points_filter['max']];
                } elseif(isset($points_filter['isable'])) {
                    $where['rpacket_t_points'] = ['elt', $points_filter['isable']];
                }
            }
        }
        //仅我能兑换的会员级别
        if(intval($_GET['isable']) == 1){
            ...
            $where['rpacket_t_mgradelimit'] = ['elt', $member_info['member_grade_level']];
        }
