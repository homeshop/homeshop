...
class store_msgControl extends BaseSellerControl {
    ...
    public function msg_infoOp() {
        ...
        if(!in_array($_SESSION['seller_id'], $sm_readids)){
            ...
            $model_storemsg->editStoreMsg(['sm_id' => $sm_id], $update);
    ...
    public function mark_as_readOp() {
        ...
        foreach($smids as $val){
            ...
            if(empty($read_info)){
                ...
                // 更新店铺消息表
                $storemsg_info = $model_storemsg->getStoreMsgInfo(['sm_id' => $val]);
                $sm_readids = explode(',', $storemsg_info['sm_readids']);
                $sm_readids[] = $_SESSION['seller_id'];
                $sm_readids = array_unique($sm_readids);
                $update = [];
                $update['sm_readids'] = implode(',', $sm_readids).',';
                $model_storemsg->editStoreMsg(['sm_id' => $val], $update);
            }
        }
    ...
    public function msg_settingOp() {
        ...
        // 店铺消息模板列表
        $smt_list = Model('store_msg_tpl')->getStoreMsgTplList([], 'smt_code,smt_name,smt_message_switch,smt_message_forced,smt_short_switch,smt_short_forced,smt_mail_switch,smt_mail_forced');
        
        // 店铺接收设置
        $setting_list = Model('store_msg_setting')->getStoreMsgSettingList(['store_id' => $_SESSION['store_id']], '*', 'smt_code');
    ...
    public function edit_msg_settingOp() {
        ...
        // 店铺消息模板
        $smt_info = Model('store_msg_tpl')->getStoreMsgTplInfo(['smt_code' => $code], 'smt_code,smt_name,smt_message_switch,smt_message_forced,smt_short_switch,smt_short_forced,smt_mail_switch,smt_mail_forced');
        if(empty($smt_info)){
            return false;
        }
        
        // 店铺消息接收设置
        $setting_info = Model('store_msg_setting')->getStoreMsgSettingInfo([
            'smt_code' => $code,
    ...
    public function save_msg_settingOp() {
        ...
        $obj_validate->validateparam = [
            [
                "input" => $_POST["short_number"],
                "require" => "false",
                "validator" => "mobile",
                "message" => '请填写正确的手机号码'
            ],
            [
                "input" => $_POST["mail_number"],
                "require" => "false",
                "validator" => "email",
                "message" => '请填写正确的邮箱'
            ],
    ...
    private function profile_menu($menu_key = '') {
        ...
        $menu_array = [
            [
                'menu_key' => 'msg_list',
                'menu_name' => '消息列表',
                'menu_url' => urlShop('store_msg', 'index')
            ],
            ['menu_key' => 'sys_list', 'menu_name' => '系统公告', 'menu_url' => urlShop('store_msg', 'sys_list')],
            [
                'menu_key' => 'msg_setting',
                'menu_name' => '消息接收设置',
                'menu_url' => urlShop('store_msg', 'msg_setting')
            ],
        ];
        if(!$this->checkIsAdmin()){
            unset($menu_array[2]);
