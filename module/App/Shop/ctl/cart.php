...
class cartControl extends BaseBuyControl {
    ...
    public function __construct() {
        ...
        //允许不登录就可以访问的op
        $op_arr = ['ajax_load', 'add', 'del'];
        if(!in_array($op, $op_arr) && !$_SESSION['member_id']){
            redirect(urlLogin('login', 'index', ['ref_url' => request_uri()]));
        }
    ...
    public function indexOp() {
        ...
        //购物车列表
        $cart_list = $model_cart->listCart('db', ['buyer_id' => $_SESSION['member_id']]);
        ...
        $condition['voucher_t_store_id'] = ['in', array_keys($store_cart_list)];
        ...
        if($_SESSION['member_id']){
            ...
            if(!empty($cart_list)){
                foreach($cart_list as $k => $cart){
                    ...
                    $cart_array['list'][$k]['goods_url'] = urlShop('goods', 'index', ['goods_id' => $cart['goods_id']]);
                }
            }
        } else {
            //登录前
            ...
            foreach($cart_list as $key => $cart){
                ...
                $value['goods_url'] = urlShop('goods', 'index', ['goods_id' => $cart['goods_id']]);
    ...
    public function addOp() {
        ...
        if(is_numeric($_GET['goods_id'])){
            ...
            //优惠套装加入购物车(单套)
            if(!$_SESSION['member_id']){
                exit(json_encode(['msg' => '请先登录', 'UTF-8']));
            }
            ...
            $bl_info = $model_bl->getBundlingInfo(['bl_id' => $bl_id]);
            if(empty($bl_info) || $bl_info['bl_state'] == '0'){
                exit(json_encode(['msg' => '该优惠套装已不存在，建议您单独购买', 'UTF-8']));
            }
            
            //检查每个商品是否符合条件,并重新计算套装总价
            $bl_goods_list = $model_bl->getBundlingGoodsList(['bl_id' => $bl_id]);
            $goods_id_array = [];
            $bl_amount = 0;
        ...
        if($insert){
            ...
            $data = [
                'state' => 'true',
                'num' => $model_cart->cart_goods_num,
                'amount' => ncPriceFormat($model_cart->cart_all_price)
            ];
        } else {
            $data = ['state' => 'false'];
        }
    ...
    private function _check_goods($goods_info, $quantity) {
        if(empty($quantity)){
            exit(json_encode(['msg' => Language::get('wrong_argument', 'UTF-8')]));
        }
        if(empty($goods_info)){
            exit(json_encode(['msg' => Language::get('cart_add_goods_not_exists', 'UTF-8')]));
        }
        if($goods_info['store_id'] == $_SESSION['store_id']){
            exit(json_encode(['msg' => Language::get('cart_add_cannot_buy', 'UTF-8')]));
        }
        if(intval($goods_info['goods_storage']) < 1){
            exit(json_encode(['msg' => Language::get('cart_add_stock_shortage', 'UTF-8')]));
        }
        if(intval($goods_info['goods_storage']) < $quantity){
            exit(json_encode(['msg' => Language::get('cart_add_too_much', 'UTF-8')]));
        }
        if($goods_info['is_virtual'] || $goods_info['is_fcode'] || $goods_info['is_book']){
            exit(json_encode(['msg' => '该商品不允许加入购物车，请直接购买', 'UTF-8']));
        }
    }
    ...
    public function updateOp() {
        ...
        if(empty($cart_id) || empty($quantity)){
            exit(json_encode(['msg' => Language::get('cart_update_buy_fail', 'UTF-8')]));
        }
        ...
        $cart_info = $model_cart->getCartInfo(['cart_id' => $cart_id, 'buyer_id' => $_SESSION['member_id']]);
        if($cart_info['bl_id'] == '0'){
            ...
            if(intval($goods_info['goods_storage']) < $quantity){
                ...
                $model_cart->editCart(['goods_num' => $goods_info['goods_storage']], [
                    'cart_id' => $cart_id,
                    'buyer_id' => $_SESSION['member_id']
                ]);
                exit(json_encode($return));
            ...
            //优惠套装商品
            $model_bl = Model('p_bundling');
            $bl_goods_list = $model_bl->getBundlingGoodsList(['bl_id' => $cart_info['bl_id']]);
            $goods_id_array = [];
            foreach($bl_goods_list as $goods){
                $goods_id_array[] = $goods['goods_id'];
            }
        ...
        $update = $model_cart->editCart($data, ['cart_id' => $cart_id, 'buyer_id' => $_SESSION['member_id']]);
        if($update){
            $return = [];
            $return['state'] = 'true';
            ...
            $return = ['msg' => Language::get('cart_update_buy_fail', 'UTF-8')];
        }
        exit(json_encode($return));
    }
    ...
    public function delOp() {
        ...
        if($_SESSION['member_id']){
            //登录状态下删除数据库内容
            $delete = $model_cart->delCart('db', ['cart_id' => $cart_id, 'buyer_id' => $_SESSION['member_id']]);
            if($delete){
                $data['state'] = 'true';
            ...
            //未登录时删除cookie的购物车信息
            $delete = $model_cart->delCart('cookie', ['goods_id' => $cart_id]);
            if($delete){
                $data['state'] = 'true';
                $data['quantity'] = $model_cart->cart_goods_num;
                $data['amount'] = $model_cart->cart_all_price;
