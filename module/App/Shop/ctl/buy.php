...
class buyControl extends BaseBuyControl {
    ...
    public function payOp() {
        ...
        $pay_info = $model_order->getOrderPayInfo([
            'pay_sn' => $pay_sn,
        ...
        $condition['order_state'] = ['in', [ORDER_STATE_NEW, ORDER_STATE_PAY]];
        $order_list = $model_order->getOrderList($condition, '', '*', '', '', [], true);
        if(empty($order_list)){
        ...
        if($order_list[0]['if_buyer_repay'] && $order_list[0]['pay_sn1'] == ''){
            ...
            $update = $model_order->editOrder([
                'pay_sn' => $pay_sn_new,
                'pay_sn1' => $pay_sn
            ], ['order_id' => $order_list[0]['order_id'], 'order_type' => 2]);
            if(!$update){
                showMessage('支付失败', 'index.php?act=member_order', 'html', 'error');
            } else {
        ...
        foreach($order_list as $key => $order_info){
            if(!in_array($order_info['payment_code'], ['offline', 'chain'])){
    ...
    public function pd_payOp() {
        ...
        $pd_info = $model_order->getPdRechargeInfo([
            'pdr_sn' => $pay_sn,
        ...
        $model_payment = Model('payment');
        $condition = [];
        $condition['payment_code'] = ['not in', ['offline', 'predeposit', 'wxpay']];
    ...
    public function pay_okOp() {
        ...
        //查询支付单信息
        $model_order = Model('order');
        $pay_info = $model_order->getOrderPayInfo(['pay_sn' => $pay_sn, 'buyer_id' => $_SESSION['member_id']]);
        if(empty($pay_info)){
            showMessage(Language::get('cart_order_pay_not_exists'), 'index.php?act=member_order', 'html', 'error');
        }
        Tpl::output('pay_info', $pay_info);
    ...
    public function load_addrOp() {
        ...
        if(!empty($_GET['id']) && intval($_GET['id']) > 0){
            $model_addr->delAddress(['address_id' => intval($_GET['id']), 'member_id' => $_SESSION['member_id']]);
        }
        $condition = [];
    ...
    public function add_addrOp() {
        $model_addr = Model('address');
        if(chksubmit()){
            $count = $model_addr->getAddressCount(['member_id' => $_SESSION['member_id']]);
            if($count >= 20){
                exit(json_encode(['state' => false, 'msg' => '最多允许添加20个有效地址']));
            }
            //验证表单信息
            ...
            $obj_validate->validateparam = [
                [
                    ...
                    "require" => "true",
                    "message" => Language::get('cart_step1_input_receiver')
                ],
                [
                    "input" => $_POST["area_id"],
                    "require" => "true",
                    "validator" => "Number",
                    "message" => Language::get('cart_step1_choose_area')
                ]
            ];
            ...
            if($insert_id){
                exit(json_encode(['state' => true, 'addr_id' => $insert_id]));
            } else {
                exit(json_encode(['state' => false, 'msg' => '新地址添加失败']));
            }
        } else {
    ...
    public function load_invOp() {
        ...
        if(intval($_GET['del_id']) > 0){
            $model_inv->delInv(['inv_id' => intval($_GET['del_id']), 'member_id' => $_SESSION['member_id']]);
        }
        ...
        if(chksubmit()){
            ...
            if($_POST['invoice_type'] == 2){
                if(empty($_POST['inv_company']) || empty($_POST['inv_code']) || empty($_POST['inv_reg_addr'])){
                    exit(json_encode(['state' => false, 'msg' => Language::get('nc_common_save_fail', 'UTF-8')]));
                }
            }
            $data = [];
            ...
            if($insert_id){
                exit(json_encode(['state' => 'success', 'id' => $insert_id]));
            } else {
                exit(json_encode(['state' => 'fail', 'msg' => Language::get('nc_common_save_fail', 'UTF-8')]));
            }
        } else {
            Tpl::showpage('buy_address.add', 'null_layout');
        }
