...
class store_orderControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        $order_list = $model_order->getStoreOrderList($_SESSION['store_id'], $_GET['order_sn'], $_GET['buyer_name'], $_GET['state_type'], $_GET['query_start_date'], $_GET['query_end_date'], $_GET['skip_off'], '*', [
            'order_goods',
            'order_common',
            'member'
        ]);
    ...
    public function show_orderOp() {
        ...
        $order_info = $model_order->getOrderInfo($condition, ['order_common', 'order_goods', 'member']);
        if(empty($order_info)){
        ...
        //取得订单操作日志
        $order_log_list = $model_order->getOrderLogList(['order_id' => $order_info['order_id']], 'log_id asc');
    ...
    private function profile_menu($menu_type = '', $menu_key = '') {
        ...
        switch($menu_type) {
            case 'list':
                $menu_array = [
                    [
                        'menu_key' => 'store_order',
                        'menu_name' => Language::get('nc_member_path_all_order'),
                        'menu_url' => 'index.php?act=store_order'
                    ],
                    [
                        'menu_key' => 'state_new',
                        'menu_name' => Language::get('nc_member_path_wait_pay'),
                        'menu_url' => 'index.php?act=store_order&op=index&state_type=state_new'
                    ],
                    [
                        'menu_key' => 'state_pay',
                        'menu_name' => Language::get('nc_member_path_wait_send'),
                        'menu_url' => 'index.php?act=store_order&op=store_order&state_type=state_pay'
                    ],
                    [
                        'menu_key' => 'state_notakes',
                        'menu_name' => '待自提',
                        'menu_url' => 'index.php?act=store_order&op=store_order&state_type=state_notakes'
                    ],
                    [
                        'menu_key' => 'state_send',
                        'menu_name' => Language::get('nc_member_path_sent'),
                        'menu_url' => 'index.php?act=store_order&op=index&state_type=state_send'
                    ],
                    [
                        'menu_key' => 'state_success',
                        'menu_name' => Language::get('nc_member_path_finished'),
                        'menu_url' => 'index.php?act=store_order&op=index&state_type=state_success'
                    ],
                    [
                        'menu_key' => 'state_cancel',
                        'menu_name' => Language::get('nc_member_path_canceled'),
                        'menu_url' => 'index.php?act=store_order&op=index&state_type=state_cancel'
                    ],
                ];
                break;
        }
