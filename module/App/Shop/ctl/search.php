...
class searchControl extends BaseHomeControl {
    ...
    public function indexOp() {
        ...
        if(intval($_GET['cate_id']) > 0){
            $goods_class_array = $this->_model_search->getLeftCategory([$_GET['cate_id']]);
        } elseif($_GET['keyword'] != '') {
            if(cookie('his_sh') == ''){
                $his_sh_list = [];
            } else {
        ...
        if(!empty($goods_list)){
            if(is_null($indexer_count)){
                ...
                // 商品多图
                $goodsimage_more = $model_goods->getGoodsImageList([
                    'goods_commonid' => [
                        'in',
                        $commonid_array
                    ]
                ], '*', 'is_default desc,goods_image_id asc');
                // 店铺
    ...
    public function get_booth_goodsOp() {
        ...
        $child = (!empty($goods_class[$gc_id]['child'])) ? explode(',', $goods_class[$gc_id]['child']) : [];
        $childchild = (!empty($goods_class[$gc_id]['childchild'])) ? explode(',', $goods_class[$gc_id]['childchild']) : [];
        $gcid_array = array_merge([$gc_id], $child, $childchild);
        // 查询添加到推荐展位中的商品id
        $boothgoods_list = Model('p_booth')->getBoothGoodsList([
            'gc_id' => [
                'in',
                $gcid_array
            ]
        ...
        $goods_list = Model('goods')->getGoodsOnlineList(['goods_id' => ['in', $goodsid_array]], $fieldstr);
        if(empty($goods_list)){
            return false;
        }
    ...
    public function get_gc_goods_recommendOp() {
        ...
        $goods_list = [];
        if($rec_gc_id > 0){
            $rec_list = Model('goods_recommend')->getGoodsRecommendList(['rec_gc_id' => $rec_gc_id], '', '', '*', '', 'rec_goods_id');
            if(!empty($rec_list)){
