...
class store_activityControl extends BaseSellerControl {
    ...
    public function activity_applyOp() {
        ...
        $list = $activity_detail_model->getGoodsJoinList([
            'activity_id' => "$activity_id",
            'store_id' => "{$_SESSION['store_id']}",
            'activity_detail_state_in' => "'0','1','3'",
            'group' => 'activity_detail_state asc'
        ]);
        ...
        if(trim($_GET['name']) != ''){
            $condition['goods_name'] = ['like', '%'.trim($_GET['name']).'%'];
        }
        $condition['store_id'] = $_SESSION['store_id'];
        if(!empty($item_ids)){
            $condition['goods_id'] = ['not in', $item_ids];
        }
    ...
    public function activity_apply_saveOp() {
        ...
        $list = $activity_detail->getList([
            'store_id' => "{$_SESSION['store_id']}",
            'activity_id' => "$activity_id"
        ]);
        $ids = [];//已经存在的活动内容编号
        $ids_state2 = [];//已经存在的被拒绝的活动编号
        if(is_array($list) and !empty($list)){
            foreach($list as $ad){
                $ids[] = $ad['item_id'];
        ...
        foreach($_POST['item_id'] as $item_id){
            ...
            if(!in_array($item_id, $ids)){
                ...
                $activity_detail->updateList($input, ['item_id' => $item_id]);
            }
        }
    ...
    private function profile_menu($menu_key = '') {
        ...
        $menu_array = [
            1 => [
                'menu_key' => 'store_activity',
                'menu_name' => Language::get('nc_member_path_activity_list'),
                'menu_url' => 'index.php?act=store_activity&op=store_activity'
            ],
            2 => [
                'menu_key' => 'activity_apply',
                'menu_name' => Language::get('nc_member_path_join_activity'),
                'menu_url' => ''
            ]
        ];
