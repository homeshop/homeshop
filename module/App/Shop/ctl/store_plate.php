...
class store_plateControl extends BaseSellerControl {
    ...
    public function plate_listOp() {
        ...
        if(trim($_GET['p_name']) != ''){
            $where['plate_name'] = ['like', '%'.trim($_GET['p_name']).'%'];
        }
        if(in_array($_GET['p_position'], ['0', '1'])){
    ...
    public function plate_addOp() {
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                ...
                ["input" => $_POST["p_content"], "require" => "true", "message" => '请填写版式内容']
            ];
    ...
    public function plate_editOp() {
        ...
        $plate_info = Model('store_plate')->getStorePlateInfo([
            'plate_id' => $plate_id,
    ...
    public function drop_plateOp() {
        ...
        $return = Model('store_plate')->delStorePlate([
            'plate_id' => ['in', $plateid_array],
            'store_id' => $_SESSION['store_id']
        ]);
        if($return){
            ...
            case 'plate_list':
                $menu_array = [
                    [
                        'menu_key' => 'plate_list',
                        'menu_name' => '版式列表',
                        'menu_url' => urlShop('store_plate', 'plate_list')
                    ]
                ];
                break;
            case 'plate_add':
                $menu_array = [
                    ...
                    [
                        'menu_key' => 'plate_add',
                        'menu_name' => '添加版式',
                        'menu_url' => urlShop('store_plate', 'plate_add')
                    ]
                ];
                break;
            case 'plate_edit':
                $menu_array = [
                    ...
                    [
                        'menu_key' => 'plate_edit',
                        'menu_name' => '编辑版式',
                        'menu_url' => urlShop('store_plate', 'plate_edit')
                    ]
                ];
                break;
        }
