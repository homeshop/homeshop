...
class predepositControl extends SystemControl {
    ...
    public function recharge_editOp() {
        ...
        try {
            ...
            QueueClient::push('addConsume', [
                ...
                'consume_amount' => $info['pdr_amount'],
                'consume_time' => TIMESTAMP,
                'consume_remark' => '管理员更改充值单[' . $info['pdr_sn'] . ']状态,充值成功'
            ]);
    ...
    public function pd_cash_delOp() {
        ...
        if($id > 0){
            ...
            try {
                ...
                $member_info = $model_member->getMemberInfo(['member_id' => $info['pdc_member_id']]);
                //扣除冻结的预存款
    ...
    public function export_step1Op() {
        $condition = [];
        if($_GET['member_name']){
            $condition['pdr_member_name'] = ['like', '%' . $_GET['member_name'] . '%'];
        }
        if($_GET['member_id']){
            $condition['pdr_member_id'] = ['like', '%' . $_GET['member_id'] . '%'];
        }
        ...
        if($start_unixtime || $end_unixtime){
            $condition['pdr_add_time'] = ['time', [$start_unixtime, $end_unixtime]];
        }
        ...
        if(in_array($$_GET['sortname'], $param) && in_array($_GET['sortorder'], ['asc', 'desc'])){
            $order = $_GET['sortname'] . ' ' . $_GET['sortorder'];
        }
        if($_GET['id'] != ''){
            $id_array = explode(',', $_GET['id']);
            $condition['pdr_id'] = ['in', $id_array];
        }
        $model_pd = Model('predeposit');
    ...
    private function createExcel($data = []) {
        ...
        //header
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_yc_no')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_yc_member')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_yc_ctime')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_yc_ptime')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_yc_pay')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_yc_money')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_yc_paystate')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_yc_memberid')];
        foreach((array)$data as $k => $v){
            $tmp = [];
            $tmp[] = ['data' => $v['pdr_sn']];
            $tmp[] = ['data' => $v['pdr_member_name']];
            $tmp[] = ['data' => date('Y-m-d H:i:s', $v['pdr_add_time'])];
            if(intval($v['pdr_payment_time'])){
                if(date('His', $v['pdr_payment_time']) == 0){
                    $tmp[] = ['data' => date('Y-m-d', $v['pdr_payment_time'])];
                } else {
                    $tmp[] = ['data' => date('Y-m-d H:i:s', $v['pdr_payment_time'])];
                }
            } else {
                $tmp[] = ['data' => ''];
            }
            $tmp[] = ['data' => $v['pdr_payment_name']];
            $tmp[] = ['format' => 'Number', 'data' => ncPriceFormat($v['pdr_amount'])];
            $tmp[] = ['data' => $v['pdr_payment_state']];
            $tmp[] = ['data' => $v['pdr_member_id']];
            $excel_data[] = $tmp;
        }
    ...
    public function export_cash_step1Op() {
        ...
        if($start_unixtime || $end_unixtime){
            $condition['pdc_add_time'] = ['time', [$start_unixtime, $end_unixtime]];
        }
        if(!empty($_GET['member_name'])){
            $condition['pdc_member_name'] = ['like', '%' . $_GET['member_name'] . '%'];
        }
        if(!empty($_GET['member_id'])){
            $condition['pdc_member_id'] = ['like', '%' . $_GET['member_id'] . '%'];
        }
        if(!empty($_GET['user_name'])){
            $condition['pdc_bank_user'] = ['like', '%' . $_GET['user_name'] . '%'];
        }
     ...
     * @param array $data
     */
    private function createCashExcel($data = []) {
        Language::read('export');
        import('libraries.excel');
        $excel_obj = new Excel();
        $excel_data = [];
        ...
        $excel_obj->setStyle(['id' => 's_title', 'Font' => ['FontName' => '宋体', 'Size' => '12', 'Bold' => '1']]);
        //header
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_tx_no')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_tx_member')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_tx_money')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_tx_ctime')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_tx_state')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_tx_memberid')];
        foreach((array)$data as $k => $v){
            $tmp = [];
            $tmp[] = ['data' => $v['pdc_sn']];
            $tmp[] = ['data' => $v['pdc_member_name']];
            $tmp[] = ['format' => 'Number', 'data' => ncPriceFormat($v['pdc_amount'])];
            $tmp[] = ['data' => date('Y-m-d H:i:s', $v['pdc_add_time'])];
            $tmp[] = ['data' => $v['pdc_payment_state']];
            $tmp[] = ['data' => $v['pdc_member_id']];
            $excel_data[] = $tmp;
        }
        $excel_data = $excel_obj->charset($excel_data, CHARSET);
        $excel_obj->addArray($excel_data);
     ...
     * @param array $data
     */
    private function createmxExcel($data = []) {
        Language::read('export');
        import('libraries.excel');
        $excel_obj = new Excel();
        $excel_data = [];
        ...
        $excel_obj->setStyle(['id' => 's_title', 'Font' => ['FontName' => '宋体', 'Size' => '12', 'Bold' => '1']]);
        //header
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_mx_member')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_mx_ctime')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_mx_av_money')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_mx_freeze_money')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_mx_system')];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => L('exp_mx_mshu')];
        foreach((array)$data as $k => $v){
            $tmp = [];
            $tmp[] = ['data' => $v['lg_member_name']];
            $tmp[] = ['data' => date('Y-m-d H:i:s', $v['lg_add_time'])];
            if(floatval($v['lg_av_amount']) == 0){
                $tmp[] = ['data' => ''];
            } else {
                $tmp[] = ['format' => 'Number', 'data' => ncPriceFormat($v['lg_av_amount'])];
            }
            if(floatval($v['lg_freeze_amount']) == 0){
                $tmp[] = ['data' => ''];
            } else {
                $tmp[] = ['format' => 'Number', 'data' => ncPriceFormat($v['lg_freeze_amount'])];
            }
            $tmp[] = ['data' => $v['lg_admin_name']];
            $tmp[] = ['data' => $v['lg_desc']];
            $excel_data[] = $tmp;
        }
        $excel_data = $excel_obj->charset($excel_data, CHARSET);
