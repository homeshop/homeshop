...
class settingControl extends SystemControl {
    ...
    private $links = [
        ['url' => 'act=setting&op=base', 'lang' => 'web_set'],
        ['url' => 'act=setting&op=dump', 'lang' => 'dis_dump']
    ];
    ...
    public function ajax_categoryOp() {
        ...
        $list = $model->field('gc_title,gc_keywords,gc_description')->where(['gc_id' => intval($_GET['id'])])->find();
    ...
    public function seo_updateOp() {
        ...
        if(chksubmit()){
            ...
            foreach((array)$seo as $key => $value){
                $model_seo->where(['type' => $key])->update($value);
            }
            dkcache('seo');
            showMessage(L('nc_common_save_succ'));
        } else {
    ...
    public function seo_categoryOp() {
        if(chksubmit()){
            $where = ['gc_id' => intval($_POST['category'])];
            $input = [];
