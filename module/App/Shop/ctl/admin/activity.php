    ...
    public function activity_xmlOp() {
        ...
        foreach($list as $val){
            ...
            if($val['activity_state'] == 0 || $val['activity_end_date'] < time()){
                $o .= '<a class="btn red confirm-del-on-click" href="javascript:;" data-href="index.php?act=activity&op=del&activity_id=' . $val['activity_id'] . '"><i class="fa fa-trash-o"></i>删除</a>';
            }
            
            $o .= '<span class="btn"><em><i class="fa fa-cog"></i>设置<i class="arrow"></i></em><ul>';
            
            $o .= '<li><a href="index.php?act=activity&op=edit&activity_id=' . $val['activity_id'] . '">编辑活动</a></li>';
            
            $o .= '<li><a href="index.php?act=activity&op=detail&id=' . $val['activity_id'] . '">处理申请</a></li>';
            ...
            $i['activity_sort'] = '<span class="editable" title="可编辑" style="width:50px;" data-live-inline-edit="activity_sort">' . $val['activity_sort'] . '</span>';
            
            $i['activity_title'] = '<span class="editable" title="可编辑" style="width:333px;" data-live-inline-edit="activity_title">' . $val['activity_title'] . '</span>';
...
EOB;
            ...
            $i['activity_state'] = $val['activity_state'] == 1 ? '<span class="yes"><i class="fa fa-check-circle"></i>开启</span>' : '<span class="no"><i class="fa fa-ban"></i>关闭</span>';
    ...
    public function ajaxOp() {
        if(in_array($_GET['branch'], ['activity_title', 'activity_sort'])){
            $activity = Model('activity');
            $update_array = [];
            switch($_GET['branch']) {
                /**
        ...
        } elseif(in_array($_GET['branch'], ['activity_detail_sort'])) {
    ...
    public function detail_xmlOp() {
        ...
        foreach($list as $val){
            $o = '<a class="btn green" href="' . urlShop('goods', 'index', ['goods_id' => $val['item_id']]) . '"><i class="fa fa-list-alt"></i>查看</a>';
            
            $o .= '<span class="btn"><em><i class="fa fa-cog"></i>设置<i class="arrow"></i></em><ul>';
            
            if($val['activity_detail_state'] != 1){
                $o .= '<li><a class="confirm-on-click" href="javascript:;" data-href="index.php?act=activity&op=deal&state=1&activity_detail_id=' . $val['activity_detail_id'] . '">通过</a></li>';
            }
            
            if($val['activity_detail_state'] != 2){
                $o .= '<li><a class="confirm-on-click" href="javascript:;" data-href="index.php?act=activity&op=deal&state=2&activity_detail_id=' . $val['activity_detail_id'] . '">拒绝</a></li>';
            }
            
            if($val['activity_detail_state'] != 1){
                $o .= '<li><a class="confirm-on-click" href="javascript:;" data-href="' . 'index.php?act=activity&op=del_detail&activity_detail_id=' . $val['activity_detail_id'] . '">删除</a></li>';
            }
            ...
            $i['activity_detail_sort'] = '<span class="editable" title="可编辑" style="width:50px;" data-live-inline-edit="activity_detail_sort">' . $val['activity_detail_sort'] . '</span>';
    ...
    public function dealOp() {
        ...
        //创建活动内容对象
        $activity_detail = Model('activity_detail');
        if($activity_detail->updates(['activity_detail_state' => intval($_GET['state'])], $id)){
