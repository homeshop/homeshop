...
class pointsControl extends SystemControl {
    ...
    public function checkmemberOp() {
        ...
        $obj_member = Model('member');
        $member_info = $obj_member->getMemberInfo(['member_name' => $name]);
        if(is_array($member_info) && count($member_info) > 0){
            if(strtoupper(CHARSET) == 'GBK'){
        ...
        if($_POST['query'] != ''){
            switch($_POST['qtype']) {
                ...
                case 'pl_membername_like':
                    $where['pl_membername'] = ['like', "%{$_POST['query']}%"];
                    break;
                case 'pl_adminname_like':
                    $where['pl_adminname'] = ['like', "%{$_POST['query']}%"];
                    break;
            }
        }
        $order = '';
        $param = ['pl_id', 'pl_memberid', 'pl_membername', 'pl_points', 'pl_addtime'];
