...
class goodsControl extends SystemControl {
    ...
    private $links = [
        ['url' => 'act=goods&op=goods', 'text' => '所有商品'],
        ['url' => 'act=goods&op=lockup_list', 'text' => '违规下架'],
        ['url' => 'act=goods&op=waitverify_list', 'text' => '等待审核'],
        ['url' => 'act=goods&op=goods_set', 'text' => '商品设置'],
    ];
    ...
    public function goodsOp() {
        //父类列表，只取到第二级
        $gc_list = Model('goods_class')->getGoodsClassList(['gc_parent_id' => 0]);
    ...
    public function get_xmlOp() {
        ...
        if($_GET['goods_commonid'] != ''){
            $condition['goods_commonid'] = ['like', '%' . $_GET['goods_commonid'] . '%'];
        }
        if($_GET['store_name'] != ''){
            $condition['store_name'] = ['like', '%' . $_GET['store_name'] . '%'];
        }
        if($_GET['brand_name'] != ''){
            $condition['brand_name'] = ['like', '%' . $_GET['brand_name'] . '%'];
        }
        ...
        foreach($goods_list as $value){
            ...
            $operation .= "<li><a href='" . urlShop('goods', 'index', ['goods_id' => $storage_array[ $value['goods_commonid'] ]['goods_id']]) . "' target=\"_blank\">查看商品详细</a></li>";
    ...
    private function getGoodsState() {
        return ['1' => '出售中', '0' => '仓库中', '10' => '违规下架'];
    }
    
    private function getGoodsVerify() {
        return ['1' => '通过', '0' => '未通过', '10' => '等待审核'];
    }
    ...
    public function goods_delOp() {
        ...
        Model('goods')->delGoodsAll(['goods_commonid' => $common_id]);
    ...
    public function get_goods_sku_listOp() {
        ...
        $goods_list = $model_goods->getGoodsList(['goods_commonid' => $commonid], 'goods_id,goods_spec,store_id,goods_price,goods_serial,goods_storage,goods_image');
        if(empty($goods_list)){
            showDialog('参数错误', '', '', 'CUR_DIALOG.close();');
        }
    ...
    public function export_csvOp() {
        ...
        if($_GET['id'] != ''){
            ...
            $condition['goods_commonid'] = ['in', $id_array];
        }
        if($_GET['goods_name'] != ''){
            $condition['goods_name'] = ['like', '%' . $_GET['goods_name'] . '%'];
        }
