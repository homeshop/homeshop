...
class typeControl extends SystemControl {
    ...
    public function get_xmlOp() {
        ...
        $order = '';
        $param = ['type_id', 'type_name', 'type_sort', 'class_id', 'class_name'];
    ...
    public function type_addOp() {
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                ["input" => $_POST["t_mane"], "require" => "true", "message" => $lang['type_add_name_no_null']],
                [
                    "input" => $_POST["t_sort"],
                    "require" => "true",
                    'validator' => 'Number',
        ...
        $spec_list = $model_spec->specList(['order' => 'sp_sort asc'], '', 'sp_id,sp_name,class_id,class_name');
        $s_list = [];
    ...
    public function type_editOp() {
        ...
        if(chksubmit()){
            ...
            if($_POST['brand']['form_submit'] == 'ok'){
                $model_type->delType('type_brand', ['type_id' => $type_id]);
            ...
            if($_POST['spec']['form_submit'] == 'ok'){
                $model_type->delType('type_spec', ['type_id' => $type_id]);
            ...
            if(is_array($_POST['at_value']) && !empty($_POST['at_value'])){
                ...
                foreach($attribute_array as $v){
                    ...
                    if(isset($v['form_submit']) && $v['form_submit'] == 'ok' && !in_array($v['a_id'], $del_array)){             //原属性已修改
                        ...
                        $attr_array['attr_sort'] = $v['sort'];
                        $attr_array['attr_show'] = intval($v['show']);
                        $return = $model_type->typeUpdate($attr_array, [
                            'type_id' => $type_id,
                ...
                if(!empty($_POST['a_del'])){
                    ...
                    $model_type->delType('attribute_value', ['in_attr_id' => $del_id]);  //删除属性值
                    $model_type->delType('attribute', ['in_attr_id' => $del_id]);    //删除属性
                }
            ...
            if(!empty($_POST['custom'])){
                ...
                foreach($_POST['custom'] as $key => $val){
                    ...
                    $model_custom->editTypeCustom(['custom_name' => $val['name']], ['custom_id' => $key]);
                }
                if(!empty($custom_del_array)){
                    $model_custom->delTypeCustom(['custom_id' => ['in', $custom_del_array]]);
                }
            }
            ...
            $type_array['class_id'] = $_POST['class_id'];
            $type_array['class_name'] = $_POST['class_name'];
            $return = $model_type->typeUpdate($type_array, ['type_id' => $type_id], 'type');
            if($return){
                $url = [
                    [
                        'url' => 'index.php?act=type&op=type',
        ...
        // 属性列表
        $type_info = $model_type->typeList(['type_id' => intval($_GET['t_id'])]);
        if(!type_info){
            showMessage($lang['param_error']);
        }
        ...
        unset($brand_list);
        // 类型与品牌关联列表
        $brand_related = $model_type->typeRelatedList('type_brand', ['type_id' => intval($_GET['t_id'])], 'brand_id');
        $b_related = [];
        ...
        ksort($s_list);
        // 规格关联列表
        $spec_related = $model_type->typeRelatedList('type_spec', ['type_id' => intval($_GET['t_id'])], 'sp_id');
        $sp_related = [];
        ...
        $custom_list = Model('type_custom')->getTypeCustomList(['type_id' => intval($_GET['t_id'])]);
        ...
        // 属性
        $attr_list = $model_type->typeRelatedList('attribute', [
        ...
        if($_POST['form_submit']){
            ...
            if($error != ''){
                ...
                if(!empty($attr_value) && is_array($attr_value)){
                    ...
                    // 删除属性
                    $model->table('attribute_value')->where([
                        'attr_value_id' => [
                            'in',
                            implode(',', $del_array)
                        ]
                ...
                $attr_value_list = $model->table('attribute_value')->where(['attr_id' => intval($_POST['attr_id'])])->order('attr_value_sort asc, attr_value_id asc')->select();
                ...
                $return = $model->table('attribute')->where(['attr_id' => intval($_POST['attr_id'])])->update($data);
    ...
    public function type_delOp() {
        if(empty($_GET['id'])){
            exit(json_encode(['state' => false, 'msg' => L('param_error')]));
        }
        //属性模型
        $model_type = Model('type');
        ...
        //属性列表
        $type_list = $model_type->typeList(['in_type_id' => $id]);
        
        if(is_array($type_list) && !empty($type_list)){
            
            //删除属性值表
            $attr_list = $model_type->typeRelatedList('attribute', ['in_type_id' => $id], 'attr_id');
            if(is_array($attr_list) && !empty($attr_list)){
                ...
                $return1 = $model_type->delType('attribute_value', ['in_attr_id' => $attrs_id]); //删除属性值
                $return2 = $model_type->delType('attribute', ['in_attr_id' => $attrs_id]);       //删除属性
                if(!$return1 || !$return2){
                    exit(json_encode(['state' => false, 'msg' => L('type_index_del_related_attr_fail')]));
                }
            }
            
            //删除对应品牌
            $return = $model_type->delType('type_brand', ['in_type_id' => $id]);
            if(!$return){
                exit(json_encode(['state' => false, 'msg' => L('type_index_del_related_brand_fail')]));
            }
            
            //删除对应规格
            $return = $model_type->delType('type_spec', ['in_type_id' => $id]);
            if(!$return){
                exit(json_encode(['state' => false, 'msg' => L('type_index_del_related_type_fail')]));
            }
            
            //删除类型
            $return = $model_type->delType('type', ['in_type_id' => $id]);
            if(!$return){
                exit(json_encode(['state' => false, 'msg' => L('type_index_del_fail')]));
            }
            
            // 删除自定义属性
            Model('type_custom')->delTypeCustom(['type_id' => ['in', $id]]);
            
            $this->log(L('nc_delete,type_index_type_name') . '[ID:' . $id . ']', 1);
            exit(json_encode(['state' => true, 'msg' => L('type_index_del_succ')]));
        } else {
