...
class complainControl extends SystemControl {
    ...
    private $links = [
        ['url' => 'act=complain&op=complain_new_list', 'text' => '新投诉'],
        ['url' => 'act=complain&op=complain_appeal_list', 'text' => '待申诉'],
        ['url' => 'act=complain&op=complain_talk_list', 'text' => '对话中'],
        ['url' => 'act=complain&op=complain_handle_list', 'text' => '待仲裁'],
        ['url' => 'act=complain&op=complain_finish_list', 'text' => '已关闭'],
        ['url' => 'act=complain&op=complain_subject_list', 'text' => '主题设置'],
        ['url' => 'act=complain&op=complain_setting', 'text' => '时效设置']
    ];
    ...
    */
    public function get_new_xmlOp() {
        ...
        if($_POST['query'] != '' && in_array($_POST['qtype'], ['complain_accuser', 'complain_accused'])){
            $condition[ $_POST['qtype'] ] = $_POST['query'];
        }
        $sort_fields = ['accuser_name', 'complain_id', 'accused_name', 'accuser_id', 'accused_id'];
        ...
        foreach($complain_list as $k => $complain_info){
            ...
            if(in_array($_GET['state'], [self::STATE_TALK, self::STATE_HANDLE])){
            ...
            $list['accused_name'] = "<a class='open' title='{$complain_info['accused_name']}' href='" . urlShop('show_store', 'index', ['store_id' => $complain_info['accused_id']]) . "' target='blank'>{$complain_info['accused_name']}</a>";
