...
class orderControl extends SystemControl {
    ...
    public function get_xmlOp() {
        ...
        foreach($order_list as $order_id => $order_info){
            ...
            $list['order_sn'] = $order_info['order_sn'] . str_replace([1, 2, 3], [
                    null,
                    ' [预定]',
                    '[门店自提]'
                ], $order_info['order_type']);
            $list['order_from'] = str_replace([1, 2], ['PC端', '移动端'], $order_info['order_from']);
            ...
            $list['evaluation_state'] = str_replace([0, 1, 2], ['未评价', '已评价', '未评价'], $order_info['evaluation_state']);
    ...
    private function _order_cancel($order_info) {
        ...
        if(TIMESTAMP - 86400 < $order_info['api_pay_time']){
            ...
            exit(json_encode(['state' => false, 'msg' => '该订单曾尝试使用第三方支付平台支付，须在' . $_hour . '小时以后才可取消']));
        }
        ...
        if($result['state']){
            exit(json_encode(['state' => true, 'msg' => '取消成功']));
        } else {
            exit(json_encode(['state' => false, 'msg' => '取消失败']));
        }
    }
    ...
    private function _order_receive_pay($order_info, $post) {
        ...
        //预定支付尾款时需要用到已经支付的状态
        $order_list = $model_order->getOrderList([
            'pay_sn' => $order_info['pay_sn'],
            'order_state' => ['in', [ORDER_STATE_NEW, ORDER_STATE_PAY]]
        ]);
        ...
        if($result['state']){
            ...
            QueueClient::push('addConsume', [
                'member_id' => $order_info['buyer_id'],
                'member_name' => $order_info['buyer_name'],
                'consume_amount' => $api_pay_amount,
                'consume_time' => TIMESTAMP,
                'consume_remark' => '管理员更改订单为已收款状态，订单号：' . $order_info['order_sn']
            ]);
        }
        return $result;
    }
    ...
    public function show_orderOp() {
        ...
        $model_order = Model('order');
        $order_info = $model_order->getOrderInfo(['order_id' => $order_id], ['order_goods', 'order_common', 'store']);
        
        foreach($order_info['extend_order_goods'] as $value){
            $value['image_60_url'] = cthumb($value['goods_image'], 60, $value['store_id']);
            $value['image_240_url'] = cthumb($value['goods_image'], 240, $value['store_id']);
            $value['goods_type_cn'] = orderGoodsType($value['goods_type']);
            $value['goods_url'] = urlShop('goods', 'index', ['goods_id' => $value['goods_id']]);
        ...
        //订单变更日志
        $log_list = $model_order->getOrderLogList(['order_id' => $order_info['order_id']]);
        ...
        //商家信息
        $store_info = Model('store')->getStoreInfo(['store_id' => $order_info['store_id']]);
        Tpl::output('store_info', $store_info);
        
        //商家发货信息
        if(!empty($order_info['extend_order_common']['daddress_id'])){
            $daddress_info = Model('daddress')->getAddressInfo(['address_id' => $order_info['extend_order_common']['daddress_id']]);
        ...
        if($order_info['order_state'] == ORDER_STATE_CANCEL){
            $order_info['close_info'] = $model_order->getOrderLogInfo([
                'order_id' => $order_info['order_id'],
        ...
        if($order_info['order_state'] == ORDER_STATE_PAY){
            $order_info['pay_info'] = $model_order->getOrderLogInfo([
                'order_id' => $order_info['order_id'],
    ...
    public function export_step1Op() {
        ...
        if(preg_match('/^[\d,]+$/', $_GET['order_id'])){
            ...
            $condition['order_id'] = ['in', $_GET['order_id']];
        }
        $this->_get_condition($condition);
        $sort_fields = [
            'buyer_name',
    ...
    private function createExcel($data = []) {
        ...
        //header
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '订单编号'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '订单来源'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '下单时间'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '订单金额(元)'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '订单状态'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '支付单号'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '支付方式'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '支付时间'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '充值卡支付(元)'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '预存款支付(元)'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '发货物流单号'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '退款金额(元)'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '订单完成时间'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '是否评价'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '店铺ID'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '店铺名称'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '买家ID'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '买家账号'];
        //data
        foreach((array)$data as $k => $order_info){
            ...
            $tmp = [];
            $tmp[] = ['data' => $list['order_sn']];
            $tmp[] = ['data' => $list['order_from']];
            $tmp[] = ['data' => $list['add_time']];
            $tmp[] = ['data' => $list['order_amount']];
            $tmp[] = ['data' => $list['order_state']];
            $tmp[] = ['data' => $list['pay_sn']];
            $tmp[] = ['data' => $list['payment_code']];
            $tmp[] = ['data' => $list['payment_time']];
            $tmp[] = ['data' => $list['rcb_amount']];
            $tmp[] = ['data' => $list['pd_amount']];
            $tmp[] = ['data' => $list['shipping_code']];
            $tmp[] = ['data' => $list['refund_amount']];
            $tmp[] = ['data' => $list['finnshed_time']];
            $tmp[] = ['data' => $list['evaluation_state']];
            $tmp[] = ['data' => $list['store_id']];
            $tmp[] = ['data' => $list['store_name']];
            $tmp[] = ['data' => $list['buyer_id']];
            $tmp[] = ['data' => $list['buyer_name']];
            $excel_data[] = $tmp;
        }
    ...
    private function _get_condition(& $condition) {
        if($_REQUEST['query'] != '' && in_array($_REQUEST['qtype'], [
                'order_sn',
                'store_name',
                'buyer_name',
                'pay_sn'
        ...
        if($_GET['keyword'] != '' && in_array($_GET['keyword_type'], [
                'order_sn',
                'store_name',
                'buyer_name',
                'pay_sn',
            ...
            if($_GET['jq_query']){
                ...
                $condition[ $_GET['keyword_type'] ] = ['like', "%{$_GET['keyword']}%"];
            }
        }
        if(!in_array($_GET['qtype_time'], ['add_time', 'payment_time', 'finnshed_time'])){
        ...
        if($_GET['qtype_time'] && ($start_unixtime || $end_unixtime)){
            $condition[ $_GET['qtype_time'] ] = ['time', [$start_unixtime, $end_unixtime]];
        }
        if($_GET['payment_code']){
            if($_GET['payment_code'] == 'wxpay'){
                $condition['payment_code'] = ['in', ['wxpay', 'wx_saoma', 'wx_jsapi']];
            } else {
        ...
        if(!in_array($_GET['query_amount'], ['order_amount', 'shipping_fee', 'refund_amount'])){
            $_GET['query_amount'] = null;
        }
        if(floatval($_GET['query_start_amount']) > 0 && floatval($_GET['query_end_amount']) > 0 && $_GET['query_amount']){
            $condition[ $_GET['query_amount'] ] = [
                'between',
                floatval($_GET['query_start_amount']) . ',' . floatval($_GET['query_end_amount'])
            ];
        }
        if(in_array($_GET['order_from'], ['1', '2'])){
