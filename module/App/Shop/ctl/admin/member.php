...
class memberControl extends SystemControl {
    ...
    public function member_editOp() {
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                $result = $model_member->editMember(['member_id' => intval($_POST['member_id'])], $update_array);
                if($result){
                    $url = [
                        [
    ...
    public function member_addOp() {
        ...
        if(chksubmit()){
            ...
            $obj_validate = new Validate();
            $obj_validate->validateparam = [
                ["input" => $_POST["member_name"], "require" => "true", "message" => $lang['member_add_name_null']],
                ["input" => $_POST["member_passwd"], "require" => "true", "message" => '密码不能为空'],
                [
                    "input" => $_POST["member_email"],
                    "require" => "true",
                    'validator' => 'Email',
    ...
    public function ajaxOp() {
        switch($_GET['branch']) {
            ...
            case 'check_user_name':
                ...
                $condition['member_id'] = ['neq', intval($_GET['member_id'])];
    ...
    public function export_csvOp() {
        ...
        if($_GET['id'] != ''){
            ...
            $condition['member_id'] = ['in', $id_array];
        }
        if($_GET['query'] != ''){
