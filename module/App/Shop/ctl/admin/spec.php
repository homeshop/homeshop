...
class specControl extends SystemControl {
    ...
    public function get_xmlOp() {
        ...
        $order = '';
        $param = ['sp_id', 'sp_name', 'sp_sort', 'class_id', 'class_name'];
    ...
    public function spec_addOp() {
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                ["input" => $_POST["s_name"], "require" => "true", "message" => $lang['spec_add_name_no_null']]
            ];
    ...
    public function spec_editOp() {
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                $return = $model_spec->specUpdate($param, ['sp_id' => intval($_POST['s_id'])], 'spec');
                if($return){
                    $url = [
                        [
                            'url' => 'index.php?act=spec&op=spec',
    ...
    public function spec_delOp() {
        ...
        //规格列表
        $spec_list = $model_spec->specList(['in_sp_id' => $id]);
        
        if(is_array($spec_list) && !empty($spec_list)){
            // 删除类型与规格关联表
            $return = $model_spec->delSpec('type_spec', ['in_sp_id' => $id]);
            if(!$return){
                exit(json_encode(['state' => false, 'msg' => L('nc_common_save_fail')]));
            }
            
            //删除规格值表
            $return = $model_spec->delSpec('spec_value', ['in_sp_id' => $id]);
            if(!$return){
                exit(json_encode(['state' => false, 'msg' => L('nc_common_save_fail')]));
            }
            
            //删除规格表
            $return = $model_spec->delSpec('spec', ['in_sp_id' => $id]);
            if(!$return){
                exit(json_encode(['state' => false, 'msg' => L('nc_common_save_fail')]));
            }
