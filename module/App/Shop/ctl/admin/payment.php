...
class paymentControl extends SystemControl {
    ...
    public function indexOp() {
        ...
        $payment_list = $model_payment->getPaymentList(['payment_code' => ['neq', 'predeposit']]);
    ...
    public function editOp() {
        ...
        if(chksubmit()){
            ...
            $model_payment->editPayment($data, ['payment_id' => $payment_id]);
            showMessage(Language::get('nc_common_save_succ'), 'index.php?act=payment&op=index');
        }
        
        $payment_id = intval($_GET["payment_id"]);
        $payment = $model_payment->getPaymentInfo(['payment_id' => $payment_id]);
