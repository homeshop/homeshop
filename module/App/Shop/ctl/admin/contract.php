...
class contractControl extends SystemControl {
    ...
    public function citemlist_xmlOp() {
        $model_contract = Model('contract');
        $list = $model_contract->contractItemList([], 'cti_state desc,cti_sort asc,cti_name asc');
        $data = [];
        $data['now_page'] = 1;
        $data['total_num'] = count($list);
        foreach($list as $val){
            $o = '<a class="btn blue" href="' . urlAdminShop('contract', 'citemedit', [
                    'itemid' => $val['cti_id'],
    ...
    public function citemeditOp() {
        ...
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                [
                    "input" => $_POST['item_name'],
                    "require" => "true",
                    "validator" => "Length",
                    "min" => "1",
                    "max" => "50",
                    "message" => '项目名称不能为空且不能大于50个字符'
                ],
                [
                    "input" => $_POST['item_cost'],
                    "require" => "true",
                    "validator" => "double",
                    "min" => "1",
                    "message" => '保证金应为大于1的数字'
                ],
                ["input" => $_POST['item_desc'], "require" => "true", "message" => '请添加项目描述'],
                [
                    "input" => $_POST['item_sort'],
                    "require" => "true",
                    "validator" => "Number",
                    "min" => "1",
                    "message" => '排序应为大于1的正整数'
            ...
            $rs = $model_contract->editContractItem(['cti_id' => $itemid_id], $update_arr);
            if($rs){
                //记录日志
    ...
    public function contractlist_xmlOp() {
        ...
        foreach($c_list as $val){
            $o = '<a class="btn blue" href="' . urlAdminShop('contract', 'contractedit', [
                    'ct_id' => $val['ct_id'],
                ]) . '"><i class="fa fa-pencil-square-o"></i>编辑</a>';
            
            $o .= "<span class='btn'><em><i class='fa fa-cog'></i>设置 <i class='arrow'></i></em><ul>";
            $o .= "<li><a href='" . urlAdminShop('contract', 'contractinfo', ['ct_id' => $val['ct_id']]) . "'>查看详情</a></li>";
            $o .= "<li><a href='" . urlAdminShop('contract', 'costedit', ['ct_id' => $val['ct_id']]) . "'>编辑保证金</a></li>";
            $o .= "<li><a href='" . urlAdminShop('contract', 'costlog', ['ct_id' => $val['ct_id']]) . "'>保证金日志</a></li>";
            $o .= "</ul>";
    ...
    public function contracteditOp() {
        ...
        if(chksubmit()){
            ...
            try {
                ...
                $result = $this->saveContractLog($item_info, [
                    'store_id' => $c_info['ct_storeid'],
                    'store_name' => $c_info['ct_storename']
                ], $log_msg);
                if(!$result){
                    throw new Exception(L('nc_common_save_fail'));
                }
                ...
                //如果修改了关闭状态，则更新店铺商品状态
                if($c_info['ct_closestate_key'] != $_POST['c_state']){
                    QueueClient::push('updateStoreGoodsContract', [
                        'store_id' => $c_info['ct_storeid'],
                        'item_id' => $c_info['ct_itemid']
                    ]);
                }
                showDialog(L('nc_common_save_succ'), 'index.php?act=contract&op=contractlist', 'succ');
            } catch(Exception $e) {
    ...
    public function applylist_xmlOp() {
        ...
        if($_REQUEST['advanced']){
            ...
            if(trim($sdate) && trim($edate)){
                ...
                $where['cta_addtime'] = ['between', "$sdate,$edate"];
            } elseif(trim($sdate)) {
                $sdate = strtotime($sdate);
                $where['cta_addtime'] = ['egt', $sdate];
            } elseif(trim($edate)) {
                $edate = strtotime($edate);
                $where['cta_addtime'] = ['elt', $edate];
            }
        } else {
            if(strlen($q = trim($_REQUEST['query']))){
                switch($_REQUEST['qtype']) {
        ...
        foreach($apply_list as $val){
            $o = '';
            if(in_array($val['cta_auditstate_key'], ['notaudit', 'costpay'])){
                $o = '<a class="btn blue" href="' . urlAdminShop('contract', 'applyedit', [
                        'aid' => $val['cta_id'],
                    ]) . '"><i class="fa fa-pencil-square-o"></i>编辑</a>';
            } else {
                $o .= '<a class="btn green" href="' . urlAdminShop('contract', 'applyinfo', [
                        'aid' => $val['cta_id'],
    ...
    public function applyeditOp() {
        ...
        $where = [];
        $where['cta_id'] = $aid;
        $where['cta_auditstate'] = [
            'in',
            [$this->join_auditstate_arr['notaudit']['sign'], $this->join_auditstate_arr['costpay']['sign']]
        ];
        ...
        foreach($this->join_auditstate_arr as $k => $v){
            if($apply_info['cta_auditstate_key'] == 'notaudit' && in_array($k, [
                    'notaudit',
                    'auditpass',
                    'auditfailure'
                ])
            ...
            if($apply_info['cta_auditstate_key'] == 'costpay' && in_array($k, ['costpay', 'costpass', 'costfailure'])){
                $curr_applystatearr[ $k ] = $v;
            }
        }
        if(chksubmit()){
            ...
            try {
                ...
                $result = $model_contract->editContractApply(['cta_id' => $aid], $update_arr);
                if(!$result){
                    throw new Exception(L('nc_common_save_fail'));
                }
                //编辑店铺保障服务信息
                ...
                if($_POST['apply_state'] == 'costpass'){//如果保证金通过审核，则状态修改为已加入
                    ...
                    $update_arr['ct_cost'] = ['exp', 'ct_cost+' . $apply_info['cta_cost']];
                }
                ...
                $result = $this->saveContractLog($item_info, [
                    'store_id' => $apply_info['cta_storeid'],
                    'store_name' => $apply_info['cta_storename']
                ], $log_msg);
                if(!$result){
                    throw new Exception(L('nc_common_save_fail'));
                }
    ...
    public function costeditOp() {
        ...
        if(chksubmit()){
            $obj_validate = new Validate();
            $obj_validate->validateparam = [
                ["input" => $_POST['operatetype'], "require" => "true", "message" => '请选择增减类型'],
                [
                    "input" => $_POST['price'],
                    "require" => "true",
                    "validator" => "Number",
                    "min" => "0",
                    "message" => '金额不能为空且为大于0数字'
                ],
                ["input" => $_POST['clog_desc'], "require" => "true", "message" => '原因描述必填且小于200个字符']
            ];
            $error = $obj_validate->validate();
            if($error){
                showDialog($error, '', 'error');
            ...
            try {
                ...
                //编辑店铺保障服务信息
                $update_arr = [];
                $update_arr['ct_cost'] = ['exp', 'ct_cost+' . $price];
                $where = [];
                $where['ct_id'] = $ct_id;
                $result = $model_contract->editContract($where, $update_arr);
                if(!$result){
    ...
    public function quitlist_xmlOp() {
        ...
        if($_REQUEST['advanced']){
            ...
            if(trim($sdate) && trim($edate)){
                $sdate = strtotime($sdate);
                $edate = strtotime($edate);
                $where['ctq_addtime'] = ['between', "$sdate,$edate"];
            } elseif(trim($sdate)) {
                $sdate = strtotime($sdate);
                $where['ctq_addtime'] = ['egt', $sdate];
            } elseif(trim($edate)) {
                $edate = strtotime($edate);
                $where['ctq_addtime'] = ['elt', $edate];
            }
        } else {
            if(strlen($q = trim($_REQUEST['query']))){
                switch($_REQUEST['qtype']) {
        ...
        foreach($quit_list as $val){
            ...
            if($val['ctq_auditstate_key'] == 'notaudit'){
                $o = '<a class="btn blue" href="' . urlAdminShop('contract', 'quitedit', [
                        'qid' => $val['ctq_id'],
                    ]) . '"><i class="fa fa-pencil-square-o"></i>编辑</a>';
            } else {
                $o .= '<a class="btn green" href="' . urlAdminShop('contract', 'quitinfo', [
                        'qid' => $val['ctq_id'],
                    ]) . '"><i class="fa fa-list-alt"></i>查看</a>';
            }
    ...
    public function quiteditOp() {
        ...
        if(chksubmit()){
            ...
            try {
                ...
                $result = $model_contract->editQuitApply(['ctq_id' => $qid], $update_arr);
                if(!$result){
                    throw new Exception(L('nc_common_save_fail'));
                }
                //编辑店铺保障服务信息
                ...
                //新增保障项目日志
                $log_itemarr = ['cti_id' => $quit_info['ctq_itemid'], 'cti_name' => $quit_info['ctq_itemname']];
                $log_storearr = ['store_id' => $quit_info['ctq_storeid'], 'store_name' => $quit_info['ctq_storename']];
                $logmsg = '';
                ...
                if($_POST['quit_state'] == 'auditpass'){//如果通过审核
                    QueueClient::push('updateStoreGoodsContract', [
                        'store_id' => $quit_info['ctq_storeid'],
                        'item_id' => $quit_info['ctq_itemid']
                    ]);
                }
    ...
    public function contractgoodsOp() {
        ...
        //更新商品信息
        QueueClient::push('updateStoreGoodsContract', ['store_id' => $store_id, 'item_id' => $item_id]);
    ...
    private function show_menu($menu_key = '') {
        $menu_array = [
            1 => [
                'menu_key' => 'applylist',
                'menu_name' => '服务加入申请',
                'menu_url' => 'index.php?act=contract&op=applylist'
            ],
            2 => [
                'menu_key' => 'quitlist',
                'menu_name' => '服务退出申请',
                'menu_url' => 'index.php?act=contract&op=quitlist'
            ],
            3 => [
                'menu_key' => 'contractlist',
                'menu_name' => '店铺保障服务',
                'menu_url' => 'index.php?act=contract&op=contractlist'
            ],
            4 => [
                'menu_key' => 'citemlist',
                'menu_name' => '保障服务管理',
                'menu_url' => 'index.php?act=contract&op=citemlist'
            ]
        ];
