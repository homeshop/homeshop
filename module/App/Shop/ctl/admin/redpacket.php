...
class redpacketControl extends SystemControl {
    ...
    public function rptaddOp() {
        if(chksubmit()){
            ...
            $obj_validate->validateparam = [
                [
                    "input" => $_POST['rpt_title'],
                    "require" => "true",
                    "validator" => "Length",
                    "min" => "1",
                    "max" => "50",
                    "message" => '模版名称不能为空且小于50个字符'
                ],
                ["input" => $_POST['rpt_gettype'], "require" => "true", "message" => '请选择领取方式'],
                ["input" => $_POST['rpt_sdate'], "require" => "true", "message" => '请选择有效期开始时间'],
                ["input" => $_POST['rpt_edate'], "require" => "true", "message" => '请选择有效期结束时间'],
                [
                    "input" => $_POST['rpt_price'],
                    "require" => "true",
                    "validator" => "Number",
                    "min" => "1",
                    "message" => '面额不能为空且为大于1的整数'
                ],
                [
                    "input" => $_POST['rpt_total'],
                    "require" => "true",
                    "validator" => "Number",
                    "min" => "1",
                    "message" => '可发放数量不能为空且为大于1的整数'
                ],
                [
                    "input" => $_POST['rpt_orderlimit'],
                    "require" => "true",
                    "validator" => "Double",
                    "min" => "0",
                    "message" => '模版使用消费限额不能为空且必须是数字'
                ],
                [
                    "input" => $_POST['rpt_desc'],
                    "require" => "true",
                    "validator" => "Length",
                    "min" => "1",
                    "max" => "200",
                    "message" => '模版描述不能为空且小于200个字符'
                ]
            ];
    ...
    public function rptlist_xmlOp() {
        ...
        if($_REQUEST['advanced']){
            ...
            if(trim($_GET['sdate']) && trim($_GET['edate'])){
                ...
                $where['rpacket_t_updatetime'] = ['between', "$sdate,$edate"];
            } elseif(trim($_GET['sdate'])) {
                $sdate = strtotime($_GET['sdate']);
                $where['rpacket_t_updatetime'] = ['egt', $sdate];
            } elseif(trim($_GET['edate'])) {
                $edate = strtotime($_GET['edate']);
                $where['rpacket_t_updatetime'] = ['elt', $edate];
            }
            ...
            if($pdates){
                $where['pdates'] = ['exp', implode(' and ', $pdates)];
            }
        } else {
        ...
        foreach($list as $val){
            ...
            $o .= "<li><a href='" . urlAdminShop('redpacket', 'rptedit', ['tid' => $val['rpacket_t_id']]) . "'>编辑信息</a></li>";
            $o .= "<li><a href='" . urlAdminShop('redpacket', 'rptinfo', ['tid' => $val['rpacket_t_id']]) . "'>查看详细</a></li>";
            $o .= "</ul>";
            ...
            $i['rpacket_t_recommendtext'] = $val['rpacket_t_recommend'] == '1' ? '<span class="yes"><i class="fa fa-check-circle"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
    ...
    public function rpteditOp() {
        ...
        if(chksubmit()){
            ...
            $rs = Model('redpacket')->editRptTemplate(['rpacket_t_id' => $t_id], $update_arr);
            if($rs){
    ...
    */
    ...
    public function rplist_xmlOp() {
        $t_id = intval($_GET['tid']);
        if($t_id <= 0){
            echo Tpl::flexigridXML([]);
            exit;
        }
        $model_redpacket = Model('redpacket');
        $list = $model_redpacket->getRedpacketList(['rpacket_t_id' => $t_id], '*', 0, $_REQUEST['rp'], 'rpacket_id desc');
        $data = [];
        $data['now_page'] = $model_redpacket->shownowpage();
        $data['total_num'] = $model_redpacket->gettotalnum();
        foreach($list as $val){
    ...
    public function export_step1Op() {
        ...
        $t_id = intval($_GET['tid']);
        //查询红包模板
        $rpt_info = $model_redpacket->getRptTemplateInfo(['rpacket_t_id' => $t_id]);
        if(!$rpt_info){
            showDialog(L('param_error'), 'index.php?act=redpacket&op=rptlist');
        }
        $where = [];
     ...
     * @param array $data
     */
    private function createExcel($data = [], $rpt_info) {
        ...
        //header
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '红包编码'];
        if($rpt_info['rpacket_t_gettype_key'] == 'pwd'){
            $excel_data[0][] = ['styleid' => 's_title', 'data' => '卡密'];
        }
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '领取方式'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '有效期'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '面额'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '每人限领'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '消费限额'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '会员级别'];
        $excel_data[0][] = ['styleid' => 's_title', 'data' => '状态'];
        ...
        foreach((array)$data as $k => $v){
            ...
            $tmp = [];
            $tmp[] = ['data' => $list['rpacket_code']];
            if($rpt_info['rpacket_t_gettype_key'] == 'pwd'){
                $tmp[] = ['data' => $list['rpacket_pwd']];
            }
            $tmp[] = ['data' => $list['rpacket_t_gettype_text']];
            $tmp[] = ['data' => $list['rpacket_expiry_date']];
            $tmp[] = ['data' => $list['rpacket_price']];
            $tmp[] = ['data' => $list['rpacket_t_eachlimit']];
            $tmp[] = ['data' => $list['rpacket_limit']];
            $tmp[] = ['data' => $list['rpacket_t_mgradelimittext']];
            $tmp[] = ['data' => $list['rpacket_t_state_text']];
            $tmp[] = ['data' => $list['rpacket_state_text']];
            $tmp[] = ['data' => $list['rpacket_owner_name']];
            $tmp[] = ['data' => $list['rpacket_active_date']];
            $excel_data[] = $tmp;
        }
