...
class billControl extends SystemControl {
    ...
    private $links = [
        ['url' => 'act=bill&op=index', 'lang' => 'nc_manage'],
    ];
    ...
    public function show_billOp() {
        ...
        $bill_info = $model_bill->getOrderBillInfo(['ob_id' => $ob_id]);
        if(!$bill_info){
            showMessage('参数错误', '', 'html', 'error');
        }
        ...
        if($if_start_date || $if_end_date){
            $order_condition['finnshed_time'] = ['between', "{$start_unixtime},{$end_unixtime}"];
        } else {
            $order_condition['finnshed_time'] = [
                'between',
                "{$bill_info['ob_start_date']},{$bill_info['ob_end_date']}"
            ];
        }
    ...
    public function get_bill_info_xmlOp() {
        ...
        if($_GET['query_type'] == 'refund'){
            ...
            if($_POST['query'] != '' && in_array($_POST['qtype'], ['refund_sn', 'order_sn', 'buyer_name'])){
                $refund_condition[ $_POST['qtype'] ] = ['like', "%{$_POST['query']}%"];
            }
            $sort_fields = ['refund_amount', 'commis_amount', 'refund_type', 'admin_time', 'buyer_id', 'store_id'];
            ...
            foreach($refund_list as $refund_info){
                ...
                $list['refund_type'] = str_replace([1, 2], ['退款 ', '退货'], $refund_info['refund_type']);
            ...
            if($_POST['query'] != '' && in_array($_POST['qtype'], ['order_sn'])){
                $order_info = $model_order->getOrderInfo(['order_sn' => $_POST['query']]);
                if($order_info){
            ...
            $order_list = $model_order->getOrderList(['order_id' => ['in', $order_id_array]]);
            $data = [];
            ...
            //订单列表
            $model_order = Model('order');
            if($_POST['query'] != '' && in_array($_POST['qtype'], ['order_sn', 'buyer_name'])){
                $order_condition[ $_POST['qtype'] ] = ['like', "%{$_POST['query']}%"];
            }
            if($_GET['order_sn'] != ''){
                $order_condition['order_sn'] = ['like', "%{$_GET['order_sn']}%"];
            }
            if($_GET['buyer_name'] != ''){
                if($_GET['jq_query']){
                    $order_condition['buyer_name'] = $_GET['buyer_name'];
                } else {
                    $order_condition['buyer_name'] = ['like', "%{$_GET['buyer_name']}%"];
                }
            }
    ...
    public function bill_checkOp() {
        ...
        $update = $model_bill->editOrderBill(['ob_state' => BILL_STATE_SYSTEM_CHECK], $condition);
        if($update){
    ...
    public function bill_payOp() {
        ...
        if(chksubmit()){
            ...
            if($update){
                ...
                $cost_condition['cost_time'] = ['between', "{$bill_info['ob_start_date']},{$bill_info['ob_end_date']}"];
                $model_store_cost->editStoreCost(['cost_state' => 1], $cost_condition);
    ...
    public function export_orderOp() {
        ...
        $end_unixtime = $if_end_date ? $end_unixtime + 86400 - 1 : null;
        if($if_start_date || $if_end_date){
            $condition['finnshed_time'] = ['between', "{$start_unixtime},{$end_unixtime}"];
        } else {
            $condition['finnshed_time'] = ['between', "{$bill_info['ob_start_date']},{$bill_info['ob_end_date']}"];
        }
        ...
        if($_REQUEST['query'] != '' && in_array($_REQUEST['qtype'], ['order_sn', 'buyer_name'])){
            $condition[ $_REQUEST['qtype'] ] = ['like', "%{$_REQUEST['query']}%"];
        }
        if($_GET['order_sn'] != ''){
            $condition['order_sn'] = ['like', "%{$_GET['order_sn']}%"];
        }
        if($_GET['buyer_name'] != ''){
            if($_GET['jq_query']){
                $condition['buyer_name'] = $_GET['buyer_name'];
            } else {
                $condition['buyer_name'] = ['like', "%{$_GET['buyer_name']}%"];
            }
        }
        ...
        $data = $model_order->getOrderList($condition, '', '*', 'order_id desc', $limit, ['order_goods']);
        ...
        $export_data = [];
        $export_data[0] = ['订单编号', '订单金额', '运费', '佣金', '下单日期', '成交日期', '商家', '商家编号', '买家', '买家编号', '商品'];
        $order_totals = 0;
        $shipping_totals = 0;
        $commis_totals = 0;
        $k = 0;
    ...
    public function export_bookOp() {
        ...
        if(preg_match('/^[\d,]+$/', $_GET['order_id'])){
            $_GET['order_id'] = explode(',', trim($_GET['order_id'], ','));
            $condition['book_order_id'] = ['in', $_GET['order_id']];
        }
        $model_order_book = Model('order_book');
        
        $condition['book_store_id'] = $bill_info['ob_store_id'];
        $condition['book_cancel_time'] = ['between', "{$bill_info['ob_start_date']},{$bill_info['ob_end_date']}"];
        unset($order_condition['finnshed_time']);
        ...
        $data = $model_order->getOrderList(['order_id' => ['in', $order_id_array]], '', '*', 'order_id desc');
        
        $export_data = [];
        $export_data[0] = ['订单编号', '订单金额', '运费', '未退定金', '下单日期', '取消日期', '商家', '商家编号', '买家', '买家编号'];
        $deposit_amount = 0;
        $k = 0;
        foreach($data as $v){
            //该订单算佣金
    ...
    public function export_refund_orderOp() {
        ...
        $condition['store_id'] = $bill_info['ob_store_id'];
        $condition['goods_id'] = ['gt', 0];
        if(preg_match('/^[\d,]+$/', $_GET['refund_id'])){
            $_GET['refund_id'] = explode(',', trim($_GET['refund_id'], ','));
            $condition['refund_id'] = ['in', $_GET['refund_id']];
        }
        if($_GET['query'] != '' && in_array($_GET['qtype'], ['refund_sn', 'order_sn', 'buyer_name'])){
            $condition[ $_GET['qtype'] ] = ['like', "%{$_GET['query']}%"];
        }
        $if_start_date = preg_match('/^20\d{2}-\d{2}-\d{2}$/', $_GET['query_start_date']);
        $if_end_date = preg_match('/^20\d{2}-\d{2}-\d{2}$/', $_GET['query_end_date']);
        $start_unixtime = $if_start_date ? strtotime($_GET['query_start_date']) : null;
        ...
        $end_unixtime = $if_end_date ? $end_unixtime + 86400 - 1 : null;
        if($if_start_date || $if_end_date){
            $condition['admin_time'] = ['between', "{$start_unixtime},{$end_unixtime}"];
        } else {
            $condition['admin_time'] = ['between', "{$bill_info['ob_start_date']},{$bill_info['ob_end_date']}"];
        }
        $sort_fields = ['refund_amount', 'commis_amount', 'refund_type', 'admin_time', 'buyer_id', 'store_id'];
        if(in_array($_GET['sortorder'], ['asc', 'desc']) && in_array($_GET['sortname'], $sort_fields)){
        ...
        $export_data = [];
        $export_data[0] = ['退单编号', '订单编号', '退单金额', '退单佣金', '类型', '退款日期', '商家', '商家编号', '买家', '买家编号'];
        $refund_amount = 0;
        $commis_totals = 0;
        $k = 0;
        foreach($data as $v){
            ...
            $export_data[ $k + 1 ][] = str_replace([1, 2], ['退款', '退货'], $v['refund_type']);
    ...
    public function get_bill_xmlOp() {
        ...
        foreach($bill_list as $bill_info){
            $list = [];
            if(in_array($bill_info['ob_state'], [2, 3])){
    ...
    private function _get_bill_condition($condition) {
        if($_GET['query_year'] && $_GET['query_month']){
            ...
            $condition['os_month'] = ['between', $_GET['query_year'] . '01,' . $_GET['query_year'] . '12'];
        }
        if(!empty($_GET['os_month'])){
            $condition['os_month'] = intval($_GET['os_month']);
        }
        if($_REQUEST['query'] != '' && in_array($_REQUEST['qtype'], ['ob_no', 'ob_id', 'ob_store_name'])){
        ...
        if($_GET['ob_store_name'] != ''){
            if($_GET['jq_query']){
                ...
                $condition['ob_store_name'] = ['like', "%{$_GET['ob_store_name']}%"];
            }
        }
        $sort_fields = [
            'ob_id',
        ...
        if(in_array($_REQUEST['sortorder'], ['asc', 'desc']) && in_array($_REQUEST['sortname'], $sort_fields)){
