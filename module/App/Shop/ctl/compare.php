...
class compareControl extends BaseHomeControl {
    ...
    public function indexOp() {
        ...
        if($gid_arr){
            //查询商品信息
            $goods_list = $gcommon_id_arr = [];
            $goods_listtmp = $model->getGoodsList([
                'goods_id' => ['in', $gid_arr],
                'goods_state' => 1,
                'goods_verify' => 1
            ]);
            if(!empty($goods_listtmp)){
                foreach($goods_listtmp as $k => $v){
                    $v['goods_spec_arr'] = ($t = $v['goods_spec']) ? unserialize($t) : [];
                    $goods_list[$v['goods_id']] = $v;
                    $gcommon_id_arr[] = $v['goods_commonid'];
                }
            }
            if(!empty($gcommon_id_arr)){
                $gcommon_listtmp = $model->getGoodsCommonList([
                    'goods_commonid' => [
                        'in',
                        $gcommon_id_arr
                    ]
                ...
                if(!empty($gcommon_listtmp)){
                    foreach($gcommon_listtmp as $k => $v){
                        $v['goods_attr_arr'] = ($t = trim($v['goods_attr'])) ? unserialize($t) : [];
                        $v['spec_name_arr'] = ($t = trim($v['spec_name'])) ? unserialize($t) : [];
                        $v['spec_value_arr'] = ($t = trim($v['spec_value'])) ? unserialize($t) : [];
            ...
            for($j = 0; $j < self::MAXNUM; $j++){
                $compare_list[$i][] = $compare_listtmp[$j] ? [
            ...
            for($j = 0; $j < self::MAXNUM; $j++){
                $compare_list[$i][] = $compare_listtmp[$j] ? ($compare_listtmp[$j]['brand_id'] ? [
    ...
    public function addcompareOp() {
        $gid = intval($_GET['id']);
        if($gid <= 0){
            exit(json_encode(['done' => false, 'msg' => '参数错误']));
        }
        if(cookie('comparegoods')){//如果cookie已经存在
            ...
            if($comparegoods_str){
                ...
                if(count($comparegoods_arr) >= self::MAXNUM){
                    exit(json_encode(['done' => false, 'msg' => '对比栏已满，您可以删除不需要的栏内商品再继续添加哦！']));
                }
        ...
        exit(json_encode(['done' => true, 'msg' => '']));
    }
    ...
    public function delcompareOp() {
        ...
        exit(json_encode(['done' => true, 'gid_str' => $gid_str]));
    }
}
