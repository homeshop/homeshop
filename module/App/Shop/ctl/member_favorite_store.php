...
class member_favorite_storeControl extends BaseMemberControl {
    ...
    public function favoritestoreOp() {
        ...
        if(!empty($favorites_info)){
            echo json_encode([
                'done' => false,
                'msg' => Language::get('favorite_already_favorite_store', 'UTF-8')
            ]);
            die;
        }
        //判断店铺是否为当前会员所有
        if($fav_id == $_SESSION['store_id']){
            echo json_encode(['done' => false, 'msg' => Language::get('favorite_no_my_store', 'UTF-8')]);
            die;
        }
        //添加收藏
        $insert_arr = [];
    ...
    public function fslistOp() {
        $favorites_model = Model('favorites');
        $favorites_list = $favorites_model->getStoreFavoritesList(['member_id' => $_SESSION['member_id']], '*', 10);
    ...
    private function _getStoreGoods($store_id) {
        ...
        $condition = [];
        $condition['store_id'] = $store_id;
        $condition['goods_promotion_type'] = ['neq', 0];
        ...
        $condition = [];
        $condition['store_id'] = $store_id;
        $condition['goods_addtime'] = ['between', [$weak_start, $weak_end]];
        ...
        return [
            'new' => $new_goods_list,
            'promotion' => $promotion_goods_list,
            'hot' => $hot_goods_list,
            'sign' => $sign
        ];
    }
    ...
    public function moreOp() {
        ...
        if(empty($_GET['curpage'])){
            $store_info = [];
            $favorites_model = Model('favorites');
            $favorites_list = $favorites_model->getStoreFavoritesList(['member_id' => $_SESSION['member_id']], '*');
            if(!empty($favorites_list) && is_array($favorites_list)){
                $favorites_id = [];//收藏的店铺编号
                foreach($favorites_list as $key => $favorites){
                    $fav_id = $favorites['fav_id'];
        ...
        // 计算新品、促销、热销、商品数量
        $model_goods->cls();
        $new_count = $salenum_count = $model_goods->getGoodsOnlineCount(['store_id' => $store_id]);
        $promotion_count = $model_goods->getGoodsOnlineCount([
            'store_id' => $store_id,
            'goods_promotion_type' => ['neq', 0]
        ]);
        Tpl::output('count', ['new' => $new_count, 'promotion' => $promotion_count, 'hot' => $salenum_count]);
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        $menu_array = [
            [
                'menu_key' => 'fav_store',
                'menu_name' => Language::get('nc_member_path_collect_store'),
                'menu_url' => 'index.php?act=member_favorite_store&op=fslist'
            ]
        ];
        if($menu_key == 'more'){
            $menu_array[] = ['menu_key' => 'more', 'menu_name' => '收藏店铺详细'];
        }
