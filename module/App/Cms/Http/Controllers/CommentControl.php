...
class  CommentControl extends CMSHomeControl {
    ...
    public function comment_saveOp() {
        ...
        if(!empty($_SESSION['member_id'])){
            ...
            if(!empty($_POST['comment_id'])){
                $comment_detail = $model_comment->getOne(['comment_id' => $_POST['comment_id']]);
            ...
            if($result){
                ...
                $update = [];
                $update[ $count_field ] = ['exp', $count_field . '+1'];
                $condition = [];
    ...
    public function comment_listOp() {
        ...
        if($comment_object_id > 0 && $comment_type > 0){
            ...
            if(!empty($comment_quote_id)){
                $comment_quote_list = $model_cms_comment->getListWithUserInfo([
                    'comment_id' => [
                        'in',
                        $comment_quote_id
                    ]
    ...
    public function comment_dropOp() {
        ...
        if($comment_id > 0){
            $model_comment = Model('cms_comment');
            $comment_info = $model_comment->getOne(['comment_id' => $comment_id]);
            if($comment_info['comment_member_id'] == $_SESSION['member_id']){
                $result = $model_comment->drop(['comment_id' => $comment_id]);
                if($result){
    ...
    public function comment_upOp() {
        ...
        if($comment_id > 0){
            ...
            if(!$is_exist){
                ...
                $model_comment = Model('cms_comment');
                $model_comment->modify(['comment_up' => ['exp', 'comment_up+1']], ['comment_id' => $comment_id]);
            } else {
                $data['result'] = 'false';
                $data['message'] = '顶过了';
            }
