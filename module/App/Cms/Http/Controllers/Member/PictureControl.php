<?php namespace App\Cms\Http\Controllers\Member;

use App\Cms\Http\Controllers\CMSMemberControl;
use App\Cms\Http\Controllers\Language;
use App\Cms\Http\Controllers\Tpl;

/**
 * cms用户中心画报
 */
class  PictureControl extends CMSMemberControl {
    ...
    public function picture_listOp() {
        ...
        if(!empty($_GET['picture_state'])){
            ...
            $condition['picture_state'] = ['in', [self::ARTICLE_STATE_PUBLISHED, self::ARTICLE_STATE_VERIFY]];
        }
     ...
     * 获得画报列表
     */
    private function get_picture_list($condition = []) {
        if(!empty($_GET['keyword'])){
            $condition['picture_title'] = ['like', '%' . $_GET['keyword'] . '%'];
        }
        ...
        $picture_image_array = $model_picture_image->getList(['image_picture_id' => ['in', $picture_ids]]);
        $picture_image_list = [];
        if(!empty($picture_image_array)){
            foreach($picture_image_array as $value){
                $image = ['name' => $value['image_name'], 'path' => $value['image_path']];
    ...
    public function picture_editOp() {
        ...
        if($picture_detail){
            ...
            $model_picture_image = Model('cms_picture_image');
            $picture_image_list = $model_picture_image->getList(['image_picture_id' => $picture_id], null);
            Tpl::output('picture_image_list', $picture_image_list);
    ...
    public function picture_dropOp() {
        ...
        if($picture_auth){
            $model_picture = Model('cms_picture');
            $result = $model_picture->drop(['picture_id' => $picture_id]);
            if($result){
    ...
    private function picture_state_change($picture_state_new) {
        ...
        if($picture_auth){
            $model_picture = Model('cms_picture');
            $result = $model_picture->modify(['picture_state' => $picture_state_new], ['picture_id' => $picture_id]);
