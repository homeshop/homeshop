 ...
 * cms用户中心文章
 */
class  ArticleControl extends CMSMemberControl {
    ...
    public function article_listOp() {
        ...
        if(!empty($_GET['article_state'])){
            ...
            $condition['article_state'] = ['in', [self::ARTICLE_STATE_PUBLISHED, self::ARTICLE_STATE_VERIFY]];
        }
     ...
     * 获得文章列表
     */
    private function get_article_list($condition = []) {
        if(!empty($_GET['keyword'])){
            $condition['article_title'] = ['like', '%' . $_GET['keyword'] . '%'];
        }
    ...
    public function article_dropOp() {
        ...
        if($article_auth){
            $model_article = Model('cms_article');
            $result = $model_article->drop(['article_id' => $article_id]);
            if($result){
    ...
    private function article_state_change($article_state_new) {
        ...
        if($article_auth){
            $model_article = Model('cms_article');
            $result = $model_article->modify(['article_state' => $article_state_new], ['article_id' => $article_id]);
