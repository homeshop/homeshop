...
class  ArticleControl extends CMSHomeControl {
    ...
    public function article_detailOp() {
        ...
        //计数加1
        $model_article->modify(['article_click' => ['exp', 'article_click+1']], ['article_id' => $article_id]);
        //文章心情
        $article_attitude_list = [];
    ...
    public function article_comment_detailOp() {
        ...
        $article_hot_comment = $model_article->getList(['article_state' => self::ARTICLE_STATE_PUBLISHED], null, 'article_comment_count desc', '*', 10);
    ...
    public function article_searchOp() {
        $condition = [];
        $condition['article_title'] = ["like", '%' . trim($_GET['keyword']) . '%'];
        $condition['article_state'] = self::ARTICLE_STATE_PUBLISHED;
        $model_article = Model('cms_article');
