...
class  NavigationControl extends SystemControl {
    ...
    public function cms_navigation_saveOp() {
        ...
        $validate_array = [
            [
                'input' => $_POST['navigation_title'],
                'require' => 'true',
                "validator" => "Length",
                "min" => "1",
                "max" => "20",
                'message' => Language::get('navigation_title_error')
            ],
            [
                'input' => $_POST['navigation_link'],
                'require' => 'true',
                "validator" => "Length",
                "min" => "1",
                "max" => "255",
                'message' => Language::get('navigation_link_error')
            ],
            [
                'input' => $_POST['navigation_sort'],
                'require' => 'true',
                'validator' => 'Range',
                'min' => 0,
                'max' => 255,
                'message' => Language::get('navigation_sort_error')
            ],
        ];
    ...
    public function update_navigation_titleOp() {
        ...
        $error = $obj_validate->validate();
        if($error != ''){
            echo json_encode(['result' => false, 'message' => Language::get('navigation_title_error')]);
            die;
        } else {
    ...
    public function update_navigation_linkOp() {
        ...
        $error = $obj_validate->validate();
        if($error != ''){
            echo json_encode(['result' => false, 'message' => Language::get('navigation_link_error')]);
            die;
        } else {
    ...
    private function update_navigation($column, $new_value) {
        ...
        $model = Model("cms_navigation");
        $result = $model->modify([$column => $new_value], ['navigation_id' => $navigation_id]);
        if($result){
            echo json_encode(['result' => true, 'message' => 'success']);
            die;
        } else {
            echo json_encode(['result' => false, 'message' => Language::get('nc_common_save_fail')]);
            die;
        }
    }
    ...
    public function cms_navigation_dropOp() {
        ...
        $model = Model('cms_navigation');
        $condition = [];
        $condition['navigation_id'] = ['in', $navigation_id];
    ...
    private function show_menu($menu_key) {
        $menu_array = [
            'list' => [
                ...
                'menu_url' => 'index.php?act=cms_navigation&op=cms_navigation_list'
            ],
            'add' => [
                'menu_type' => 'link',
                'menu_name' => Language::get('nc_new'),
                'menu_url' => 'index.php?act=cms_navigation&op=cms_navigation_add'
            ],
        ];
