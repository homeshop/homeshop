    ...
    public function cms_article_list_xmlOp() {
        ...
        if($_REQUEST['advanced']){
            if(strlen($q = trim((string)$_REQUEST['article_title']))){
                $condition['article_title'] = ['like', '%' . $q . '%'];
            }
        ...
        foreach($list as $val){
            ...
            $o .= '<li><a target="_blank" href="' . CMS_SITE_URL . '/index.php?act=article&op=article_detail&article_id=' . $val['article_id'] . '">查看文章</a></li>';
            ...
            $i = [];
            $i['operation'] = $o;
            $i['article_sort'] = '<span class="editable" title="可编辑" style="width:50px;" data-live-inline-edit="article_sort">' . $val['article_sort'] . '</span>';
...
EOB;
            ...
            $i['article_click'] = '<span class="editable" title="可编辑" style="width:50px;" data-live-inline-edit="article_click">' . $val['article_click'] . '</span>';
            $i['article_commend_flag'] = $val['article_commend_flag'] == 1 ? '<span class="yes"><i class="fa fa-check-circle"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
            $i['article_commend_image_flag'] = $val['article_commend_image_flag'] == 1 ? '<span class="yes"><i class="fa fa-check-circle"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
            $i['article_comment_flag'] = $val['article_comment_flag'] == 1 ? '<span class="yes"><i class="fa fa-check-circle"></i>开启</span>' : '<span class="no"><i class="fa fa-ban"></i>关闭</span>';
            $i['article_attitude_flag'] = $val['article_attitude_flag'] == 1 ? '<span class="yes"><i class="fa fa-check-circle"></i>开启</span>' : '<span class="no"><i class="fa fa-ban"></i>关闭</span>';
    ...
    public function cms_article_verifyOp() {
        if(intval($_REQUEST['verify_state']) === 1){
            ...
            $this->cms_article_state_modify(self::ARTICLE_STATE_DRAFT, ['article_verify_reason' => $_POST['verify_reason']]);
        }
    }
     ...
     * 修改文章状态
     */
    private function cms_article_state_modify($new_state, $param = []) {
        $article_id = $_REQUEST['article_id'];
        $model_article = Model('cms_article');
        $param['article_state'] = $new_state;
        $model_article->modify($param, ['article_id' => ['in', $article_id]]);
    ...
    public function update_article_sortOp() {
        ...
        if($new_sort > 255){
            ...
            $result = $model_class->modify(['article_sort' => $new_sort], ['article_id' => $_GET['id']]);
            if($result){
                echo json_encode(['result' => true, 'message' => 'class_add_success']);
                die;
            } else {
                echo json_encode(['result' => false, 'message' => Language::get('class_add_fail')]);
                die;
            }
        }
    }
    ...
    public function update_article_clickOp() {
        ...
        $model_class = Model("cms_article");
        $result = $model_class->modify(['article_click' => $_GET['value']], ['article_id' => $_GET['id']]);
        if($result){
    ...
    private function get_article_state_list() {
        $array = [];
        $array[ self::ARTICLE_STATE_DRAFT ] = ['text' => Language::get('cms_text_draft')];
        $array[ self::ARTICLE_STATE_VERIFY ] = ['text' => Language::get('cms_text_verify')];
        $array[ self::ARTICLE_STATE_PUBLISHED ] = ['text' => Language::get('cms_text_published')];
        $array[ self::ARTICLE_STATE_RECYCLE ] = ['text' => Language::get('cms_text_recycle')];
        return $array;
    }
    ...
    private function show_menu($menu_key) {
        $menu_array = [
            'list' => [
                'menu_type' => 'link',
                'menu_name' => Language::get('nc_list'),
                'menu_url' => 'index.php?act=cms_article&op=cms_article_list'
            ],
            'list_verify' => [
                'menu_type' => 'link',
                'menu_name' => Language::get('cms_article_list_verify'),
                'menu_url' => 'index.php?act=cms_article&op=cms_article_list_verify'
            ],
            'list_published' => [
                'menu_type' => 'link',
                'menu_name' => Language::get('cms_article_list_published'),
                'menu_url' => 'index.php?act=cms_article&op=cms_article_list_published'
            ],
        ];
