 ...
 * cms文章分类
 */
class  PictureControl extends SystemControl {
    ...
    public function cms_picture_list_xmlOp() {
        ...
        if($_REQUEST['advanced']){
            if(strlen($q = trim((string)$_REQUEST['picture_title']))){
                $condition['picture_title'] = ['like', '%' . $q . '%'];
            }
        ...
        foreach($list as $val){
            ...
            $o .= '<li><a target="_blank" href="' . CMS_SITE_URL . '/index.php?act=picture&op=picture_detail&picture_id=' . $val['picture_id'] . '">查看画报</a></li>';
            ...
            $i = [];
            $i['operation'] = $o;
            $i['picture_sort'] = '<span class="editable" title="可编辑" style="width:50px;" data-live-inline-edit="picture_sort">' . $val['picture_sort'] . '</span>';
...
EOB;
            ...
            $i['picture_click'] = '<span class="editable" title="可编辑" style="width:50px;" data-live-inline-edit="picture_click">' . $val['picture_click'] . '</span>';
            $i['picture_commend_flag'] = $val['picture_commend_flag'] == 1 ? '<span class="yes"><i class="fa fa-check-circle"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';
            $i['picture_comment_flag'] = $val['picture_comment_flag'] == 1 ? '<span class="yes"><i class="fa fa-check-circle"></i>开启</span>' : '<span class="no"><i class="fa fa-ban"></i>关闭</span>';
    ...
    public function cms_picture_verifyOp() {
        if(intval($_REQUEST['verify_state']) === 1){
            ...
            $this->cms_picture_state_modify(self::ARTICLE_STATE_DRAFT, ['picture_verify_reason' => $_POST['verify_reason']]);
        }
    }
     ...
     * 修改文章状态
     */
    private function cms_picture_state_modify($new_state, $param = []) {
        $picture_id = $_REQUEST['picture_id'];
        $model_picture = Model('cms_picture');
        $param['picture_state'] = $new_state;
        $model_picture->modify($param, ['picture_id' => ['in', $picture_id]]);
    ...
    public function update_picture_sortOp() {
        ...
        if($new_sort > 255){
            echo json_encode(['result' => false, 'message' => Language::get('picture_sort_error')]);
            die;
        } else {
            $model_class = Model("cms_picture");
            $result = $model_class->modify(['picture_sort' => $new_sort], ['picture_id' => $_GET['id']]);
            if($result){
    ...
    public function update_picture_clickOp() {
        ...
        $model_class = Model("cms_picture");
        $result = $model_class->modify(['picture_click' => $_GET['value']], ['picture_id' => $_GET['id']]);
        if($result){
    ...
    public function cms_picture_dropOp() {
        ...
        $model_picture = Model('cms_picture');
        $condition = [];
        $condition['picture_id'] = ['in', $picture_id];
    ...
    private function show_menu($menu_key) {
        $menu_array = [
            'list' => [
                ...
                'menu_url' => 'index.php?act=cms_picture&op=cms_picture_list'
            ],
            'list_verify' => [
                'menu_type' => 'link',
                'menu_name' => Language::get('cms_article_list_verify'),
                'menu_url' => 'index.php?act=cms_picture&op=cms_picture_list_verify'
            ],
            'list_published' => [
                'menu_type' => 'link',
                'menu_name' => Language::get('cms_article_list_published'),
                'menu_url' => 'index.php?act=cms_picture&op=cms_picture_list_published'
            ],
        ];
