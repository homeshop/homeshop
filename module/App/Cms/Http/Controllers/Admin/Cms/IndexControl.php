...
class  IndexControl extends SystemControl {
    ...
    public function drop_page_moduleOp() {
        ...
        if(empty($module_id)){
            ...
            $model_index_module = Model('cms_index_module');
            $result = $model_index_module->drop(['module_id' => $module_id]);
            if(!$result){
                $data['result'] = false;
                $data['message'] = '删除失败';
            }
    ...
    public function update_page_module_titleOp() {
        ...
        if($error != ''){
            echo json_encode(['result' => false, 'message' => '标题名称不能为空且必须小于10个字']);
            die;
        } else {
            $update = ['module_title' => $new_title];
    ...
    public function update_page_module_styleOp() {
        $update = ['module_style' => $_POST['module_style']];
        $result = $this->update_page_module($update, $_POST['module_id']);
        echo $result;
    }
    ...
    public function update_page_module_viewOp() {
        $update = ['module_view' => $_POST['module_view']];
        $result = $this->update_page_module($update, $_POST['module_id']);
        echo $result;
    }
    ...
    private function update_page_module($update, $module_id) {
        ...
        if(empty($module_id)){
            ...
            $model_index_module = Model('cms_index_module');
            $result = $model_index_module->modify($update, ['module_id' => $module_id]);
            if(!$result){
                $data['result'] = false;
                $data['message'] = '操作失败';
            }
    ...
    public function update_page_module_indexOp() {
        ...
        if(!empty($page_module_id_string)){
            ...
            foreach($page_module_id_array as $module_id){
                $result = $model_index_module->modify(['module_sort' => $index], ['module_id' => $module_id]);
                $index++;
            }
        }
        echo json_encode($data);
    ...
    public function cms_index_previewOp() {
        $model_index_module = Model('cms_index_module');
        $module_list = $model_index_module->getList(['module_state' => 1], null, 'module_sort asc');
    ...
    public function save_moduleOp() {
        ...
        //生成模板
        $model_module_frame = Model('cms_module_frame');
        $module_frame = $model_module_frame->getOne(['frame_name' => $_POST['frame_name']]);
    ...
    public function drop_moduleOp() {
        ...
        $module_detail = $model_cms_module->getOne(['module_id' => $module_id]);
        //模块不存在或者是系统自带模块
        if(empty($module_detail) || intval($module_detail['module_class']) === 1){
            $this->return_error('参数错误');
        }
        //删除模块数据库记录
        $model_cms_module->drop(['module_id' => $module_id]);
        $model_cms_index_module = Model('cms_index_module');
        $model_cms_index_module->drop(['module_name' => $module_detail['module_name']]);
        //删除模板文件
    ...
    public function module_editOp() {
        ...
        $model_index_module = Model('cms_index_module');
        $module_detail = $model_index_module->getOne(['module_id' => $module_id]);
    ...
    public function save_page_moduleOp() {
        ...
        $model_cms_index_module = Model('cms_index_module');
        $result = $model_cms_index_module->modify(['module_content' => base64_encode(serialize($module_content))], ['module_id' => $_POST['module_id']]);
        if($result){
    ...
    private function show_menu($menu_key) {
        $menu_array = [
            'cms_index' => [
                ...
                'menu_url' => 'index.php?act=cms_index&op=cms_index'
            ],
        ];
        if($menu_key == 'module_edit'){
            $menu_array['module_edit'] = [
                'menu_type' => 'link',
    ...
    public function get_article_listOp() {
        ...
        if($_GET['search_type'] == 'article_id'){
            ...
            $condition['article_title'] = ['like', '%' . trim($_GET['search_keyword']) . '%'];
        }
    ...
    public function get_store_listOp() {
        //获取店铺列表
        $condition = [];
        $condition['store_name'] = ['like', '%' . $_GET['search_keyword'] . '%'];
    ...
    public function get_member_listOp() {
        //获取店铺列表
        $condition = [];
        $condition['member_name'] = ['like', '%' . trim($_GET['search_keyword']) . '%'];
    ...
    public function get_brand_listOp() {
        ...
        $brand_list = $model_brand->getBrandPassedList([], '*', 6);
