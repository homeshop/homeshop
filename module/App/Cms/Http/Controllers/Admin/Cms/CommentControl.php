...
class  CommentControl extends SystemControl {
    ...
    public function comment_manage_xmlOp() {
        ...
        foreach($list as $val){
            ...
            $o = '<a class="btn red confirm-del-on-click" href="index.php?act=cms_comment&op=comment_drop&comment_id=' . $val['comment_id'] . '"><i class="fa fa-trash"></i>删除</a>';
            $o .= '<a class="btn green" target="_blank" href="' . CMS_SITE_URL . DS . 'index.php?act=' . $channel['key'] . '&op=' . $channel['key'] . '_detail&' . $channel['key'] . '_id=' . $val['comment_object_id'] . '"><i class="fa fa-list-alt"></i>查看</a>';
            $i = [];
            $i['operation'] = $o;
    ...
    private function get_type_array() {
        $type_array = [];
        $type_array[1] = ['name' => Language::get('cms_text_artcile'), 'key' => 'article'];
        $type_array[2] = ['name' => Language::get('cms_text_picture'), 'key' => 'picture'];
