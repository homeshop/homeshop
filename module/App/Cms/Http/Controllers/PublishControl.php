...
class  PublishControl extends CMSMemberControl {
    ...
    public function publish_article_saveOp() {
        ...
        if(!empty($_POST['article_id'])){
            ...
            if($article_auth){
                $model_article->modify($param, ['article_id' => $article_id]);
                $model_tag_relation->drop(['relation_type' => 1, 'relation_object_id' => $article_id]);
            } else {
    ...
    public function publish_picture_saveOp() {
        ...
        if(!empty($_POST['picture_id'])){
            ...
            if($picture_auth){
                //删除原画报图片
                $model_picture_image->drop(['image_picture_id' => $picture_id]);
                $model_picture->modify($param, ['picture_id' => $picture_id]);
                $model_tag_relation->drop(['relation_type' => 2, 'relation_object_id' => $picture_id]);
            } else {
                showMessage(Language::get('wrong_argument'), '', '', 'error');
            }
        } else {
